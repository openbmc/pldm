...
class InventoryManager
{
  ...
  private:
     ...
     *
     * @param[in] eidValue - Remote MCTP endpoint
     * @param[in] descriptors - Descriptors of the firmware device
     */
    void updateFirmwareDeviceName(const eid& eidValue,
                                  const Descriptors& descriptors);
     ...
     */
    FirmwareDeviceName obtainDeviceNameFromDescriptors(
        const Descriptors& descriptors);
