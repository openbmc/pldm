    ...
    for (const auto& [eid, index] : deviceUpdaterInfos)
    {
        ...
        if (!inserted)
        {
            ...
            totalNumComponentUpdates -=
                std::get<pkg::ApplicableComponents>(fwDeviceIDRecord).size();
        }
    }
 ...
 */
static std::vector<pkg::Descriptor> sortedDescriptors(
    const pkg::Descriptors& descriptors)
{
    std::vector<pkg::Descriptor> sorted(descriptors.begin(), descriptors.end());
    ...
    TotalComponentUpdates& totalNumComponentUpdates)
{
    ...
    std::unordered_map<eid, std::vector<pkg::Descriptor>> sortedDescriptorMap;
    ...
    {
        const auto deviceIDDescriptors = sortedDescriptors(
    ...
    {
        const auto recordDescriptors = sortedDescriptors(
        ...
        for (const auto& [eid, downstreamDeviceInfo] : downstreamDescriptorMap)
        {
            ...
            {
                const auto deviceDescriptors = sortedDescriptors(descriptors);
                if (std::includes(
