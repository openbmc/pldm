...
{
    static std::unordered_map<uint16_t, std::string> activationMethods = {
        {0, "Automatic"},
        {1, "Self-Contained"},
        {2, "Medium-specific reset"},
        {4, "System reboot"},
        {8, "DC power cycle"},
        {16, "AC power cycle"}};

    auto iterator = activationMethods.find(compActivationModification.value);
    if (iterator != activationMethods.end())
    {
...
void UpdateManager::createMessageRegistry(
    const mctp_eid_t eid, const FirmwareDeviceIDRecord& fwDeviceIDRecord,
    const size_t comp_index, const std::string messageID,
    const std::string& resolution)
{
...
void UpdateManager::activatePackage()
{
    ...
    for (const auto& [eid, deviceUpdaterPtr] : deviceUpdaterMap)
    {
        ...
        for (size_t compID = 0; compID < applicableComponents.size(); compID++)
        {
            createMessageRegistry(eid, deviceUpdaterPtr->fwDeviceIDRecord,
                                  compID, targetDetermined);
        }
