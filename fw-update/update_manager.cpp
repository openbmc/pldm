...
int UpdateManager::processPackage(const std::filesystem::path& packageFilePath)
{
    ...
    if (packageFd < 0)
    {
        error("Failed to open the PLDM fw update package file '{FILE}', error - {ERROR}.",
    ...
    {
        error("Failed to get the PLDM fw update package file '{FILE}' size, error - {ERROR}.",
              "ERROR", errno, "FILE", packageFilePath);
        close(packageFd);
        packageFd = -1;
        std::filesystem::remove(packageFilePath);
    ...
    packageMap = static_cast<uint8_t*>(mmap(
        nullptr, packageSize, PROT_READ, MAP_SHARED, packageFd, 0));
    if (packageMap == MAP_FAILED)
    {
        error("Failed to map the PLDM fw update package file '{FILE}', error - {ERROR}.",
              "ERROR", errno, "FILE", packageFilePath);
        close(packageFd);
        packageFd = -1;
        std::filesystem::remove(packageFilePath);
    ...
    try
    {
        parser = std::make_unique<PackageParser>(packageMap, packageSize);
    }
