...
int UpdateManager::processPackage(const std::filesystem::path& packageFilePath)
{
    ...
    try
    {
        // Use MMapHandler with path - it automatically handles open, fstat, mmap, and close
        packageMMapHandler = std::make_unique<pldm::utils::MMapHandler>(packageFilePath);
        packageSize = packageMMapHandler->size();
        packageMap = reinterpret_cast<uint8_t*>(packageMMapHandler->data());
    }
