...
class FirmwareInventoryManager
{
  public:
    ...
    explicit FirmwareInventoryManager(
        ...
        std::unique_ptr<BoardPathProvider> provider = nullptr) :
        configurations(config),
        boardPathProvider(provider ? std::move(provider)
                                   : std::make_unique<BoardPathProvider>())
    {}
  ...
  private:
    ...
    /**
     *  @brief 智能指標，安全管理 BoardPathProvider 生命週期
     */
    std::unique_ptr<BoardPathProvider> boardPathProvider;
};
