    ...
    std::function<void()> taskCompletionCallback)
{
    ...
    auto targetArgs = conditionConfigManager.targetArgs(softwareName);
    auto conditionArg = generateArg(targetArgs, boardName);
    const bool includeApplyTimeArg =
        std::find(targetArgs.begin(), targetArgs.end(), "applyTime") !=
        targetArgs.end();
    ...
    softwareMap.insert_or_assign(softwareIdentifier,
                                 std::make_unique<FirmwareInventory>(
                                     ...
                                     generatedId, activeVersion, boardPath));
}
