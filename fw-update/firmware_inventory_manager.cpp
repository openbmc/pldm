    ...
    const pldm::utils::DBusHandler* handler)
{
    ...
    pldm::utils::DBusHandler defaultHandler;
    const pldm::utils::DBusHandler* useHandler =
        handler ? handler : &defaultHandler;
    auto boardPath = getBoardPath(*inventoryPath, useHandler);
...
std::optional<std::filesystem::path> getBoardPath(
    const InventoryPath& path, const pldm::utils::DBusHandler* handler)
{
    pldm::utils::DBusHandler defaultHandler;
    const pldm::utils::DBusHandler* useHandler =
        handler ? handler : &defaultHandler;
    ...
    try
    {
        response = useHandler->getAncestors(path.c_str(), {boardInterface});
    }
