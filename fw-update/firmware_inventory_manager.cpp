    ...
    const Descriptors& descriptors, const ComponentInfo& componentInfo)
{
    ...
    auto boardPath = getBoardPath(*inventoryPath);
    if (!boardPath)
    {
    ...
    const auto boardDeviceName = std::format("{}_{}", boardName, softwareName);
    const auto softwarePath = std::format("/xyz/openbmc_project/software/{}_{}",
                                          boardDeviceName, generateSwId());
