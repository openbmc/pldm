#include "fw_inventory_manager.hpp"
    ...
    for (const auto& eid : eids)
    {
        ...
        rc = handler.registerRequest(
            ...
            std::move(std::bind_front(
                &InventoryManager::queryDeviceIdentifiersRespHandler, this)));
        if (rc)
        {
...
void InventoryManager::queryDeviceIdentifiersRespHandler(
...
void InventoryManager::sendGetFirmwareParametersRequest(mctp_eid_t eid)
{
    ...
    rc = handler.registerRequest(
        ...
        std::move(requestMsg),
        std::move(std::bind_front(
            &InventoryManager::getFirmwareParametersRespHandler, this)));
    if (rc)
    {
...
void InventoryManager::getFirmwareParametersRespHandler(
    mctp_eid_t eid, const pldm_msg* response, size_t respMsgLen)
{
    if (response == nullptr || !respMsgLen)
    {
    ...
    auto activeCompImageSetVer = utils::toString(activeCompImageSetVerStr);
    size_t versionHash = std::hash<std::string>{}(activeCompImageSetVer);
    if (!versionMap.contains(versionHash))
    {
        using VersionEnum =
            sdbusplus::xyz::openbmc_project::Software::server::Version;
        versionMap.emplace(
            versionHash,
            std::make_unique<Version>(pldm::utils::DBusHandler::getBus(),
                                      swRootPath + std::to_string(versionHash),
                                      activeCompImageSetVer,
                                      VersionEnum::VersionPurpose::Other));
    }
