    ...
    size_t offset)
{
    ...
    while (deviceIdRecCount-- && (pkgHdrRemainingSize > 0))
    {
        ...
        if (rc)
        {
            error("Decoding firmware device ID record failed - '{RC}'", "RC",
                  rc);
...
void PackageParser::validatePkgTotalSize(uintmax_t pkgSize)
{
    ...
    for (const auto& componentImageInfo : componentImageInfos)
    {
        ...
        if (compLocOffset != calcPkgSize)
        {
            ...
            error(
                "Validating the component location offset for '{COMPONENT_VERSION}' failed.",
    ...
    {
        error(
            "Package size {PKG_SIZE} does not match '{CALCULATED_PACKAGE_SIZE}'.",
                            ...
                            uintmax_t pkgSize)
{
    ...
    {
        error(
            "Parsing package header failed for '{CALCULATED_CHECKSUM}' and '{PACKAGE_HEADER_CHECKSUM}'",
