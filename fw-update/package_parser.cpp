...
void PackageParser::parse(const std::vector<uint8_t>& pkgHdr)
{
    ...
    info("Parse package header done");
    ...
    info("Parse firmware device ID records done");
    ...
    info("Parse downstream device ID records done");
    ...
    info("Parse component image information done");
}
