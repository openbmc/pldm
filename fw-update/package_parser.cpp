    ...
    size_t offset)
{
    ...
    while (deviceIdRecCount-- && (pkgHdrRemainingSize > 0))
    {
               ...
               (recordDescriptors.length > 0))
        {
            ...
            {
                error(
                    "Decoding descriptor type, length and value failed with response code '{RC}'",
                    "RC", rc);
                throw InternalFailure();
            }
...
void PackageParser::validatePkgTotalSize(uintmax_t pkgSize)
{
    ...
    for (const auto& componentImageInfo : componentImageInfos)
    {
        ...
        if (compLocOffset != calcPkgSize)
        {
            ...
            error(
                "Validating the component location offset for version '{COMPONENT_VERSION}' failed.",
                            ...
                            uintmax_t pkgSize)
{
    ...
    {
        error(
            "Parsing package header failed for calculated checksume '{CALCULATED_CHECKSUM}' and header checksum '{PACKAGE_HEADER_CHECKSUM}'",
