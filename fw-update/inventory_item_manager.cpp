    ...
    const std::string& activeVersion)
{
    if (!inventoryPathMap.at(eid).empty())
    {
        ...
        createVersion(softwarePath, activeVersion,
                      VersionPurpose::Other);
        createAssociation(softwarePath, "running", "ran_on",
                          devicePath);
    }
}
                                         ...
                                         VersionPurpose purpose)
{
    ...
    auto softwareVersion = std::make_unique<SoftwareVersionIntf>(
        utils::DBusHandler::getBus(), path.c_str());
    softwareVersion->version(version.c_str());
    softwareVersion->purpose(purpose);
    softwareVersions.emplace_back(std::move(softwareVersion));
}
