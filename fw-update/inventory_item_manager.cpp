...
Software::Software(Context& ctx, const std::string& path,
    ...
    blocksTransition(
        std::make_unique<ActivationBlocksTransitionIntf>(ctx, path.c_str())),
    activation(std::make_unique<SoftwareActivationIntf>(ctx, path.c_str())),
    version(std::make_unique<SoftwareVersionIntf>(ctx, path.c_str()))
{
    ...
    this->activation->activation(SoftwareActivationIntf::Activations::Active);
