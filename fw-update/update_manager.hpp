...
class UpdateManager
{
  public:
    ...
    explicit UpdateManager(
        ...
        event(event),
        handler(handler), instanceIdDb(instanceIdDb),
        localDescriptorMap(std::move(descriptorMap)),
        descriptorMap(localDescriptorMap),
        localComponentInfoMap(std::move(componentInfoMap)),
        componentInfoMap(localComponentInfoMap), totalNumComponentUpdates(0),
