...
class FirmwareCondition : public std::enable_shared_from_this<FirmwareCondition>
{
  public:
    ...
    void unitStateChange(sdbusplus::message_t& msg)
    {
        ...
        {
            info("Service completed, {JOB}", "JOB", returnedJobPath);
            if (postRoutine)
            {
                (*postRoutine)();
                self.reset();
  ...
  private:
    ...
    std::shared_ptr<FirmwareCondition> self = nullptr;
};
