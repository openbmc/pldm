        ...
        try
        {
            ...
            if (data.contains("Components") && data["Components"].is_array())
            {
                for (const auto& component : data["Components"])
                {
                    ...
                    {
                        auto [it, inserted] = insert_or_assign(
                            component["Component"], FirmwareConditionPathPair());
                        auto& [pre, post] = it->second;
                        if (component.contains("PreCondition") &&
                            component["PreCondition"].is_string())
                        {
                            pre = component["PreCondition"];
                        }
                        if (component.contains("PostCondition") &&
                            component["PostCondition"].is_string())
                        {
                            post = component["PostCondition"];
                        }
                    }
                    else
                    {
