            ...
            if (data.contains("Components") && data["Components"].is_array())
            {
                for (const auto& component : data["Components"])
                {
                        ...
                        component["Component"].is_string())
                    {
                        ...
                        if (component.contains("PreUpdateTarget") &&
                            component["PreUpdateTarget"].is_string())
                        {
                            pre = component["PreUpdateTarget"];
                        }
                        if (component.contains("PostUpdateTarget") &&
                            component["PostUpdateTarget"].is_string())
                        {
                            post = component["PostUpdateTarget"];
                        }
                    }
                    else
                    {
