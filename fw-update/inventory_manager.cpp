...
void InventoryManager::discoverFDs(const std::vector<mctp_eid_t>& eids)
{
    for (const auto& eid : eids)
    {
        ...
        if (rc)
        {
            ...
            error(
                "Encode query device identifiers req for endpoint ID '{EID}' failed with response code '{RC}'",
        ...
        {
            error(
                "Send QueryDeviceIdentifiers request for endpoint ID '{EID}' failed with response code '{RC}'",
                "EID", unsigned(eid), "RC", rc);
        }
    }
}
                                              ...
                                              size_t respMsgLen)
{
    ...
    {
        error(
            "No response received for QueryDeviceIdentifiers for endpoint ID '{EID}'",
            "EID", unsigned(eid));
        return;
    }
    ...
    {
        error(
            "Decoding QueryDeviceIdentifiers response for endpoint ID '{EID}' failed with response code '{RC}'",
            "EID", unsigned(eid), "RC", rc);
        return;
    }
    ...
    {
        error(
            "QueryDeviceIdentifiers response for endpoint ID '{EID}' failed with completion code '{CC}'",
    ...
    while (descriptorCount-- && (deviceIdentifiersLen > 0))
    {
        ...
        {
            error(
                "Decoding descriptor type, length and value for endpoint ID '{EID}' failed with response code '{RC}'",
                "EID", unsigned(eid), "RC", rc);
            return;
        }
        ...
        else
        {
            ...
            {
                error(
                    "Decoding Vendor-defined descriptor value for endpoint ID '{EID}' failed with response code '{RC}'",
                    "EID", unsigned(eid), "RC", rc);
                return;
            }
...
void InventoryManager::sendGetFirmwareParametersRequest(mctp_eid_t eid)
{
    ...
    if (rc)
    {
        instanceIdDb.free(eid, instanceId);
        error(
            "Encode get firmware parameters req for endpoint ID '{EID}' failed with response code '{RC}'",
            "EID", unsigned(eid), "RC", rc);
        return;
    }
    ...
    {
        error(
            "Send GetFirmwareParameters request for endpoint ID '{EID}' failed with response code '{RC}'",
            "EID", unsigned(eid), "RC", rc);
    }
}
                                             ...
                                             size_t respMsgLen)
{
    ...
    {
        error(
            "No response received for GetFirmwareParameters for endpoint ID '{EID}'",
            "EID", unsigned(eid));
    ...
    {
        error(
            "Decoding GetFirmwareParameters response for endpoint ID '{EID}' failed with response code '{RC}'",
            "EID", unsigned(eid), "RC", rc);
        return;
    }
    ...
    {
        error(
            "GetFirmwareParameters response for endpoint ID '{EID}' failed with completion code '{CC}'",
    ...
    while (fwParams.comp_count-- && (compParamTableLen > 0))
    {
        ...
        {
            error(
                "Decoding component parameter table entry for endpoint ID '{EID}' failed with response code '{RC}'",
                "EID", unsigned(eid), "RC", rc);
            return;
        }
