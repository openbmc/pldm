...
void InventoryManager::discoverFDs(const std::vector<mctp_eid_t>& eids)
{
    for (const auto& eid : eids)
    {
        ...
        if (rc)
        {
            ...
            error(
                "Encode query device identifiers req for '{EID}' failed - '{RC}'",
        ...
        {
            error(
                "Send QueryDeviceIdentifiers request for '{EID}' failed - '{RC}'",
                "EID", unsigned(eid), "RC", rc);
        }
    }
}
                                              ...
                                              size_t respMsgLen)
{
    ...
    if (completionCode)
    {
        error("QueryDeviceIdentifiers response for '{EID}' failed - '{CC}'",
                                             ...
                                             size_t respMsgLen)
{
    ...
    {
        error(
            "Decoding GetFirmwareParameters response for '{EID}' failed - '{RC}'",
            "EID", unsigned(eid), "RC", rc);
        return;
    }
    ...
    {
        error("GetFirmwareParameters response for '{EID}' failed - '{CC}'",
