...
void InventoryManager::getDeviceName(mctp_eid_t eid, const std::string& UUID,
                                     std::string& deviceName)
{
    info("Checking Entity Manager for hardcoded Device Name for EID {EID}",
              "EID", eid);
    ...
    std::vector<std::string> ifaceList = {
        "xyz.openbmc_project.Configuration.PldmDevice.Type",
        "xyz.openbmc_project.Configuration.PldmDevice.Name",
        "xyz.openbmc_project.Configuration.PldmDevice.UUID"};
    utils::GetSubTreePathsResponse objectPaths;
    utils::DBusHandler dbusHandler;
    try
    {
        objectPaths = dbusHandler.getSubTreePaths(inventoryPath, depth,
                                                  ifaceList);
    }
    catch (const std::exception& e)
    {
        error("Error getting subtree paths for {PATH}", "PATH",
                   inventoryPath);
    ...
    for (const auto& objectPath : objectPaths)
    {
        ...
        {
            error("Error getting UUID from path {PATH}", "PATH",
                  objectPath);
            continue;
        }
