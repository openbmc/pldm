    ...
    enum transfer_op_flag transferOperationFlag)
{
    ...
    auto instanceId = instanceIdDb.next(eid);
    auto request = new (requestMsg.data()) pldm_msg;
    pldm_get_downstream_firmware_params_req requestParameters{
        dataTransferHandle, transferOperationFlag};
    ...
    mctp_eid_t eid, const pldm_msg* response, size_t respMsgLen)
{
    ...
    if (downstreamFirmwareParams.completion_code)
    {
        error(
            ...
            "EID", eid, "CC", downstreamFirmwareParams.completion_code);
        return;
    }
           ...
           (paramTableLen > 0))
    {
        ...
        const auto compClassification = compEntry.comp_classification;
