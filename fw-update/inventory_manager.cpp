    ...
    mctp_eid_t eid, const pldm_msg* response, size_t respMsgLen)
{
    ...
    std::vector<pldm_downstream_device*> downstreamDeviceFields(
        numberOfDownstreamDevices);
    std::vector<variable_field> downstreamDescriptorDatas(
        numberOfDownstreamDevices);

    rc = pldm_extract_downstream_devices(
        &downstreamDevicesData, numberOfDownstreamDevices,
        downstreamDeviceFields.data(), downstreamDescriptorDatas.data());
    if (rc)
    {
        error(
    ...
    for (uint16_t deviceCount = 0; deviceCount < numberOfDownstreamDevices;
         deviceCount++)
    {
        const auto& downstreamDeviceField =
            downstreamDeviceFields.at(deviceCount);
        const auto& downstreamDescriptorData =
            downstreamDescriptorDatas.at(deviceCount);
        auto downstreamDescriptorCount =
            downstreamDeviceField->downstream_descriptor_count;
        std::vector<variable_field>
            descriptorFields(downstreamDescriptorCount);

        rc = pldm_extract_descriptors(&downstreamDescriptorData,
        ...
        for (const auto& descriptorField : descriptorFields)
        {
            ...
            variable_field descriptorData{};
            rc = decode_descriptor_type_length_value(
                descriptorField.ptr, descriptorField.length, &descriptorType,
                &descriptorData);
            if (rc)
            {
                error(
