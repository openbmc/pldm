    ...
    mctp_eid_t eid, const pldm_msg* response, size_t respMsgLen)
{
    ...
    pldm_query_downstream_identifiers_resp responseData{};
    size_t respDataLen = respMsgLen + sizeof(responseData);
    ...
    rc = decode_query_downstream_identifiers_index(
        &deviceData, &responseData, respMsgLen, devices.data(), devicesLength,
    ...
    switch (responseData.transfer_flag)
    {
        case PLDM_START:
            ...
            sendQueryDownstreamIdentifiersRequest(
                eid, responseData.next_data_transfer_handle, PLDM_GET_NEXTPART);
            break;
        case PLDM_MIDDLE:
            sendQueryDownstreamIdentifiersRequest(
                eid, responseData.next_data_transfer_handle, PLDM_GET_NEXTPART);
