...
namespace fw_update
{
sdbusplus::async::task<int> InventoryManager::sendRecvPldmMsgOverMctp(
...
sdbusplus::async::task<int> InventoryManager::discoverFDsTask()
{
...
sdbusplus::async::task<int> InventoryManager::startFirmwareDiscoveryFlow(
    mctp_eid_t eid)
{
    uint8_t rc = 0;
...
sdbusplus::async::task<int> InventoryManager::queryDeviceIdentifiers(
    mctp_eid_t eid)
{
    ...
    if (rc)
    {
        ...
        error(
            "Failed to encode QueryDeviceIdentifiers request for endpoint ID {EID}, response code {RC}",
            "EID", eid, "RC", rc);
        co_return rc;
    }
    ...
    {
        error(
            "Failed to send QueryDeviceIdentifiers request for endpoint ID {EID}, response code {RC} ",
            "EID", eid, "RC", rc);
        co_return rc;
    }
...
sdbusplus::async::task<int> InventoryManager::getFirmwareParameters(
    mctp_eid_t eid)
{
    auto instanceId = instanceIdDb.next(eid);
    Request requestMsg(
...
sdbusplus::async::task<int> InventoryManager::parseGetFWParametersResponse(
    mctp_eid_t eid, const pldm_msg* response, size_t respMsgLen)
{
    if (response == nullptr || !respMsgLen)
    {
