...
void InventoryManager::refreshInventoryPath(const MctpInfo& mctpInfo)
{
    ...
    {
        if (std::get<eid>(mctpInfo) == eidValue)
        {
...
void InventoryManager::updateFirmwareDeviceName(const eid& eidValue,
                                                const Descriptors& descriptors)
{
    ...
    if (firmwareDeviceName.empty())
    {
        firmwareDeviceName = "Firmware_Device_" + std::to_string(eidValue);
    }

    firmwareDeviceNameMap.insert_or_assign(eidValue, firmwareDeviceName);
}
