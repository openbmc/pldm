...
void InventoryManager::removeFDs(const MctpInfos& mctpInfos)
{
    for (const auto& mctpInfo : mctpInfos)
    {
        ...
        firmwareInventoryManager.removeById(eid);
    }
}
...
void InventoryManager::refreshInventoryPath(const MctpInfo& mctpInfo)
{
    ...
    for (const auto& [configDbusPath, configMctpInfo] : configurations)
    {
        ...
        {
            firmwareInventoryManager.refreshInventoryPath(eid, configDbusPath);
            break;
        }
    }
}
    ...
    mctp_eid_t eid, const pldm_msg* response, size_t respMsgLen)
{
    ...
    foreach_pldm_downstream_device_parameters_entry(params, entry, rc)
    {
        ...
        {
            firmwareInventoryManager.emplace(
                softwareIdentifier,
...
{
    return firmwareInventoryManager.handleRequest(eid, command, request,
                                                  reqMsgLen);
}
