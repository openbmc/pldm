                               ...
                               TaskCallback&& taskCallback)
{
    ...
    try
    {
        ...
        auto slot = method.call_async(callback);
        asyncSlots.push_back(std::move(slot));
    }
    catch (const std::exception& e)
    {
                                          ...
                                          const std::string& args) const
{
    ...
    if (!args.empty())
    {
        auto argPath = std::filesystem::path(args);
        if (!argPath.has_filename())
        {
            error("Bad argument to a service unit: {ARG}", "ARG", args);
            throw std::runtime_error("Bad argument to a service unit");
        }
        auto prunedArg = argPath.filename().string();
        service += std::format("@{}", prunedArg);
    }
    service += ".service";
    return service;
}
