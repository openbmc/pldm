    ...
    const pkg::Descriptors& descriptors)
{
    for (size_t index = 0; index < fwDeviceIDRecords.size(); ++index)
    {
        ...
        std::vector<pkg::Descriptor> deviceIDDescriptors(
            std::get<pkg::Descriptors>(fwDeviceIDRecords[index]).begin(),
            std::get<pkg::Descriptors>(fwDeviceIDRecords[index]).end());
        std::sort(deviceIDDescriptors.begin(), deviceIDDescriptors.end());

        std::vector<pkg::Descriptor> sortedDescriptors(descriptors.begin(),
                                                       descriptors.end());
