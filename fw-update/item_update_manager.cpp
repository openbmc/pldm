...
bool ItemUpdateManager::processPackage()
{
    activation = std::make_unique<Activation>(
        pldm::utils::DBusHandler::getBus(), objPathWithHash,
...
{
    objPathWithHash = std::format("{}_{}", objPath, utils::generateSwId());
    auto dupFd = dup(fd);
    if (dupFd < 0)
    {
