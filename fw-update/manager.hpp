  ...
  public:
    ...
    explicit Manager(
        const pldm::utils::DBusHandler*
            dbusHandler, // D-Bus handler for inventory queries
        Event& event,    // io_context event object
        requester::Handler<requester::Request>& handler, // PLDM request handler
        pldm::InstanceIdDb& instanceIdDb)                // InstanceId database
        :
