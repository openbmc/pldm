...
TEST(SystemdInterfaceAsyncTest, ExecuteDoesNotInvokeCallbackSynchronously)
{
    ...
    systemdInterface.execute(
        "pldm-test-async-condition", "arg",
        [&callbackInvoked](bool /*success*/) { callbackInvoked = true; });

    // execute() should not block or synchronously invoke callback.
