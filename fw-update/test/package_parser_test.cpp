#include "fw-update/package_parser.hpp"

#include <libpldm/edac.h>

#include <array>
#include <bit>
#include <typeinfo>

#include <gmock/gmock.h>
#include <gtest/gtest.h>

using namespace pldm::fw_update;

/** @brief Append the PackageHeaderChecksum of the package header
 *
 *  Computing it here rather than hardcoding it keeps the test packages
 *  maintainable, patching a header field does not require recomputing a
 *  constant by hand.
 */
void appendChecksum(std::vector<uint8_t>& fwPkgHdr)
{
    const uint32_t check = pldm_edac_crc32(fwPkgHdr.data(), fwPkgHdr.size());
    const auto checksum =
        std::bit_cast<std::array<uint8_t, sizeof(check)>>(check);
    fwPkgHdr.insert(fwPkgHdr.end(), checksum.begin(), checksum.end());
}

void imageGenerate(std::vector<uint8_t>& image, size_t size)
{
    for (size_t i = 1; i <= size; ++i)
    {
        image.push_back(static_cast<uint8_t>(i));
    }
}

void imageInsert(std::vector<uint8_t>& fwPkgHdr, std::vector<uint8_t>& image)
{
    // Insert the image at the end of the package header
    fwPkgHdr.insert(fwPkgHdr.end(), image.begin(), image.end());
}

/** @brief Check one entry of the parsed component image information
 *
 *  @param[in] actual - the parsed entry
 *  @param[in] package - the package the entry was parsed from
 *  @param[in] offset - where in @a package the component image is expected
 *  @param[in] size - the expected size of the component image
 */
void expectCompImageInfo(
    const pkg::ComponentImageInfo& actual, const std::vector<uint8_t>& package,
    pkg::CompClassification classification, pkg::CompIdentifier identifier,
    pkg::CompComparisonStamp stamp, pkg::CompOptions options,
    pkg::ReqCompActivationMethod method, size_t offset, size_t size,
    const pkg::CompVersion& version)
{
    using Pos = pkg::ComponentImageInfoPos;

    EXPECT_EQ(std::get<static_cast<size_t>(Pos::CompClassificationPos)>(actual),
              classification);
    EXPECT_EQ(std::get<static_cast<size_t>(Pos::CompIdentifierPos)>(actual),
              identifier);
    EXPECT_EQ(
        std::get<static_cast<size_t>(Pos::CompComparisonStampPos)>(actual),
        stamp);
    EXPECT_EQ(std::get<static_cast<size_t>(Pos::CompOptionsPos)>(actual),
              options);
    EXPECT_EQ(
        std::get<static_cast<size_t>(Pos::ReqCompActivationMethodPos)>(actual),
        method);

    // The component image has to be a view into the package, not a copy
    const auto& image = std::get<pkg::CompImage>(actual);
    EXPECT_EQ(image.data(), package.data() + offset);
    EXPECT_EQ(image.size(), size);

    EXPECT_EQ(std::get<pkg::CompVersion>(actual), version);
}

TEST(PackageParser, ValidPkgSingleDescriptorSingleComponent)
{
    std::vector<uint8_t> fwPkgHdr{
        };

    constexpr uintmax_t pkgSize = 166;
    constexpr uintmax_t pkgHeaderSize = 139;
    constexpr uintmax_t pkgImageSize = pkgSize - pkgHeaderSize;
    std::vector<uint8_t> compImage;
    imageGenerate(compImage, pkgImageSize);
    imageInsert(fwPkgHdr, compImage);
    auto parser = parsePkgHeader(fwPkgHdr);
    ASSERT_NE(parser, nullptr);
    auto obj = parser.get();
    EXPECT_EQ(typeid(*obj).name(), typeid(WrapPackageParser).name());

    auto outfwDeviceIDRecords = parser->getFwDeviceIDRecords();
    pkg::FirmwareDeviceIDRecords fwDeviceIDRecords{
        {1,
         {0},
         "VersionString2",
         {{PLDM_FWUP_UUID,
           std::vector<uint8_t>{0x16, 0x20, 0x23, 0xC9, 0x3E, 0xC5, 0x41, 0x15,
                                0x95, 0xF4, 0x48, 0x70, 0x1D, 0x49, 0xD6,
                                0x75}}},
         {}},
    };
    EXPECT_EQ(outfwDeviceIDRecords, fwDeviceIDRecords);

    const auto& outCompImageInfos = parser->getComponentImageInfos();
    ASSERT_EQ(outCompImageInfos.size(), 1);
    expectCompImageInfo(outCompImageInfos[0], fwPkgHdr, 10, 100, 0xFFFFFFFF, 0,
                        0, 139, 27, "VersionString3");
}

TEST(PackageParser, ValidPkgMultipleDescriptorsMultipleComponents)
{
    std::vector<uint8_t> fwPkgHdr{
        };

    appendChecksum(fwPkgHdr);

    const uintmax_t pkgHeaderSize = fwPkgHdr.size();
    EXPECT_EQ(pkgHeaderSize, 326);

    constexpr uintmax_t pkgSize = 407;
    const uintmax_t pkgImageSize = (pkgSize - pkgHeaderSize) / 3;
    std::vector<uint8_t> compImage1, compImage2, compImage3;
    imageGenerate(compImage1, pkgImageSize);
    imageGenerate(compImage2, pkgImageSize);
    imageGenerate(compImage3, pkgImageSize);
    imageInsert(fwPkgHdr, compImage1);
    imageInsert(fwPkgHdr, compImage2);
    imageInsert(fwPkgHdr, compImage3);

    auto parser = parsePkgHeader(fwPkgHdr);
    ASSERT_NE(parser, nullptr);
    auto obj = parser.get();
    EXPECT_EQ(typeid(*obj).name(), typeid(WrapPackageParser).name());

    EXPECT_EQ(fwPkgHdr.size(), pkgSize);

    auto outfwDeviceIDRecords = parser->getFwDeviceIDRecords();
    pkg::FirmwareDeviceIDRecords fwDeviceIDRecords{
        {1,
         {0, 1},
         "VersionString2",
         {{PLDM_FWUP_UUID,
           std::vector<uint8_t>{0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D, 0x47, 0x18,
                                0xA0, 0x30, 0xFC, 0x8A, 0x56, 0x58, 0x7D,
                                0x5B}},
          {PLDM_FWUP_IANA_ENTERPRISE_ID,
           std::vector<uint8_t>{0x47, 0x16, 0x00, 0x00}},
          {PLDM_FWUP_VENDOR_DEFINED,
           std::make_tuple("OpenBMC", std::vector<uint8_t>{0x12, 0x34})}},
         {}},
        {0,
         {0, 1, 2},
         "VersionString3",
         {{PLDM_FWUP_UUID,
           std::vector<uint8_t>{0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D, 0x47, 0x18,
                                0xA0, 0x30, 0xFC, 0x8A, 0x56, 0x58, 0x7D,
                                0x5C}}},
         {}},
        {0,
         {0},
         "VersionString4",
         {{PLDM_FWUP_UUID,
           std::vector<uint8_t>{0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D, 0x47, 0x18,
                                0xA0, 0x30, 0xFC, 0x8A, 0x56, 0x58, 0x7D,
                                0x5D}}},
         {}},
    };
    EXPECT_EQ(outfwDeviceIDRecords, fwDeviceIDRecords);

    const auto& outCompImageInfos = parser->getComponentImageInfos();
    ASSERT_EQ(outCompImageInfos.size(), 3);
    expectCompImageInfo(outCompImageInfos[0], fwPkgHdr, 10, 100, 0xFFFFFFFF, 0,
                        0, 326, 27, "VersionString5");
    expectCompImageInfo(outCompImageInfos[1], fwPkgHdr, 10, 200, 0xFFFFFFFF, 0,
                        1, 353, 27, "VersionString6");
    expectCompImageInfo(outCompImageInfos[2], fwPkgHdr, 11, 300, 0xFFFFFFFF, 1,
                        12, 380, 27, "VersionString7");
}

TEST(PackageParser, InvalidPkgHeaderInfoIncomplete)
{
    std::vector<uint8_t> fwPkgHdr{0xF0, 0x18, 0x87, 0x8C, 0xCB, 0x7D,
                                  0x49, 0x43, 0x98, 0x00, 0xA0, 0x2F,
                                  0x05, 0x9A, 0xCA, 0x02};

    auto parser = parsePkgHeader(fwPkgHdr);
    EXPECT_EQ(parser, nullptr);
}

TEST(PackageParser, InvalidPkgNotSupportedHeaderFormat)
{
    std::vector<uint8_t> fwPkgHdr{
        0x12, 0x44, 0xD2, 0x64, 0x8D, 0x7D, 0x47, 0x18, 0xA0, 0x30,
        0xFC, 0x8A, 0x56, 0x58, 0x7D, 0x5B, 0x02, 0x8B, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x19, 0x0C, 0xE5,
        0x07, 0x00, 0x08, 0x00, 0x01, 0x0E, 0x56, 0x65, 0x72, 0x73,
        0x69, 0x6F, 0x6E, 0x53, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x31};

    auto parser = parsePkgHeader(fwPkgHdr);
    EXPECT_EQ(parser, nullptr);
}

TEST(PackageParser, InvalidPkgBadChecksum)
{
    std::vector<uint8_t> fwPkgHdr{
        };

    constexpr uintmax_t pkgSize = 166;
    constexpr uintmax_t pkgHeaderSize = 139;
    constexpr uintmax_t pkgImageSize = pkgSize - pkgHeaderSize;
    std::vector<uint8_t> compImage;
    imageGenerate(compImage, pkgImageSize);
    imageInsert(fwPkgHdr, compImage);
    EXPECT_EQ(parsePkgHeader(fwPkgHdr), nullptr);
}

TEST(PackageParser, InvalidPkgZeroComponentSize)
{
    // As InvalidPkgBadChecksum, but with a ComponentSize of 0
    std::vector<uint8_t> fwPkgHdr{
        };
    appendChecksum(fwPkgHdr);

    constexpr uintmax_t pkgHeaderSize = 139;
    constexpr uintmax_t pkgImageSize = 27;
    ASSERT_EQ(fwPkgHdr.size(), pkgHeaderSize);

    std::vector<uint8_t> compImage;
    imageGenerate(compImage, pkgImageSize);
    imageInsert(fwPkgHdr, compImage);

    EXPECT_EQ(parsePkgHeader(fwPkgHdr), nullptr);
}

TEST(PackageParser, InvalidPkgEmptyComponentVersionString)
{
    // Rejected by libpldm++ rather than by this repository, the test pins the
    // behaviour down so that it is noticed if that ever changes.
    //
    // As InvalidPkgBadChecksum, but with a ComponentVersionStringLength of 0.
    // PackageHeaderSize and ComponentLocationOffset shrink by the 14 bytes of
    // the dropped version string.
    std::vector<uint8_t> fwPkgHdr{
        0xF0, 0x18, 0x87, 0x8C, 0xCB, 0x7D, 0x49, 0x43, 0x98, 0x00, 0xA0,
        0x2F, 0x05, 0x9A, 0xCA, 0x02, 0x01, 0x7D, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x19, 0x0C, 0xE5, 0x07, 0x00, 0x08,
        0x00, 0x01, 0x0E, 0x56, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x53,
        0x74, 0x72, 0x69, 0x6E, 0x67, 0x31, 0x01, 0x2E, 0x00, 0x01, 0x01,
        0x00, 0x00, 0x00, 0x01, 0x0E, 0x00, 0x00, 0x01, 0x56, 0x65, 0x72,
        0x73, 0x69, 0x6F, 0x6E, 0x53, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x32,
        0x02, 0x00, 0x10, 0x00, 0x16, 0x20, 0x23, 0xC9, 0x3E, 0xC5, 0x41,
        0x15, 0x95, 0xF4, 0x48, 0x70, 0x1D, 0x49, 0xD6, 0x75, 0x01, 0x00,
        0x0A, 0x00, 0x64, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00,
        0x00, 0x7D, 0x00, 0x00, 0x00, 0x1B, 0x00, 0x00, 0x00, 0x01, 0x00};
    appendChecksum(fwPkgHdr);

    constexpr uintmax_t pkgHeaderSize = 125;
    constexpr uintmax_t pkgImageSize = 27;
    ASSERT_EQ(fwPkgHdr.size(), pkgHeaderSize);

    std::vector<uint8_t> compImage;
    imageGenerate(compImage, pkgImageSize);
    imageInsert(fwPkgHdr, compImage);

    EXPECT_EQ(parsePkgHeader(fwPkgHdr), nullptr);
}

TEST(PackageParser, InvalidPkgComparisonStampWithoutUseComparisonStamp)
{
    // As InvalidPkgBadChecksum, but with a ComponentComparisonStamp of
    // 0x12345678 while the UseComponentComparisonStamp option is not set
    std::vector<uint8_t> fwPkgHdr{
        };
    appendChecksum(fwPkgHdr);

    constexpr uintmax_t pkgHeaderSize = 139;
    constexpr uintmax_t pkgImageSize = 27;
    ASSERT_EQ(fwPkgHdr.size(), pkgHeaderSize);

    std::vector<uint8_t> compImage;
    imageGenerate(compImage, pkgImageSize);
    imageInsert(fwPkgHdr, compImage);

    EXPECT_EQ(parsePkgHeader(fwPkgHdr), nullptr);
}
