...
// sequence and verify two distinct rejection codes:
//
//   - Out-of-sequence completes / requestFwData (phase gate fires):
//     PLDM_FWUP_COMMAND_NOT_EXPECTED per DSP0267 Sec. 12.6-12.9.
//   - In-sequence TransferComplete(success) but with insufficient
//     bytes served (the FD lied about completion): PLDM_ERROR_INVALID_DATA.
//     The command is expected in DOWNLOAD -- only the payload is invalid.
//
// Expected behavior:
//   Against origin/master (unfixed): coverage / out-of-sequence tests
//   FAIL -- handlers return PLDM_SUCCESS and the attack succeeds.
//   Against phase-tracking-fix: out-of-sequence tests return
//   PLDM_FWUP_COMMAND_NOT_EXPECTED; coverage-failure tests return
//   PLDM_ERROR_INVALID_DATA; and the normal-flow regression still
//   succeeds.
...
{
    // FD drives the handshake through UpdateComponent ->
    // TransferComplete(success) without issuing a single
    // RequestFirmwareData. The phase gate is in Download, so
    ...
    EXPECT_EQ(completionCodeOf(response), PLDM_ERROR_INVALID_DATA)
        << "TransferComplete(success) with zero bytes served must be "
           "rejected with PLDM_ERROR_INVALID_DATA (coverage check; the "
           "command is in-sequence but the success claim is false).";
}

// ============================================================
// Full attack chain: a malicious FD claims TransferComplete /
// VerifyComplete / ApplyComplete success without ever issuing a
// RequestFirmwareData. Verifies the phase gates and the bytes-
// served cross-check together block every step.
// ============================================================
...
TEST_F(PhaseTrackingTest, FullAttackChainBlocked)
{
    ...
    // RequestFirmwareData. Bytes-served gate must fire with
    // PLDM_ERROR_INVALID_DATA (in-sequence command, false payload).
    {
...
{
    // High-water-mark bypass scenario:
    ...
    auto* tcMsg = reinterpret_cast<const pldm_msg*>(tcReq.data());
    auto tcResp = updater.transferComplete(tcMsg, 1);
    EXPECT_EQ(completionCodeOf(tcResp), PLDM_ERROR_INVALID_DATA)
        << "Sparse [512, 1024) coverage of a 1024-byte component must "
           "be rejected with PLDM_ERROR_INVALID_DATA. A high-water-mark "
           "tracker would have accepted this and let the attack succeed.";
}
...
TEST_F(PhaseTrackingTest, OverlappingRequestsCounted)
{
    ...
    EXPECT_EQ(completionCodeOf(tcResp), PLDM_ERROR_INVALID_DATA)
        << "Overlapping/duplicate requests covering only [0, 768) of a "
           "1024-byte component must be rejected with "
...
TEST_F(PhaseTrackingTest, OffsetOverflowDoesNotInflateCoverage)
{
    ...
    EXPECT_EQ(completionCodeOf(tcResp), PLDM_ERROR_INVALID_DATA)
        << "After only an overflow-prone near-UINT32_MAX request, "
           ...
           "must be rejected with PLDM_ERROR_INVALID_DATA.";
}
...
TEST_F(PhaseTrackingTest, RequestBeyondCompSizeIgnored)
{
    ...
    EXPECT_EQ(completionCodeOf(tcResp), PLDM_ERROR_INVALID_DATA)
        << "Coverage of [0,991) U [992,1024) is 1023 bytes -- one byte "
           ...
           "Rejection is PLDM_ERROR_INVALID_DATA (in-sequence command, "
           "false success claim).";
}

// ============================================================
// Backward phase-gate regression: after a successful transfer
// has advanced phase to Verify, a second TransferComplete must
// not be accepted again -- the phase gate at the top of
// transferComplete must fire with PLDM_FWUP_COMMAND_NOT_EXPECTED.
// Distinct from the coverage-failure path (in-sequence + false
// payload, PLDM_ERROR_INVALID_DATA): here the command itself
// arrived out of sequence.
// ============================================================

TEST_F(PhaseTrackingTest, TransferCompleteRejectedAfterPhaseAdvancedToVerify)
{
    auto updater = makeUpdater();
    advanceToDownload(updater);

    // Pull the full 1024 bytes in two halves (matches the normal-flow
    // pattern), then send a compliant TransferComplete(success). Phase
    // advances Download -> Verify.
    auto issueFwDataRequest = [&](uint32_t offset, uint32_t length,
                                  uint8_t instanceId) {
        std::vector<uint8_t> req(
            sizeof(pldm_msg_hdr) + sizeof(pldm_request_firmware_data_req), 0);
    ...
    EXPECT_EQ(completionCodeOf(issueFwDataRequest(0, 512, 0x70)), PLDM_SUCCESS);
    EXPECT_EQ(completionCodeOf(issueFwDataRequest(512, 512, 0x71)),
              PLDM_SUCCESS);

    auto firstReq =
        makeRequest(PLDM_TRANSFER_COMPLETE, 0x72, PLDM_FWUP_TRANSFER_SUCCESS);
    auto* firstMsg = reinterpret_cast<const pldm_msg*>(firstReq.data());
    auto firstResp = updater.transferComplete(firstMsg, 1);
    ASSERT_EQ(completionCodeOf(firstResp), PLDM_SUCCESS)
        << "First TransferComplete with full coverage must succeed; "
           "phase should advance Download -> Verify.";

    // Now in Verify. A second TransferComplete arriving here is a
    // backward-phase attack: the FD attempting to resend completion
    // for a phase the UA has already left. The phase gate must fire
    // with COMMAND_NOT_EXPECTED -- this is the true out-of-sequence
    // path, distinct from the in-sequence coverage-failure path.
    auto secondReq =
        makeRequest(PLDM_TRANSFER_COMPLETE, 0x73, PLDM_FWUP_TRANSFER_SUCCESS);
    auto* secondMsg = reinterpret_cast<const pldm_msg*>(secondReq.data());
    auto secondResp = updater.transferComplete(secondMsg, 1);
    EXPECT_EQ(completionCodeOf(secondResp), PLDM_FWUP_COMMAND_NOT_EXPECTED)
        << "TransferComplete arriving in Verify (phase already advanced) "
           "must be rejected by the phase gate with COMMAND_NOT_EXPECTED, "
           "not the coverage-failure path's PLDM_ERROR_INVALID_DATA.";
}
