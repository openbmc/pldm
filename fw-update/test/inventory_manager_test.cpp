...
TEST_F(InventoryManagerTest, handleQueryDownstreamIdentifierResponse)
{
    ...
    constexpr size_t respDataLength = respPayloadLength + downstreamDeviceLen;
    std::array<uint8_t, respDataLength> respDataBuf{0};
    auto respData = new (respDataBuf.data())
    ...
    auto rc = decode_query_downstream_identifiers_resp(
        responseMsg, respPayloadLength, respData, respDataLength, &deviceData,
        &total_descriptors);
    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(respData->transfer_flag, PLDM_START_AND_END);
