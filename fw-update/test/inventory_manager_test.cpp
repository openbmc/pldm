...
TEST_F(InventoryManagerTest, handleQueryDownstreamIdentifierResponse)
{
    ...
    rc = decode_query_downstream_identifiers_resp(
        responseMsg, queryDownstreamIdentifiersResp.size() - hdrSize,
        &resp_data,
        queryDownstreamIdentifiersResp.size() - hdrSize + sizeof(resp_data),
        &deviceData, &total_descriptors);
    EXPECT_EQ(rc, PLDM_SUCCESS);
    EXPECT_EQ(resp_data.transfer_flag, PLDM_START_AND_END);
