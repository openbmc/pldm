...
TEST_F(InventoryManagerTest, handleQueryDownstreamIdentifierResponse)
{
    ...
    std::array<uint8_t, respPayloadLength> respDataBuf;
    ...
    auto rc = decode_query_downstream_identifiers_resp(
        responseMsg, respPayloadLength, respData, respPayloadLength,
        &deviceData, &total_descriptors);
    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(resp_data.transfer_flag, PLDM_START_AND_END);
