...
class FirmwareInventoryManagerTest : public FirmwareInventoryManager
{
  public:
    ...
    FirmwareInventoryManagerTest(const Configurations& config,
                                 std::unique_ptr<BoardPathProvider> provider) :
        FirmwareInventoryManager(config, std::move(provider))
    {}
...
TEST(DeleteFirmwareEntry_RemovesEntry, goodtest)
{
    ...
    auto boardPathProviderMock = std::make_unique<BoardPathProviderTest>();
    FirmwareInventoryManagerTest inventoryManager(
        configurations, std::move(boardPathProviderMock));
