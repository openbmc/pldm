...
void DeviceUpdater::sendPassCompTableRequest(size_t offset)
{
    ...
    const auto compVersion = utils::toString(comp.component_version_string);
...
void DeviceUpdater::sendUpdateComponentRequest(size_t offset)
{
    ...
    CompIdentifier compIdentifier = comp.component_identifier;
    auto compImage =
        CompImage{comp.component_image.ptr,
                  comp.component_image.ptr + comp.component_image.length};
                                      ...
                                      size_t payloadLength)
{
    ...
    const auto compImagePtr = comp.component_image.ptr;
    const auto& compImageSize = comp.component_image.length;
    ...
    memcpy(response.data() + sizeof(pldm_msg_hdr) + sizeof(completionCode),
           compImagePtr + offset, length - padBytes);
    rc = encode_request_firmware_data_resp(
