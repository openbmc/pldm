...
void DeviceUpdater::startFwUpdateFlow()
{
    ...
    {
        error("Failed to allocate instance id in startFwUpdateFlow: {ERROR}",
              "ERROR", e.what());
    }
    // NumberOfComponents
...
void DeviceUpdater::sendPassCompTableRequest(size_t offset)
{
    ...
    {
        error(
            "Failed to allocate instance id in sendPassCompTableRequest: {ERROR}",
            "ERROR", e.what());
    }
    // TransferFlag
    const auto& applicableComponents =
...
void DeviceUpdater::sendUpdateComponentRequest(size_t offset)
{
    ...
    {
        error(
            "Failed to allocate instance id in sendUpdateComponentRequest: {ERROR}",
            "ERROR", e.what());
    }
    const auto& applicableComponents =
        std::get<ApplicableComponents>(fwDeviceIDRecord);
