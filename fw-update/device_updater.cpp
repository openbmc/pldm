    ...
    maxTransferSize(maxTransferSize), updateManager(updateManager)
{
    ...
    if (auto pRecord = std::get_if<FirmwareDeviceIDRecord>(&deviceIDRecord))
    {
        applicableComponents = std::get<ApplicableComponents>(*pRecord);
        fwDevicePkgData = std::get<FirmwareDevicePackageData>(*pRecord);
        compImageSetVersion = std::get<ComponentImageSetVersion>(*pRecord);
    }
    else if (auto pRecord =
    ...
    {
        error("Invalid device ID record type");
...
void DeviceUpdater::sendPassCompTableRequest(size_t offset)
{
    ...
    const auto& compTuple = compImageInfos[applicableComponents[offset]];
    const auto& comp = std::get<pldm_package_component_image_information>(compTuple);
    CompClassification compClassification = comp.component_classification;
    CompIdentifier compIdentifier = comp.component_identifier;
    ...
    CompComparisonStamp compComparisonStamp = comp.component_comparison_stamp;
    const auto& compVersion = std::get<CompVersion>(compTuple);
...
void DeviceUpdater::sendUpdateComponentRequest(size_t offset)
{
    ...
    CompClassification compClassification = comp.component_classification;
    CompIdentifier compIdentifier = comp.component_identifier;
    const auto& compImage = std::get<CompImage>(compTuple);
    auto compImageSize = compImage.size();
    // ComponentClassificationIndex
    CompClassificationIndex compClassificationIndex{};
    auto compKey = std::make_pair(compClassification, compIdentifier);
    if (compInfo.contains(compKey))
    ...
    updateOptionFlags.bits.bit0 = comp.component_options.value;
    const auto& compVersion = std::get<CompVersion>(compTuple);
    variable_field compVerStrInfo{};
    compVerStrInfo.ptr = reinterpret_cast<const uint8_t*>(compVersion.data());
    compVerStrInfo.length = static_cast<uint8_t>(compVersion.size());
    ...
    auto rc = encode_update_component_req(
        ...
        comp.component_comparison_stamp, compImageSize, updateOptionFlags,
                                      ...
                                      size_t payloadLength)
{
    ...
    const auto& compTuple = compImageInfos[applicableComponents[componentIndex]];
    const auto& compImage = std::get<CompImage>(compTuple);
    auto compImageSize = compImage.size();
