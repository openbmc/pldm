...
sdbusplus::message::object_path SoftwareUpdate::startUpdate(
    sdbusplus::message::unix_fd image, RequestedApplyTimes applyTime)
{
    ...
    info(
        "Sending init update event for EID {EID}, image fd {FD}, apply time {APPLYTIME}",
        "EID", updater.eid, "FD", image.fd, "APPLYTIME", applyTime);
    updater.sendInitUpdateEvent(std::move(image), applyTime);
