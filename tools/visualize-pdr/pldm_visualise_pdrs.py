...
def fetch_pdr(handle_number, entity_association_pdr, state_sensor_pdr,
              ...
              fail, hostname, uname, passwd, port):
...
def fork_process_for_pdr(hostname, uname, passwd, port):
    ...
    for i in range(1, 1000):
        pool.apply_async(fetch_pdr, args=(i, entity_association_pdr,
                         ...
                         fru_record_set_pdr, tl_pdr, fail_count,
                         hostname, uname, passwd, port))
    pool.close()
    pool.join()
    ...
    print("Number of Entity Association PDR's : ", len(entity_association_pdr.keys()))
