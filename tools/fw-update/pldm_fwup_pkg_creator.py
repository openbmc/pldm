...
def analyze_package(pkg_file_path):
    ...
    with open(pkg_file_path, "rb") as f:
        ...
        print(
            "  PackageHeaderFormatRevision: {} (DSP0267 v1.{}.0)".format(
        ...
        package_header_size = struct.unpack("<H", f.read(2))[0]
        print(
            "  PackageHeaderSize          : {} bytes".format(
                package_header_size
            )
        )
        ...
        print(
            "  ComponentBitmapBitLength   : {}".format(
                component_bitmap_bit_length
            )
        )
        ...
        pkg_ver_str = f.read(pkg_ver_str_len).decode("ascii", errors="replace")
        print(
            "  PackageVersionString       : '{}' (type={}, len={})".format(
        ...
        for dev_idx in range(device_id_record_count):
            ...
            f.read(1)  # ComponentImageSetVersionStringType
            ...
            print(
                "    DeviceUpdateOptionFlags  : 0x{:08X}".format(
                    dev_update_flags
                )
            )
            print(
                "    CompImgSetVersionString  : '{}'".format(comp_set_ver_str)
            )
            print("    ApplicableComponents     : {}".format(applicable))
            print("    FWDevicePkgDataLength    : {}".format(fw_pkg_data_len))
            if format_revision >= 4:
                print(
                    "    ReferenceManifestLength  : {}".format(
                        ref_manifest_len
                    )
                )
        ...
        if format_revision >= 2:
            ...
            downstream_count = struct.unpack("<B", f.read(1))[0]
            print(
                "  DownstreamDeviceIDRecordCount : {}".format(downstream_count)
            )
            ...
            for ds_idx in range(downstream_count):
                ...
                print(
                    "    DescriptorCount                  : {}".format(
                        descriptor_count
                    )
                )
                print(
                    "    DownstreamDeviceUpdateOptionFlags: 0x{:08X}".format(
                        ds_flags
                    )
                )
                print(
                    "    SelfContainedActivMinVerString   : '{}'".format(
                        ds_min_ver_str
                    )
                )
                print(
                    "    ApplicableComponents             : {}".format(
                        applicable
                    )
                )
                print(
                    "    DSDevicePkgDataLength            : {}".format(
                        ds_pkg_data_len
                    )
                )
                if format_revision >= 4:
        ...
        for comp_idx in range(component_image_count):
            ...
            stamp_note = (
                " (default/unused)" if comparison_stamp == 0xFFFFFFFF else ""
            )
            print(
        ...
        computed_header_checksum = (
            binascii.crc32(f.read(header_end_pos)) & 0xFFFFFFFF
        )
        header_ok = (
            "OK"
        ...
        if format_revision >= 4:
            ...
            computed_payload_checksum = (
                binascii.crc32(payload_data) & 0xFFFFFFFF
            )
            payload_ok = (
                "OK"
...
def main():
    ...
    analyze_parser = subparsers.add_parser(
        "analyze",
        help="Analyze and display PLDM FW update package header contents",
    )
