...
def extract_component_images(pkg_file_path, output_dir):
    ...
    with open(pkg_file_path, "rb") as f:
        ...
        for i in range(component_image_count):
            classification = struct.unpack("<H", f.read(2))[0]  # ComponentClassification
            identifier = struct.unpack("<H", f.read(2))[0]      # ComponentIdentifier
            f.read(4)  # ComponentComparisonStamp
            f.read(2)  # ComponentOptions
            f.read(2)  # RequestedComponentActivationMethod
            location_offset = struct.unpack("<I", f.read(4))[0]  # ComponentLocationOffset
...
def main():
    ...
    create_parser.add_argument("metadatafile", help="Path of metadata JSON file")
    create_parser.add_argument(
        "images",
        nargs="+",
        help=(
