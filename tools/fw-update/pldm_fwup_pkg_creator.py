...
descriptor_type_name_length = {
    ...
    0x0003: ["PnP Vendor ID", 3],
    0x0004: ["ACPI Vendor ID", 4],
    0x0005: ["IEEE Assigned Companmy ID", 3],
...
def write_fw_device_identification_area(
    pldm_fw_up_pkg, metadata, package_header_format_revision, component_bitmap_bit_length
):
    """
    ...
    for device in devices:
        ...
        if package_header_format_revision == 1:
            format_string = (
                "<HBIBBH"
...
def write_downstream_device_identification_area(
    ...
    if downstream_device_id_record_count > max_downstream_device_id_record_count:
        sys.exit(
            "ERROR: there can be only upto 255 entries in the                "
            " FirmwareDeviceIdentificationArea section"
        )
    ...
    for downstream_device in downstream_devices:
        ...
        for option in downstream_device["DownstreamDeviceUpdateOptionFlags"]:
            if option not in supported_downstream_device_update_option_flags:
                sys.exit("ERROR: unsupported DownstreamDeviceUpdateOptionFlag entry")
            downstream_device_update_option_flags[option] = 1
        downstream_device_record_length += 4

        # ComponentImageSetVersionStringType supports only ASCII for now
        downstream_device_self_contained_activation_min_version_string_type = string_types["ASCII"]
        downstream_device_record_length += 1
        ...
        if downstream_device_update_option_flags[0]:
            ...
            downstream_device_self_contained_activation_min_version_string = downstream_device[
                "DownstreamDeviceSelfContainedActivationMinVersionString"
            ]
            check_string_length(downstream_device_self_contained_activation_min_version_string)
            downstream_device_record_length += len(downstream_device_self_contained_activation_min_version_string)
            ...
            downstream_device_self_contained_activation_min_version_string_type = 0
        ...
        downstream_device_record_length += downstream_device_applicable_components_bitfield_length
        ...
        format_string = (
            ...
            + "s"
            + str(len(downstream_device_self_contained_activation_min_version_string))
            + "s"
        )
        pldm_fw_up_pkg.write(
            struct.pack(
                ...
                len(downstream_device_self_contained_activation_min_version_string),
                ...
                downstream_device_self_contained_activation_min_version_string.encode("ascii"),
            )
        )
        if downstream_device_update_option_flags[0]:
            pldm_fw_up_pkg.write(
...
def write_component_image_info_area(pldm_fw_up_pkg, metadata, package_header_format_revision, image_files):
    """
...
def main():
    ...
    try:
        with open(args.pldmfwuppkgname, "w+b") as pldm_fw_up_pkg:
            package_header_format_revision, component_bitmap_bit_length = write_pkg_header_info(
                pldm_fw_up_pkg, metadata
            )
            if package_header_format_revision not in expected_package_format_versions:
                sys.exit(
