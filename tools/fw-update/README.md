# PLDM Firmware Update Package Creator and Parser

This repository contains Python scripts for both creating and parsing PLDM (Platform Level Data Model) firmware update packages. These scripts adhere to the DSP0267 specification v1.1.0, specifically Section 7, and provide a comprehensive solution for generating and analyzing PLDM firmware update packages.

## PLDM Firmware Update Package Creator
...
## PLDM Firmware Update Package Parser

### Overview

The PLDM Firmware Update Package Parser is a Python script that allows you to parse the contents of a PLDM firmware update package and extract metadata information from it.

### Features

- Extracts metadata information from a PLDM firmware update package.
- Parses the package header information, firmware device identification area, and component image information area.
- Supports various data types, such as ASCII and UTF-8 strings.
- Outputs the parsed metadata in JSON format.

### Requirements

- Python 3.x
- Python argparse module
	- Install using: sudo pip3 install argparse (example for Ubuntu)
- Python enum module
	- Install using: sudo pip3 install enum34 (example for Ubuntu)
- Python bitarray module
	- Install using: sudo pip3 install bitarray (example for Ubuntu)
	
### Usage

	python pldm_fwup_pkg_parser.py [-h]
                                        pldm_fw_up_pkg output_json

	Positional Arguments:
		pldm_fw_up_pkg      Path to the PLDM firmware update package you want to parse.
		output_json         Desired path for the output JSON file.
		
After the script finishes execution, the output JSON file will be generated at the specified path.