...
- FirmwareDeviceIDRecords: which is a list of Individual Firmware Device ID
  ...
  - ReferenceManifestLength: Supported, generated by the script (PackageHeaderFormatVersion >= 4)
  ...
  - ReferenceManifestData: Supported (PackageHeaderFormatVersion >= 4), optional field
...
- DownstreamDeviceIDRecords: which is a list of Individual Downstream Device ID
  ...
  - DownstreamDeviceReferenceManifestLength: Supported, generated by the script (PackageHeaderFormatVersion >= 4)
  ...
  - DownstreamDeviceReferenceManifestData: Supported (PackageHeaderFormatVersion >= 4), optional field
    - If specified in metadata file, must be provided as a hex string
    - Example: "DownstreamDeviceReferenceManifestData": "AABBCCDDEEFF00112233445566778899"
...
The following tools are provided to verify and analyze PLDM firmware update packages:

### verify_pldm_package.py

A simple checksum verification tool that validates both header and payload checksums.

**Usage:**
```bash
./verify_pldm_package.py <package_file>
```

**Example:**
```bash
./verify_pldm_package.py fw_package.pkg
```

**Output:**
- Package format version
- Package header size
- Header checksum (stored vs calculated)
- Payload checksum (stored vs calculated)
- Verification result (PASSED/FAILED)

### parse_pldm_package.py

A comprehensive package parser and analyzer that displays detailed information about all fields in the PLDM package.

**Usage:**
```bash
./parse_pldm_package.py <package_file> [--verbose]
```

**Options:**
- `--verbose, -v`: Show detailed byte-level information including hex dumps

**Example:**
```bash
# Basic parsing
./parse_pldm_package.py fw_package.pkg

# Detailed parsing with hex dumps
./parse_pldm_package.py fw_package.pkg --verbose
```

**Output includes:**
- Package Header Information (UUID, version, release date, etc.)
- Firmware Device ID Records (descriptors, applicable components, version strings)
- Downstream Device ID Records (if present)
- Component Image Information (classification, location, size, version)
- Checksum verification results
- Optional: Hex dumps of all data fields

### Additional Documentation

- **CHECKSUM_VERIFICATION.md**: Detailed guide on verifying CRC32 implementation compatibility between Python and C
- **VERIFICATION_GUIDE.md**: Step-by-step guide for package verification workflow

For more information on the verification process, see `CHECKSUM_VERIFICATION.md`.
