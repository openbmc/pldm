    ...
    for (auto& [tid, terminus] : termini)
    {
        ...
        if (terminus->doesSupport(PLDM_PLATFORM))
        {
            ...
            if (eventMessageGlobalEnable != PLDM_EVENT_MESSAGE_GLOBAL_DISABLE)
            {
                ...
                if (rc)
                {
                    error("setEventReceiver failed, rc={RC}", "RC",
                          static_cast<int>(rc));
                }
            }
        }
...
exec::task<int> PlatformManager::getPDRs(std::shared_ptr<Terminus> terminus)
{
    ...
    do
    {
        ...
        {
            error(
                "getPDRs() failed to get fist part of record handle {RECORD} from terminus {TID} with return code {RC}",
                "RECORD", static_cast<int>(recordHndl), "TID",
                static_cast<int>(tid), "RC", static_cast<int>(rc));
            terminus->pdrs.clear();
            co_return rc;
        }
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    error(
                        "getPDRs() failed to get middle part of record handle {RECORD} from terminus {TID} with return code {RC}",
                        "RECORD", static_cast<int>(recordHndl), "TID",
                        static_cast<int>(tid), "RC", static_cast<int>(rc));
                    terminus->pdrs.clear();
                    co_return rc;
