...
#define DEFAULT_MESSAGE_BUFFER_SIZE 256
    ...
    for (auto& [tid, terminus] : termini)
    {
        ...
        if (terminus->doesSupportCommand(PLDM_PLATFORM, PLDM_GET_PDR))
        {
            ...
            // Continue to next steps although the parsing PDR is failed at some
            // records
            if (!terminus->parseTerminusPDRs())
            {
                lg2::error("Failed to parse PDRs for terminus with TID: {TID}",
                           "TID", tid);
            }
        }
...
exec::task<int> PlatformManager::getPDRs(std::shared_ptr<Terminus> terminus)
{
    ...
    constexpr uint16_t recvBufSize =
        MAXIMUM_RECEIVE_SIZE - sizeof(pldm_msg_hdr) - sizeof(pldm_pdr_hdr);
