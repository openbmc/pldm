...
exec::task<int> PlatformManager::initTerminus()
{
    ...
    for (const auto& tid : tids)
    {
        ...
        {
            lg2::info("Terminus {TID} removed before initialization, skipping.", "TID", tid);
            continue;
        }
                                         ...
                                         PLDM_GET_FRU_RECORD_TABLE_METADATA))
        {
            ...
            if (!termini.contains(tid))
            {
                lg2::error("Terminus {TID} removed during getFRURecordTableMetadata", "TID", tid);
                continue;
            }
            ...
            terminus->doesSupportCommand(PLDM_FRU, PLDM_GET_FRU_RECORD_TABLE))
        {
            ...
            if (!termini.contains(tid))
            {
                lg2::error("Terminus {TID} removed during getFRURecordTables", "TID", tid);
                continue;
            }
        ...
        if (fruData.size())
        {
            if (!termini.contains(tid))
            {
                lg2::error("Terminus {TID} removed during eventMessageBufferSize", "TID", tid);
                continue;
            }
        ...
        if (!termini.contains(tid)) continue;
