...
exec::task<int> PlatformManager::initTerminus()
{
    for (auto& [tid, terminus] : termini)
    {
        ...
        /* Need terminus name from PDRs before creating Fru Inventory object */
        if (fruData.size())
        {
            createGernalFruDbus(tid, fruData.data(), fruData.size());
        }
    ...
    pldm_tid_t tid, const uint8_t* fruData, const size_t fruLen)
{
    ...
    termini[tid]->createGernalFruDbus(fruData, fruLen);
}
