...
struct StateSensorInfo
{
    ...
    /** @brief State set ID and supported state values of each component
     *         sensor, indexed by its composite sensor offset
     */
    std::vector<std::pair<StateSetId, PossibleStates>> componentInfo;
};
...
class StateSensor
{
  ...
  private:
    ...
    /** @brief The state set interface of each component sensor, indexed by
     *         its composite sensor offset, nullptr when the state set has no
     *         D-Bus interface. The interfaces are owned by the StateSets of
     *         the entity, which outlives the sensor.
     */
