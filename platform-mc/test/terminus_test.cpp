...
TEST(TerminusTest, parseSensorAuxiliaryNamesPDR_TruncatedBodyRejected)
{
    // Attack: the generic pldm_pdr_hdr is complete, so the outer
    // header guard passes, but the PDR body is truncated before the
    // fixed sensor-aux prefix (terminus_handle + sensor_id +
    // sensor_count) is complete. parseSensorAuxiliaryNamesPDR must
    // reject it on the offsetof(names) length check rather than read
    // sensor_count past the end.
    auto event = sdeventplus::Event::get_default();
    auto t1 = pldm::platform_mc::Terminus(
        1, 1 << PLDM_BASE | 1 << PLDM_PLATFORM, event);
    std::vector<uint8_t> maliciousPdr{
        ...
        0x0,                             // recordChangeNumber
        0x0,
        0x0,                             // dataLength (completes pldm_pdr_hdr)
        0x0,
        0x0 // terminus_handle only — sensor_id / sensor_count missing
    };
    t1.pdrs.emplace_back(maliciousPdr);
    EXPECT_NO_THROW(t1.parseTerminusPDRs());
