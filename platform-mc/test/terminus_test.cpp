...
TEST(TerminusTest, parseEntityAssociationPDRTest)
{
    ...
    std::vector<uint8_t> unmappedContainerPdr{
        0x5, 0x0, 0x0,
        0x0,                         // record handle
        0x1,                         // PDRHeaderVersion
        PLDM_PDR_ENTITY_ASSOCIATION, // PDRType
        ...
        0x1,  // associationType physical
        0xff,
        0xff, // container entityType with no Item interface
        0x1,
        0x0,  // container entity instance number = 1
        0x1,
    ...
    t1.pdrs.emplace_back(unmappedContainerPdr);
    t1.parseTerminusPDRs();
    ...
    /* A container with no entity D-Bus object is replaced by the terminus
     * inventory path for an entity type which the terminus may contain
     * directly
     */
    auto containedByTerminusEntity = t1.getEntity({PLDM_ENTITY_PROC, 1, 3});
    ...
    ASSERT_NE(nullptr, hiddenEntity);
    EXPECT_TRUE(hiddenEntity->getContainers().empty());
}
...
TEST(TerminusTest, pldmEntityContainedByTerminusTest)
{
    /* A memory controller is contained by a logical processor which has no
     * D-Bus object, so the terminus contains it directly
     */
    EXPECT_TRUE(pldm::dbus_api::isPldmEntityContainedByTerminus(
        PLDM_ENTITY_MEMORY_CONTROLLER));
    EXPECT_FALSE(
        pldm::dbus_api::isPldmEntityContainedByTerminus(PLDM_ENTITY_FAN));
    EXPECT_FALSE(pldm::dbus_api::isPldmEntityContainedByTerminus(0xFFFF));
}
