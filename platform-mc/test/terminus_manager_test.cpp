...
{
    // lookup a unmapped EID to TID
    auto mappedTid = terminusManager.toTID(1);
    EXPECT_EQ(mappedTid, std::nullopt);

    // map null EID(0)
    mappedTid = terminusManager.mapToTID(0);
    EXPECT_EQ(mappedTid, std::nullopt);

    // map booadcast EID(0xff)
    mappedTid = terminusManager.mapToTID(0xff);
    EXPECT_EQ(mappedTid, std::nullopt);

    // lookup reserved TID(0)
    auto mappedEid = terminusManager.toEID(0);
    EXPECT_EQ(mappedEid, std::nullopt);

    // lookup reserved TID(0xff)
