...
TEST(StateSensor, pathAndFunctional)
{
    ...
    // Constructed disabled and reachable, no reading yet
    EXPECT_FALSE(sensor.functional());
    EXPECT_TRUE(sensor.available());
    EXPECT_FALSE(sensor.getReading().has_value());

    // Enabled -> functional and reachable, raw wire values stored
    ...
    // A responded reading with a non-enabled opState -> reachable but not
    // functional
...
TEST(StateSensor, handleErrGetStateSensorReadings)
{
    ...
    // A successful read marks the component reachable
    sensor.updateReading(PLDM_SENSOR_ENABLED, 1, 2);
    EXPECT_TRUE(sensor.functional());
    EXPECT_TRUE(sensor.available());
    ASSERT_TRUE(sensor.getReading().has_value());

    // A failed read marks the component unavailable and drops the stored
    // state; Functional stays driven by the last sensorOpState, not the
    // communication error
