    ...
    Terminus::findTerminusName()
{
    ...
    if (it != entityAuxiliaryNamesTbl.end())
    {
        ...
        return std::pair<std::string_view, uint16_t>{entityNames[0].second,
                                                     key.type};
    }
...
bool Terminus::createInventoryPath(std::string tName, uint16_t entityType)
{
    ...
    inventoryPath = std::format("/xyz/openbmc_project/inventory/system/{}/{}",
                                segment, tName);
    try
    {
    ...
    {
        lg2::error("Failed to create Inventory interface for device {PATH}",
                   "PATH", inventoryPath);
    }
