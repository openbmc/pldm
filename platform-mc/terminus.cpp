    ...
    const std::shared_ptr<pldm_compact_numeric_sensor_pdr> pdr)
{
    ...
    {
        sensorName = "PLDM_Device_Sensor_" + std::to_string(sensorId) + "_" +
                     std::to_string(tid);
    }
    ...
    try
    {
        ...
        auto ptData = reinterpret_cast<uint8_t*>(&pdrData);
        auto vPdrData = std::vector<uint8_t>(ptData, ptData + sizeof(pdrData));
        std::shared_ptr<SensorIntf> sensor =
            std::make_shared<CompactNumericSensor>(
                tid, true, vPdrData, sensorId, sensorName, inventoryPath);
        sensorObjects.emplace_back(sensor);
    }
    catch (const std::exception& e)
    ...
    const std::shared_ptr<pldm_numeric_effecter_value_pdr> pdr)
{
    ...
    {
        sensorName =
            mctpMedium + "_" + "PLDM_Effecter_" + std::to_string(sensorId);
    }
    else
    {
        sensorName = "PLDM_Effecter_" + std::to_string(sensorId) + "_" +
                     std::to_string(tid);
    }
    ...
    try
    {
        ...
        auto vPdrData = std::vector<uint8_t>(ptData, ptData + sizeof(pdrData));
        std::shared_ptr<SensorIntf> sensor =
            std::make_shared<NumericEffecterDbus>(tid, true, vPdrData, sensorId,
