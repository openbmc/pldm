...
std::shared_ptr<pldm_platform_file_descriptor_pdr> Terminus::parseFileDescriptorPDR(
    const std::vector<uint8_t>& pdrData)
{
    const uint8_t* ptr = pdrData.data();
    auto parsedPdr = std::make_shared<pldm_platform_file_descriptor_pdr>();
    auto rc =
        decode_pldm_platform_file_descriptor_pdr(ptr, pdrData.size(), parsedPdr.get());
    if (rc)
    {
        return nullptr;
    }
