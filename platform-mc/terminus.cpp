...
Terminus::Terminus(mctp_eid_t eid, uint8_t tid, sdeventplus::Event& event,
                   requester::Handler<requester::Request>& handler,
                   dbus_api::Requester& requester, uint64_t& supportedTypes) :
    eid(eid),
...
std::tuple<SensorId, uint8_t, std::vector<std::pair<std::string, std::string>>>
    Terminus::parseSensorAuxiliaryNamesPDR(const std::vector<uint8_t>& pdrData)
{
    ...
    return std::make_tuple(pdr->sensor_id, pdr->sensor_count, nameStrings);
}
