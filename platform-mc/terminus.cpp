    ...
    Terminus::parseEntityAuxiliaryNamesPDR(const std::vector<uint8_t>& pdrData)
{
    ...
    auto decodedPdr = (struct pldm_entity_auxiliary_names_pdr*)vPdr.data();
    ...
    decodedPdr->names = (struct pldm_entity_auxiliary_name*)vNames.data();
         ...
         std::views::iota(0, static_cast<int>(decodedPdr->name_string_count)))
    {
        ...
        const size_t u16NameStringLen =
            pldm::utils::str16len(decodedPdr->names[count].name);
