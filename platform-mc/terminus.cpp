...
int Terminus::fetchEntityManagerConfiguration()
{
    ...
    for (const auto& [objPath, interfaces] : objects)
    {
        ...
        {
            // -------- Endpoint level --------
            if (std::find(interfaceFilter.begin(), interfaceFilter.end(), ifName) != interfaceFilter.end())
            {
                lg2::info("Found interface {IF} on path {PATH}", "IF", ifName, "PATH", objPath.str);
                auto invIt = props.find("Inventory");

                if (invIt != props.end() && std::holds_alternative<std::string>(invIt->second))
                {
                    lg2::info("Setting inventory name from Entity Manager: {NAME} on {PATH}",
                               "NAME", std::get<std::string>(invIt->second), "PATH", objPath.str);
            ...
            {
                lg2::warning(
                    "SensorsMap missing Name or SensorID on {PATH}",
                    "PATH", objPath.str);
                continue;
            }
            ...
            {
                lg2::warning(
                    "SensorsMap invalid property type on {PATH}",
                    "PATH", objPath.str);
                continue;
            }
            ...
            {
                lg2::warning(
                    "SensorID overflow: {SID} on {PATH}",
                    "SID", sensorId, "PATH", objPath.str);
                continue;
            }
            ...
            if (!updated)
            {
                ...
                lg2::info(
                    "EM sensor rename applied: SID={SID}, Name={NAME}",
                    "SID", sensorId, "NAME", name);
            }
        }
    }
