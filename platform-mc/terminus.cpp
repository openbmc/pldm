...
bool Terminus::createSoftwareInventoryPath(std::string tName)
{
    ...
    try
    {
        ...
        softwareInventoryIntf->purpose(dbus_api::versionserver::VersionPurpose::Other);
        return true;
    }
    catch (const sdbusplus::exception_t& e)
    {
        lg2::error(
            "Failed to create Software Inventory interface for device {PATH}",
            "PATH", softwareInventoryPath);
    }
                                      ...
                                      const size_t fruLen)
{
    ...
    {
        lg2::info("Terminus ID {TID}: Created SoftwareInventory path {PATH}.", "TID",
                   tid, "PATH", static_cast<std::string>(tmp.value()));
    }
