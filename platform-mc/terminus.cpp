...
Terminus::Terminus(pldm_tid_t tid, uint64_t supportedTypes,
    ...
    supportedTypes(supportedTypes), event(event), terminusManager(terminusManager)
{}
    ...
    Terminus::parseFileDescriptorPDR(const std::vector<uint8_t>& pdrData)
{
    ...
    auto rc = decode_pldm_file_descriptor_pdr(ptr, pdrData.size(), parsedPdr.get());
    if (rc)
    {
        return nullptr;
    }
...
void Terminus::constructFilePathRecursive(std::shared_ptr<FileInfoStruct> file, const sdbusplus::message::object_path& basePath)
{
    file->objPath = basePath;
    std::string cleanName = std::regex_replace(file->name, std::regex("[^a-zA-Z0-9_/]+"), "_");
...
void Terminus::buildFileHierarchy()
{
    ...
    // Construct a virtual top most file to the file info map, use 0xffff as its key and its parent
    std::map<uint16_t, std::shared_ptr<FileInfoStruct>> fileInfoMap;
    const uint16_t topMostId = 0xffff;
    auto topMostInfo = std::make_shared<FileInfoStruct>(FileInfoStruct{terminusName, topMostId, {}, nullptr, {}});
    ...
    for (const auto& pdr : fileDescriptorPdrs)
    {
        ...
        std::string fileName((const char*)(pdr->file_name.ptr), pdr->file_name.length - 1);
        lg2::debug("Adding file {FILE} to file info map.", "FILE", fileName);
        auto fileInfo = std::make_shared<FileInfoStruct>(FileInfoStruct{fileName, pdr->superior_directory_file_identifier, {}, pdr, {}});
    ...
    const sdbusplus::message::object_path basePath("/xyz/openbmc_project/file/");
    ...
    for (const auto& file : fileInfoMap)
    {
        ...
        if (static_cast<FileClassification>(fileInfo->pdr->file_classification)
        ...
        try
        {
            auto fileObject = std::make_shared<FileDescriptor>(bus, fileInfo->objPath, tid, false, fileInfo->pdr, terminusManager);
            lg2::info("Created File Descriptor object {NAME}", "NAME", fileInfo->name);
