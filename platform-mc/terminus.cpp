...
void Terminus::parseTerminusPDRs()
{
    ...
    auto assocPath = getAssociatedPath();
    if (assocPath && !assocPath->empty())
    {
        std::string pathStr(*assocPath);
        sdbusplus::object_path path(pathStr);
        inventoryPath = path.parent_path().str;
        lg2::info(
            "Terminus ID {TID}: Using associated EM inventory path {PATH}.",
    ...
    else
    {
        ...
        {
            lg2::info("Terminus ID {TID}: Created Inventory path {PATH}.",
                      "TID", tid, "PATH", inventoryPath);
        }
    }
