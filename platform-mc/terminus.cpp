...
bool Terminus::createInventoryPath(std::string tName, uint16_t entityType)
{
    ...
    try
    {
        ...
        terminusInventoryItemIntf = std::make_shared<InventoryItemIntf>(
            utils::DBusHandler::getBus(), inventoryPath.c_str());
        terminusInventoryItemIntf->prettyName(tName);
        terminusInventoryItemIntf->present(true);
        terminusStateSets = std::make_shared<StateSets>(
            inventoryPath, terminusInventoryItemIntf);
        return true;
    }
                                    ...
                                    std::string_view typeName)
{
    ...
    /* The entity instance number is unique within the container of the
     * entity, not within the terminus, so the container ID is part of the
     * name.
     */
    return std::format("Terminus_{}_{}_{}_{}", tid, typeName, key.instanceIdx,
                       key.containerId);
}
...
void Terminus::addEntityAssociations()
{
    ...
    for (const auto& association : entityAssociations)
    {
        ...
        {
            /* The overall terminus entity is exposed by the terminus
             * inventory path.
             */
        ...
        for (const auto& child : association.children)
        {
            ...
            /* A container with no D-Bus object hides the entities it
             * contains. An entity type which the terminus may contain
             * directly is contained by the terminus inventory path instead.
             */
            const std::string& childContainerPath =
                containerPath.empty() &&
                        pldm::dbus_api::isPldmEntityContainedByTerminus(
                            child.type)
                    ? inventoryPath
                    : containerPath;

            if (childContainerPath.empty())
            {
                continue;
            }
