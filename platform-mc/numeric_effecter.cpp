...
NumericEffecter::NumericEffecter(
    ...
    name(effecterName), tid(tid), terminusManager(terminusManager), pdr(pdr)
{
    if (!pdr)
    {
                                  ...
                                  double pendingValue, double presentValue)
{
    ...
    for (auto& intf : interfaces)
    {
        if (intf)
        {
            ...
            {
                lg2::error(
                    "Exception in effecter interface for {NAME}: {ERROR}",
                    "NAME", name, "ERROR", e.what());
            }
        }
    }
...
sdbusplus::async::task<int> NumericEffecter::setNumericEffecterEnable(
    pldm_effecter_oper_state state)
{
    ...
    struct pldm_platform_set_numeric_effecter_enable_req req = {
        ...
        .effecter_operational_state = static_cast<uint8_t>(state)};
...
sdbusplus::async::task<int> NumericEffecter::setNumericEffecterValue(
    double effecterValue)
{
    Request request(sizeof(pldm_msg_hdr) +
                    PLDM_PLATFORM_SET_NUMERIC_EFFECTER_VALUE_MAX_REQ_BYTES);
    auto requestMsg = reinterpret_cast<pldm_msg*>(request.data());
...
sdbusplus::async::task<int> NumericEffecter::getNumericEffecterValue()
{
    Request request(
