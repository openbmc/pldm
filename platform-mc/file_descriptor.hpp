...
class FileDescriptor : virtual public FileInterface
{
  ...
  private:
     ...
     *  @return coroutine task returning int
     */
    exec::task<int> readFileBySection(const FD& fd, const SectionOffset& offset,
     ...
     *  @return coroutine task returning int
     */
    exec::task<int> readFileByLength(const FD& fd, const size_t& offset,
