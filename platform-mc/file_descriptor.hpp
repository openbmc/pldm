...
#include <xyz/openbmc_project/Common/File/common.hpp>
 ...
 *
 * This class represents a File Descriptor object introduced by File Descriptor PDR
 * to D-Bus.
 */
class FileDescriptor : virtual public FileInterface
{
  ...
  private:
    ...
    // [ChauLy] TODO: support multiple socket pairs based on FileMaximumFileDescriptorCount
    int sockets[2];

    void setPurpose(const FileClassification& classification);
    exec::task<int> writeDataToSocket(const uint32_t& dataSize, const uint8_t* const data,
    ...
    exec::task<int> readFile(const uint16_t& fd, const uint32_t& offset, const uint32_t& readSize);
    exec::task<int> readTask(uint32_t offset, uint32_t length, bool exclusivity);
    sdbusplus::message::unix_fd read(uint32_t offset, uint32_t length, bool exclusivity) override;
