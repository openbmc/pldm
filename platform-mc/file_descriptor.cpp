...
using PurposeType =
    sdbusplus::common::xyz::openbmc_project::common::File::PurposeType;
...
exec::task<int> FileDescriptor::readFile(const FD& fd, const size_t& offset,
                                         ...
                                         const size_t& length, bool exclusivity)
{
    ...
    if (rc == PLDM_SUCCESS)
    {
        ...
        if (fileSize)
        {
            ...
            rc = co_await readFile(curFd, offset, readLength);
            if (rc)
            {
                lg2::error(
