...
FileDescriptor::FileDescriptor(sdbusplus::bus_t& bus, const std::string& path,
    ...
    std::shared_ptr<pldm_file_descriptor_pdr> pdr, TerminusManager& terminusManager) :
    FileInterface(bus, path.c_str()), tid(tid), terminusManager(terminusManager)
{
    ...
    std::string fileName((const char*)(pdr->file_name.ptr), pdr->file_name.length);
    name(fileName);
    source(SourceType::System);
    if (pdr->oem_file_classification)
    {
        oemClassName.assign((const char*)(pdr->oem_file_classification_name.ptr), pdr->oem_file_classification_name.length);
    }
...
exec::task<int> FileDescriptor::writeDataToSocket(const uint32_t& dataSize, const uint8_t* const data,
                                                const int& socket, const int& maxFd)
{
    ...
    while (remaining > 0)
    {
        if (retries == 5)
        {
            lg2::error("Failed to write to output socket for terminus ID {TID}, FileIdentifier {ID} after {RETRY} retries.",
        ...
        if (retval < 0)
        {
            lg2::error("Failed to select output socket for terminus ID {TID}, FileIdentifier {ID}, error number - {ERR}.", "TID", tid, "ID", identifier, "ERR", errno);
            co_return PLDM_ERROR;
        }
        if (retval == 0)
        {
            lg2::error("Select timeout for terminus ID {TID}, FileIdentifier {ID}, error number - {ERR}.",
        ...
        if ((retval > 0) && (FD_ISSET(socket, &wfd)))
        {
            ...
            if (bytes < 0)
            {
                lg2::error("Failed to write to output socket for terminus ID {TID}, FileIdentifier {ID}, error number - {ERR}.",
                            "TID", tid, "ID", identifier, "ERR", errno);
        ...
        {
            lg2::debug("Write socket FD is not set for terminus ID {TID}, FileIdentifier {ID}.", "TID", tid, "ID", identifier);
            retries++;
            continue;
        }
    }
...
exec::task<int> FileDescriptor::readFileToSocket(const FD& fd, const SectionOffset& offset,
                                                const SectionOffset& length, std::vector<uint8_t>& readBuffer)
{
    int rc;
    ...
    while (!isEnd)
    {
        ...
        rc = co_await sendMultipartReceive(
            terminusManager, tid,
            transferOperation, transferCtx, dataTransferHandle,
            requestSectionOffset, requestSectionLength, completionCode,
            transferFlag, nextDataTransferHandle, buffer,
            dataIntegrityChecksum);
        if (rc || completionCode)
        {
            lg2::error("Failed to send MultipartReceive request for {LEN} bytes at offset {OFF} for TID {TID}, file descriptor {FD} with error {RC}, complete code {CC}",
                        "LEN", requestSectionLength, "OFF", requestSectionOffset, "TID", tid, "FD", fd, "RC", rc, "CC", completionCode);
            co_return rc;
        }
        ...
        if (buffer.size() > 0)
        {
            ...
            recvDataSize += buffer.size();
        }
        
        auto calculatedCRC = pldm_edac_crc32(readBuffer.data(), readBuffer.size());
        if (calculatedCRC != dataIntegrityChecksum)
        {
            lg2::info("Checksum mismatched while sending MultipartReceive request for TID {TID}, file descriptor {FD}.\nRe-requesting current part",
                    "TID", tid, "FD", fd);
        ...
        if (transferFlag != PLDM_BASE_MULTIPART_RECEIVE_TRANSFER_FLAG_ACK_COMPLETION)
        {
            rc = co_await writeDataToSocket(buffer.size(), buffer.data(), sockets[0], maxFd);
            if (rc)
            {
                lg2::error("Failed to write part data of {LEN} bytes to the output socket {SOCKET} with error {RC}",
        ...
        switch (transferFlag)
        {
            ...
            case PLDM_BASE_MULTIPART_RECEIVE_TRANSFER_FLAG_ACK_COMPLETION:
                ...
                break;
            default:
                lg2::error("Received bad transfer flag {FLAG} while sending MultipartReceive request for TID {TID}, file descriptor {FD}.",
...
exec::task<int> FileDescriptor::readFile(const uint16_t& fd, const uint32_t& offset, const uint32_t& readSize)
{
    ...
    if (isRegular)
    {
        ...
        if (rc)
        {
            lg2::error("Failed to read {LEN} bytes at offset {OFF} from terminus ID {TID}, file descriptor {FD}, error {RC}.",
    ...
    else
    {
        ...
        rc = co_await negotiateTransferParameters(terminusManager, tid, reqPartSize, reqProtocol,
                                                respPartSize, respProtocol);
        if (rc)
        {
            respPartSize = 0x100;
            lg2::info("Failed to negotiate part size {PARTSIZE} for terminus ID {TID}, use {DEFAULT}", "PARTSIZE", reqPartSize, "TID", tid, "DEFAULT", respPartSize);
        }
        ...
        while ((remaining > 0) && !lastRead)
        {
            ...
            if (rc)
            {
                lg2::error("Failed to read {LEN} bytes at offset 0 from terminus ID {TID}, file descriptor {FD}, error {RC}.",
...
exec::task<int> FileDescriptor::readTask(uint32_t offset, uint32_t length, bool exclusivity)
{
    int rc;
    uint16_t curFd;
    ...
    if (rc == PLDM_SUCCESS)
    {
        ...
        rc = co_await dfHeartbeat(
            terminusManager, tid, curFd, requesterMaxInterval, responderMaxInterval);

        if (rc == PLDM_SUCCESS)
        {
            // [ChauLy] TODO: Support getting file size from File Size Numeric Sensor
            rc = co_await readFile(curFd, offset, length);
            if (rc)
            {
                lg2::error("Failed to read {LEN} bytes at offset {OFF} for terminus ID {TID}, FileIdentifier {ID}, error {RC}.",
                            "LEN", length, "OFF", offset, "TID", tid, "ID", identifier, "RC", rc);
            }
        }
        else
        {
            lg2::error("Failed to get file size from terminus ID {TID}, FileIdentifier {ID}, error {RC}.",
                       "TID", tid, "ID", identifier, "RC", rc);
        }
    }
    else
    {
        lg2::error("Failed to open file session from terminus ID {TID}, FileIdentifier {ID}, error {RC}.",
                    "TID", tid, "ID", identifier, "RC", rc);
        co_return rc;
    }
    ...
    if (rc)
    {
        lg2::error("Failed to close file session from terminus ID {TID}, FileIdentifier {ID}, error {RC}.",
                   "TID", tid, "ID", identifier, "RC", rc);
    }
    co_return rc;
}

sdbusplus::message::unix_fd FileDescriptor::read(uint32_t offset, uint32_t length, bool exclusivity)
{
    if (isDirectory)
    {
        elog<NotAllowed>(NotAllowedMetaData::REASON("Reading a directory is not allowed"));
    }
    ...
    if (exclusivity)
    {
        ...
        {
            elog<NotAllowed>(NotAllowedMetaData::REASON("File does not support exclusive read"));
        }
    }

	/* Create a socketpair */
    if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, sockets) == -1) {
        lg2::error("Failed to setup socketpair for terminus ID {TID}, FileIdentifier {ID}, error number {ERR}.",
                    "TID", tid, "ID", identifier, "ERR", errno);
        elog<InternalFailure>();
    }

    // [ChauLy] TODO: Support multiple task handles based on FileMaximumFileDescriptorCount
    if (taskHandle.has_value()) 
    {
        auto& [scope, rcOpt] = *taskHandle;
        if (!rcOpt.has_value())
        {
            lg2::error("Failed to read from terminus ID {TID}, FileIdentifier {ID} - file is busy.",
    ...
    scope.spawn(readTask(offset, length, exclusivity) |
                    stdexec::then([&](int rc) {
                        lg2::info("Read finishes for terminus ID {TID}, FileIdentifier {ID}, return code {RC}.",
                                   "TID", tid, "ID", identifier, "RC", rc);
                        rcOpt.emplace(rc);
                        close(sockets[0]);
                        close(sockets[0]);
