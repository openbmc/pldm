...
exec::task<int> TerminusManager::initMctpTerminus(const MctpInfo& mctpInfo)
{
    ...
    while ((type < PLDM_MAX_TYPES))
    {
        ...
        if (!terminus)
        {
            lg2::error("Terminus {TID} removed during discovery. Aborting.",
                       "TID", tid);
            co_return PLDM_ERROR;
        }
        ...
        {
            lg2::error("Terminus {TID} removed from map during version fetch.",
                       "TID", tid);
            co_return PLDM_ERROR;
        }
    ...
    {
        lg2::warning("Terminus {TID} removed before final configuration.",
                     "TID", tid);
        co_return PLDM_ERROR;
    }
