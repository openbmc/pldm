...
exec::task<int> TerminusManager::getTidOverMctp(mctp_eid_t eid, pldm_tid_t* tid)
{
    ...
    {
        lg2::error("Failed to allocate instance id in getTidOverMctp: {ERROR}",
                   "ERROR", e.what());
        co_return PLDM_ERROR;
    }
...
exec::task<int> TerminusManager::setTidOverMctp(mctp_eid_t eid, pldm_tid_t tid)
{
    ...
    {
        lg2::error("Failed to allocate instance id in setTidOverMctp: {ERROR}",
                   "ERROR", e.what());
        co_return PLDM_ERROR;
    }
    ...
    size_t* responseLen)
{
    ...
    {
        lg2::error("Failed to allocate instance id in sendRecvPldmMsg: {ERROR}",
                   "ERROR", e.what());
        co_return PLDM_ERROR_NOT_READY;
    }
