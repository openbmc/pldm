...
void TerminusManager::discoverTerminus(const MctpInfos& mctpInfos)
{
    _mctpInfos = mctpInfos;
    ...
    sdeventplus::source::EventBase& /* source */)
{
    ...
    {
        auto mctpInfo = _mctpInfos.back();
        _mctpInfos.pop_back();
        sendGetTID(mctpInfo.first);
    }
    else
    {
                                       ...
                                       size_t respMsgLen)
{
    ...
    if (rc)
    {
        std::cerr << "decode_get_tid_resp response failed, EID="
                  << unsigned(eid) << ", RC=" << rc << "\n";
        return scanTerminus();
    }
    ...
    if (!assignedTid)
    {
        ...
        if (!ret)
        {
            std::cerr << "mapTID failed to map TID for EID=" << unsigned(eid)
                      << "\n";
            return scanTerminus();
        }
        assignedTid = ret.value();
    }
    ...
    sdeventplus::source::EventBase& /* source */)
{
    ...
    {
        auto _terminus = _terminuses.back();
