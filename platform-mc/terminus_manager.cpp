...
exec::task<int> TerminusManager::initMctpTerminus(const MctpInfo& mctpInfo)
{
    ...
    if (!isDiscovery)
    {
        ...
        try
        {
            auto terminuPtr = std::make_shared<Terminus>(tid, supportedTypes);
            if (!terminuPtr->isConstructed)
            {
                error("Failed to construct Terminus for Terminus {TID}", "TID",
                      static_cast<int>(tid));
                co_return PLDM_ERROR;
            }
            termini[tid] = terminuPtr;
        }
