...
std::optional<mctp_eid_t> TerminusManager::toEID(PldmTID tid)
{
...
std::optional<PldmTID> TerminusManager::toTID(mctp_eid_t eid)
{
...
std::optional<PldmTID> TerminusManager::mapToTID(mctp_eid_t eid)
{
    if (eid == 0 || eid == 0xff)
    {
        return std::nullopt;
...
void TerminusManager::unmapTID(PldmTID tid)
{
...
requester::Coroutine TerminusManager::getTID(mctp_eid_t eid, PldmTID& tid)
{
...
requester::Coroutine TerminusManager::setTID(mctp_eid_t eid, PldmTID tid)
{
    auto instanceId = requester.getInstanceId(eid);
...
{
    mctp_eid_t eid = terminus->getEid();
