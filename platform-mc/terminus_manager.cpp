...
exec::task<int> TerminusManager::initMctpTerminus(const MctpInfo& mctpInfo)
{
    ...
    if (rc)
    {
        error("failed to get PLDM Types tid='{TID}'", "TID",
              static_cast<int>(tid));
        co_return PLDM_ERROR;
    }
...
exec::task<int> TerminusManager::getTidOverMctp(mctp_eid_t eid, pldm_tid_t* tid)
{
    ...
    if (rc)
    {
        ...
        error("encode_get_tid_req failed. rc='{RC}'", "RC",
              static_cast<int>(rc));
        co_return rc;
    }
    ...
    if (rc)
    {
        error("decode_get_tid_resp failed. rc='{RC}'", "RC",
              static_cast<int>(rc));
        co_return rc;
    }
                                              ...
                                              uint64_t& supportedTypes)
{
    ...
    if (rc)
    {
        error("encode_get_types_req failed. rc='{RC}'", "RC",
              static_cast<int>(rc));
        co_return rc;
    }
    ...
    if (rc)
    {
        error("decode_get_types_resp failed. rc='{RC}'", "RC",
              static_cast<int>(rc));
        co_return rc;
    }
    co_return completionCode;
