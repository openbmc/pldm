...
void TerminusManager::removeMctpTerminus(const TerminusInfos& mctpInfos)
{
    ...
    for (const auto& [tid, mctpInfo] : mctpInfos)
    {
        ...
        if (!unmapTid(it->first))
        {
            lg2::error(
                "Failed to unmap TID {TID} EID {EID} networkId {NETWORK}.",
                "TID", it->first, "EID", std::get<0>(mctpInfo), "NETWORK",
                std::get<3>(mctpInfo));
        }
        termini.erase(it);
        mctpInfoAvailTable.erase(mctpInfo);
...
exec::task<int> TerminusManager::initMctpTerminus(
    const MctpInfo& mctpInfo, [[maybe_unused]] const pldm_tid_t assignedTid)
{
    int rc = 0;
    bool isMapped = false;
    pldm_tid_t tid = 0;
#if defined(PLDM_TRANSPORT_WITH_AF_MCTP)
    /* In AF-MCTP (network) mode, TID is pre-assigned and already stored in
     * mctpInfoTable by the discovery layer (via allocateOrGetTid → mapTid).
     * Skip GetTID and storeTerminusInfo; just send SetTID. */
    tid = assignedTid;
    ...
    if (!isMapped)
    {
        /* Demux only: terminus has no TID yet, allocate one from pool and
         * assign it. */
...
exec::task<int> TerminusManager::setTidOverMctp(const MctpInfo& mctpInfo,
                                                pldm_tid_t tid)
{
    auto instanceIdResult = instanceIdDb.next(tid);
    if (!instanceIdResult)
    {
        co_return PLDM_ERROR;
    }
