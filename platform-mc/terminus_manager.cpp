    ...
    const MctpInfo& mctpInfo, [[maybe_unused]] const pldm_tid_t assignedTid)
{
    ...
    /* In AF-MCTP (network) mode, TID is pre-assigned by the discovery layer.
     * Skip GetTID; store the assigned TID directly and send SetTID. */
    tid = assignedTid;
    if (!storeTerminusInfo(mctpInfo, tid))
    {
        lg2::error("Failed to store Terminus Info for terminus {TID}.", "TID",
                   tid);
        co_return PLDM_ERROR;
    }
