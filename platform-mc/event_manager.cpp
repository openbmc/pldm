...
int EventManager::getNextPartParameters(
    uint8_t eventTid, uint16_t eventId, std::vector<uint8_t> eventMessage,
    uint8_t transferFlag, uint8_t eventClass,
    ...
    uint32_t* eventIdToAcknowledge)
{
    ...
    if (transferFlag == PLDM_PLATFORM_TRANSFER_START_AND_END)
    {
        ...
        {
            eventHandlers.at(eventClass)(eventTid, eventId, eventMessage.data(),
                                         eventMessage.size());
        }
    }
    ...
    pldm_tid_t tid, uint32_t pollDataTransferHandle)
{
    ...
    while (eventId != PLDM_PLATFORM_EVENT_ID_NONE)
    {
        ...
        uint8_t eventTid = PLDM_PLATFORM_EVENT_ID_NONE;
        ...
        else
        {
            auto ret = getNextPartParameters(
                eventTid, eventId, eventMessage, transferFlag, eventClass,
