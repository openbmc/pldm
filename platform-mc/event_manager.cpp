...
int EventManager::triggerNumericSensorThresholdEvent(
    NumericSensor& sensor, uint8_t previousEventState, uint8_t nextEventState,
    double value, bool isFinal)
{
    ...
    {
        int rc = triggerNumericSensorThresholdEvent(sensor, *prevIt,
                                                    normalState, value, false);
        if (rc != PLDM_SUCCESS)
        {
            lg2::error(
                "Error handling {PREV} to current for numeric sensor {SENSOR}",
                "PREV", int(*prevIt), "SENSOR", sensor.sensorName);
        }
        return triggerNumericSensorThresholdEvent(sensor, normalState, *nextIt,
                                                  value);
    }
        ...
        if (gettingBetter)
        {
            if (currIt != prevIt)
            {
                ...
                sensor.triggerThresholdEvent(
                    std::get<0>(event), std::get<1>(event), value, true, true);
            }
            sensor.triggerThresholdEvent(std::get<0>(event), std::get<1>(event),
                                         value, false, false);
        }
        else
        {
    ...
    if (isFinal)
    {
        sensor.triggerThresholdNormal(value);
    }
    return PLDM_SUCCESS;
}
                                            ...
                                            size_t sensorDataLength)
{
    ...
    return triggerNumericSensorThresholdEvent(*sensor, previousEventState,
                                              eventState, value);
}
