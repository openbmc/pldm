...
requester::Coroutine EventManager::pollForPlatformEventTask(tid_t tid)
{
    ...
    while (eventId != 0)
    {
        ...
        std::cerr << "req pollForPlatformEventMessage. tid=" << unsigned(tid)
                  << " transferOperationFlag="
                  << unsigned(transferOperationFlag)
                  << " dataTransferHandle=" << unsigned(dataTransferHandle)
                  << " eventIdToAcknowledge=" << unsigned(eventIdToAcknowledge)
                  << "\n";
        ...
        {
            std::cerr << "pollForPlatformEventMessage failed. tid="
                      << unsigned(tid) << " transferOperationFlag="
                      << unsigned(transferOperationFlag)
                      << " rc=" << unsigned(rc)
        ...
        std::cerr << "resp pollForPlatformEventMessage. tid=" << unsigned(tid)
                  << " transferOperationFlag="
                  << unsigned(transferOperationFlag)
                  << " transferFlag=" << unsigned(transferFlag)
                  << " eventID=" << unsigned(eventId) << " rc=" << unsigned(rc)
                  << " eventDataSize=" << unsigned(eventDataSize)
                  << " checksum=" << unsigned(eventDataIntegrityChecksum)
                  << " cc=" << unsigned(completionCode) << "\n";
        ...
        std::cerr << "pollForPlatformEventTask 11\n";
        if (transferOperationFlag == PLDM_ACKNOWLEDGEMENT_ONLY)
        {
            std::cerr << "pollForPlatformEventTask 22\n";
            if (eventId == 0xffff)
            {
                std::cerr << "pollForPlatformEventTask 33\n";
        ...
        {
            std::cerr << "pollForPlatformEventTask 44\n";
            if (transferFlag == PLDM_START || transferFlag == PLDM_MIDDLE)
            {
                std::cerr << "pollForPlatformEventTask 55\n";
            ...
            else
            {
                ...
                {
                    std::cerr << "pollForPlatformEventTask 99\n";
                    if (eventDataIntegrityChecksum ==
                        crc32(eventMessage.data(), eventMessage.size()))
                    {
                        std::cerr << "pollForPlatformEventTask aa\n";
                        handlePlatformEvent(
                            eventTid, eventClass, eventMessage.data(),
                            eventMessage.size(), platformEventStatus);
                    }
                    ...
                    {
                        std::cerr
                            << "pollForPlatformEventMessage checksum error, tid="
                            << unsigned(tid) << " eventId=" << unsigned(eventId)
                            << " eventClass=" << unsigned(eventClass) << "\n";
                    }
                }
            ...
            std::cerr << "pollForPlatformEventTask bb\n";
        }
        std::cerr << "pollForPlatformEventTask cc\n";
    }
    ...
    uint32_t& eventDataIntegrityChecksum)
{
    ...
    if (rc)
    {
        std::cerr << "encode_poll_for_platform_event_message_req tid="
    ...
    if (rc)
    {
        std::cerr << "decode_poll_for_platform_event_message_resp tid="
                  << unsigned(tid) << " rc=" << unsigned(rc)
                  << " responseLen=" << unsigned(responseLen) << "\n";
        co_return rc;
    }
