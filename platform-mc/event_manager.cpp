                                      ...
                                      size_t eventDataSize)
{
    if (eventClass == PLDM_SENSOR_EVENT)
    {
        ...
        if (rc)
        {
            error("Failed to decode sensor event data, rc='{RC}'.", "RC", rc);
            return rc;
        }
...
exec::task<int> EventManager::pollForPlatformEventTask(pldm_tid_t tid)
{
    ...
    while (eventId != 0)
    {
        ...
        {
            error(
                "pollForPlatformEventMessage failed. tid='{TID}' eventId='{EVENTID}' rc='{RC}' cc='{CC}'",
                                   ...
                                   size_t eventDataSize)
{
    ...
    if (rc)
    {
        error("Failed to decode CPER event data, rc='{RC}'", "RC", rc);
        return rc;
    }
    ...
    if (fd < 0)
    {
        error("Failed to generate temp file:'{ERRORNO}'", "ERRORNO",
              std::strerror(errno));
        return PLDM_ERROR;
    }
    close(fd);
