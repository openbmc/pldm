                                      ...
                                      size_t eventDataSize)
{
    if (eventClass == PLDM_SENSOR_EVENT)
    {
        ...
        if (rc)
        {
            error("Failed to decode sensor event data return code {RC}.", "RC",
                  rc);
            return rc;
        }
...
exec::task<int> EventManager::pollForPlatformEventTask(pldm_tid_t tid)
{
    ...
    while (eventId != 0)
    {
        ...
        {
            error(
                "pollForPlatformEventMessage for terminus {TID} with event {EVENTID} is failed with return code {RC} and complete code {CC}",
    ...
    uint32_t& eventDataIntegrityChecksum)
{
    ...
    {
        error(
            "encode_poll_for_platform_event_message_req for terminus {TID} is failed with error {RC}",
            "TID", tid, "RC", rc);
        co_return rc;
    }
    ...
    {
        error(
            "decode_poll_for_platform_event_message_resp for terminus {TID} is failed with error {RC}",
            "TID", tid, "RC", rc);
        co_return rc;
    }
                                   ...
                                   size_t eventDataSize)
{
    ...
    if (rc)
    {
        error("Failed to decode CPER event data with error {RC}'", "RC", rc);
        return rc;
    }
    ...
    if (fs::exists(dirStatus))
    {
        ...
        {
            error("Failed to create '{DIRNAME}' directory", "DIRNAME", dirName);
            return PLDM_ERROR;
        }
    }
    else
    ...
    if (fd < 0)
    {
        error("Failed to generate temp file with error {ERRORNO}", "ERRORNO",
              std::strerror(errno));
        return PLDM_ERROR;
    }
    close(fd);
    ...
    {
        error("Failed to save CPER to '{FILENAME}', error - {ERROR}.",
                                      ...
                                      const std::string& dataPath)
{
    auto createDump = [](std::map<std::string, std::string>& addData) {
        ...
        {
            error("Failed to create D-Bus Dump entry, error - {ERROR}.",
                  "ERROR", e);
        }
    };
                                                ...
                                                const double threshold)
{
    ...
    {
        error("Message Registry messageID is not recognised, '{MESSAGEID}'",
              "MESSAGEID", messageId);
        return PLDM_ERROR;
    }
