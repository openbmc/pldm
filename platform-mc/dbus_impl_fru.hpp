    ...
    public EntityIntf
{
  public:
    ...
    PldmEntityReq(sdbusplus::bus_t& bus, const std::string& path) :
        ...
        BoardIntf(bus, path.c_str()), EntityIntf(bus, path.c_str()) {};
    ...
    /** @brief Set value of entityType in PLDM.Entity */
    uint16_t entityType(uint16_t value);

    /** @brief Set value of entityInstanceNumber in PLDM.Entity */
    uint16_t entityInstanceNumber(uint16_t value);

    /** @brief Set value of containerID in PLDM.Entity */
