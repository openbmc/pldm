...
class TerminusManager
{
  ...
  private:
     ...
     * @param[in] assignedTid - preallocated TID for the terminus
     *  @return coroutine return_value - PLDM completion code
     */
    exec::task<int> initMctpTerminus(
        const MctpInfo& mctpInfo, const pldm_tid_t assignedTid);

    /** @brief Send getTID PLDM command to destination EID and then return the
     *         value of tid in reference parameter.
     *
     *  @param[in] eid - Destination EID
     *  @param[out] tid - Terminus TID returned from the endpoint
     *  @return coroutine return_value - PLDM completion code
     */
