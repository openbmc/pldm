...
class TerminusManager
{
  public:
    ...
    explicit TerminusManager(
        ...
        std::vector<std::shared_ptr<Terminus>>& terminuses, Manager* manager) :
        event(event),
        handler(handler), requester(requester), terminuses(terminuses),
        tidPool(255, 0), manager(manager)
    {}

    void discoverTerminus(const std::vector<mctp_eid_t>& _eids);
