...
void SensorManager::startPolling(pldm_tid_t tid)
{
    ...
    {
        info("TID:'{TID}' sensor poll timer already exists.", "TID",
             (unsigned)tid);
        return;
    }
    // numeric sensor
    ...
    {
        info("TID:'{TID}' no sensor to polling.", "TID", (unsigned)tid);
        return;
    }
...
void SensorManager::doSensorPolling(pldm_tid_t tid)
{
    ...
    scope.spawn(stdexec::just() |
        ...
        {
            info("Stopped sensor polling for tid '{TID}'", "TID", tid);
            rcOpt = PLDM_SUCCESS;
        }
    }),
...
exec::task<int> SensorManager::doSensorPollingTask(pldm_tid_t tid)
{
    ...
    do
    {
        ...
        if (verbose)
        {
            info("TID:'{TID}' start sensor polling at {NOW}.", "TID", tid,
    ...
    SensorManager::getSensorReading(std::shared_ptr<NumericSensor> sensor)
{
    ...
    if (rc)
    {
        error("encode_get_sensor_reading_req failed, tid='{TID}', rc='{RC}'.",
              "TID", tid, "RC", rc);
        co_return rc;
    }
