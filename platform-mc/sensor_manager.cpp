    ...
    pollingTime(SENSOR_POLLING_TIME), verbose(verbose)
{
    ...
    getPrioritySensorNameSpaces(configJson);
}

void SensorManager::getPrioritySensorNameSpaces(
    const std::filesystem::path& configJson)
{
    ...
    {
        std::string nameSpace = "/xyz/openbmc_project/sensors/" + name + "/";
...
void SensorManager::startPolling(pldm_tid_t tid)
{
    ...
    if (!prioritySensors[tid].size() && !roundRobinSensors[tid].size())
    {
