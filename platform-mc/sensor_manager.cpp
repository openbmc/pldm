...
exec::task<int> SensorManager::doSensorPollingTask(pldm_tid_t tid)
{
    ...
    do
    {
        ...
        /* The state sensors get a budget of their own, so the numeric sensors
         * of a terminus cannot hold their round robin back.
         */
        sd_event_now(event.get(), CLOCK_MONOTONIC, &t0);
        t1 = t0;
        ...
        budgetSpent = budgetSpent || (stateToBeUpdated > 0);
    } while (budgetSpent);
