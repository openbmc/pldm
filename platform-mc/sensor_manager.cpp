...
SensorManager::SensorManager(
    ...
    std::map<pldm_tid_t, std::shared_ptr<Terminus>>& termini, bool verbose,
    ...
    pollingTime(SENSOR_POLLING_TIME), verbose(verbose)
{
    ...
    {
        error("Parsing json file failed. File path {FILE_PATH}", "FILE_PATH",
...
void SensorManager::startPolling(pldm_tid_t tid)
{
    ...
    {
        info("Terminus ID {TID}: sensor poll timer already exists.", "TID",
             (unsigned)tid);
        return;
    }
    // numeric sensor
    ...
    {
        info("Terminus ID {TID}: no sensor to polling.", "TID", (unsigned)tid);
        return;
    }
...
void SensorManager::doSensorPolling(pldm_tid_t tid)
{
    ...
    scope.spawn(stdexec::just() |
        ...
        {
            info("Stopped sensor polling for Terminus ID {TID}", "TID", tid);
            rcOpt = PLDM_SUCCESS;
        }
    }),
...
exec::task<int> SensorManager::doSensorPollingTask(pldm_tid_t tid)
{
    ...
    do
    {
        ...
        if (verbose)
        {
            info("Terminus ID {TID}: start sensor polling at {NOW}.", "TID",
                 tid, "NOW", pldm::utils::getCurrentSystemTime());
        }
        ...
        {
            sd_event_now(event.get(), CLOCK_MONOTONIC, &t1);
            info("end sensor polling at '{END}'. duration(us):'{DELTA}'", "END",
                 pldm::utils::getCurrentSystemTime(), "DELTA", t1 - t0);
        }
    ...
    SensorManager::getSensorReading(std::shared_ptr<NumericSensor> sensor)
{
    ...
    {
        error(
            "encode_get_sensor_reading_req failed, Terminus ID {TID}, return code {RC}.",
            "TID", tid, "RC", rc);
        co_return rc;
    }
    ...
    {
        error(
            "Failed to decode response of GetSensorReading, Terminus ID {TID}, return code {RC}.",
            "TID", tid, "RC", rc);
    ...
    {
        error(
            "Failed to decode response of GetSensorReading, Terminus ID {TID}, return code {RC}, complete code {CC}.",
