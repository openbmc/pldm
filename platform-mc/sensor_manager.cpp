...
exec::task<int> SensorManager::doSensorPollingTask(pldm_tid_t tid)
{
    ...
    do
    {
         ...
         */
        budgetSpent = (toBeUpdated > 0);

        rc = co_await pollStateSensors(tid, terminus, budgetSpent);
        if (rc != PLDM_SUCCESS)
        {
            co_return rc;
        }
...
exec::task<int> SensorManager::pollStateSensors(
    pldm_tid_t tid, std::shared_ptr<Terminus> terminus, bool& budgetSpent)
{
    uint64_t t0 = 0;
    uint64_t t1 = 0;
    uint64_t elapsed = 0;
    ...
    auto toBeUpdated = stateSensors.size();
    ...
    budgetSpent = budgetSpent || (toBeUpdated > 0);
