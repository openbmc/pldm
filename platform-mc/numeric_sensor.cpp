...
void NumericSensor::setSensorUnit(uint8_t base_unit)
{
    ...
    std::string& associationPath) : tid(tid), sensorName(sensorName)
{
    ...
    maxValue = getSensorDataValue(pdr->sensor_data_size, pdr->max_readable);
    minValue = getSensorDataValue(pdr->sensor_data_size, pdr->min_readable);
