    ...
    sensorId(pdr->sensor_id), sensorName(sensorName), isPriority(false)
{
    ...
    {
        error(
            "Failed to create association interface for numeric sensor {PATH}",
            "PATH", path);
        return;
    }
    ...
    {
        error("Failed to create Value interface for numeric sensor {PATH}",
              "PATH", path);
        return;
    }
    ...
    {
        error(
            "Failed to create Availability interface for numeric sensor {PATH}",
            "PATH", path);
        return;
    }
    ...
    {
        error(
            "Failed to create Operation status interface for numeric sensor {PATH}",
            "PATH", path);
        return;
    }
    ...
    if (hasWarningThresholds)
    {
        ...
        {
            error(
                "Failed to create Threshold warning interface for numeric sensor {PATH}",
                "PATH", path);
            return;
        }
    ...
    if (hasCriticalThresholds)
    {
        ...
        {
            error(
                "Failed to create Threshold critical interface for numeric sensor {PATH}",
                "PATH", path);
            return;
        }
    ...
    /* Success construct the sensor object */
    isConstructed = true;
}
    ...
    sensorId(pdr->sensor_id), sensorName(sensorName), isPriority(false)
{
    ...
    {
        error(
            "Failed to create Association interface for compact numeric sensor {PATH}",
            "PATH", path);
        return;
    }
    associationDefinitionsIntf->associations(
    ...
    {
        error(
            "Failed to create Value interface for compact numeric sensor {PATH}",
            "PATH", path);
        return;
    }
    valueIntf->maxValue(unitModifier(conversionFormula(maxValue)));
    ...
    {
        error(
            "Failed to create Availability interface for compact numeric sensor {PATH}",
            "PATH", path);
        return;
    }
    availabilityIntf->available(true);
    ...
    {
        error(
            "Failed to create Operational status interface for compact numeric sensor {PATH}",
            "PATH", path);
        return;
    }
    operationalStatusIntf->functional(!sensorDisabled);
    ...
    if (hasWarningThresholds)
    {
        ...
        {
            error(
                "Failed to create Warning threshold interface for compact numeric sensor {PATH}",
                "PATH", path);
            return;
        }
        thresholdWarningIntf->warningHigh(unitModifier(warningHigh));
    ...
    if (hasCriticalThresholds)
    {
        ...
        {
            error(
                "Failed to create Critical threshold interface for compact numeric sensor {PATH}",
                "PATH", path);
            return;
        }
        thresholdCriticalIntf->criticalHigh(unitModifier(criticalHigh));
