...
template <typename upperErrorObj, typename lowerErrorObj>
auto logThresholdHelper(pldm::utils::Direction direction,
                        const std::string& sensorObjPath, double value,
                        SensorUnit sensorUnit, double threshold)
    -> std::optional<sdbusplus::message::object_path>
{
    switch (direction)
    {
        case pldm::utils::Direction::HIGH:
            return logThresholdHelper<upperErrorObj>(sensorObjPath, value,
                                                     sensorUnit, threshold);
        case pldm::utils::Direction::LOW:
            return logThresholdHelper<lowerErrorObj>(sensorObjPath, value,
                                                     sensorUnit, threshold);
        default:
            break;
    }
    ...
    pldm::utils::Level level, pldm::utils::Direction direction, double value)
{
    ...
    {
        case pldm::utils::Level::WARNING:
            log = logThresholdHelper<Errors::ReadingAboveUpperWarningThreshold,
                                     Errors::ReadingBelowLowerWarningThreshold>(
                direction, sensorObjPath, value, sensorUnit, threshold);
            break;
        case pldm::utils::Level::CRITICAL:
            log =
