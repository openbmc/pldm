    ...
    std::optional<sdbusplus::message::object_path>& log)
{
    if (PLDM_THRESHOLD_LOGGING)
    {
        if (log)
        {
            ...
            {
                lg2::error("Error trying to resolve: {LOG} : {ERROR}", "LOG",
                           std::string(*log), "ERROR", ec.what());
            }
            log.reset();
        }
    }
    ...
    -> std::optional<sdbusplus::message::object_path>
{
    if (PLDM_THRESHOLD_LOGGING)
    {
        ...
        {
            lg2::error("Unable to create log entry for {SENSOR} : {ERROR}",
                       "SENSOR", sensorObjPath, "ERROR", ec.what());
        }
    }
    return std::nullopt;
}
