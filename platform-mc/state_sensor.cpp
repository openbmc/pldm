                                    ...
                                    uint8_t previousEventState)
{
    ...
    if (sensorOffset < stateSets.size() && stateSets[sensorOffset])
    {
        ...
        // Get event data (messageID, message, level, resolution, additionalData)
        auto [messageID, message, level, resolution, additionalData] =
            stateSet->getEventData();
        ...
        if (!messageID.empty())
        {
            ...
            // Create log entry with sensor name and state type as first argument
...
void StateSensor::createLogEntry(const std::string& messageID,
                                 const std::string& sensorName,
                                 ...
                                 const std::string& resolution, Level level)
{
    ...
    try
    {
        ...
        lg2::info(
            ...
            "MID", messageID, "SEV", severity, "ARGS",
            sensorName + "," + arg2);
    }
    catch (const std::exception& e)
    {
