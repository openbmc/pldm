...
HealthStateSetIntf::HealthStateSetIntf(
    ...
    const std::string& inventoryPath, uint16_t inventoryEntityType)
{
    ...
    /* The association restricts its endpoint to the inventory items whose
     * Redfish resource exposes a health property, so an item outside that set
     * is left unlinked. */
    if (inventoryPath.empty() ||
        !pldm::dbus_api::isHealthAssociationEndpoint(inventoryEntityType))
    {
        return;
    }
    ...
    const std::string& inventoryPath, uint16_t inventoryEntityType)
{
    ...
    {
        case PLDM_STATE_SET_HEALTH_STATE:
            return std::make_unique<HealthStateSetIntf>(
                bus, path, inventoryPath, inventoryEntityType);
        default:
            return nullptr;
    }
}
    ...
    tid(tid), offset(offset), info(info), stateSetName(stateSetName), name(name)
{
    ...
    stateSetIntf = createStateSetIntf(info->compositeInfo[offset].first, bus,
                                      path, inventoryPath, inventoryEntityType);
    if (!stateSetIntf)
    {
        throw sdbusplus::xyz::openbmc_project::Common::Error::InvalidArgument();
    }
