...
void generatePDR(const std::string& dir, std::map<RecordHandle, Entry>& pdrRepo)
{
    ...
    static std::map<Type, std::function<void(void)>> generators = {
        {
        ...
        [](const auto& json) {
            ...
            for (const auto& entry : entries)
            {
                ...
                pdr->effecter_id = entry.value("id", 0);
                pdr->entity_type = entry.value("type", 0);
                pdr->entity_instance = entry.value("instance", 0);
                pdr->container_id = entry.value("container", 0);
