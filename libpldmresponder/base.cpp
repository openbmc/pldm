...
Response getPLDMTypes(const pldm_msg* request, size_t payloadLength)
{
    ...
    encode_get_types_resp(responsePtr->hdr.instance_id, PLDM_SUCCESS,
...
Response getPLDMCommands(const pldm_msg* request, size_t payloadLength)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        encode_get_commands_resp(responsePtr->hdr.instance_id, rc, nullptr,
                                 responsePtr);
        return response;
    }
    ...
    {
        encode_get_commands_resp(responsePtr->hdr.instance_id,
    ...
    encode_get_commands_resp(responsePtr->hdr.instance_id, PLDM_SUCCESS,
...
Response getPLDMVersion(const pldm_msg* request, size_t payloadLength)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        encode_get_version_resp(responsePtr->hdr.instance_id, rc, 0, 0, nullptr,
                                4, responsePtr);
        return response;
    }
    ...
    {
        encode_get_version_resp(responsePtr->hdr.instance_id,
    ...
    encode_get_version_resp(responsePtr->hdr.instance_id, PLDM_SUCCESS, 0,
