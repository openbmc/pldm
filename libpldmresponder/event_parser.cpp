...
EventParser::EventParser(const std::string& dirPath)
{
    fs::path dir(dirPath);
...
StateToDBusValue EventParser::mapStateToDBusVal(const Json& eventStates,
...
const EventDBusInfo& EventParser::getEventInfo(const EventEntry& entry)
{
    return eventMap.at(entry);
}

int EventParser::eventAction(const EventEntry& entry, EventState state)
{
    try
    {
