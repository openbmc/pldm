                         ...
                         uint8_t* effecterValue)
{
    ...
    // When the powerOnTime is 0, it means the function TPO is disabled.
    if (powerOnTime < timeNow && 0 != powerOnTime)
    {
        std::cerr << "Error setting TPO time, effecterValue=" << powerOnTime
                  << "\n";
        return PLDM_ERROR;
    }
