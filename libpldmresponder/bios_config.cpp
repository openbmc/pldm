...
void BIOSConfig::buildAndStoreAttrTables(const Table& stringTable)
{
    ...
    for (auto& attr : biosAttributes)
    {
        ...
        {
            error(
                "Error while Constructing Table Entry {ATTRIBUTE_NAME}, {ERR_EXCEP}",
                "ATTRIBUTE_NAME", attr->name, "ERR_EXCEP", e);
        }
    }
...
void BIOSConfig::load(const fs::path& filePath, ParseHandler handler)
{
    ...
    if (fs::exists(filePath))
    {
        ...
        {
            error("Failed to parse JSON config file {FILE_PATH}, {ERR_EXCEP}",
                  "FILE_PATH", filePath.c_str(), "ERR_EXCEP", e);
        }
    }
}
    ...
    const DbusChObjProperties& chProperties, uint32_t biosAttrIndex)
{
    ...
    {
        error(
            "Could not find handle for BIOS string {ATTRIBUTE_NAME}, {ERR_EXCEP}",
            "ATTRIBUTE_NAME", attrName.c_str(), "ERR_EXCEP", e);
        return;
    }
