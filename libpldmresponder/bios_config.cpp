...
void BIOSConfig::buildAndStoreAttrTables(const Table& stringTable)
{
    ...
    {
        error("Failed to read BaseBIOSTable property - '{ERROR}'", "ERROR", e);
    }
    ...
    for (auto& attr : biosAttributes)
    {
        ...
        {
            error(
                "Error constructing table entry for '{ATTRIBUTE}' - '{ERROR}'",
    ...
    if (fs::exists(filePath))
    {
        try
        {
            ...
            for (auto& entry : entries)
            {
                ...
                {
                    error(
                        "Failed to parse JSON config file(entry handler) at {PATH} - '{ERROR}'",
    ...
    switch (attrType)
    {
        ...
        case PLDM_BIOS_ENUMERATION_READ_ONLY:
        {
            ...
            for (uint8_t handle : handles)
            {
                ...
                info(
                    "BIOS '{ATTRIBUTE}' updated to {VALUE} by BMC: {CHECK_BMC} ",
        ...
        case PLDM_BIOS_INTEGER_READ_ONLY:
        {
            ...
            auto chkBMC = isBMC ? "true" : "false";
            info("BIOS '{ATTRIBUTE}' updated to '{VALUE}' by BMC: {CHECK_BMC}",
...

{
    ...
    switch (attrType)
    {
        ...
        case PLDM_BIOS_INTEGER_READ_ONLY:
        {
            ...
            {
                error("Integer: out of bound, '{ATTRIBUTE_VALUE}'",
        ...
        case PLDM_BIOS_STRING_READ_ONLY:
        {
            ...
            {
                error(
                    "String: Length error, '{ATTRIBUTE_VALUE}' and '{LENGTH}'",
        ...
        default:
            error("ReadOnly or Unsupported '{TYPE}'", "TYPE", attrType);
            return PLDM_ERROR;
    };
}
    ...
    const DbusChObjProperties& chProperties, uint32_t biosAttrIndex)
{
    ...
    {
        error("Missing handle for '{ATTRIBUTE}' - {ERROR}", "ATTRIBUTE",
              attrName, "ERROR", e);
        return;
    }
    ...
    {
        error(
            "Attribute {ATTRIBUTE} not found in attribute table and '{ATTR_HANDLE}'",
    ...
    {
        error(
            "Could not update the attribute value table for '{ATTR_HANDLE}' and '{TYPE}'",
    ...
    if (rc != PLDM_SUCCESS)
    {
        error("could not setAttrValue on base bios table and dbus - '{RC}'",
              "RC", rc);
    }
}
    ...
    const PendingAttributes& pendingAttributes)
{
    ...
    for (auto& attribute : pendingAttributes)
    {
        ...
        {
            error("Attribute '{TYPE}' not supported", "TYPE", attributeType);
            continue;
        }
