...

{
    ...
    switch (attrType)
    {
        ...
        case PLDM_BIOS_INTEGER_READ_ONLY:
        {
            ...
            {
                error(
                    "Integer out of bound for attribute value '{ATTRIBUTE_VALUE}', the lower bound '{LOWER}', the upper bound '{UPPER}' and the scalar value '{SCALAR}'.",
    ...
    const DbusChObjProperties& chProperties, uint32_t biosAttrIndex)
{
    ...
    {
        error(
            "Failed to find attribute {ATTRIBUTE} attribute in BIOS Attribute table with attribute handle '{ATTR_HANDLE}'",
    ...
    {
        error("Attribute value table not present in PLDM_BIOS_ATTR_VAL_TABLE");
        return;
    }
