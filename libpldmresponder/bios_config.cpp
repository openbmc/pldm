...
void BIOSConfig::buildAttrTables(const Table& stringTable)
{
    ...
    BIOSTable::appendPadAndChecksum(attrTable);
    BIOSTable::appendPadAndChecksum(attrValueTable);
...
std::optional<Table> BIOSConfig::buildStringTable()
{
    ...
    {
        BIOSStringTable::constructEntry(table, elem);
    }

    BIOSTable::appendPadAndChecksum(table);
...
int BIOSConfig::setAttrValue(const void* entry, size_t size)
{
    ...
    auto destTable =
        BIOSAttrValTable::updateTable(*attrValueTable, entry, size);
    ...
    auto attrValHeader = BIOSAttrValTable::decodeHeader(attrValueEntry);

    auto attrEntry =
        BIOSAttrTable::findByHandle(*attrTable, attrValHeader.attrHandle);
    if (!attrEntry)
    {
        return PLDM_ERROR;
    }
    ...
    try
    {
        auto attrHeader = BIOSAttrTable::decodeHeader(attrEntry);
