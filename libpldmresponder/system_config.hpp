...
using PropValue = std::variant<std::vector<std::string>>;
using PropertyMap = std::map<std::string, PropValue>;
...
class SystemConfig
{
  ...
  private:
    ...
    void ibmCompatibleAddedCallback(sdbusplus::message::message& msg)
    {
        ...
        auto names = std::get<Interface>(properties.at(namesProperty));
