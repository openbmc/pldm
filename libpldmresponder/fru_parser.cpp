...
void FruParser::setupDefaultDBusLookup(const fs::path& masterJsonPath)
{
    ...
    {
        error("Parsing FRU Dbus Lookup Map config file at {PATH} failed.",
...

    {
        ...
        {
            error("FRU DBus lookup map format '{ERROR}'", "ERROR", e);
...
void FruParser::setupFruRecordMap(const std::string& dirPath)
{
    for (auto& file : fs::directory_iterator(dirPath))
    {
        ...
        if (data.is_discarded())
        {
            error("Parsing FRU config file at {PATH} failed.", "PATH",
                  file.path().c_str());
            throw InternalFailure();
        }
