...
{
    setupDefaultDBusLookup(masterJsonPath);
...
void FruParser::setupDefaultDBusLookup(const std::string& masterJsonPath)
{
    ...
    if (data.is_discarded())
    {
        ...
        std::cerr << "Parsing FRU config file failed, FILE=" << masterJsonPath;
        throw InternalFailure();
    }
    std::map<Interface, EntityType> defIntfToEntityType =
        static_cast<std::map<Interface, EntityType>>(
            data.value("FruDbusLookupMap", emptyJson));
