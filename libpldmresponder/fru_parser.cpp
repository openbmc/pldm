...
const DBusLookupInfo& FruParser::inventoryLookup()
{
    return lookupInfo.value();
}

const FruRecordInfos& FruParser::getRecordInfo(Interface intf)
{
...
void FruParser::setupFruRecordMap(const std::string& dirPath)
{
    for (auto& file : fs::directory_iterator(dirPath))
    {
        ...
        {
            FruRecordInfos recordInfo{fruInfo};
            recordMap.emplace(itemName, recordInfo);
        }
    }
}
