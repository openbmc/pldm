...
void FruParser::setupFRUInfo(const std::string& dirPath)
{
    ...
    {
        std::cerr<<"FRU config directory does not exist or empty, DIR="<<dirPath;
    ...
    {
        std::cerr<<"FRU D-Bus lookup JSON does not exist, PATH="<<masterFilePath;
        throw InternalFailure();
    }
...
void FruParser::setupFruRecordMap(const std::string& dirPath)
{
    for (auto& file : fs::directory_iterator(dirPath))
    {
        ...
        if (data.is_discarded())
        {
            ...
            std::cerr<<"Parsing FRU master config file failed, FILE="<<file.path();
            throw InternalFailure();
        }
