...
class Handler : public CmdHandler
{
  public:
    Handler(const pldm::utils::DBusHandler* dBusIntf,
            ...
            sdeventplus::Event& event,bool verbose=false, bool buildPDRLazily = false,
        ...
        event(event),verbose(verbose), pdrJsonsDir(pdrJsonsDir), pdrCreated(false)
    {
        if (!buildPDRLazily)
        {
        ...
        respHandlers.emplace(PLDM_GET_PDR,
                         [this](const pldm_msg* request, size_t payloadLength) {
        ...
        respHandlers.emplace(PLDM_SET_STATE_EFFECTER_STATES,
                         [this](const pldm_msg* request, size_t payloadLength) {
                             return this->setStateEffecterStatesResp(request,
                                                                 payloadLength);
                         });
    ...
    void _processFetchPDREvent(uint32_t nextRecordHandle, sdeventplus::source::EventBase& source); 
