                 ...
                 const BIOSStringTable& stringTable)
{
    ...
    auto setDbusProperty = [&dBusMap](const auto& variant) {
        pldm::responder::DBusHandler().setDbusProperty(
            dBusMap->objectPath.c_str(), dBusMap->propertyName.c_str(),
...
std::string stringToUtf8(uint8_t stringType, const std::vector<uint8_t>& data)
{
    switch (stringType)
    {
        ...
        case HEX:
            return std::string(data.begin(), data.end()); // TODO:
        case UTF_16BE:
        case UTF_16LE: // TODO
            return std::string(data.begin(), data.end());
...
int setAttrValueDbusBacked(const variable_field* attributeData,
                           ...
                           const BIOSStringTable& stringTable)
{
    ...
    {
        std::cerr << "setAttrValueDbusBacked Error: " << e.what() << std::endl;
        return PLDM_ERROR;
    }
}
