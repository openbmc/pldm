...
int parseBiosJsonFile(const char* dirPath, std::string fileName, Json& fileData)
{
    int rc = 0;
...
int setupValueLookup(const char* dirPath)
{
    ...
    for (const auto& entry : entries)
    {
        ...
        if (entry.count("dbus") != 0)
        {
            ...
            if (dBusMap->objectPath.empty())
            {
                log<level::ERR>("object_path of dbus is empty");
                rc = -1;
            }
            if (dBusMap->interface.empty())
            {
                log<level::ERR>("interface of dbus is empty");
                rc = -1;
            }
            if (dBusMap->propertyName.empty())
            {
                log<level::ERR>("property_name of dbus is empty");
                rc = -1;
            }
            if (rc != 0)
            {
        ...
        // Defaulting all the types of attributes to BIOSEnumeration
        internal::valueMap.emplace(std::move(attr),
                                   std::make_tuple(false, strType, minStrLen,
                                                   maxStrLen, defaultStrLen,
                                                   std::move(defaultStr)));
    }
