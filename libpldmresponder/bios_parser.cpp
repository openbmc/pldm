...
int setupValueLookup(const char* filePath)
{
    ...
    for (const auto& entry : entries)
    {
        ...
        if (entry.count("dbus") != 0)
        {
            ...
            {
                dBusMap.value().dBusValToMap = std::make_optional<internal::DbusValToValMap>();
                std::string propertyType = dBusEntry.value("property_type", "");
                if (populateMapping(propertyType, dBusEntry["property_values"],
                                    ...
                                    dBusMap.value().dBusValToMap.value()) < 0)
                {
                    log<level::ERR>(
                        ...
                        phosphor::logging::entry("attribute_name=%s",
                                                 attrName.c_str()),
                        phosphor::logging::entry("FILE=%s", filePath));
                    continue;
                }
            }
        }
