...
void generate_pldm_numeric_effecter_pdr(const Json& json, T& repo)
{
    ...
    for (const auto& e : entries)
    {
        ...
        dbusMapping.objectPath = e.value("path", empty);
        dbusMapping.interface = e.value("interface", empty);
        dbusMapping.propertyName = e.value("property", empty);
        objDbus.emplace_back(std::move(dbusMapping));
        add(pdr->effecter_id, std::move(objDbus));
