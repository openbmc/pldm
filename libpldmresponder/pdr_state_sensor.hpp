    ...
    for (const auto& e : entries)
    {
        ...
        for (const auto& sensor : sensors)
        {
            ...
            catch (const std::exception& e)
            {
                ...
                intfSignal.push_back(std::make_unique<
                    ...
                    [&](sdbusplus::message::message& msg) {
                        ...
                        pldm::utils::DBusMapping dbusMapping =
                            pldm::utils::DBusMapping{opath, iface, propertyName,
                                                     propertyType};

                        /*handler.addDbusObjMaps(
                        pdr->sensor_id,std::make_tuple(std::move(dbusMappings),
                        std::move(dbusIdToVal)), TypeId::PLDM_SENSOR_ID);*/
                        std::cerr << " Creating the PDR inside Interface added "
                                     "signal watcher code"
                                  << std::endl;
                        PdrEntry pdrEntry{};
