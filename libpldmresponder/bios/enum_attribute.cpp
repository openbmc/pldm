...
BIOSEnumAttribute::BIOSEnumAttribute(const Json& entry) : BIOSAttribute(entry)
{
    ...
    BIOSEnumAttribute::getValueIndices(const std::string& value)
{
    ...
    uint8_t index = iter - possibleValues.begin();
    return {index};
}
...
{
    auto defaultValueIndices = getValueIndices(defaultValue);
    if (readOnly)
    {
    ...
    try
    {
        ...
        return getValueIndices(currentValue);
    }
...
int BIOSEnumAttribute::setAttrValueOnDbus(
                                       ...
                                       Table& attrTable, Table& attrValueTable)
{
    ...
    auto defaultIndices = getValueIndices(defaultValue);
