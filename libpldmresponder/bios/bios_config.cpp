...
{
    BIOSStringTable biosTableString(stringTable);
    load(jsonDir / stringJsonFile, [&biosTableString, this](const Json& entry) {
        biosAttributes.push_back(
            std::make_unique<BIOSStringAttribute>(entry, biosTableString));
    });
    ...
    {
        attr->constructEntry(biosTableString, attrTable, attrValueTable);
    }
