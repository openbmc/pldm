...
void generate_pldm_numeric_effecter_pdr(const Json& json, T& repo)
{
    ...
    for (const auto& e : entries)
    {
        ...
        pdr->effecter_data_size = e.value("effecter_data_size", PLDM_UINT8);
        ...
        pdr->max_set_table = e.value("max_set_table", 0);
        pdr->min_set_table = e.value("min_set_table", 0);
        pdr->range_field_format = e.value("range_field_format", 0);
        pdr->range_field_support = e.value("range_field_support", 0);
        pdr->nominal_value = e.value("nominal_value", 0);
        pdr->normal_max = e.value("normal_max", 0);
        pdr->normal_min = e.value("normal_min", 0);
        pdr->rated_max = e.value("rated_max", 0);
        pdr->rated_min = e.value("rated_min", 0);
        pdr->has_description_pdr = e.value("effecter_init", false);
