...
template <typename T>
void generate_pldm_pdr_hdr(T* pdr, const uint32_t record_handle,
                           const uint8_t version, const uint8_t type,
                           const uint16_t record_change_num,
                           const uint16_t length)
{
    pdr->hdr.record_handle = record_handle;
    pdr->hdr.version = version;
    pdr->hdr.type = type;
    pdr->hdr.record_change_num = record_change_num;
    pdr->hdr.length = length;
}
...
void generate_pldm_state_effecter_pdr(const Json& json, T& repo)
{
    ...
    for (const auto& e : entries)
    {
        ...
        generate_pldm_pdr_hdr(pdr, repo.getNextRecordHandle(), 1,
                              PLDM_STATE_EFFECTER_PDR, 0,
                              pdrSize - sizeof(pldm_pdr_hdr));
    ...
    for (const auto& e : entries)
    {
        ...
        pldm_numeric_effecter_valuer_pdr* pdr =
            reinterpret_cast<pldm_numeric_effecter_valuer_pdr*>(
                pdrEntry.data());
        generate_pldm_pdr_hdr(
            pdr, repo.getNextRecordHandle(), 1, PLDM_NUMERIC_EFFECTER_PDR, 0,
