                            ...
                            const char* biosJsonDir)
{
    ...
    { // no persisted table, constructing fresh table and file
        ...
        auto respPayloadLength = response.size();
                              ...
                              const BIOSTable& BIOSStringTable)
{
    ...
    while (1)
    {
        stringHandle temp = tableEntry->string_handle;
        ...
        if (std::distance(tableData, response.data() + response.size()) <=
            PLDM_BIOS_TABLE_PAD_AND_CHKSUM_MAX_SIZE)
        {
            log<level::INFO>("Reached end of BIOS string table");
            break;
        }
 ...
 */
Table construct(const BIOSTable& BIOSStringTable, const char* biosJsonDir)
{
 ...
 */
Table constructValue(const BIOSTable& BIOSAttributeTable,
                     const BIOSTable& BIOSStringTable)
{
    ...
    while (1)
    {
        ...
        std::move(entryToAttrValTable.begin(), entryToAttrValTable.end(),
                               ...
                               uint8_t instanceID, const char* biosJsonDir)
{
    ...
    { /* no persisted table, constructing fresh table and response*/
        auto attributeTable =
            bios_type_enum::construct(BIOSStringTable, biosJsonDir);
                                    ...
                                    const char* biosJsonDir)
{
    ...
    { // no persisted table, constructing fresh table and data
        Table attributeValueTable =
            bios_type_enum::constructValue(BIOSAttributeTable, BIOSStringTable);
        // calculate pad
