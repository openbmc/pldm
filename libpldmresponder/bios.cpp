...
std::map<AttrType, AttrValTableEntryConstructHandler> AttrValTableConstructMap{
    ...
    {PLDM_BIOS_INTEGER_READ_ONLY, bios_type_integer::constructAttrValueEntry},
};
...
{
    auto attrName = findStringName(attr_entry->string_handle, BIOSStringTable);
    if (attrName.empty())
    {
        log<level::ERR>("invalid string handle",
                        entry("STRING_HANDLE=%d", attr_entry->string_handle));
        return;
    }

    AttrValTableConstructMap.at(attr_entry->attr_type)(
        attr_entry, attrName, BIOSStringTable, attributeValueTable);
}
