...
Response Handler::setDateTime(const pldm_msg* request, size_t payloadLength)
{
    ...
    if (timeowner == "xyz.openbmc_project.Time.Owner.Owners.BMC")
    {
        log<level::ERR>(
            "Not Allowed Error.Setting BmcTime with BMC owner is not allowed",
 ...
 */
Response getBIOSStringTable(const pldm_msg* request, BIOSTable& BIOSStringTable)

{
    ...
    {
        encode_get_bios_table_resp(request->hdr.instance_id, PLDM_SUCCESS,
 ...
 *  @param[in] biosJsonDir - path where the BIOS json files are present
 */
Response getBIOSAttributeTable(const pldm_msg* request,
 ...
 *  @param[in] BIOSStringTable - the string table
 */
Response getBIOSAttributeValueTable(const pldm_msg* request,
                         ...
                         const char* biosJsonDir, const char* biosTablePath)
{
    ...
    switch (tableType)
    {
        case PLDM_BIOS_STRING_TABLE:
            ...
            response = getBIOSStringTable(request, BIOSStringTable);
            break;
        case PLDM_BIOS_ATTR_TABLE:
            ...
            {
                response = getBIOSAttributeTable(request, BIOSAttributeTable,
                                                 BIOSStringTable, biosJsonDir);
            }
            break;
        case PLDM_BIOS_ATTR_VAL_TABLE:
            ...
            else
            {
                response = getBIOSAttributeValueTable(
                    request, BIOSAttributeValueTable, BIOSAttributeTable,
                    BIOSStringTable);
            }
            break;
        default:
