                        ...
                        const char* biosJsonDir, Table& attributeTable)
{
    ...
    for (const auto& [key, value] : attributeMap)
    {
        ...
        /*
        printf("wkr: attr table\n");
        for (auto i = 0UL; i < stringAttrTable.size(); i++)
        {
            printf("0x%02x ", stringAttrTable[i]);
        }
        printf("wkr\n");
        */
                               ...
                               uint8_t instanceID, const char* biosJsonDir)
{
    ...
    { // no available type directory is found
        ...
        for (const auto& file : fs::directory_iterator(dir))
        {
            ...
            else if (filename == bios_parser::bIOSStrJson)
            {
                bios_type_string::constructAttrTable(
                    BIOSStringTable, biosJsonDir, attributeTable);
                found = true;
            }
            // test code, need to delete, begin
            /*
            printf("wkr: filename=%s\n", filename.c_str());
            for(auto i = 0UL; i < attributeTable.size(); i++)
            {
                printf("%x ", attributeTable[i]);
            }
            printf("wkr: \n");
            */
            // end
