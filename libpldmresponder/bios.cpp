                               ...
                               uint8_t instanceID, const char* biosJsonDir)
{
    ...
    { // no persisted table, constructing fresh table and response
        ...
        {
            auto filename = file.path().filename().c_str();
            if (filename == bios_parser::bIOSEnumJson)
            {
                bios_type_enum::constructAttrTable(BIOSStringTable, filename,
                                                   attributeTable);
                found = true;
            }
            else if (filename == bios_parser::bIOSStrJson)
            {
                bios_type_string::constructAttrTable(BIOSStringTable, filename,
                                                     attributeTable);
                found = true;
            }
        }
