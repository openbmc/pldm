...
size_t getTableTotalsize(size_t sizeWithoutPad)
{
    ...
    return padSize + sizeof(uint32_t) /* checksum */;
}
    ...
    BIOSTableRow& attrValueTableEntry)
{
    ...
    uint8_t pv_num;
    pldm_bios_table_attr_entry_enum_decode_pv_num(attrTableEntry, &pv_num);
...
BIOSTableRow
    constructAttrTableEntry(const struct pldm_bios_attr_table_entry* entry,
                            const BIOSTable& BIOSStringTable)
{
