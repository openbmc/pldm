    ...
    try
    {
        value = pldm::utils::DBusHandler()
                    .getDbusProperty<std::variant<EpochTimeUS>>(
                        hostTimePath, "Elapsed", timeInterface, true);
    }
...
Response Handler::setDateTime(const pldm_msg* request, size_t payloadLength)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        encode_set_date_time_resp(request->hdr.instance_id, PLDM_ERROR,
    ...
    try
    {
        ...
        encode_set_date_time_resp(request->hdr.instance_id, PLDM_SUCCESS,
                                  responsePtr, response.size());
    }
    catch (std::exception& e)
    {

        std::cerr << "Error Setting time,PATH=%s" << setTimePath
                  << "TIME INTERACE=%s" << setTimeInterface << "ERROR=%s"
                  << e.what() << "\n";
