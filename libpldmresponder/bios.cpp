...

{
    ...
    {
        encode_get_bios_table_resp(instanceID, PLDM_SUCCESS, 0,
                              ...
                              const BIOSTable& BIOSStringTable)
{
    ...
    auto string_entry = pldm_bios_table_string_find_by_string(
    ...
    return pldm_bios_table_string_entry_decode_handle(string_entry);
}
                           ...
                           const BIOSTable& BIOSStringTable)
{
    Table table;
    BIOSStringTable.load(table);
    auto string_entry = pldm_bios_table_string_find_by_handle(
    ...
    name.resize(str_length);
    pldm_bios_table_string_entry_decode_string(string_entry, name.data(),
                                               name.size() + 1);
    return name;
}
...
void constructAttrValueEntry(
    ...
    BIOSTableRow& attrValueTableEntry)
{
    CurrentValues currVals;
    try
    {
    ...
    size_t size = pldm_bios_table_attr_value_entry_encode_enum_length(
        currValStrIndices.size());
    attrValueTableEntry.resize(size);
    pldm_bios_table_attr_value_entry_encode_enum(
        attrValueTableEntry.data(), attrValueTableEntry.size(),
...
using AttrValTableEntryConstructHandler =
    std::function<void(const struct pldm_bios_attr_table_entry* tableEntry,
                       const std::string& attrName,
                       const BIOSTable& BIOSStringTable, BIOSTableRow& row)>;
...
BIOSTableRow
    constructAttrValueTableEntry(const struct pldm_bios_attr_table_entry* entry,
                                 const BIOSTable& BIOSStringTable)
{
    ...
    AttrValTableConstructMap.at(entry->attr_type)(entry, attrName,
                                                  BIOSStringTable, row);
    return row;
}
                                    ...
                                    const char* /*biosJsonDir*/)
{
    ...
    traverseBIOSAttrTable(
        ...
        [&BIOSStringTable, &attributeValueTable](
            const struct pldm_bios_attr_table_entry* table_entry) {
            BIOSTableRow row =
                constructAttrValueTableEntry(table_entry, BIOSStringTable);
            attributeValueTable.insert(attributeValueTable.end(), row.begin(),
                                       row.end());
        });
