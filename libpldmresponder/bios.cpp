                         ...
                         const char* biosJsonDir)
{
    ...
    for (const auto& [key, value] : attributeMap)
    {
        ...
        {
            log<level::ERR>("Exception fetching handle for the string, will "
                            "skip the attribute",
        ...
        for (const auto& elem : possiVals)
        {
            try
            {
                ...
                possiValsByHdl.push_back(hdl);
            }
            catch (InternalFailure& e)
            {
                log<level::ERR>("Exception creating possible values,will skip "
                                "the row in attribute table constructiion");
                continue;
            }
        }
                              ...
                              const BIOSTable& BIOSStringTable)
{
    ...
    while (1)
    {
        ...
        std::vector<uint8_t> currValStrIndices(numCurrVals);
        try
        {
            currValStrIndices =
        ...
        {
            log<level::ERR>("Could not construct the current values for "
                            "attribute, will skip the row",
                            entry("NAME=%s", attrName.c_str()));
            if (std::distance(tableData, response.data() + tableLen) <=
                padChksumMax)
            {
