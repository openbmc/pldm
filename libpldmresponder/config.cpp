...
void Handler::systemCompatibleCallback(sdbusplus::message_t& msg)
{
    ...
    if (!names.empty())
    {
        ...
        auto systype = names.front();
        std::size_t found = systype.find_last_of(".");
        systemType = systype.substr(found + 1);
    }
 ...
 */
std::filesystem::path Handler::getPlatformName()
{
    if (!systemType.empty())
    {
