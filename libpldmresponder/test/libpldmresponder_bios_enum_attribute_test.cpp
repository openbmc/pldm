...
TEST_F(TestBIOSEnumAttribute, GetValueIndexThrowsOnUnknownValue)
{
    ...
    EXPECT_THROW(
        getValueIndex(attr, "unknown", {"Concurrent", "Disruptive"}),
...
TEST_F(TestBIOSEnumAttribute, GetValueIndexReturnsCorrectIndex)
{
    ...
    BIOSEnumAttribute attr{json, nullptr};
    EXPECT_EQ(getValueIndex(attr, "Concurrent", {"Concurrent", "Disruptive"}), 0);
    EXPECT_EQ(getValueIndex(attr, "Disruptive", {"Concurrent", "Disruptive"}), 1);
}
