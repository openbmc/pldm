...
TEST(FruImpl, updateAssociationTreeTest)
{
    ...
    MockFruImpl mockedFruHandler(FRU_JSONS_DIR, FRU_MASTER_JSON,
    ...
    dbus::InterfaceMap motherboardIface{ {"xyz.openbmc_project.Inventory.Decorator.LocationCode", {}} };
    //const pldm::responder::dbus::InterfaceMap ifaceMap{ {"", {}} }; 

    EXPECT_CALL(mockedFruHandler,
                getEntityByObjectPath(systemIface)).WillOnce(Return(std::make_optional(systemEntity)));

    EXPECT_CALL(mockedFruHandler,
                getEntityByObjectPath(chassisIface)).WillOnce(Return(std::make_optional(chassisEntity)));

    EXPECT_CALL(mockedFruHandler,
                getEntityByObjectPath( motherboardIface)).WillOnce(Return(std::make_optional(motherboardEntity)));
    ...
    node = NULL;
    node = pldm_entity_association_tree_find(entityTree.get(), &motherboardEntity);
    ...
    EXPECT_TRUE(node != NULL);
    GTEST_COUT << "Entity Type of Parent of Motherboard is ........." << (test_node->parent).entity_type << std::endl;
    ...
    //rc = mockedFruHandler.updateAssociationTree(
      //  objects, "/xyz/openbmc_project/inventory/system/chassis");
    //rc = mockedFruHandler.updateAssociationTree(
      //  objects, "/xyz/openbmc_project/inventory/system/chassis/motherboard");
    GTEST_COUT << "After Update Tree.........." << std::endl;
}
