  ...
  protected:
    void SetUp() override
    {
        ...
        handler = std::make_unique<HostPDRHandler>(
            0, 8, event, repo.get(), "", entityTree.get(), bmcEntityTree.get(),
            instanceIdDb, nullptr, pldm::utils::EntityMaps{});
    }

    void seedTemporaryState()
    {
        handler->mergedHostPdrs = true;
        handler->pendingStateSensorPDRs.emplace_back(std::vector<uint8_t>{1});
        handler->pendingFruRecordSetPDRs.emplace_back(std::vector<uint8_t>{2});
    }
