...
class FruImpl
{
  public:
        ...
        oemFruHandler(oemFruHandler)
    {
        ...
        std::vector<std::string> hotPlugIntf = parser.getHotPluggableInterfaces();

        subscribeFruPresence(inventoryObjPath, hotPlugIntf, itemInterface,
                             hotplugMatch);
    }
  ...
  private:
    ...
    void subscribeFruPresence(
        const std::string& inventoryObjPath, std::vector<std::string> fruInterfaces,
     ...
     */
    std::vector<std::unique_ptr<sdbusplus::bus::match::match>> hotplugMatch;
};
