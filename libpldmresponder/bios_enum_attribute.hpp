...
class BIOSEnumAttribute : public BIOSAttribute
{
  public:
    ...
    /** @brief */
    std::vector<uint16_t>
        getValueDisplayNamesHandle(const BIOSStringTable& stringTable,
                                   const std::vector<std::string>& vdn);
  ...
  private:
    ...
    /** @brief Get handles of value display names
     *  @param[in] stringTable - The bios string table
     *  @param[in] vdn - value display names
     *  @return the handles
     */
    std::vector<uint16_t>
        getValueDisplayNamesHandle(const BIOSStringTable& stringTable,
