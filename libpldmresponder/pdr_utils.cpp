...
pldm_entity_association_tree* Repo::getEntityAssociationTree() const
{
    return entityTree;
}
...
void Repo::addEntityAssocitionTree(std::string path, pldm_entity* entity,
                                   Type type)
{
...
void Repo::addPdrEntityAssoctition()
{
...
StateIdtoDbusVal populateMapping(const std::string& type,
                                 const Json& dBusValues,
                                 const PossibleValues& pv)
{
    size_t pos = 0;
    pldm::utils::PropertyValue value;
    StateIdtoDbusVal valueMap;
