                              ...
                              pdr_utils::RepoInterface& repo)
{
    ...
    for (const auto& e : entries)
    {
        ...
        uint16_t parent_entity_type = e.value("parentEntityType", 0);
        uint16_t parent_entity_instance = e.value("parentEntityInstanceNum", 0);
        pldm_entity entity{};
        entity.entity_type = pdr->entity_type;

        pldm_entity parentEntity{};
        parentEntity.entity_type = parent_entity_type;
        parentEntity.entity_instance_num = parent_entity_instance;
