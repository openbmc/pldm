...
void getPDR(const pldm_msg_payload* request, pldm_msg* response)
{
    ...
    decode_get_pdr_req(request, &recordHandle, &dataTransferHandle,
                       &transferOpFlag, &reqSizeBytes, &recordChangeNum);
...
void setStateEffecterStates(const pldm_msg_payload* request, pldm_msg* response)
{
    ...
    int rc = decode_set_state_effecter_states_req(
        request, &effecterId, &compEffecterCnt, stateField.data());
    if (rc == PLDM_SUCCESS)
    {
        rc = setBmcEffecter(effecterId, compEffecterCnt, stateField.data());
    }

    encode_set_state_effecter_states_resp(0, rc, response);
}
