...
void Handler::addDbusObjMaps(uint16_t effecterId, DbusMappings&& dbusMappings,
                             DbusValMaps&& dbusValMaps)
{
    dbusObjMaps.emplace(effecterId, std::make_tuple(std::move(dbusMappings),
...
void Handler::generate(const std::string& dir, Repo& repo)
{
    ...
    const std::map<Type, generateHandler> generateHandlers = {
        {PLDM_STATE_EFFECTER_PDR,
         [this](const auto& json, RepoInterface& repo) {
             pdr_state_effecter::generateStateEffecterHandler<Handler>(
                 *this, json, repo);
         }},
        {PLDM_NUMERIC_EFFECTER_PDR,
         [this](const auto& json, RepoInterface& repo) {
             pdr_numeric_effecter::generateNumericEffecterHandler<Handler>(
                 *this, json, repo);
         }}};
