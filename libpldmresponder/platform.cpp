...
using namespace pldm::responder::platformStateEffecter;
                                          ...
                                          size_t payloadLength)
{
    ...
    if (rc == PLDM_SUCCESS)
    {
        const pldm::utils::DBusHandler dBusIntf;
        IndexedRepo pdrRepo;
        getRepoByType(PDR_JSONS_DIR, PLDM_NUMERIC_EFFECTER_PDR, pdrRepo);
        rc = setNumericEffecterValueHandler<pldm::utils::DBusHandler>(
            dBusIntf, pdrRepo, effecterId, effecterDataSize, effecterValue,
