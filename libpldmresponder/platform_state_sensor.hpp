...
uint8_t getSensorOpState(
    const DBusInterface& dBusIntf,
    std::map<State, pldm::utils::PropertyValue> stateToDbusValue,
    ...
    std::vector<get_sensor_state_field>& stateField)
{
    ...
    Repo stateSensorPDRs(stateSensorPdrRepo.get(), NULL);
    ...
    try
    {
        const auto& [dbusMappings, dbusValMaps] =
            handler.getDbusObjMaps(sensorId, PLDM_SENSOR_ID);
        ...
        for (size_t i = 0; i < compSensorCnt; i++)
        {
            ...
            uint8_t sensorOpState = getSensorOpState<DBusInterface>(
                dBusIntf, dbusValMaps[i], dbusMapping);
            uint8_t presentState = getPresentState<DBusInterface>(
                dBusIntf, dbusMapping.objectPath);
            stateField.push_back(
                {sensorOpState, presentState, UNKNOWN, UNKNOWN});
        }
    }
