...
int FruImpl::setFRUTable(const std::vector<uint8_t>& fruData, bool updateDBus)
{
    ...
    {
        // auto tlv = reinterpret_cast<const
        // pldm_fru_record_tlv*>(fruData.data());
        if (record->record_type == PLDM_FRU_RECORD_TYPE_OEM)
        {
            std::cerr << " Process fru record" << std::endl;
                                    ...
                                    size_t payloadLength)
{
    ...
    rc = encode_set_fru_record_table_resp(
        request->hdr.instance_id, PLDM_SUCCESS, 0 /* nextDataTransferHandle */, response.size() - sizeof(pldm_msg_hdr),
        responsePtr);
