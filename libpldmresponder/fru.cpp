...
void FruImpl::removeIndividualFRU(const std::string& fruObjPath)
{
    ...
    {
        hasError = true;
        error("Failed to remove contained entity from PDR. RC = {RC}", "RC",
    ...
    {
        hasError = true;
        error("Failed to delete entity from association tree. RC = {RC}", "RC",
              rc);
    }

    rc = pldm_entity_association_tree_delete_node(bmcEntityTree, &removeEntity);
    if (rc)
    {
        hasError = true;
        error("Failed to delete entity from BMC association tree. RC = {RC}",
              "RC", rc);
    }
    ...
    if (!hasError)
    {
        ...
        info(
            ...
            static_cast<unsigned>(removeEntity.entity_instance_num),
            "ENTITY_ID",
        ...
        for (const auto& ids : effecterIDs)
        {
            ...
            if (rc != 0)
            {
                error("Failed to delete PDR by effecter ID {ID}. RC = {RC}",
                      "ID", ids, "RC", rc);
                continue;
            }
        ...
        for (const auto& ids : sensorIDs)
        {
            ...
            int rc = pldm_pdr_delete_by_sensor_id(pdrRepo, ids, false,
                                                  &delSensorHdl);
        ...
        // need to send both remote and local records. Phyp keeps track of bmc
        // only records
        ...
        if (!handlesTobeModified.empty())
        {
            platformHandler->sendPDRRepositoryChgEventbyPDRHandles(
                handlesTobeModified,
                std::vector<uint8_t>{PLDM_RECORDS_MODIFIED});
        }
    }
    else
    {
