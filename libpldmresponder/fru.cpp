...
void FruImpl::removeIndividualFRU(const std::string& fruObjPath)
{
    ...
    auto updateRecordHdlBmc = pldm_entity_association_pdr_remove_contained_entity(pdrRepo, removeEntity,
                                                        &bmcEventDataOps, false);
    std::cout << "\n pldm_entity_association_pdr_remove_contained_entity updateRecordHdlBmc "
         << updateRecordHdlBmc << std::endl;

    auto updateRecordHdlHost = pldm_entity_association_pdr_remove_contained_entity(
        pdrRepo, removeEntity, &hostEventDataOps, true);
    std::cout << "\n pldm_entity_association_pdr_remove_contained_entity updateRecordHdlHost " 
 ...
 /*   sendPDRRepositoryChgEventbyPDRHandles(
        ...
        std::move(std::vector<uint8_t>(1, PLDM_RECORDS_DELETED))); need to send both remote and local records. Phyp keeps track of bmc only records
                                 ...
                                 const std::string& fruObjectPath)
{
    ...
    try
    {
        ...
        //sm00
      /*  std::cout << "\nprinting the entityTree before adding the node" << std::endl;
        size_t num = 0;
        pldm_entity* out = nullptr;
        pldm_entity_association_tree_visit(entityTree,&out, &num);
        free(out);*/
        ...
        //sm00
      /*  std::cout << "\nprinting the entityTree after adding the node" << std::endl;
        num = 0;
        out = nullptr;
        pldm_entity_association_tree_visit(entityTree,&out, &num);
        free(out);*/
        ...
        for (const auto& object : objects)
        {
            if (object.first.str == fruObjectPath)
            {
                ...
                newRecordHdl = populateRecords(interfaces, recordInfos, entity,
                                               fruObjectPath, true);
                std::cout << "\npopulateRecords returned newRecordHdl "
    ...
    uint8_t bmcEventDataOps = PLDM_INVALID_OP;
    auto updatedRecordHdlBmc = pldm_entity_association_pdr_add_contained_entity(pdrRepo, entity,
                                                     parentEntity, &bmcEventDataOps, false);
    std::cout << "\npldm_entity_association_pdr_add_contained_entity updatedRecordHdlBmc "
      << updatedRecordHdlBmc << std::endl;

    uint8_t hostEventDataOps = PLDM_INVALID_OP;

    auto updatedRecordHdlHost = pldm_entity_association_pdr_add_contained_entity(
        pdrRepo, entity, parentEntity, &hostEventDataOps, true);
    std::cout << "\npldm_entity_association_pdr_add_contained_entity updatedRecordHdlHost "
                                       ...
                                       const std::string& fruInterface)
{
    ...
    if (fruInterface == "xyz.openbmc_project.Inventory.Item.PCIeDevice")
    {
        ...
        std::cout << "after finding the port objects " << portObjects.size() << std::endl;
    }
