    ...
    std::vector<uint8_t>&& eventDataOps)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        error("Failed to encode the repository change event data, rc = {RC}",
    ...
    if (rc != PLDM_SUCCESS)
    {
        ...
        error("Failed to encode the platform event message, rc = {RC}", "RC",
    ...
    auto platformEventMessageResponseHandler = [](mctp_eid_t /*eid*/,
        ...
        {
            error("Failed to decode platform event message: rc={RC}, cc={CC}",
