    ...
    const fru_parser::FruRecordInfos& recordInfos, const pldm_entity& entity)
{
    ...
    static uint32_t lastUsedBMCRecordHandle = 0;
    ...
    for (auto const& [recType, encType, fieldInfos] : recordInfos)
    {
        ...
        if (tlvs.size())
        {
            if (numRecs == numRecsCount)
            {
                ...
                lastUsedBMCRecordHandle = nextRecordHandle();
                std::cout << "\ngot bmc record handle as " << lastUsedBMCRecordHandle << " in fru.cpp \n";
