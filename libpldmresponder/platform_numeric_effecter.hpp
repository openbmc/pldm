                                   ...
                                   size_t effecterValueLength)
{
    ...
    try
    {
        dBusIntf.updateDbusProperty(dbusMapping, rc.value());
    }
    ...
    // auto updateDbusProperty = [&dBusIntf, &dbusObjs](const auto& value) ->
    // int {
    //     try
    //     {
    //         dBusIntf.setDbusProperty(dbusObjs[0].objectPath.c_str(),
    //                                  dbusObjs[0].propertyName.c_str(),
    //                                  dbusObjs[0].interface.c_str(), value);
    //     }
    //     catch (const sdbusplus::exception::SdBusError& e)
    //     {
    //         std::cerr << "Error setting TPO property, ERROR=" << e.what()
    //                   << " PROPERTY=" << dbusObjs[0].propertyName.c_str()
    //                   << " INTERFACE=" << dbusObjs[0].interface.c_str()
    //                   << " PATH=" << dbusObjs[0].objectPath.c_str() << "\n";
    //         return PLDM_ERROR;
    //     }
