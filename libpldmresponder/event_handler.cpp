...
EventHandler::EventHandler(const std::string& dirPath)
{
    fs::path dir(dirPath);
    ...
    for (const auto& entry : entries)
    {
        ...
        eventMap.emplace(eventEntry, std::make_tuple(std::move(dbusInfo),
                                                     std::move(eventStateMap)));
    }
}

StateToDBusValue EventHandler::mapStateToDBusVal(const Json& eventStates,
                                                 ...
                                                 std::string_view type)
{
    ...
    pldm::utils::PropertyValue propValue{};
    for (; stateIt != eventStates.end(); ++stateIt, ++propIt)
    {
        if (type == "uint8_t")
        {
            propValue = static_cast<uint8_t>(propIt.value());
        }
        else if (type == "uint16_t")
        {
            propValue = static_cast<uint16_t>(propIt.value());
        }
        else if (type == "uint32_t")
        {
            propValue = static_cast<uint32_t>(propIt.value());
        }
        else if (type == "uint64_t")
        {
            propValue = static_cast<uint64_t>(propIt.value());
        }
        else if (type == "int16_t")
        {
            propValue = static_cast<int16_t>(propIt.value());
        }
        else if (type == "int32_t")
        {
            propValue = static_cast<int32_t>(propIt.value());
        }
        else if (type == "int64_t")
        {
            propValue = static_cast<int64_t>(propIt.value());
        }
        else if (type == "bool")
        {
            propValue = static_cast<bool>(propIt.value());
        }
        else if (type == "double")
        {
            propValue = static_cast<double>(propIt.value());
        }
        else if (type == "string")
        {
            propValue = static_cast<std::string>(propIt.value());
        }
        else
        {
        ...
        eventStateMap.emplace((*stateIt).get<uint8_t>(), propValue);
    }

    return eventStateMap;
}

const EventDBusInfo& EventHandler::getEventInfo(const EventEntry& entry)
{
    return eventMap.at(entry);
}

int EventHandler::eventAction(const EventEntry& entry, EventState state)
{
    try
    {
