...
namespace responder
{
namespace platformStateEffecter
{
 ...
 *            terms of PLDM completion codes if atleast one state fails to be
 *            set
 */
template <class DBusInterface>
int setStateEffecterStatesHandler(
    const DBusInterface& dBusIntf, const Repo& pdrRepo, effecter::Id effecterId,
    const std::vector<set_effecter_state_field>& stateField)
{
                 ...
                 if (iter == stateSet->second.end())
                 {
                     std::cerr
                         ...
                         << "found for PLDM_BOOT_PROGRESS_STATE, "
                            "EFFECTER_ID="
                         << effecterId
                         << " FIELD=" << stateField[currState].effecter_state
    ...
    for (uint8_t currState = 0; currState < compEffecterCnt; ++currState)
    {
        ...
        {
            std::cerr << "Invalid state set value, EFFECTER_ID=" << effecterId
