...
static Response processRxMsg(const std::vector<uint8_t>& requestMsg)
{
    ...
    {
        log<level::ERR>("Received PLDM response message",
                        entry("header Byte 1=0x%02x", requestMsg[2]),
                        entry("header Byte 2=0x%02x", requestMsg[3]),
                        entry("header Byte 3=0x%02x", requestMsg[4]));
    }
    else
    {
        ...
        {
            log<level::ERR>("Created invalid PLDM response ",
                            entry("header Byte 1=0x%02x", requestMsg[2]),
                            entry("header Byte 2=0x%02x", requestMsg[3]),
                            entry("header Byte 3=0x%02x", requestMsg[4]));
        }
        ...
        else
        {
            ...
            else
            {
                log<level::ERR>("Failure to read peeked length packet",
                                entry("Peeked Length=%zu", peekedLength),
                                entry("Read Length=%zu", recvDataLength));
                exit(EXIT_FAILURE);
            }
        }
    } while (true);
