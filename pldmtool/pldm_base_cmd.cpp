...
class SetTID : public CommandInterface
{
  public:
    ...
    explicit SetTID(const char* type, const char* name,
                              CLI::App* app) :
        CommandInterface(type, name, app)
    {
        app->add_option(
    ...
    std::pair<int, std::vector<uint8_t>> createRequestMsg() override
    {
        ...
        auto request = reinterpret_cast<pldm_msg*>(requestMsg.data());
        auto rc = encode_set_tid_req(
            instanceId, tid, request);
        if (rc != PLDM_SUCCESS)
        {
            std::cerr << "Failed to encode_set_event_receiver_req, rc = " << rc
                      << std::endl;
        }
        return {rc, requestMsg};
    }
...
void registerCommand(CLI::App& app)
{
    ...
    commands.push_back(std::make_unique<SetTID>(
        "base", "SetTID", setTID));
}
