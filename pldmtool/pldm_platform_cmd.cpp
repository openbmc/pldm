...
#pragma GCC diagnostic pop
                                ...
                                ordered_json& output)
    {
        ...
        int rc = decode_pldm_platform_file_descriptor_pdr(data, data_length,
                                                          &pdr);
        if (rc)
        {
        ...
        if (pdr.file_name.length > 0 && pdr.file_name.ptr != nullptr)
        {
            output["fileName"] = std::string(
                reinterpret_cast<const char*>(pdr.file_name.ptr),
        ...
        if (pdr.oem_file_classification && pdr.oem_file_classification_name.length > 0 &&
            pdr.oem_file_classification_name.ptr != nullptr)
        {
            output["oemFileClassificationName"] = std::string(
