...
class GetPDR : public CommandInterface
{
  ...
  private:
    ...
    ordered_json printCommonPDRHeader(const pldm_pdr_hdr* hdr)
    {
        ordered_json header;
    ...
    ordered_json printStateSensorPDR(const uint8_t* data)
    {
    ...
    ordered_json printPDRFruRecordSet(uint8_t* data)
    {
        if (data == NULL)
        {
            return {};
    ...
    ordered_json printPDREntityAssociation(uint8_t* data)
    {
        ...
        std::cout << peadata.dump(4) << std::endl;
        return peadata;
    }

    ordered_json printNumericEffecterPDR(uint8_t* data)
    {
    ...
    ordered_json printStateEffecterPDR(const uint8_t* data)
    {
    ...
    ordered_json printTerminusLocatorPDR(const uint8_t* data)
    {
                     ...
                     uint8_t* data)
    {
        ...
        ordered_json header = printCommonPDRHeader(pdr);
        output.insert(output.end(), header.begin(), header.end());

        ordered_json pdrtypeinfo;
        switch (pdr->type)
        {
        ...
        output.insert(output.end(), pdrtypeinfo.begin(), pdrtypeinfo.end());
