...
class GetNumericEffecterValue : public CommandInterface
{
  public:
    ...
    void parseResponseMsg(pldm_msg* responsePtr, size_t payloadLength) override
    {
        ...
        std::array<uint8_t, 4> pendingValue{};
        std::array<uint8_t, 4> presentValue{};
       ...
       switch(effecterDataSize)
        {
           case PLDM_EFFECTER_DATA_SIZE_UINT8:
           {
              uint8_t *pendingData = reinterpret_cast<uint8_t*>(pendingValue.data());
              uint8_t *presentData = reinterpret_cast<uint8_t*>(presentValue.data());
              output["pendingValue"] = *pendingData;
              output["presentValue"] = *presentData;
              break;
           }
           case PLDM_EFFECTER_DATA_SIZE_SINT8:
           {
              int8_t *pendingData = reinterpret_cast<int8_t*>(pendingValue.data());
              int8_t *presentData = reinterpret_cast<int8_t*>(presentValue.data());
              output["pendingValue"] = *pendingData;
              output["presentValue"] = *presentData;
              break;
           }
           case PLDM_EFFECTER_DATA_SIZE_UINT16:
           {
              uint16_t *pendingData = reinterpret_cast<uint16_t*>(pendingValue.data());
              uint16_t *presentData = reinterpret_cast<uint16_t*>(presentValue.data());
              output["pendingValue"] = *pendingData;
              output["presentValue"] = *presentData;
              break;
           }
           case PLDM_EFFECTER_DATA_SIZE_SINT16:
           {
              int16_t *pendingData = reinterpret_cast<int16_t*>(pendingValue.data());
              int16_t *presentData = reinterpret_cast<int16_t*>(presentValue.data());
              output["pendingValue"] = *pendingData;
              output["presentValue"] = *presentData;
              break;
           }
           case PLDM_EFFECTER_DATA_SIZE_UINT32:
           {
              uint32_t *pendingData = reinterpret_cast<uint32_t*>(pendingValue.data());
              uint32_t *presentData = reinterpret_cast<uint32_t*>(presentValue.data());
              output["pendingValue"] = *pendingData;
              output["presentValue"] = *presentData;
              break;
           }
           case PLDM_EFFECTER_DATA_SIZE_SINT32:
           {
              int32_t *pendingData = reinterpret_cast<int32_t*>(pendingValue.data());
              int32_t *presentData = reinterpret_cast<int32_t*>(presentValue.data());
              output["pendingValue"] = *pendingData;
              output["presentValue"] = *presentData;
              break;
           }
