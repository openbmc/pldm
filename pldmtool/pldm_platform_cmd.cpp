  ...
  private:
    ...
    void printNumericSensorPDR(const uint8_t* data, ordered_json& output)
    {
        struct pldm_numeric_sensor_value_pdr* pdr =
            (struct pldm_numeric_sensor_value_pdr*)data;
        if (!pdr)
        {
        ...
        output["entityType"] = getEntityName(pdr->entity_type);
        output["entityInstanceNumber"] = int(pdr->entity_instance_num);
        output["containerID"] = int(pdr->container_id);
        output["sensorInit"] = unsigned(pdr->sensor_init);
        output["sensorAuxiliaryNamesPDR"] =
        ...
        switch (pdr->sensor_data_size)
        {
            ...
            case PLDM_SENSOR_DATA_SIZE_SINT8:
                output["hysteresis"] = unsigned(pdr->hysteresis.value_s8);
                output["maxReadable"] = unsigned(pdr->max_readable.value_s8);
                output["minReadable"] = unsigned(pdr->min_readable.value_s8);
                break;
            ...
            case PLDM_SENSOR_DATA_SIZE_SINT16:
                output["hysteresis"] = unsigned(pdr->hysteresis.value_s16);
                output["maxReadable"] = unsigned(pdr->max_readable.value_s16);
                output["minReadable"] = unsigned(pdr->min_readable.value_s16);
                break;
            ...
            case PLDM_SENSOR_DATA_SIZE_SINT32:
                output["hysteresis"] = unsigned(pdr->hysteresis.value_s32);
                output["maxReadable"] = unsigned(pdr->max_readable.value_s32);
                output["minReadable"] = unsigned(pdr->min_readable.value_s32);
                break;
            default:
                break;
        }

        output["supportedThresholds"] =
            unsigned(pdr->supported_thresholds.byte);
        output["thresholAndHysteresisVolatility"] =
            unsigned(pdr->threshold_and_hysteresis_volatility.byte);
        output["stateTransitionInterval"] =
            unsigned(pdr->state_transition_interval);
        output["updateInterval"] = unsigned(pdr->update_interval);
        output["rangeFieldFormat"] = unsigned(pdr->range_field_format);
        output["rangeFieldSupport"] = unsigned(pdr->range_field_support.byte);
        ...
        switch (pdr->range_field_format)
        {
            case PLDM_RANGE_FIELD_FORMAT_UINT8:
                ...
                output["fatalLeow"] = unsigned(pdr->fatal_low.value_u8);
                break;
            case PLDM_RANGE_FIELD_FORMAT_SINT8:
                output["nominalValue"] = unsigned(pdr->nominal_value.value_s8);
                output["normalMax"] = unsigned(pdr->normal_max.value_s8);
                output["normalMin"] = unsigned(pdr->normal_min.value_s8);
                output["warningHigh"] = unsigned(pdr->warning_high.value_s8);
                output["warningLow"] = unsigned(pdr->warning_low.value_s8);
                output["criticalHigh"] = unsigned(pdr->critical_high.value_s8);
                output["criticalLow"] = unsigned(pdr->critical_low.value_s8);
                output["fatalHigh"] = unsigned(pdr->fatal_high.value_s8);
                output["fatalLeow"] = unsigned(pdr->fatal_low.value_s8);
                break;
            case PLDM_RANGE_FIELD_FORMAT_UINT16:
                output["nominalValue"] = unsigned(pdr->nominal_value.value_u16);
                output["normalMax"] = unsigned(pdr->normal_max.value_u16);
                ...
                output["fatalLeow"] = unsigned(pdr->fatal_low.value_u16);
                break;
            case PLDM_RANGE_FIELD_FORMAT_SINT16:
                output["nominalValue"] = unsigned(pdr->nominal_value.value_s16);
                output["normalMax"] = unsigned(pdr->normal_max.value_s16);
                output["normalMin"] = unsigned(pdr->normal_min.value_s16);
                output["warningHigh"] = unsigned(pdr->warning_high.value_s16);
                output["warningLow"] = unsigned(pdr->warning_low.value_s16);
                output["criticalHigh"] = unsigned(pdr->critical_high.value_s16);
                output["criticalLow"] = unsigned(pdr->critical_low.value_s16);
                output["fatalHigh"] = unsigned(pdr->fatal_high.value_s16);
                output["fatalLeow"] = unsigned(pdr->fatal_low.value_s16);
                break;
            case PLDM_RANGE_FIELD_FORMAT_UINT32:
                output["nominalValue"] = unsigned(pdr->nominal_value.value_u32);
                output["normalMax"] = unsigned(pdr->normal_max.value_u32);
                ...
                output["fatalLeow"] = unsigned(pdr->fatal_low.value_u32);
                break;
            case PLDM_RANGE_FIELD_FORMAT_SINT32:
                output["nominalValue"] = unsigned(pdr->nominal_value.value_s32);
                output["normalMax"] = unsigned(pdr->normal_max.value_s32);
                output["normalMin"] = unsigned(pdr->normal_min.value_s32);
                output["warningHigh"] = unsigned(pdr->warning_high.value_s32);
                output["warningLow"] = unsigned(pdr->warning_low.value_s32);
                output["criticalHigh"] = unsigned(pdr->critical_high.value_s32);
                output["criticalLow"] = unsigned(pdr->critical_low.value_s32);
                output["fatalHigh"] = unsigned(pdr->fatal_high.value_s32);
                output["fatalLeow"] = unsigned(pdr->fatal_low.value_s32);
                break;
            case PLDM_RANGE_FIELD_FORMAT_REAL32:
                output["nominalValue"] = unsigned(pdr->nominal_value.value_f32);
                output["normalMax"] = unsigned(pdr->normal_max.value_f32);
                output["normalMin"] = unsigned(pdr->normal_min.value_f32);
                output["warningHigh"] = unsigned(pdr->warning_high.value_f32);
                output["warningLow"] = unsigned(pdr->warning_low.value_f32);
                output["criticalHigh"] = unsigned(pdr->critical_high.value_f32);
                output["criticalLow"] = unsigned(pdr->critical_low.value_f32);
                output["fatalHigh"] = unsigned(pdr->fatal_high.value_f32);
                output["fatalLeow"] = unsigned(pdr->fatal_low.value_f32);
                break;
            default:
                break;
        }
