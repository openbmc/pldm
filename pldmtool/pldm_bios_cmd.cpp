...
class GetBIOSTableHandler : public CommandInterface
{
  public:
    ...
    json displayAttributeValueEntry(
        ...
        const std::optional<Table>& stringTable, bool verbose)
    {
        ...
        json attrvaldata = json::object({});
        if (verbose)
        {
            attrvaldata["AttributeHandle"] = attrHandle;
            attrvaldata["AttributeType"] = attrTypeMap.at(attrType);
        }
        switch (attrType)
        {
            ...
            case PLDM_BIOS_ENUMERATION_READ_ONLY:
            {
                ...
                if (verbose)
                {
                    attrvaldata["NumberOfCurrentValues"] = (int)count;
                }
                for (size_t i = 0; i < handles.size(); i++)
                {
                    if (verbose)
                    {
                        attrvaldata["CurrentValueStringHandleIndex[" +
                                    std::to_string(i) + "]"] =
                    ...
                    {
                        attrvaldata["CurrentValue"] = displayEnumValueByIndex(
            ...
            case PLDM_BIOS_STRING_READ_ONLY:
            {
                ...
                if (verbose)
                {
                    attrvaldata["CurrentStringLength"] = currentString.length;
                    attrvaldata["CurrentString"] = std::string(
                        reinterpret_cast<const char*>(currentString.ptr),
                        currentString.length);
                }
                else
                {
                    attrvaldata["CurrentValue"] = std::string(
                        reinterpret_cast<const char*>(currentString.ptr),
                        currentString.length);
                }
...
class GetBIOSTable : public GetBIOSTableHandler
{
  ...
  private:
                                   ...
                                   const std::optional<Table>& stringTable)
    {
        ...
        {
            avdata = displayAttributeValueEntry(tableEntry, attrTable,
                                                stringTable, true);
            output.emplace_back(avdata);
        }
        pldmtool::helper::DisplayInJson(output);
    }
};
...
class GetBIOSAttributeCurrentValueByHandle : public GetBIOSTableHandler
{
  public:
    ...
    void exec()
    {
        ...
        json avdata = displayAttributeValueEntry(tableEntry, attrTable,
                                                 stringTable, false);
