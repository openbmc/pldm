...
void HostEffecterParser::parseEffecterJson(const std::string& jsonPath)
{
    ...
    try
    {
        data = validateJson(jsonFilePath);
    }
    ...
    size_t dbusInfoIndex, uint16_t effecterId)
{
    ...
    {
        effecterId = pldm::responder::pdr_utils::findStateEffecterId(
    ...
    uint16_t effecterId)
{
    ...
    if (rc != PLDM_REQUESTER_SUCCESS)
    {
        ...
        errMsg << "Failed to send message/receive response. RC = " << rc
    ...
    {
        std::ostringstream errMsg;
        errMsg << "Failed setStateEffecterStates for effecter " << effecterId
