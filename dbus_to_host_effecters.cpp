...

{
    ...
    for (auto it = dBusValues.begin(); it != dBusValues.end(); ++it)
    {
        ...
        {
            std::cerr << "Unknown D-Bus property type, TYPE=" << propertyType.c_str()
                      << "\n";
            return;
        }
    ...
    uint32_t dbusInfoIndex)
{
             ...
             0x4) // hardcoded effecter id for Phyp
    {
        ...
        try
        {
            ...
            if (currHostState != "xyz.openbmc_project.State.OperatingSystem.Status.OSStatus.Standby")
                {
                    std::cout <<"Host is not at standby. Current host state: " << currHostState.c_str() << "\n";
                    return;
                }
        }
    ...
    uint8_t& instanceId)
{
    ...
    std::vector<uint8_t> requestMsg(
        ...
        sizeof(set_effecter_state_field) * compEffCnt,0);
