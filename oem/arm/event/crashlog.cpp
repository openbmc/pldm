...
constexpr const char* systemDumpStagingDir = "/tmp/system-dumps";
 ...
 */
std::vector<std::shared_ptr<platform_mc::FileDescriptor>> findCrashlogFiles(
    platform_mc::Manager& manager, pldm_tid_t tid)
{
 ...
 */
std::vector<std::shared_ptr<platform_mc::FileDescriptor>> waitForCrashlogFiles(
    platform_mc::Manager& manager, pldm_tid_t tid)
{
    ...
    const fs::path& outputPath, uint64_t expectedSize, uint64_t& totalBytes)
{
    ...
    auto fd =
        open(outputPath.c_str(), O_CREAT | O_EXCL | O_WRONLY | O_CLOEXEC, 0600);
    if (fd < 0)
    {
...
int notifySystemDump(uint32_t sourceId, uint64_t size)
{
    try
    {
        ...
        auto method = bus.new_method_call(dumpManagerService, systemDumpPath,
    ...
    std::vector<CollectedCrashlogFile>& collectedFiles)
{
    ...
    if (!currentSize)
    {
        lg2::error("Failed to read size from PLDM crashlog file {FILE}", "FILE",
                   file->getFileName());
...
int collectCrashlog(platform_mc::Manager& manager, pldm_tid_t tid)
{
    ...
    auto workDir =
        fs::path(crashlogArchiveWorkDir) /
        ("pldm-crashlog-" + std::to_string(sourceId) + "-" + timestamp);
    auto archivePath =
        fs::path(systemDumpStagingDir) / std::to_string(sourceId);
    std::vector<CollectedCrashlogFile> collectedFiles;
    ...
    {
        auto rc =
            collectCrashlogFile(files[index], index, workDir, collectedFiles);
        if (rc != PLDM_SUCCESS)
        {
            fs::remove_all(workDir);
            fs::remove(archivePath);
                          ...
                          uint16_t sensorId, uint8_t eventState)
{
    ...
    if (eventState != deviceFileUpdatedState)
    {
        lg2::debug("Ignoring unsupported Device File state {STATE} from "
                   ...
                   "STATE", eventState, "NAME", *terminusName, "SID", sensorId);
        return PLDM_SUCCESS;
    }
    ...
    std::thread([manager, tid]() { collectCrashlog(*manager, tid); }).detach();
