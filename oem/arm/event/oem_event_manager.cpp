                             ...
                             const std::variant<uint64_t, std::string>& value)
{
    auto method = bus.new_method_call(
        service.c_str(), bootProgressObjectPath, pldm::utils::dbusProperties,
        "Set");
    ...
    pldm_tid_t tid, size_t eventDataOffset)
{
    ...
    {
        lg2::debug(
            "Ignoring Arm sensor event from terminus {TID}; "
...
int OemEventManager::updateBootProgress(uint32_t presentReading) const
{
    ...
    try
    {
        ...
        auto service = dbusHandler.getService(bootRawObjectPath,
                                              bootRawInterface);
        auto method = bus.new_method_call(
        ...
        setBootProgressProperty(
            bus, bootProgressService, bootProgressProperty,
