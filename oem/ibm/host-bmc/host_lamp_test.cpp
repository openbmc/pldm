...
bool HostLampTest::asserted(bool value)
{
    ...
    sendHostStateEffecter(effecterID);
...
uint16_t HostLampTest::getEffecterID()
{
    ...
    try
    {
        std::vector<std::vector<uint8_t>> stateEffecterPDRs{};
        auto method = bus.new_method_call(
            "xyz.openbmc_project.PLDM", "/xyz/openbmc_project/pldm",
            "xyz.openbmc_project.PLDM.PDR", "FindStateEffecterPDR");
        method.append(mctp_eid, entityType,
                      (uint16_t)PLDM_STATE_SET_IDENTIFY_STATE);

        auto reply = bus.call(method);
        reply.read(stateEffecterPDRs);
    ...
    {
        std::cerr << "host lamp test: Error get effecterID,ERROR=" << e.what()
                  << "\n";
    }
...
int HostLampTest::sendHostStateEffecter(uint16_t effecterID)
{
