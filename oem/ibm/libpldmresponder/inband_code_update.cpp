...
std::string CodeUpdate::fetchCurrentBootSide() { return currBootSide; }

std::string CodeUpdate::fetchNextBootSide() { return nextBootSide; }
...
void CodeUpdate::setVersions() {
  ...
  try {
    ...
    auto method1 =
        bus.new_method_call(MAPPER_SERVICE, ACTIVE_OBJ_PATH, PROP_INTF, "Get");
...
void CodeUpdate::processPriorityChangeNotification(
  ...
  nextBootSide =
      (newVal == 0) ? currBootSide : ((currBootSide == Tside) ? Pside : Tside);
}
...
void CodeUpdate::deleteImage() {
  ...
  auto &bus = dBusIntf->getBus();
  try {
    auto method = bus.new_method_call(UPDATER_SERVICE, SW_OBJ_PATH, DELETE_INTF,
                                      "DeleteAll");
    bus.call_noreply(method);
  } catch (const std::exception &e) {
...
void generateStateEffecterOEMPDR(platform::Handler *platformHandler,
  ...
  size_t pdrSize = 0;
  pdrSize =
      sizeof(pldm_state_effecter_pdr) + sizeof(state_effecter_possible_states);
...
void generateStateSensorOEMPDR(platform::Handler *platformHandler,
  ...
  possibleStates->state_set_id = stateSetID;
  possibleStates->possible_states_size = 2;
  auto state = reinterpret_cast<state_sensor_possible_states *>(possibleStates);
  if ((stateSetID == oem_ibm_platform::PLDM_OEM_IBM_BOOT_STATE) ||
...
template <typename... T> int executeCmd(T const &... t) {
...
int assembleCodeUpdateImage() {
  ...
  rc = executeCmd("/bin/tar", "-xf", tarImagePath.c_str(), "-C", updateDirPath);
  if (rc < 0) {
    return PLDM_ERROR;
  }
