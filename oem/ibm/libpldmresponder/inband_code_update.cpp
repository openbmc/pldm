...
void generateStateEffecterOEMPDR(oem_platform::Handler* oemPlatformHandler,
...
/*
void generateStateSensorOEMPDR(uint16_t nextSensorId,uint16_t entityInstance, 
                               uint16_t stateSetID,
                               pdr_utils::RepoInterface& repo)
{
    size_t pdrSize = 0;
...
void buildAllCodeUpdateEffecterPDR(pldm::responder::oem_platform::Handler* oemPlatformHandler,
                                   pdr_utils::RepoInterface& repo)
{
    generateStateEffecterOEMPDR(oemPlatformHandler,ENTITY_INSTANCE_0,BOOT_SIDE_ID,
                                repo);
    //generateStateEffecterOEMPDR(oemPlatformHandler,ENTITY_INSTANCE_1,BOOT_SIDE_ID, 
      //                          repo);
    //generateStateEffecterOEMPDR(oemPlatformHandler,ENTITY_INSTANCE_0,FW_UPDATE_ID, 
      //                          repo);
...
void buildAllCodeUpdateSensorPDR(uint16_t nextSensorId,
                                pdr_utils::RepoInterface& repo)
{
    generateStateSensorOEMPDR(nextSensorId++,ENTITY_INSTANCE_0,BOOT_SIDE_ID, 
                              repo);
    generateStateSensorOEMPDR(nextSensorId++,ENTITY_INSTANCE_1,BOOT_SIDE_ID, 
                              repo);
    generateStateSensorOEMPDR(nextSensorId,ENTITY_INSTANCE_0,FW_UPDATE_ID, 
                              repo);
