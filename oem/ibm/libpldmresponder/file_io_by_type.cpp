                                  ...
                                  uint64_t address)
{
    ...
    if (upstream)
    {
        ...
        if (!fileExists)
        {
            error("File '{PATH}' does not exist.", "PATH", path.c_str());
    ...
    if (file == -1)
    {
        error("File '{PATH}' does not exist.", "PATH", path.string());
        return PLDM_ERROR;
    }
                          ...
                          uint32_t& length, Response& response)
{
    ...
    {
        error("File '{PATH}' does not exist, handle '{FILE_HANDLE}'", "PATH",
              filePath.c_str(), "FILE_HANDLE", fileHandle);
        return PLDM_INVALID_FILE_HANDLE;
    }
    ...
    error("Unable to read file '{PATH}'", "PATH", filePath.c_str());
    return PLDM_ERROR;
}
