...
int PelHandler::handle(DMA* xdmaInterface, uint8_t command, bool upstream)
{
    ...
    fs::create_directories(PEL_TEMP_DIR); // use tempfs for the actual dbus call
    ...
    std::cout << "calling transferDataHost with address " << address
              << std::endl;

    while (origLength > dma::maxSize)
    {
        std::cout << "inside while loop transferDataHost with address "
                  << address << std::endl;
    ...
    std::cout
        << "last chunk of data getting thru transferDataHost from address  "
        << address << std::endl;
    ...
    std::cout << "after getService" << std::endl;
    ...
    auto severity =
        sdbusplus::xyz::openbmc_project::Logging::server::convertForMessage(
            sdbusplus::xyz::openbmc_project::Logging::server::Entry::Level::
                Error);
    std::cout << "before new_method_call" << std::endl;
