...
Response readFile(const pldm_msg* request, size_t payloadLength)
{
    ...
    {
        encode_read_file_resp(0, PLDM_ERROR_INVALID_LENGTH, length,
                              attrTable.data(), responsePtr);
        return response;
    }
    ...
    catch (std::exception& e)
    {
        log<level::ERR>("File handle does not exist in the file table",
                        entry("HANDLE=%d", fileHandle));
        encode_read_file_resp(0, PLDM_INVALID_FILE_HANDLE, length,
                              attrTable.data(), responsePtr);
        return response;
    }
    ...
    if (offset > fileSize)
    {
        log<level::ERR>("Offset exceeds file size", entry("OFFSET=%d", offset),
                        entry("FILE_SIZE=%d", fileSize));
        encode_read_file_resp(0, PLDM_DATA_OUT_OF_RANGE, length,
                              attrTable.data(), responsePtr);
        return response;
    }
    ...
    response.resize(PLDM_READ_FILE_RESP_BYTES + attrTable.size());
    responsePtr = reinterpret_cast<pldm_msg*>(response.data());

    encode_read_file_resp(0, PLDM_SUCCESS, length, attrTable.data(),
                          responsePtr);
...
Response writeFile(const pldm_msg* request, size_t payloadLength)
{
    ...
    decode_write_file_req(request->payload, payloadLength, &fileHandle, &offset,
                          &length, attrTable.data());
