                                     ...
                                     size_t payloadLength)
{
    ...
    if (offset >= fileSize)
    {
        error("Handler::readFileIntoMemory:Offset exceeds file size, OFFSET={OFFSTE} FILE_SIZE={FILE_SIZE} FILE_HANDLE{FILE_HANDLE}",
                                      ...
                                      size_t payloadLength)
{
    ...
    if (offset >= fileSize)
    {
        error("Handler::writeFileFromMemory:Offset exceeds file size, OFFSET={OFFSET} FILE_SIZE={FILE_SIZE} FILE_HANDLE{FILE_HANDLE}",
              "OFFSET", offset, "FILE_SIZE", fileSize, "FILE_HANDLE", fileHandle);
        encode_rw_file_memory_resp(request->hdr.instance_id,
                                   PLDM_WRITE_FILE_FROM_MEMORY,
                                   PLDM_DATA_OUT_OF_RANGE, 0, responsePtr);
