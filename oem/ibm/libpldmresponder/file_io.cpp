                                     ...
                                     size_t payloadLength)
{
    ...
    {
        error(
            "PLDM_READ_FILE_INTO_MEMORY length '{LEN}' is not a multiple of DMA minSize",
            "LEN", length);
        encodeRWResponseHandler(request->hdr.instance_id,
                                PLDM_READ_FILE_INTO_MEMORY,
                                PLDM_ERROR_INVALID_LENGTH, 0, responsePtr);
                                      ...
                                      size_t payloadLength)
{
    ...
    {
        error(
            "PLDM_WRITE_FILE_FROM_MEMORY length '{LEN}' is not a multiple of DMA minSize",
            "LEN", length);
        encodeRWResponseHandler(request->hdr.instance_id,
                                PLDM_WRITE_FILE_FROM_MEMORY,
                                PLDM_ERROR_INVALID_LENGTH, 0, responsePtr);
                                ...
                                oem_platform::Handler* oemPlatformHandler)
{
    ...
    {
        error(
            "Length '{LEN}' is not a multiple of DMA minSize for command {CMD}",
            "LEN", length, "CMD", cmd);
        encodeRWTypeMemoryResponseHandler(request->hdr.instance_id, cmd,
                                          PLDM_ERROR_INVALID_LENGTH, 0,
                                          responsePtr);
