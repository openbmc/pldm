                                     ...
                                     size_t payloadLength)
{
    ...
    {
        error(
            "File handle does not exist in the file table, HANDLE={FILE_HNDL}, {ERR_EXCEP}",
                                ...
                                oem_platform::Handler* oemPlatformHandler)
{
    ...
    {
        error("unknown file type, TYPE={LEN}, {ERR_EXCEP}", "LEN", fileType,
              "ERR_EXCEP", e);
        encode_rw_file_by_type_memory_resp(request->hdr.instance_id, cmd,
...
Response Handler::writeFileByType(const pldm_msg* request, size_t payloadLength)
{
    ...
    {
        error("unknown file type, TYPE={FILE_TYP}, {ERR_EXCEP}", "FILE_TYP",
...
Response Handler::fileAck(const pldm_msg* request, size_t payloadLength)
{
    ...
    {
        error("unknown file type, TYPE={TYPE}, {ERR_EXCEP}", "TYPE", fileType,
              "ERR_EXCEP", e);
