                               ...
                               uint64_t address, bool upstream)
{
    ...
    while (length > pageAlignedLength)
    {
        rc = transferDataHost(fd, xdmaFd(), pageAlignedLength, offset,
                              pageAlignedLength, address, upstream,
                              vgaMemPtr.get());
        if (rc < 0)
        {
            return rc;
        }
        offset += pageAlignedLength;
        length -= pageAlignedLength;
        address += pageAlignedLength;
    }
    rc =
        transferDataHost(fd, xdmaFd(), pageAlignedLength, offset,
                         pageAlignedLength, address, upstream, vgaMemPtr.get());
    return rc;
}
