...
Response Handler::getAlertStatus(const pldm_msg* request, size_t payloadLength)
{
    ...
    if (versionId != 0)
    {
        return CmdHandler::ccOnlyResponse(request, UNSUPPORTED_FORMAT_VERSION);
    }

    constexpr uint32_t rack_entry = 0xFF000030;
    constexpr uint32_t pri_cec_node = 0x00008030;
    rc = encode_get_alert_status_resp(request->hdr.instance_id, PLDM_SUCCESS,
                                      rack_entry, pri_cec_node, responsePtr,
