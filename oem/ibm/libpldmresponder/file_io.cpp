...
int DMA::transferHostDataToSocket(int fd, uint32_t length, uint64_t address)
{
    ...
    if (dmaFd < 0)
    {
        rc = -errno;
        error(
            "Failed to open the XDMA device for transferring host data to socket with response code '{RC}'",
            "RC", rc);
        return rc;
    }
    ...
    if (MAP_FAILED == vgaMem)
    {
        rc = -errno;
        error(
            "Failed to mmap the XDMA device for transferring host data to socket with response code '{RC}'",
            "RC", rc);
        return rc;
    }
    ...
    if (rc < 0)
    {
        rc = -errno;
        error(
            "Failed to execute the DMA operation for transfering host data to socket at address '{ADDRESS}' and length '{LENGTH}' with response code '{RC}'",
    ...
    if (rc < 0)
    {
        ...
        close(fd);
        error(
            "Closing socket as write to Unix socket failed for transfering host data to socket with response code '{RC}'",
            "RC", rc);
        return rc;
    }
    return 0;
                          ...
                          uint64_t address, bool upstream)
{
    ...
    auto mmapCleanup = [pageAlignedLength, &rc](void* vgaMem) {
        ...
        {
            error(
                "Received interrupt during DMA transfer for data between BMC and Host. Skipping Unmap.");
        }
    };
    ...
    if (dmaFd < 0)
    {
        rc = -errno;
        error(
            "Failed to open the XDMA device for data transfer between BMC and Host with response code '{RC}'",
            "RC", rc);
        return rc;
    }
    ...
    if (MAP_FAILED == vgaMem)
    {
        rc = -errno;
        error(
            "Failed to mmap the XDMA device for data transfer between BMC and Host with response code '{RC}'",
            "RC", rc);
        return rc;
    }
    ...
    if (upstream)
    {
        ...
        {
            error(
                "Upstream data transfer between BMC and Host where lseek for upstream '{UPSTREAM}' at offset '{OFFSET}' failed, error number - {ERROR_NUM}",
        ...
        {
            error(
                "Upstream data transfer between BMC and Host where file read for upstream '{UPSTREAM}' of length '{LENGTH}' at offset '{OFFSET}' failed, error number - {ERROR_NUM}",
        ...
        {
            error(
                "Upstream data transfer between BMC and Host mismatched for number of characters to read for upstream '{UPSTREAM}' and the length '{LENGTH}' read  and count '{RC}'",
    ...
    if (rc < 0)
    {
        rc = -errno;
        error(
            "Failed to execute the DMA operation on data between BMC and Host for upstream '{UPSTREAM}' of length '{LENGTH}' at address '{ADDRESS}', response code '{RC}'",
    ...
    if (!upstream)
    {
        ...
        {
            error(
                "Downstream data transfer between BMC and Host with lseek upstream '{UPSTREAM}' at offset '{OFFSET}' failed, error number - {ERROR_NUM}",
                "ERROR_NUM", errno, "UPSTREAM", upstream, "OFFSET", offset);
            return rc;
        }
        ...
        {
            error(
                "Downstream data transfer between BMC and Host where file write upstream '{UPSTREAM}' of length '{LENGTH}' at offset '{OFFSET}' failed, error number - {ERROR_NUM}",
                "ERROR_NUM", errno, "UPSTREAM", upstream, "LENGTH", length,
                "OFFSET", offset);
            return rc;
        }
                                     ...
                                     size_t payloadLength)
{
    ...
    {
        error("Read length '{LENGTH}' is not a multiple of DMA minSize",
              "LENGTH", length);
        encode_rw_file_memory_resp(request->hdr.instance_id,
                                   PLDM_READ_FILE_INTO_MEMORY,
                                   PLDM_ERROR_INVALID_LENGTH, 0, responsePtr);
                                      ...
                                      size_t payloadLength)
{
    ...
    if (length % dma::minSize)
    {
        error("Write length '{LENGTH}' is not a multiple of DMA minSize",
              "LENGTH", length);
        encode_rw_file_memory_resp(request->hdr.instance_id,
                                   PLDM_WRITE_FILE_FROM_MEMORY,
                                   PLDM_ERROR_INVALID_LENGTH, 0, responsePtr);
                                ...
                                oem_platform::Handler* oemPlatformHandler)
{
    ...
    if (length % dma::minSize)
    {
        error("Read/write length '{LENGTH}' is not a multiple of DMA minSize",
              "LENGTH", length);
        encode_rw_file_by_type_memory_resp(request->hdr.instance_id, cmd,
                                           PLDM_ERROR_INVALID_LENGTH, 0,
                                           responsePtr);
