    ...
    for (uint8_t currState = 0; currState < compEffecterCnt; ++currState)
    {
        if (stateField[currState].set_request == PLDM_REQUEST_SET)
        {
                     ...
                     stateSetId == PLDM_OEM_IBM_SYSTEM_POWER_STATE)
            {
                ...
                {
                    std::cout << "received system reboot setStateEffecterState \n";
                    systemRebootEvent =
                        std::make_unique<sdeventplus::source::Defer>(
                            event,
                            std::bind(std::mem_fn(&oem_ibm_platform::Handler::
    ...
    sdeventplus::source::EventBase& /*source */)
{
    ...
    static constexpr auto objectPathPR = "/xyz/openbmc_project/control/host0/power_restore_policy/one_time";
    static constexpr auto interfacePR = "xyz.openbmc_project.Control.Power.RestorePolicy";
    static constexpr auto propertyNamePR = "PowerRestorePolicy";
    static constexpr auto valuePR = "xyz.openbmc_project.Control.Power.RestorePolicy.Policy.AlwaysOn";
    auto rc =
        codeUpdate->setSystemReboot(objectPathPR, interfacePR, propertyNamePR, valuePR);
    if (rc != PLDM_SUCCESS)
    {   
    ...
    static constexpr auto objectPathChassis = "/xyz/openbmc_project/state/chassis0";
    static constexpr auto interfaceChassis = "xyz.openbmc_project.State.Chassis";
    static constexpr auto propertyNameChasis = "RequestedPowerTransition";
    static constexpr auto valueChassis = "xyz.openbmc_project.State.Chassis.Transition.Off";

    rc =
        codeUpdate->setSystemReboot(objectPathChassis, interfaceChassis, propertyNameChasis, valueChassis);
    if (rc != PLDM_SUCCESS)
    {
    ...
    static constexpr auto valueHost = "xyz.openbmc_project.State.Host.Transition.Off";

    rc =
        codeUpdate->setSystemReboot(objectPathHost, interfaceHost, propertyNameHost, valueHost);
    if (rc != PLDM_SUCCESS)
    {
    ...
    static constexpr auto valueBmc = "xyz.openbmc_project.State.BMC.Transition.Reboot";

    rc =
        codeUpdate->setSystemReboot(objectPathBmc, interfaceBmc, propertyNameBmc, valueBmc);
    if (rc != PLDM_SUCCESS)
    {
