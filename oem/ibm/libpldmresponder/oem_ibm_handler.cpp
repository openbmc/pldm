...
{
    std::cerr << "Building numeric effecter pdr" << std::endl;
    size_t pdrSize = 0;
    ...
    for (const auto& entity_path : procObjectPaths)
    {
        ...
        std::cerr << "Effecter id when building numeric PDR: " << effecterId
                  << std::endl;
        pdr->entity_type = entityType;
        ...
        if (entity_path.rfind('/') != std::string::npos)
        {
            ...
            std::cerr << "after emplacing the values" << std::endl;
...
{
    std::cerr << "Inside the update conatiner id method" << std::endl;
    for (auto& [key, value] : instanceMap)
    {
        std::cerr << "EffecterID inside the update container id method:" << key
                  << std::endl;
        std::cerr << "dcm id:" << (uint16_t)value.dcmId << std::endl;
        std::cerr << "proc id:" << (uint16_t)value.procId << std::endl;
        std::cerr << " Calling pldm_find_container_id" << std::endl;
        uint16_t newContainerID = pldm_find_container_id(
            pdrRepo, PLDM_ENTITY_PROC_MODULE, value.dcmId);
        std::cerr << "new container Id: " << newContainerID << std::endl;
        std::cerr << "Calling pldm_change_container_id_of_effecter"
                  << std::endl;
