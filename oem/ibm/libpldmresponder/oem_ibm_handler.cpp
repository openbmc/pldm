        ...
        if (stateField[currState].set_request == PLDM_REQUEST_SET)
        {
                     ...
                     stateSetId == 32768)
            {
                ...
                else if (stateField[currState].effecter_state == END)
                {
                    ...
                    uint32_t fileHandle{};
                    std::unique_ptr<LidHandler> lidHandler = 
                                 std::make_unique<LidHandler>(fileHandle, true);
                    std::cout << "before assembleFinalImage \n";             
                    int retc = lidHandler->assembleFinalImage();
                    std::cout << "after assembleFinalImage \n";
                    if (retc == PLDM_SUCCESS)
                    {
                        std::cout << "assembleFinalImage returned success \n";
