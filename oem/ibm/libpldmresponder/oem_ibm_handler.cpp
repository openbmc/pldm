    ...
    for (uint8_t currState = 0; currState < compEffecterCnt; ++currState)
    {
        if (stateField[currState].set_request == PLDM_REQUEST_SET)
        {
            ...
            else if (stateSetId == PLDM_OEM_IBM_SLOT_ENABLE_EFFECTER_STATE)
            {
                ...
                {
                    if (value.entity_instance_num == entityInstance &&
                        value.entity_type == PLDM_ENTITY_SLOT &&
                        stateField[currState].effecter_state ==
                            uint8_t(SlotEnableEffecterState::ENABLE))
                    {
                        // register a property change signal for the presence
                        slotEnable->createPresenceMatch(key, stateSetId);
                        // send the dbus path to collect VPD method ()
                    }
                }
            }
            else
