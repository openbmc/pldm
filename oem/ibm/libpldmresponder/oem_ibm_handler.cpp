    ...
    std::vector<uint8_t>& requestMsg, uint8_t instanceId)
{
    ...
    auto oemPlatformEventMessageResponseHandler =
        ...
        {
            error(
                "Failed to decode_platform_event_message_resp for code update event with response code '{RC}' and completion code '{CC}'",
    ...
    uint8_t sensorOffset, uint8_t eventState, uint8_t prevEventState)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        error("Failed to send event to hostwith response code '{RC}'", "RC",
              rc);
    }
    return;
}
    ...
    sdeventplus::source::EventBase& /*source */)
{
    ...
    {
        error(
            "Chassis State transition to Off failed, unable to set property RequestedPowerTransition, error - {ERROR}",
            "ERROR", e);
    }
    ...
    chassisOffMatch = std::make_unique<sdbusplus::bus::match_t>(
        ...
        if (itr != props.end())
        {
            ...
            if (propVal == "xyz.openbmc_project.State.Chassis.PowerState.Off")
            {
                ...
                {
                    error(
                        "Setting one-time restore policy failed, unable to set property PowerRestorePolicy, error - {ERROR}",
                        "ERROR", e);
                }
                ...
                {
                    error(
                        "BMC state transition to reboot failed, unable to set property RequestedBMCTransition , error - {ERROR}",
                        "ERROR", e);
                }
            }
        }
