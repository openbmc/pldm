...
class LidHandler : public FileHandler
{
  public:
                                ...
                                oem_platform::Handler* oemPlatformHandler)
    {
        ...
        if (fd == -1)
        {
            error("Could not open file '{LID_PATH}' for writing", "LID_PATH",
                  lidPath);
            return PLDM_ERROR;
        }
        close(fd);
        ...
        if (rc != PLDM_SUCCESS)
        {
            error("Write file from memory failed with response code '{RC}'",
                  "RC", rc);
            return rc;
        }
                      ...
                      oem_platform::Handler* oemPlatformHandler)
    {
        ...
        {
            error(
                "Lseek at offset '{OFFSET}' failed, error number - {ERROR_NUM}",
