                                ...
                                sdeventplus::Event& event)
{
    ...
    Pelpath = path;
    transferFileData(path, false, offset, length, address,
                               responseHdr, event);
    return -1;
}
...
int PelHandler::postDataTransferCallBack(bool IsWriteToMemOp)
{
    if (IsWriteToMemOp)
    {
        ...
        if(rc != PLDM_SUCCESS){
            error("failed to storing a pel's Post DMA operation,Pelpath:{PATH} ERROR={ERR}", "ERR",
                  errno, "PATH", Pelpath.string());  
        }
    }
    else
    {
