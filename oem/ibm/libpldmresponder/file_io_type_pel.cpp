                               ...
                               oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    {
        error(
            "Get PEL D-Bus call failed, PEL ID '{FILE_HANDLE}', error - {ERROR}",
                     ...
                     oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    try
    {
        ...
        if (rc == -1)
        {
            error("File lseek failed at offset '{OFFSET}'", "OFFSET", offset);
            return PLDM_ERROR;
        }
    ...
    {
        error(
            "Get PEL D-Bus call failed on PEL ID {FILE_HANDLE}, error - {ERROR}",
            "FILE_HANDLE", lg2::hex, fileHandle, "ERROR", e);
        return PLDM_ERROR;
    }
    return PLDM_SUCCESS;
...
int PelHandler::fileAck(uint8_t fileStatus)
{
    ...
    if (fileStatus == PLDM_SUCCESS)
    {
        ...
        {
            error(
                "HostReject ack D-Bus call failed on PEL ID '{FILE_HANDLE}', error - {ERROR}",
                "FILE_HANDLE", lg2::hex, fileHandle, "ERROR", e);
            return PLDM_ERROR;
        }
    }
