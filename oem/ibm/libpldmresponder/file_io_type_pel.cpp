                               ...
                               oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    {
        error(
            "Get PEL D-Bus call failed, PEL ID = 0x{FILE_HANDLE}, error - {ERROR}",
                     ...
                     oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    {
        error(
            "Get PEL D-Bus call failed on PEL ID 0x{FILE_HANDLE}, error - {ERROR}",
            "FILE_HANDLE", lg2::hex, fileHandle, "ERROR", e);
        return PLDM_ERROR;
    }
    return PLDM_SUCCESS;
...
int PelHandler::fileAck(uint8_t fileStatus)
{
    ...
    if (fileStatus == PLDM_SUCCESS)
    {
        ...
        {
            error(
                "Host ack D-Bus call failed on PEL ID '{FILE_HANDLE}', error - {ERROR}",
                "FILE_HANDLE", lg2::hex, fileHandle, "ERROR", e);
            return PLDM_ERROR;
        }
    }
    else
    {
        ...
        {
            error("Host reject D-Bus call failed on PEL ID '{FILE_HANDLE}', "
                      ...
                      oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    {
        error(
            "File write of length '{LENGTH}' at offset '{OFFSET}' failed, error number - {ERROR_NUM}",
