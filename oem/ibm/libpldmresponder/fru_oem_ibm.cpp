...
//  tlv = reinterpret_cast<const pldm_fru_record_tlv*>(fruData.data());
  ...
  for(int i = 0; i < record->num_fru_fields; i++)
  {
   ...
   std::cerr << std::boolalpha << updateDBus << std::endl;
    if ((tlv->type == 253))
    {
       ...
       // std::cerr << "Value of PCIEValue ptr: " << (uint16_t)pciValuePtr << std::endl;
        ...
        auto subSystemId =
            std::to_string(((unsigned int)pciValuePtr + 44) >> 2 & 0xffff);
        std::cerr << " vendorID:"<< vendorId << " deviceId:" << deviceId << " revisionId:" << revisionId << " classCode : "<< classCode << "subSystemVendorId:" <<  subSystemVendorId << "subSystemId:" << subSystemId << std::endl;
...
/*pldm_entity Handler::getEntityIDfromRSI(uint16_t fruRSI)
{
    ...
    std::cerr << "FRU RSI iinside the get entity id function:" << fruRSI << std::endl;
    pldm_entity fruentity{};
    uint8_t* pdrData = nullptr;
    uint32_t pdrSize{};
    const pldm_pdr_record* record{};
    std::cerr << " Before do while loop " << std::endl;
    do
    {
        std::cerr << " Before find record by type:" << record << std::endl;
        record = pldm_pdr_find_record_by_type(pdrRepo, PLDM_PDR_FRU_RECORD_SET,
                                              record, &pdrData, &pdrSize);
        std::cerr << "after finding the record of type FRU RECORD SET" << std::endl;
        if (record  && (true ^ pldm_pdr_record_is_remote(record)))
        {
            std::cerr << " inside if loop when record is remote" << std::endl;
            auto pdr = reinterpret_cast<pldm_pdr_fru_record_set*>(pdrData);
            std::cerr << " pdr rsi :" << pdr->fru_rsi << std::endl;
            if (fruRSI == pdr->fru_rsi)
            {
                std::cerr << "found RSI is equal" << std::endl;
                                 ...
                                 std::string subSystemId)
{
 ...
 //   const pldm_entity entity = getEntityIDfromRSI(fruRSI);
   // std::cerr << "after entity from RSI" << std::endl;
  std::cerr <<"FRU RSI inside updateDBus:" << fruRSI << std::endl; 
   ...
   do{
     record = pldm_pdr_fru_record_set_find_by_rsi(pdrRepo, fruRSI, &terminusHandle, &entityType, &entityInstanceNum, &containerId);
