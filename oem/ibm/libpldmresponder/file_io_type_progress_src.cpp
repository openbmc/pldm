                               ...
                               oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    if (!secondaryCode.empty())
    {
        for (int byte : secondaryCode)
        {
            tmpstream << std::setfill('0') << std::setw(2) << std::hex << byte
                      << " ";
        }
        std::cout << "Progress SRC Structure : " << tmpstream.str()
                  << std::endl;
    }

    // Get the primary code from the offset 40 bytes in the received buffer

    std::vector<uint8_t> primaryCodeArray(secondaryCode.begin() + 40,
                                          secondaryCode.begin() + 48);
    uint64_t primaryCode = 0;

    for (int i = 0; i < 8; i++)
        primaryCode |= (uint64_t)primaryCodeArray[i] << 8 * i;

    primaryCode  = be64toh(primaryCode);
    
    std::cout << "Primary Progress Code in Hex : " << std::hex << primaryCode << std::endl;
