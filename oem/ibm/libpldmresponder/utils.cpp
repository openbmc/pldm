...
int setupUnixSocket(const std::string& socketInterface)
{
    ...
    {
        error(
            "SetupUnixSocket: bind() call failed with errno , error - {ERROR}",
            "ERROR", errno);
        close(sock);
        return -1;
    }
    ...
    if (retval < 0)
    {
        error("SetupUnixSocket: select call failed, error - {ERROR}", "ERROR",
              errno);
        close(sock);
        return -1;
    }
    ...
    if ((retval > 0) && (FD_ISSET(sock, &rfd)))
    {
        ...
        if (fd < 0)
        {
            error("SetupUnixSocket: accept() call failed, error - {ERROR}",
                  "ERROR", errno);
            close(sock);
            return -1;
        }
...
int writeToUnixSocket(const int sock, const char* buf, const uint64_t blockSize)
{
    ...
    for (i = 0; i < blockSize; i = i + nwrite)
    {
        ...
        if (retval < 0)
        {
            error("WriteToUnixSocket: select call failed , error - {ERROR}",
                  "ERROR", errno);
            close(sock);
            return -1;
        }
        ...
        if ((retval > 0) && (FD_ISSET(sock, &wfd)))
        {
            ...
            if (nwrite < 0)
            {
                ...
                error("WriteToUnixSocket: Failed to write , error - {ERROR}",
                      "ERROR", errno);
                close(sock);
                return -1;
            }
