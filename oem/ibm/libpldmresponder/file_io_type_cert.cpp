                                ...
                                oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    if(transferFileData(filePath.c_str(), true, offset, length, address))
    {
        return PLDM_ERROR;
    }
    fs::remove(filePath);
                      ...
                      oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    if(readFile(filePath.c_str(), offset, length, response))
    {
        return PLDM_ERROR;
    }
    fs::remove(filePath);
                       ...
                       oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    if (certType == PLDM_FILE_TYPE_SIGNED_CERT)
    {
        ...
        if(!str.empty())
        {
            ...
            PropertyValue valueStatus{"xyz.openbmc_project.Certs.Entry.State.Complete"};
            DBusMapping dbusMappingStatus{certObjPath + std::to_string(fileHandle), certEntryIntf, "Status", "string"};
            try
            {
                pldm::utils::DBusHandler().setDbusProperty(dbusMappingStatus, valueStatus);
            }
            catch (const std::exception& e)
            {
                std::cerr
...
int CertHandler::newFileAvailable(uint64_t length)
{
    ...
    fs::permissions(certFilePath, fs::perms::others_read | fs::perms::owner_write);
    int fileFd = -1;
