                       ...
                       oem_platform::Handler* /*oemPlatformHandler*/)
{
    ...
    if (it == certMap.end())
    {
        error("CertHandler::write:file for type '{TYPE}' doesn't exist", "TYPE",
              certType);
        return PLDM_ERROR;
    }
    ...
    {
        error(
            "CertHandler::write:lseek at offset '{OFFSET}' failed, error - {ERROR}",
            "OFFSET", offset, "ERROR", errno);
        return PLDM_ERROR;
    }
    ...
    {
        error(
            "CertHandler::write:file write at offset '{OFFSET}' of length '{LENGTH}' failed, error - {ERROR}",
    ...
    if (certType == PLDM_FILE_TYPE_SIGNED_CERT)
    {
        ...
        if (!str.empty())
        {
            ...
            {
                error(
                    "CertHandler::write:failed to set Client certificate, error - {ERROR}",
                    "ERROR", e);
                return PLDM_ERROR;
            }
            ...
            {
                error(
                    "CertHandler::write:failed to set status property of certicate entry, error - {ERROR}",
                    "ERROR", e);
                return PLDM_ERROR;
            }
            fs::remove(filePath);
...
int CertHandler::newFileAvailable(uint64_t length)
{
    ...
    {
        error(
            "CertHandler::newFileAvailable:failed to open file of type '{TYPE}', error - {ERROR}",
            "TYPE", certType, "ERROR", errno);
        return PLDM_ERROR;
    }
                                              ...
                                              uint32_t /*metaDataValue4*/)
{
    ...
    if (certType == PLDM_FILE_TYPE_SIGNED_CERT)
    {
        ...
        {
            error(
                "newFileAvailableWithMetaData:client cert file Invalid data '{META_DATA}', file handle '{FILE_HANDLE}'",
            ...
            {
                error(
                    "newFileAvailableWithMetaData:Failed to set status property of certicate entry, error - {ERROR}",
                    "ERROR", e);
                return PLDM_ERROR;
            }
        }
    ...
    {
        error(
            "newFileAvailableWithMetaData:failed to open file for type '{TYPE}', error - {ERROR}",
            "TYPE", certType, "ERROR", errno);
        return PLDM_ERROR;
    }
    certMap.emplace(certType, std::tuple(fileFd, length));
                                     ...
                                     uint32_t /*metaDataValue4*/)
{
    if (certType == PLDM_FILE_TYPE_CERT_SIGNING_REQUEST)
    {
        ...
        {
            error(
                "CertHandler::fileAckWithMetaData:Failed to set status property of certicate entry, error - {ERROR}",
                "ERROR", e);
            return PLDM_ERROR;
        }
    }
