...
static std::map<uint8_t, std::string> linkStateMap{
...
static std::map<uint8_t, std::string> linkSpeed{
...
static std::map<uint8_t, int64_t> linkWidth{{0x01, 1}, {0x02, 2},  {0x04, 4},
...
using linkIdT = uint16_t;
using linkStatusT = std::string;
using linkTypeT = uint8_t;
using linkSpeedT = uint8_t;
using linkWidthT = int64_t;
using pcieHostBridgeLocT = std::string;
using localPortTopT = std::string;
using localPortBotT = std::string;
using localPortT = std::pair<localPortTopT, localPortBotT>;
using remotePortTopT = std::string;
using remotePortBotT = std::string;
using remotePortT = std::pair<remotePortTopT, remotePortBotT>;
using ioSlotLocationT = std::vector<std::string>;

/* Cable Attributes Info */

static std::map<uint8_t, double> cableLengthMap{
    {0x00, 0},  {0x01, 2},  {0x02, 3},
    {0x03, 10}, {0x04, 20}, {0xFF, std::numeric_limits<double>::quiet_NaN()}};

static std::map<uint8_t, std::string> cableTypeMap{
    {0x00, "optical"}, {0x01, "copper"}, {0xFF, "Unknown"}};

static std::map<uint8_t, std::string> cableStatusMap{
...
using cableLinkNumT = unsigned short int;
using localPortLocCodeT = std::string;
using ioSlotLocationCodeT = std::string;
using cablePartNumT = std::string;
using cableLengthT = double;
using cableTypeT = std::string;
using cableStatusT = std::string;
...
class PCIeInfoHandler : public FileHandler
{
  ...
  private:
    uint16_t infoType; //!< type of the information
                        ...
                        * Key - LinkID (linkIdT)
                        * Value - Tuple
                        *         link status (linkStatusT)
                        *         link type (linkTypeT)
                        *         link speed (linkSpeedT)
                        *         link width (linkWidthT)
                        *         PCIe host bridge location (pcieHostBridgeLocT)
                        *         Local port (localPortT - Pair of local port top and bottom)
                        *         Remote port (remotePortT - Pair of remote port top and bottom)
                        *         I/O slot location (ioSlotLocationT - Vector of strings)
                        */
    static std::unordered_map<
        linkIdT, std::tuple<linkStatusT, linkTypeT, linkSpeedT, linkWidthT,
                            pcieHostBridgeLocT, localPortT, remotePortT,
                            ioSlotLocationT, linkIdT>>
        topologyInformation;
    /**
     ...
     * cable attributes.
     *
     * Key - CableLinkID (cableLinkNumT)
     * Value - Tuple
     *         Link ID (linkIdT)
     *         Local port location code (localPortLocCodeT)
     *         I/O slot location code (ioSlotLocationCodeT)
     *         Cable part number (cablePartNumT)
     *         Cable length (cableLengthT)
     *         Cable type (cableTypeT)
     *         Cable status (cableStatusT)
     *
     */

    static std::unordered_map<
        cableLinkNumT,
        std::tuple<linkIdT, localPortLocCodeT, ioSlotLocationCodeT,
                   cablePartNumT, cableLengthT, cableTypeT, cableStatusT>>
        cableInformation;
    /**
     ...
     *
     * Key - LinkID (linkIdT)
     * Value - LinkType (linkTypeT)
     */
    static std::unordered_map<linkIdT, linkTypeT> linkTypeInfo;
