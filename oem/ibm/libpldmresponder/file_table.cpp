...
FileTable::FileTable(const std::string& fileTableConfigPath)
{
    ...
    for (const auto& record : data)
    {
        ...
        std::string filedir = record.value(path, "");

        fs::path fsDir(filedir);
        ...
        // Iterate file under directory
        for (const auto& filepath : fs::directory_iterator(fsDir))
        {
            ...
            fileTable.resize(tableSize + sizeof(handle) +
                             sizeof(fileNameLength) + fileNameLength +
                             sizeof(fileSize) + sizeof(traits));
            iter = fileTable.begin() + tableSize;

            // Populate the file table with the contents of the JSON entry
            std::copy_n(reinterpret_cast<uint8_t*>(&handle), sizeof(handle),
                        iter);
            ...
            std::copy_n(
                reinterpret_cast<const uint8_t*>(fsPath.filename().c_str()),
                fileNameLength, iter);
            ...
            std::copy_n(reinterpret_cast<uint8_t*>(&traits), sizeof(traits),
                        iter);
