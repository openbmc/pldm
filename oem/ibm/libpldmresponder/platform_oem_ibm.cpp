    ...
    pldm::requester::Handler<pldm::requester::Request>* handler)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        std::cerr << "BIOS Attribute update event message encode failure. PLDM error code = " << std::hex
                  << std::showbase << rc << "\n";
    ...
    auto platformEventMessageResponseHandler = [](mctp_eid_t /*eid*/,
        ...
        {
            std::cerr << "Failed to decode BIOS Attribute update platform_event_message_resp: "
                      << "rc=" << rc
    ...
    if (rc)
    {
        std::cerr << "Failed to send BIOS Attribute update the platform event message \n";
    }
