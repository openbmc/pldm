...
void DbusToFileHandler::sendNewFileAvailableCmd(uint64_t fileSize)
{
    ...
    auto newFileAvailableRespHandler = [this](mctp_eid_t /*eid*/,
        ...
        {
            error(
                "Failed to decode new file available response or remote pldm terminus returned error, response code '{RC}' and completion code '{CC}'",
                                                   ...
                                                   const uint16_t type)
{
    ...
    auto newFileAvailableRespHandler =
        ...
        {
            error(
                "Failed to decode new file available response for vmi or remote pldm terminus returned error, response code '{RC}' and completion code '{CC}'",
                "RC", rc, "CC", static_cast<unsigned>(completionCode));
            pldm::utils::reportError(
