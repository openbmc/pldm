    ...
    sdbusplus::message::object_path path)
{
    ...
    resDumpCurrentObjPath = path;

    std::string vspString{};
    try
    {
        vspString = pldm::utils::DBusHandler().getDbusProperty<std::string>(
            resDumpCurrentObjPath.str.c_str(), "VSPString", resDumpEntry);
    }
    catch (const std::exception& e)
    {
    ...
    try
    {
        resDumpReqPass =
            pldm::utils::DBusHandler().getDbusProperty<std::string>(
                resDumpCurrentObjPath.str.c_str(), "Password", resDumpEntry);
    }
    catch (const std::exception& e)
    {
        std::cerr << "failed to make a d-bus call to get Password, ERROR= "
                  << e.what() << "\n";
        return;
    }
    ...
    auto fileFunc = [&fileHandle](std::string paramBuf) {
