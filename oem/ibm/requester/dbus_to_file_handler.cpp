...
static constexpr auto resDumpInProgress =
...
void DbusToFileHandler::sendNewFileAvailableCmd(uint64_t fileSize) {
  ...
  auto rc = encode_new_file_req(instanceId, PLDM_FILE_TYPE_RESOURCE_DUMP_PARMS,
  ...
  rc = handler->registerRequest(mctp_eid, instanceId, PLDM_OEM,
                                PLDM_NEW_FILE_AVAILABLE, std::move(requestMsg),
                                std::move(newFileAvailableRespHandler));
  if (rc) {
...
void DbusToFileHandler::reportResourceDumpFailure() {
  ...
  DBusMapping dbusMapping{resDumpCurrentObjPath, resDumpProgressIntf, "Status",
                          "string"};
  try {
...
void DbusToFileHandler::processNewResourceDump(
  ...
  try {
    ...
    if (curResDumpStatus != resDumpInProgress) {
      // If the resource dump is not in progress state, we do not
      // need to proceed further and contact the hypervisor
      return;
    }
