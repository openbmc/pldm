...
void DbusToFileHandler::sendNewFileAvailableCmd(uint64_t fileSize)
{
    ...
    auto newFileAvailableRespHandler = [this](mctp_eid_t /*eid*/,
        ...
        {
            error(
                "Failed to decode new file response or Host returned error for new_file_available, response code '{RC}' and completion code '{CC}'",
                                            ...
                                            const std::string fileHandle)
{
    ...
    if (!certFile)
    {
        error("Certificate file '{PATH}' open failed", "PATH", certFilePath);
        return;
    }
                                                   ...
                                                   const uint16_t type)
{
    if (instanceIdDb == NULL)
    {
        error("Failed to send csr to remote pldm terminus.");
        pldm::utils::reportError(
    ...
    auto newFileAvailableRespHandler =
        ...
        {
            error(
                "Failed to decode new file response for vmi or Host returned error for new_file_available, response code '{RC}' and completion code '{CC}'",
                "RC", rc, "CC", static_cast<unsigned>(completionCode));
            pldm::utils::reportError(
