...
TEST_F(TestFileTable, testAllreadFileByTypeHandler)
{
    ...
    using namespace pldm::responder::internal;
...
TEST(getHandlerByType, allPaths)
{
    ...
    try
    {
        handler = getHandlerByType(fileType, fileHandle, offset, length,
                                   address, path);
    }
...
{
  public:
    DummyPelHandler(uint32_t newfileHandle, uint32_t newOffset,
                    uint32_t newLength, uint64_t newAddress, fs::path newPath) :
        PelHandler(newfileHandle, newOffset, newLength, newAddress, newPath)
    {
    }
...
TEST(writeFromMemory, allPathsForPel)
{
    ...
    auto handler = std::make_unique<DummyPelHandler>(fileHandle, offset, length,
                                                     address, path);
