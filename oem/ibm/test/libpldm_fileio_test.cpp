...
TEST(GetFileTable, GoodDecodeResponse)
{
    ...
    std::vector<uint8_t> file_table_data = {1, 2, 3, 4, 5, 6, 7, 8, 9};

    std::vector<uint8_t> responseMsg(
        hdrSize + PLDM_GET_FILE_TABLE_MIN_RESP_BYTES + file_table_data.size());
    ...
    memcpy(resp->table_data, file_table_data.data(), file_table_data.size());
    ...
    ASSERT_EQ(0, memcmp(file_table_data.data(), retFileTableData.data(),
                        retFileTableDataLength));
    ASSERT_EQ(file_table_data.size(), retFileTableDataLength);
}
...
TEST(GetFileTable, BadDecodeResponse)
{
    ...
    uint8_t completionCode = PLDM_SUCCESS;
    uint8_t transferFlag = PLDM_START_AND_END;
    std::vector<uint8_t> file_table_data(9, 0);
