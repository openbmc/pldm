...
{
    std::array<uint8_t, sizeof(pldm_msg_hdr) + PLDM_NEW_FILE_AVAILABLE_WITH_META_DATA_RESP_BYTES>
        responseMsg{};
...
{
    std::array<uint8_t, PLDM_NEW_FILE_AVAILABLE_WITH_META_DATA_RESP_BYTES + sizeof(pldm_msg_hdr)>
        responseMsg{};

    auto responsePtr = reinterpret_cast<pldm_msg*>(responseMsg.data());
    size_t payload_length = responseMsg.size() - sizeof(pldm_msg_hdr);
    auto response = reinterpret_cast<pldm_file_ack_with_meta_data_resp*>(responsePtr->payload);
    ...
    // Invoke decode the read/write file response
    auto rc =
        decode_new_file_with_metadata_resp(responsePtr, payload_length, &retCompletionCode);
...
{
    std::array<uint8_t, sizeof(pldm_msg_hdr) + PLDM_NEW_FILE_AVAILABLE_WITH_META_DATA_REQ_BYTES>
        requestMsg{};
    ...
    auto rc = encode_new_file_with_metadata_req(0, fileType, fileHandle, length,
    ...
    ASSERT_EQ(0, memcmp(request->payload + sizeof(fileTypeLe) + sizeof(fileHandleLe) +
                        sizeof(lengthLe), &fileMetaData1Le, sizeof(fileMetaData1Le)));

    ASSERT_EQ(0, memcmp(request->payload + sizeof(fileTypeLe) + sizeof(fileHandleLe) +
                        sizeof(lengthLe) + sizeof(fileMetaData1Le), &fileMetaData2Le,
                        sizeof(fileMetaData2Le)));


    ASSERT_EQ(0, memcmp(request->payload + sizeof(fileTypeLe) + sizeof(fileHandleLe) +
                        sizeof(lengthLe) + sizeof(fileMetaData1Le) + sizeof(fileMetaData2Le),
                        &fileMetaData3Le, sizeof(fileMetaData3Le)));

    ASSERT_EQ(0, memcmp(request->payload + sizeof(fileTypeLe) + sizeof(fileHandleLe) +
                        sizeof(lengthLe) + sizeof(fileMetaData1Le) + sizeof(fileMetaData2Le) +
                        sizeof(fileMetaData3Le), &fileMetaData4Le, sizeof(fileMetaData4Le)));
...
{
    std::array<uint8_t, PLDM_NEW_FILE_AVAILABLE_WITH_META_DATA_REQ_BYTES + sizeof(pldm_msg_hdr)>
        requestMsg{};

    auto requestPtr = reinterpret_cast<pldm_msg*>(requestMsg.data());
    size_t payload_length = requestMsg.size() - sizeof(pldm_msg_hdr);
    auto request = reinterpret_cast<pldm_new_file_with_metadata_req*>(requestPtr->payload);
    ...
    // Invoke decode the read file request
    auto rc = decode_new_file_with_metadata_req(requestPtr, payload_length, &retFileType,
                                  &retFileHandle, &retLength,
...
TEST(NewFileAvailableWithMetaData, testBadDecodeRequest)
{
    ...
    // Request payload message is missing
    auto rc = decode_new_file_with_metadata_req(NULL, 0, &fileType, &fileHandle, &length,
              &fileMetaData1, &fileMetaData2, &fileMetaData3, &fileMetaData4);
    ...
    // Payload length is invalid
    rc = decode_new_file_with_metadata_req(requestPtr, 0, &fileType, &fileHandle, &length,
         &fileMetaData1, &fileMetaData2, &fileMetaData3, &fileMetaData4);
    ASSERT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
}
