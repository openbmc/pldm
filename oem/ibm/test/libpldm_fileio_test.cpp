...
TEST(WriteFile, testGoodDecodeRequest)
{
    ...
    auto rc = decode_write_file_req(
        requestMsg.data(), (requestMsg.size() - length), &retFileHandle,
        &retOffset, &retLength, retfileData.data());
...
TEST(ReadWriteFile, testGoodDecodeResponse)
{
    ...
    auto rc = decode_read_file_resp(
        responseMsg.data(), (responseMsg.size() - length), &retCompletionCode,
        &retLength, retfileData.data());
...
TEST(ReadWriteFile, testBadDecodeResponse)
{
    ...
    std::array<uint8_t, PLDM_READ_FILE_RESP_BYTES> requestMsg{};
...
TEST(ReadWriteFile, testGoodEncodeRequest)
{
    ...
    std::vector<uint8_t> requestMsgWr{};

    std::vector<uint8_t> fileData = {0xA, 0x1, 0x3, 0xF};
    length = fileData.size();
    requestMsgWr.resize(sizeof(pldm_msg_hdr) + PLDM_WRITE_FILE_REQ_BYTES +
                        length);
