...
TEST(GetFileTable, BadDecodeResponse)
{
    ...
    auto rc = decode_get_file_table_resp(nullptr, 0, &completionCode,
                                         &nextTransferHandle, &transferFlag,
                                         &fileTableOffset);

    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);

    rc = decode_get_file_table_resp(response, responseMsg.size() - hdrSize - 1,
                                    &completionCode, &nextTransferHandle,
                                    &transferFlag, &fileTableOffset);
