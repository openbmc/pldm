...
TEST(FileAck, testBadDecodeRequest)
{
    ...
    // Request payload message is missing
    auto rc = decode_file_ack_req(NULL, 0, &fileType, &fileHandle);
...
TEST(FileAck, testGoodEncodeRequest)
{
    ...
    auto rc = encode_file_ack_req(0, fileType, fileHandle, request);
...
TEST(FileAck, testBadEncodeRequest)
{
    ...
    // request is NULL pointer
    auto rc = encode_file_ack_req(0, fileType, fileHandle, NULL);
