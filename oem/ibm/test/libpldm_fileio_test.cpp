...
TEST(WriteFile, testGoodDecodeRequest)
{
    ...
    uint32_t retOffset =
        decode_write_file_req(requestMsg.data(), (requestMsg.size() - length),
...
TEST(ReadFile, testGoodDecodeResponse)
{
    ...
    uint32_t offset = (uint32_t)(uintptr_t)(
        responseMsg.data() + sizeof(completionCode) + sizeof(length));
    ...
    uint32_t retOffset =
        decode_read_file_resp(responseMsg.data(), (responseMsg.size() - length),
                              &retCompletionCode, &retLength);
...
{
    std::array<uint8_t, PLDM_WRITE_FILE_RESP_BYTES> responseMsg{};
    ...
    auto rc = decode_write_file_resp(responseMsg.data(), responseMsg.size(),
                                     &retCompletionCode, &retLength);
...
TEST(ReadWriteFile, testBadDecodeResponse)
{
    ...
    // Payload length is invalid
    rc = decode_read_file_resp(responseMsg.data(), 0, &completionCode, &length);
    ASSERT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
    ...
    // Payload length is invalid
    rc = decode_write_file_resp(responseMsgWr.data(), 0, &completionCode,
                                &length);
    ASSERT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
}
...
TEST(ReadWriteFile, testBadDecodeRequest)
{
    ...
    // Request payload message is missing
    rc = decode_write_file_req(NULL, 0, &fileHandle, &length);
    ASSERT_EQ(rc, PLDM_ERROR_INVALID_DATA);

    // Payload length is invalid
    rc = decode_write_file_req(requestMsgWr.data(), 0, &fileHandle, &length);
    ASSERT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
}
...
TEST(ReadFile, testGoodEncodeResponse)
{
    ...
    std::vector<uint8_t> responseMsg{};
    uint32_t offset = 0;
    memcpy(&offset, fileData.data(), fileData.size());
    uint32_t length = fileData.size();
    ...
    // ReadFile
    auto rc = encode_read_file_resp(0, PLDM_SUCCESS, length, offset, response);
...
TEST(WriteFile, testGoodEncodeResponse)
{
    ...
    // WriteFile
    auto rc = encode_write_file_resp(0, PLDM_SUCCESS, length, response);
    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(response->hdr.request, PLDM_RESPONSE);
    ASSERT_EQ(response->hdr.instance_id, 0);
    ASSERT_EQ(response->hdr.type, PLDM_OEM);
    ASSERT_EQ(response->hdr.command, PLDM_WRITE_FILE);
    ASSERT_EQ(response->payload[0], PLDM_SUCCESS);
    ASSERT_EQ(0, memcmp(response->payload + sizeof(response->payload[0]),
                        &length, sizeof(length)));
}
...
TEST(WriteFile, testGoodEncodeRequest)
{
    ...
    // WriteFile
    auto rc = encode_write_file_req(0, fileHandle, offset, length, request);

    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(request->hdr.request, PLDM_REQUEST);
    ASSERT_EQ(request->hdr.instance_id, 0);
    ASSERT_EQ(request->hdr.type, PLDM_OEM);
    ASSERT_EQ(request->hdr.command, PLDM_WRITE_FILE);
    ASSERT_EQ(0, memcmp(request->payload, &fileHandle, sizeof(fileHandle)));
    ASSERT_EQ(0, memcmp(request->payload + sizeof(fileHandle), &offset,
                        sizeof(offset)));
    ASSERT_EQ(0, memcmp(request->payload + sizeof(fileHandle) + sizeof(offset),
...
TEST(ReadWriteFile, testBadEncodeResponse)
{
    ...
    // ReadFile
    auto rc = encode_read_file_resp(0, PLDM_ERROR, length, offset, response);
