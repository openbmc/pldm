...
class MockOemPlatformHandler : public oem_ibm_platform::Handler
{
  public:
    ...
    MOCK_METHOD(std::vector<std::string>, getslotPaths, ());
};
...
TEST(OemSetStateEffecterStatesHandler, testGoodRequest)
{
    ...
    pldm_entity e1;
    e1.entity_type = 0x1;
    e1.entity_instance_num = 0;
    e1.entity_container_id = 0x1;
    std::string DbusPath = "/abc/def/xyz";
    AssociatedEntityMap testMap;
    testMap[DbusPath] = e1;
    const AssociatedEntityMap associateMap = testMap;
...
TEST(generateStateEffecterOEMPDR, testGoodRequest)
{
    ...
    std::vector<std::string> slotpath{"/abc/def"};
    ON_CALL(*mockoemPlatformHandler, getslotPaths)
        .WillByDefault(Return(slotpath));
