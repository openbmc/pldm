...
class MockCodeUpdate : public CodeUpdate
{
  public:
    ...
    //   MOCK_METHOD(int, setNextBootSide,(const std::string&),(override));

    MOCK_METHOD(void, setVersions, (), (override));
    /*void setVersions()
    {
        std::cout << "called mocked setVersions \n";
    }*/
};

TEST(OemSetStateEffecterStatesHandler, testGoodRequest)
{
    uint16_t entityID_ = 33;
    uint16_t stateSetId_ = 32769;
    ...
    // auto mockCodeUpdate =
    // std::make_unique<MockCodeUpdate>(mockDbusHandler.get()); MockCodeUpdate
    // mockCodeUpdate(mockDbusHandler.get());
    std::unique_ptr<CodeUpdate> mockCodeUpdate =
        std::make_unique<MockCodeUpdate>(mockDbusHandler.get());
    std::unique_ptr<oem_platform::Handler> oemPlatformHandler{};

    // EXPECT_CALL(mockCodeUpdate, setVersions()).Times(1);
