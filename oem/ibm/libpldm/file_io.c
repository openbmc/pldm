			  ...
			  uint8_t *data)
{
	...
	if (payload_length !=
	    PLDM_CONST_READ_FILE_RESP_BYTES + PLDM_MAX_FILE_SIZE) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
	...
	*completion_code = response->completion_code;
	*length = le32toh(response->length);
	memcpy(data, response->file_data, response->length);
			  ...
			  struct pldm_msg *msg)
{
	...
	if (response->completion_code == PLDM_SUCCESS) {
		response->length = htole32(length);
		memcpy(response->file_data, data, response->length);
	}
			  ...
			  uint32_t *length, uint8_t *data)
{
	...
	*offset = le32toh(request->offset);
	*length = le32toh(request->length);
	memcpy(data, request->file_data, request->length);
			  ...
			  struct pldm_msg *msg)
{
	...
	request->offset = htole32(offset);
	request->length = htole32(length);
	memcpy(request->file_data, data, request->length);
