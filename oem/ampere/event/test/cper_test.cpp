...
std::vector<uint8_t> makeRecord(const EFI_GUID& sectionType, size_t sectionSize)
{
    const size_t descriptorOffset = sizeof(EFI_COMMON_ERROR_RECORD_HEADER) +
...
TEST(Cper, RejectsNullAndTruncatedInput)
{
    ...
    std::vector<uint8_t> data(
        sizeof(CommonEventData) + sizeof(EFI_COMMON_ERROR_RECORD_HEADER) - 1);
...
{
    auto data =
        makeRecord(gEfiAmpereErrorSectionGuid, sizeof(EFI_AMPERE_ERROR_DATA));
    EFI_ERROR_SECTION_DESCRIPTOR descriptor{};
