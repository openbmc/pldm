                         ...
                         EFI_AMPERE_ERROR_DATA* ampSpecHdr)
{
    ...
    if (proc.SectionLength > sectionSize ||
        ...
        proc.ErrInfoNum > (proc.SectionLength - sizeof(EFI_ARM_ERROR_RECORD)) /
                              ...
                              const EFI_ERROR_SECTION_DESCRIPTOR* secDesc)
{
    ...
    if (eventDataSize < basePos ||
        secDesc->SectionOffset > eventDataSize - basePos ||
    ...
    const EFI_GUID* ptr =
        reinterpret_cast<const EFI_GUID*>(&secDesc->SectionType);
    bool decoded = true;
