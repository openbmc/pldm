...
#include "oem/meta/event/utils.hpp"
...
class OemEventManager
{
  ...
  private:
    int processOemMetaEvent(
		...
		void handleUnifiedMemoryEvent(const uint8_t*, std::string&) const;

		void handleMemoryError(const uint8_t*, std::string&, const DimmInfo&, uint8_t generalInfo) const;

		void handleSystemPostEvent(const uint8_t*, std::string&, uint8_t generalInfo) const;
    ...
    inline auto to_hex_string(uint8_t value, size_t len = 2) const
    {
		...
		void covertToDimmString(uint8_t cpu, uint8_t channel,
