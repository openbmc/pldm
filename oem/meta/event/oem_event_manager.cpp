...
std::map<std::pair<pldm_tid_t, UnifiedError>, uint32_t> receivedEventMsgTimeStamp;
    ...
    pldm_tid_t tid, const uint8_t* eventData, size_t eventDataSize) const
{
    ...
    switch (recordType)
    {
        ...
        {
            handleUnifiedBIOSEvent(tid, data, errorLog);
            break;
        }
        default:
        {
...
void OemEventManager::handleUnifiedBIOSEvent(pldm_tid_t tid, const eventMsg& eventData,
                                             std::string& errorLog) const
{
    ...
    if (eventData.size() == eventMsgLengthWithTimestamp)
    {
        ...
        lg2::info("oem event msg: detected retry event, type {TYPE}, timestamp {TIMEST}", "TYPE", 0xF & eventData[1], "TIMEST", eventTimeStamp);
        if (receivedEventMsgTimeStamp.count(eventKey) &&
            receivedEventMsgTimeStamp[eventKey] == eventTimeStamp)
        {
            lg2::info(
                "oem event msg: TID {TID}: Event time stamp {EVTT} skip retry event",
    ...
    {
      lg2::info("oem event msg: not handle retry event, event len {LEN}", "LEN", eventData.size());
    }
