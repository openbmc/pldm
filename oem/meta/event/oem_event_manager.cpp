...
int OemEventManager::processOemMetaEvent(
    ...
    const std::map<eid, pldm::oem_meta::MctpEndpoint>& configurations) const
{
    ...
    std::string slotNumber = getSlotNumberStringByTID(configurations, tid);
    std::string message =
    ...
    uint8_t tid, size_t eventDataOffset)
{
    ...
    if (!pldm::oem_meta::checkMetaIana(configurations, tid))
    {
