...
int OemEventManager::handleOemEvent(const pldm_msg* request, size_t payloadLength,
									 ...
									 size_t eventDataOffset)
{
		...
		const std::map<std::string, pldm::requester::MctpEndpoint>& configurations =
				configurationDiscovery->getConfigurations();
		if (!checkMetaIana(tid, configurations))
		{
				lg2::error(
						"Recieve OEM Meta event from not Meta specific device");
				return PLDM_ERROR;
		}
		processOemMetaEvent(
...
void OemEventManager::covertToDimmString(uint8_t cpu, uint8_t channel,
                                         uint8_t slot, std::string& str) const
{
...
void OemEventManager::getCommonDimmLocation(
    const dimm_info& dimmInfo, std::string& dimmLocation, std::string& dimm) const
{
...
std::string OemEventManager::getSlotNumberString(pldm_tid_t tid,
        const std::map<std::string, pldm::requester::MctpEndpoint>& configurations) const
{
