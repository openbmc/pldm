...
void OemEventManager::handleSystemEvent(const uint8_t* eventData, std::string& errorLog) const
{
		...
		if (eventData[8] == 0xC1)
		{
				...
				errorLog +=
						", Error Type: " + std::string(cxlError[errorType]) +
...
void OemEventManager::handleUnifiedMemoryEvent(const uint8_t* eventData, std::string& errorLog) const
{
		...
		switch (errorType)
		{
				...
				case UnifiedError::UNIFIED_MCA_ERR:
				{
						...
						errorLog +=
								...
								", Error Info: 0x" + to_hex_string(errorInfo, 6) +
				...
				case UnifiedError::UNIFIED_MCA_ERR_EXT:
				{
						...
						errorLog +=
								...
								", Machine Check Bank: " +
								machineCheckBank[machineCheckBankNumber] +
								", Error Code: 0x" + to_hex_string(errorCode, 4);
						break;
				}
						...
						switch (eventType)
						{
								case PcieEvent::PCIE_DPC:
								{
										errorLog +=
												...
												", Status: 0x" +
												to_hex_string(
														(eventData[9] << 8) | eventData[8], 4) +
												", Source ID: 0x" +
												to_hex_string(
														(eventData[11] << 8) | eventData[10], 4);
										break;
								}
								default:
								{
						...
						switch (eventType)
						{
								...
								case MemoryEvent::MEM_CXL_POST_PPR:
								{
										errorLog +=
												...
												memoryEvent[static_cast<uint8_t>(eventType)] +
												", " +
												memoryCXLPostPprEvent[eventData[11] >> 7 &
																							0x1] +
...
void OemEventManager::handleMemoryError(const uint8_t* eventData, std::string& errorLog, const DimmInfo& dimmInfo, uint8_t generalInfo) const
{
			std::string dimmLocation, dimm;
			getCommonDimmLocation(dimmInfo, dimmLocation, dimm);
			...
			switch (eventType)
			{
					...
					case MemoryError::MEMORY_CXL_TRAINNING_ERR:
					{
							errorLog +=
									...
									", " +
									memoryCxlEvent[static_cast<uint8_t>(
											eventData[11] & 0x7)];
							break;
					}
					default:
...
void OemEventManager::handleSystemPostEvent(const uint8_t* eventData, std::string& errorLog, uint8_t generalInfo) const
{
