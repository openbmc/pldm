...
int OemEventManager::processOemMetaEvent(
    ...
    const std::map<std::string, pldm::oem_meta::MctpEndpoint>& configurations)
    const
{
    ...
    uint8_t tid, size_t eventDataOffset)
{
    ...
    if (!pldm::oem_meta::checkMetaIana(tid, configurations))
    {
    ...
    const
{
    ...
    for (const auto& [configDbusPath, mctpEndpoint] : configurations)
    {
        if (mctpEndpoint.EndpointId == tid)
        {
            ...
            {
                lg2::error(
                    "Failed to call GetAncestors while trying to retrieve the slot number, ERROR {ERROR}",
                    "ERROR", e);
                return slotNumber; // return "Unknown";
            }
                                             ...
                                             std::string& errorLog) const
{
    ...
    switch (errorType)
    {
        ...
        case UnifiedError::UNIFIED_CXL_MEM_ERR:
        {
            ...
            {
                if ((eventData[7] == cxlBdfMap[i].Bus) &&
                    ((eventData[8] >> 3) == cxlBdfMap[i].Dev) &&
                    ((eventData[8] & 0x7) == cxlBdfMap[i].Fun))
                {
                    devName = std::string(cxlBdfMap[i].Name) + "_";
                    break;
                }
            }
            ...
            errorLog += "GeneralInfo: CXL Memory Error(0x" +
                        ...
                        ", " + devName + memoryCxlEvent[eventData[11]];
            break;
        }
        default:
        {
