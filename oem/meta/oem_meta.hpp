...
class OemMETA
{
     ...
     * @param[in] platformManager - MC Platform manager
				...
				platformManager(platformManager)
		{
			...
			fileIOHandler =
					std::make_unique<pldm::responder::oem_meta::FileIOHandler>(configurationDiscovery);
			registerHandler();
		...
		pldm::requester::ConfigurationDiscoveryHandler* getMctpConfigurationHandler() const {
    ...
    void registerHandler()
    {
		...
		void registerOemEventHandler(pldm::responder::platform::Handler* platformHandler)
		{
        ...
        PLDM_OEM_EVENT_CLASS_0xFB,
         {[oemEventManager](const pldm_msg* request, size_t payloadLength,
             ...
             return oemEventManager->handleOemEvent(
                 request, payloadLength, formatVersion, tid, eventDataOffset);                 
         }});
		}

    /** @brief MCTP configurations handler*/
		std::shared_ptr<pldm::requester::ConfigurationDiscoveryHandler> configurationDiscovery{};

		std::unique_ptr<pldm::responder::oem_meta::FileIOHandler> fileIOHandler = nullptr;
    ...
    /** @brief MC Platform manager*/
		...
		std::shared_prt<oem_meta::OemEventManager> oemEventManager{};
}
