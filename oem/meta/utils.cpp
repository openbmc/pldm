...
bool checkMetaIana(pldm_tid_t tid)
{
    ...
    if (auto iter = configurationDiscovery->getConfigurations().find(tid);
        iter != configurationDiscovery->getConfigurations().end())
    {
        const MctpEndpoint& mctpEndpoint = iter->second;
        ...
        iter != configurationDiscovery->getConfigurations().end())
    {
        ...
        catch (const sdbusplus::exception_t& e)
        {
            error(
                ...
                "ERROR", e, "PATH", mctpEndpoint.objectPath.c_str(),
