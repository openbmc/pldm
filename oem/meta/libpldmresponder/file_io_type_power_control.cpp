...
int PowerControlHandler::write(const message& data)
{
    ...
    {
        error("Failed to execute Dbus call with error code {ERROR}",
              "ERROR", e);
        return PLDM_ERROR;
    }
    catch (const std::exception& e)
...
std::string PowerControlHandler::getSlotNumber()
{
    ...
    {
        error("Failed to execute Dbus call with error code {ERROR}", "ERROR", e);
    }
    catch (const std::runtime_error& e)
    {
        error("Runtime error occurred with error code {ERROR}", "ERROR", e);
    }
    catch (const std::exception& e)
    {
        error("Unexpected exception occurred with error code {ERROR}", "ERROR", e);
    }

    return result;
}

const std::string PowerControlHandler::getDbusPathOfBoardContainingTheEndpoint(
    uint8_t tid)
{
    std::string result{};
...
const std::string& PowerControlHandler::getConfigurationPathByTid(uint8_t tid)
{
    ...
    error("Failed to get Configuration for TID {TID}", "TID", tid);
