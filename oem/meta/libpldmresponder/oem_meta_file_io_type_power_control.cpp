...
int PowerControlHandler::write(const message& data)
{
    ...
    {
        error("{FUNC}: Invalid incoming data for controlling power, data size={SIZE}",
    ...
    switch (option)
    {
        case static_cast<uint8_t>(POWER_CONTROL_OPTION::SLED_CYCLE):
            dbusMapping.objectPath = std::string("/xyz/openbmc_project/state/chassis0");
            break;
	case static_cast<uint8_t>(POWER_CONTROL_OPTION::SLOT_12V_CYCLE):
	    dbusMapping.objectPath = std::string("/xyz/openbmc_project/state/chassis") + getSlotNumber();
            break;
        default:
            error("Get invalid power control option, option={OPTION}",
                "OPTION", option);
            return PLDM_ERROR;
    }
