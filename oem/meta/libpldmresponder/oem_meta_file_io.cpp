                                   ...
                                   size_t payloadLength)
{
    ...
    uint8_t highOffset = 0;
    uint8_t lowOffset = 0;
    uint8_t completionCode = PLDM_SUCCESS;

    auto rc = decode_oem_meta_read_file_io_req(
        request, payloadLength, &fileIOType, &length, &transferFlag,
    ...
    std::vector<uint8_t> data = {length, transferFlag, highOffset, lowOffset};
