                               ...
                               std::stringstream& ss)
{
    ...
    CrdCxlCode crdCxlCode = static_cast<CrdCxlCode>(pBank->part);
    switch (crdCxlCode)
    {
                                          ...
                                          std::stringstream& ss)
{
    ...
    switch (crdCxlCode)
    {
        case CrdCxlCode::health:
            ...
            {
                ss << std::format(
                    "Invalid length for Cxl health, data length {}, expect length {}\n",
                    data.size(), sizeof(CXL_CRASH_DUMP_HEALTH));
                error("Invalid length for Cxl health");
                return PLDM_ERROR;
            }
        ...
        case CrdCxlCode::getEvent:
            ...
            {
                ss << std::format(
                    "Invalid length for Cxl get event, data length {}, expect length {}\n",
                    data.size(), sizeof(CXL_CRASH_DUMP_GET_EVENT));
                error("Invalid length for Cxl get event");
                return PLDM_ERROR;
            }
        ...
        case CrdCxlCode::memBrg:
            ...
            {
                ss << std::format(
                    "Invalid length for Cxl memory bridge, data length {}, expect length {}\n",
                    data.size(), sizeof(CXL_CRASH_DUMP_MEMBRG));
                error("Invalid length for Cxl memory bridge");
                return PLDM_ERROR;
            }
        ...
        case CrdCxlCode::getLog:
            ...
            {
                ss << std::format(
                    "Invalid length for Cxl get log, data length {}, expect length {}\n",
                    data.size(), sizeof(CXL_CRASH_DUMP_CXL_LOG));
                error("Invalid length for Cxl get log");
                return PLDM_ERROR;
            }
        ...
        case CrdCxlCode::getOpcode:
            ...
            {
                ss << std::format(
                    "Invalid length for Cxl get opcode, data length {}, expect length {}\n",
                    data.size(), sizeof(CXL_CRASH_DUMP_CXL_OPCODE));
                error("Invalid length for Cxl get opcode");
                return PLDM_ERROR;
            }
        ...
        case CrdCxlCode::crashDumpEnd:
            ...
            {
                ss << std::format(
                    "Invalid length for Cxl packet, data length {}, expect length {}\n",
                    data.size(), sizeof(CXL_CRASH_DUMP_PACKET));
                error("Invalid length for Cxl dump packet");
                return PLDM_ERROR;
            }
...
std::string CrashDumpHandler::getSlotNumberString()
{
    ...
    for (const auto& [configDbusPath, mctpEndpoint] : configurations)
    {
        if (mctpEndpoint.EndpointId == tid)
        {
            ...
            {
                error(
                    "Error getting Names property with error code {ERROR} PATH {PATH} INTERFACE {INTERFACE}",
...
int CrashDumpHandler::write(const message& data)
{
    ...
    if (!res)
    {
        error("Fail to write crash dump data");
        return PLDM_ERROR;
    }
