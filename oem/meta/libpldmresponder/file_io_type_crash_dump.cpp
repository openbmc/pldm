...
{
  uint8_t  cmd[4];
  uint8_t  bank[4]; // 0:cmd, 1:fmt_ver, 2-3:reserved
  uint8_t  part;
...
static int handleCxlGetLog([[maybe_unused]] std::span<const uint8_t> data,
                              std::stringstream& ss)
{
    ss << std::format("  CXL Memory Get CXL Log\n");
#if 1
    const auto* pBank =
        reinterpret_cast<const CXL_CRASH_DUMP_CXL_LOG*>(data.data());
		ss << std::format("    part        : 0x{:01X}\n", pBank->part);
		ss << std::format("    log data    :");
...
static int handleCxlDumpPacket([[maybe_unused]] std::span<const uint8_t> data,
                              std::stringstream& ss)
{
    ss << std::format("  CXL Dump Packet\n");
#if 1
    const auto* pBank =
		...
		switch(pBank->part)
		{
			case CrdCxlCode::crashDumpBegin:
				ss << std::format("    CXL End Device: Bus: 0x{:02X} Dev: 0x{:02X} Fun: 0x{:02X} Crash Dump Begin\n", pBank->Bus, pBank->Dev, pBank->Fun);
				break;
			case CrdCxlCode::crashDumpEnd:
				ss << std::format("CXL End Device Crash Dump End\n");
				break;
			default:
				ss << std::format("Unknown error\n");
		}
...
[[maybe_unused]] static int handleCxlBank(std::span<const uint8_t> data,
                                          std::stringstream& ss)
{
    if (!data.size())
    {
