...
struct CXL_CRASH_DUMP_HEALTH_ONE {
    uint8_t   part;
    uint32_t  critical_over_temperature;      // Number of times temperature has exceeded upper critical threshold
    ...
    uint32_t  num_ddr_cor_ecc_errors;         // Number of DDR Correctable ECC errors (except for PSCAN errors)
    uint32_t  num_ddr_uncor_ecc_errors;       // Number of DDR Uncorrectable ECC errors (except for PSCAN errors)
    uint32_t  link_recovery_events;           // Number of Link recovery events
    uint32_t  time_in_throttled;              // Amount of time spent in throttled state (in seconds)
    uint32_t  warning_over_temperature;       // Number of times temperature has exceeded upper warning threshold
    uint32_t  critical_under_temperature;     // Number of times temperature has exceeded lower critical threshold
    uint32_t  warning_under_temperature;      // Number of times temperature has exceeded lower warning threshold
    uint32_t  retry_req_rx;                   // Number of RETRY.Req Control FLITs Received
};

struct CXL_CRASH_DUMP_HEALTH_TWO {
    uint8_t   part;
    uint32_t  rcmd_qs0_hi_thld;                  // Number of RCMD QS0 Hi Threshold Interrupt Triggered
    uint32_t  rcmd_qs1_hi_thld;                  // Number of RCMD QS1 Hi Threshold Interrupt Triggered
    uint32_t  num_pscan_cor_ecc_errors;          // Number of PSCAN Correctable ECC errors
    uint32_t  num_pscan_uncor_ecc_errors;        // Number of PSCAN Uncorrectable ECC errors
    uint32_t  num_ddr_dimm0_cor_ecc_errors;      // Number of DDR (DIMM0) Correctable ECC errors
    uint32_t  num_ddr_dimm0_uncor_ecc_errors;    // Number of DDR (DIMM0) Uncorrectable ECC errors
    uint32_t  num_ddr_dimm1_cor_ecc_errors;      // Number of DDR (DIMM1) Correctable ECC errors
    uint32_t  num_ddr_dimm1_uncor_ecc_errors;    // Number of DDR (DIMM1) Uncorrectable ECC errors
    uint32_t  num_ddr_dimm2_cor_ecc_errors;      // Number of DDR (DIMM2) Correctable ECC errors
    uint32_t  num_ddr_dimm2_uncor_ecc_errors;    // Number of DDR (DIMM2) Uncorrectable ECC errors
    uint32_t  num_ddr_dimm3_cor_ecc_errors;      // Number of DDR (DIMM3) Correctable ECC errors
    uint32_t  num_ddr_dimm3_uncor_ecc_errors;    // Number of DDR (DIMM3) Uncorrectable ECC errors
};

struct CXL_CRASH_DUMP_GET_EVENT {
    uint8_t   part;
    uint8_t   flags;                             // See CXL 2.0 spec. Get Event Records Output Payload for details
...
struct CXL_CRASH_DUMP_MEMBRG_ONE {
    uint8_t   part;
    ...
    uint64_t  rwd_latest_poison_hpa_log;         // rwd(write) latest poison hpa addr
    ...
    uint32_t  mst_m2s_rwd_corr_err_count;        // m2s rwd(write)correctible error count
    uint32_t  fifo_full_status;                  // fifo full status
    uint32_t  fifo_empty_status;                 // fifo empty status
};

struct CXL_CRASH_DUMP_MEMBRG_TWO {
    uint8_t   part;
    ...
    uint8_t   stat_qos_tel_dev_load_read;        // stat qos tel dev load read(fifo outstanding count)
    uint8_t   stat_qos_tel_dev_load_type2_read;  // stat qos tel dev load type2 read(fifo outstanding count)
    uint8_t   stat_qos_tel_dev_load_write;       // stat qos tel dev load write(fifo outstanding count)
...
static int handleHelloHealthOne(std::span<const uint8_t> data,
                            std::stringstream& ss)
{
    const auto* pBank = reinterpret_cast<const CXL_CRASH_DUMP_HEALTH_ONE*>(data.data());
...
static int handleHelloHealthTwo(std::span<const uint8_t> data,
                            std::stringstream& ss)
{
    const auto* pBank = reinterpret_cast<const CXL_CRASH_DUMP_HEALTH_TWO*>(data.data());
...
static int handleHelloGetEvent(std::span<const uint8_t> data,
                            std::stringstream& ss)
{
    const auto* pBank = reinterpret_cast<const CXL_CRASH_DUMP_GET_EVENT*>(data.data());
    ss << std::format("    flags                       : 0x{:04X}\n", pBank->flags);
    ss << std::format("    overflow_error_count        : 0x{:04X}\n", pBank->overflow_error_count);
    ss << std::format("    first_overflow_event_ts     : 0x{:04X} 0x{:04X}\n", pBank->first_overflow_event_ts[0], pBank->first_overflow_event_ts[1]);
    ss << std::format("    last_overflow_event_ts      : 0x{:04X} 0x{:04X}\n", pBank->last_overflow_event_ts[0], pBank->last_overflow_event_ts[1]);
    ss << std::format("    event_record_count          : 0x{:04X}\n", pBank->event_record_count);
    ss << "\n";
    return PLDM_SUCCESS;
}


static int handleHelloMemBrgOne(std::span<const uint8_t> data,
                            std::stringstream& ss)
{
    const auto* pBank = reinterpret_cast<const CXL_CRASH_DUMP_MEMBRG_ONE*>(data.data());
    ss << std::format("    m2s_req_count              : 0x{:016X}\n", pBank->m2s_req_count            );
    ss << std::format("    m2s_rwd_count              : 0x{:016X}\n", pBank->m2s_rwd_count            );
    ss << std::format("    s2m_drs_count              : 0x{:016X}\n", pBank->s2m_drs_count            );
    ss << std::format("    s2m_ndr_count              : 0x{:016X}\n", pBank->s2m_ndr_count            );
    ss << std::format("    rwd_first_poison_hpa_log   : 0x{:016X}\n", pBank->rwd_first_poison_hpa_log );
    ss << std::format("    rwd_latest_poison_hpa_log  : 0x{:016X}\n", pBank->rwd_latest_poison_hpa_log);
    ss << std::format("    req_first_hpa_log          : 0x{:016X}\n", pBank->req_first_hpa_log        );
    ss << std::format("    rwd_first_hpa_log          : 0x{:016X}\n", pBank->rwd_first_hpa_log        );
...
static int handleHelloMemBrgTwo(std::span<const uint8_t> data,
                            std::stringstream& ss)
{
    const auto* pBank = reinterpret_cast<const CXL_CRASH_DUMP_MEMBRG_TWO*>(data.data());
    ss << std::format("    m2s_rwd_credit_count            : 0x{:016X}\n", pBank->m2s_rwd_credit_count);
    ss << std::format("    m2s_req_credit_count            : 0x{:016X}\n", pBank->m2s_req_credit_count);
    ss << std::format("    s2m_ndr_credit_count            : 0x{:016X}\n", pBank->s2m_ndr_credit_count);
    ss << std::format("    s2m_drc_credit_count            : 0x{:016X}\n", pBank->s2m_drc_credit_count);
    ss << std::format("    rx_fsm_status_rx_deinit         : 0x{:016X}\n", pBank->rx_fsm_status_rx_deinit);
    ss << std::format("    rx_fsm_status_m2s_req           : 0x{:016X}\n", pBank->rx_fsm_status_m2s_req);
    ss << std::format("    rx_fsm_status_m2s_rwd           : 0x{:016X}\n", pBank->rx_fsm_status_m2s_rwd);
    ss << std::format("    rx_fsm_status_ddr0_ar_req       : 0x{:016X}\n", pBank->rx_fsm_status_ddr0_ar_req);
    ss << std::format("    rx_fsm_status_ddr0_aw_req       : 0x{:016X}\n", pBank->rx_fsm_status_ddr0_aw_req);
    ss << std::format("    rx_fsm_status_ddr0_w_req        : 0x{:016X}\n", pBank->rx_fsm_status_ddr0_w_req);
    ss << std::format("    rx_fsm_status_ddr1_ar_req       : 0x{:016X}\n", pBank->rx_fsm_status_ddr1_ar_req);
    ss << std::format("    rx_fsm_status_ddr1_aw_req       : 0x{:016X}\n", pBank->rx_fsm_status_ddr1_aw_req);
    ss << std::format("    rx_fsm_status_ddr1_w_req        : 0x{:016X}\n", pBank->rx_fsm_status_ddr1_w_req);
    ss << std::format("    tx_fsm_status_tx_deinit         : 0x{:016X}\n", pBank->tx_fsm_status_tx_deinit);
    ss << std::format("    tx_fsm_status_s2m_ndr           : 0x{:016X}\n", pBank->tx_fsm_status_s2m_ndr);
    ss << std::format("    tx_fsm_status_s2m_drc           : 0x{:016X}\n", pBank->tx_fsm_status_s2m_drc);
    ss << std::format("    stat_qos_tel_dev_load_read      : 0x{:016X}\n", pBank->stat_qos_tel_dev_load_read);
    ss << std::format("    stat_qos_tel_dev_load_type2_read: 0x{:016X}\n", pBank->stat_qos_tel_dev_load_type2_read);
    ss << std::format("    stat_qos_tel_dev_load_write     : 0x{:016X}\n", pBank->stat_qos_tel_dev_load_write);
    ss << "\n";
    return PLDM_SUCCESS;
}

static int handleHelloBank(std::span<const uint8_t> data,
                            std::stringstream& ss)
{
		if (!data.size())
    {
        error("invalid length for hello bank");
        return PLDM_ERROR;
    }

		switch (static_cast<uint8_t>(data[0]))
		{
			case HelloSecretCode::healthOne:
				if (data.size() < sizeof(CXL_CRASH_DUMP_HEALTH_ONE))
				{
					error("Invalid length for Hello Bank");
					return PLDM_ERROR;
				}
				handleHelloHealthOne(data, ss);
				break;
			case HelloSecretCode::healthTwo:
				if (data.size() < sizeof(CXL_CRASH_DUMP_HEALTH_TWO))
				{
					error("Invalid length for Hello Bank");
					return PLDM_ERROR;
				}
				handleHelloHealthTwo(data, ss);
				break;
			case HelloSecretCode::getEvent:
				if (data.size() < sizeof(CXL_CRASH_DUMP_GET_EVENT))
				{
					error("Invalid length for Hello Bank");
					return PLDM_ERROR;
				}
				handleHelloGetEvent(data, ss);
				break;
			case HelloSecretCode::memBrgOne:
				if (data.size() < sizeof(CXL_CRASH_DUMP_MEMBRG_ONE))
				{
					error("Invalid length for Hello Bank");
					return PLDM_ERROR;
				}
				handleHelloMemBrgOne(data, ss);
				break;
			case HelloSecretCode::memBrgTwo:
				if (data.size() < sizeof(CXL_CRASH_DUMP_MEMBRG_TWO))
				{
					error("Invalid length for Hello Bank");
					return PLDM_ERROR;
				}
				handleHelloMemBrgTwo(data, ss);
				break;
			default:
				error("Invalid length for Hello Bank");
				return PLDM_ERROR;
...
int CrashDumpHandler::write(const message& data)
{
    ...
    switch (pHdr->bankHdr.bankType)
    {
        ...
        case BankType::wdtReg:
            ...
            break;
				case BankType::helloType:
						res = handleHelloBank(bData, ss);
						break;
        case BankType::ctrl:
