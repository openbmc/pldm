...
template <size_t N> struct CrdHwAssertBank
{
...
static int handleCxlHealthOne([[maybe_unused]] std::span<const uint8_t> data,
                              std::stringstream& ss)
{
    ss << std::format("  CXL Health\n");
#if 1
    const auto* pBank =
        reinterpret_cast<const CXL_CRASH_DUMP_HEALTH_ONE*>(data.data());
    ss << std::format("    part                       : 0x{:04X}\n",
                      pBank->part);
...
static int handleCxlHealthTwo([[maybe_unused]] std::span<const uint8_t> data,
                              std::stringstream& ss)
{
    ss << std::format("  CXL Health\n");
#if 1
    const auto* pBank =
        reinterpret_cast<const CXL_CRASH_DUMP_HEALTH_TWO*>(data.data());
    ss << std::format("    part                           : 0x{:04X}\n",
                      pBank->part);
...
static int handleCxlMemBrgOne([[maybe_unused]] std::span<const uint8_t> data,
                              std::stringstream& ss)
{
    ss << std::format("  CXL Memory Bridge\n");
#if 1
    const auto* pBank =
        reinterpret_cast<const CXL_CRASH_DUMP_MEMBRG_ONE*>(data.data());
    ss << std::format("    part                       : 0x{:04X}\n",
                      pBank->part);
...
static int handleCxlMemBrgTwo([[maybe_unused]] std::span<const uint8_t> data,
                              std::stringstream& ss)
{
    ss << std::format("  CXL Memory Bridge\n");
#if 1
    const auto* pBank =
        reinterpret_cast<const CXL_CRASH_DUMP_MEMBRG_TWO*>(data.data());
    ss << std::format("    part                             : 0x{:04X}\n",
                      pBank->part);
                                          ...
                                          std::stringstream& ss)
{
    ...
    {
        case CrdCxlCode::healthOne:
            if (data.size() < sizeof(CXL_CRASH_DUMP_HEALTH_ONE))
            {
                error("Invalid length for Cxl Bank");
                return PLDM_ERROR;
            }
            handleCxlHealthOne(data, ss);
            break;
        case CrdCxlCode::healthTwo:
            if (data.size() < sizeof(CXL_CRASH_DUMP_HEALTH_TWO))
            {
                error("Invalid length for Cxl Bank");
                return PLDM_ERROR;
            }
            handleCxlHealthTwo(data, ss);
            break;
        case CrdCxlCode::getEvent:
            ...
            break;
        case CrdCxlCode::memBrgOne:
            if (data.size() < sizeof(CXL_CRASH_DUMP_MEMBRG_ONE))
            {
                error("Invalid length for Cxl Bank");
                return PLDM_ERROR;
            }
            handleCxlMemBrgOne(data, ss);
            break;
        case CrdCxlCode::memBrgTwo:
            if (data.size() < sizeof(CXL_CRASH_DUMP_MEMBRG_TWO))
            {
                error("Invalid length for Cxl Bank");
                return PLDM_ERROR;
            }
            handleCxlMemBrgTwo(data, ss);
            break;
        default:
