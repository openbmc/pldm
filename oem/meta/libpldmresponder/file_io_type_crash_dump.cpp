                              ...
                              std::stringstream& ss)
{
    ...
    {
        if (!pBank->OpcodeCmd[i].Indx && !pBank->OpcodeCmd[i].Opcode && !pBank->OpcodeCmd[i].Effect){
					continue;
				}
...
int CrashDumpHandler::handleCxlCrashdump(std::span<const uint8_t> data)
{
    ...
    switch (crdCxlCode)
    {
        ...
        case CrdCxlCode::crashDumpEnd:
        {
            ...
            auto prefix = "host" + getSlotNumberString() + "_crashdump_CXL_";
...
int CrashDumpHandler::write(const message& data)
{
    ...
    if (!res)
    {
        error("Fail to write crash dump data, bank type {BTYPE}", "BTYPE",
              pHdr->bankHdr.bankType);
        return PLDM_ERROR;
    }
