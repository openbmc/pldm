...
int BIOSVersionHandler::write(const message& data)
{
    ...
    {
        error("{FUNC}: Failed to execute Dbus call with error code {ERROR}",
              "FUNC", std::string(__func__), "ERROR", e);
    }
    catch (const std::exception& e)
    {
        error("{FUNC}: Failed to set BIOS version with error code {ERROR}",
              "FUNC", std::string(__func__), "ERROR", e);
        return PLDM_ERROR;
    }
...
std::string BIOSVersionHandler::getSlotNumber()
{
    ...
    {
        error("{FUNC}: Runtime error occurred with error code {ERROR}", "FUNC",
              std::string(__func__), "ERROR", e);
    }
    catch (const std::exception& e)
    {
        error("{FUNC}: Unexpected exception occurred with error code {ERROR}",
              "FUNC", std::string(__func__), "ERROR", e);
    }
...
const std::string BIOSVersionHandler::getDbusPathOfBoardContainingTheEndpoint(
    uint8_t tid)
{
    std::string result{};
    ...
    {
        error("{FUNC}: Failed to call GetAncestors with error code {ERROR}",
              "FUNC", std::string(__func__), "ERROR", e);
