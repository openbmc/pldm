			    ...
			    uint8_t *type, ver32_t *version)
{
	...
	struct PLDM_GetCommands_Request *request =
	    (struct PLDM_GetCommands_Request *)msg;
	*type = request->type;
			     ...
			     const bitfield8_t *commands, struct pldm_msg *msg)
{
	...
	struct PLDM_GetCommands_Response *response =
			  ...
			  uint8_t *completion_code, bitfield8_t *types)
{
	...
	struct PLDM_GetTypes_Response *response =
	    (struct PLDM_GetTypes_Response *)msg;
			     ...
			     uint8_t *completion_code, bitfield8_t *commands)
{
	...
	struct PLDM_GetCommands_Response *response =
	    (struct PLDM_GetCommands_Response *)msg;
	*completion_code = response->completion_code;
	if (PLDM_SUCCESS != *completion_code) {
		return PLDM_SUCCESS;
	};
			   ...
			   struct pldm_msg *msg)
{
	...
	struct PLDM_GetVersion_Request *request =
			    ...
			    size_t version_size, struct pldm_msg *msg)
{
	struct pldm_header_info header = {0};
	int rc = PLDM_SUCCESS;
	struct PLDM_GetVersion_Response *response =
			   ...
			   uint8_t *type)
{
	...
	struct PLDM_GetVersion_Request *request =
	    (struct PLDM_GetVersion_Request *)msg;
			    ...
			    uint8_t *transfer_flag, ver32_t *version)
{
	...
	struct PLDM_GetVersion_Response *response =
	    (struct PLDM_GetVersion_Response *)msg;
	*completion_code = response->completion_code;
	if (PLDM_SUCCESS != *completion_code) {
		return PLDM_SUCCESS;
	};
