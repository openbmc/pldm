    ...
    uint32_t *section_offset, uint32_t *section_length)
{
	...
	if (handle == 0 && request->transfer_opflag != PLDM_XFER_COMPELTE) {
		return PLDM_ERROR_INVALID_DATA;
	}
					     ...
					     bitfield8_t *protocol_support)
{
	...
	uint16_t req_part_size = le16toh(request->part_size);
	// If a requester supports less than the minimum transfer size, it's
	// interpreted as a lack of support for multipart transfers.
	if (req_part_size < PLDM_MULTIPART_TRANSFER_MIN_SIZE) {
		return PLDM_ERROR_UNSUPPORTED_PLDM_CMD;
	}

	*part_size = req_part_size;
    ...
    const bitfield8_t *protocol_support, struct pldm_msg *msg)
{
	...
	// If a responder supports less than the minimum transfer size, it's
	// interpreted as a lack of support for multipart transfers.
	if (part_size < PLDM_MULTIPART_TRANSFER_MIN_SIZE) {
		return PLDM_ERROR_UNSUPPORTED_PLDM_CMD;
	}
					      ...
					      bitfield8_t *protocol_support)
{
	...
	// If a responder supports less than the minimum transfer size, it's
	// interpreted as a lack of support for multipart transfers.
	uint16_t resp_part_size = le16toh(resp->part_size);
	if (resp_part_size < PLDM_MULTIPART_TRANSFER_MIN_SIZE) {
		return PLDM_ERROR_UNSUPPORTED_PLDM_CMD;
	}
	*completion_code = resp->completion_code;
	*part_size = resp_part_size;
