			    ...
			    struct pldm_msg *msg)
{
	...
	size_t payload_length = sizeof(type) + sizeof(version);
			  ...
			  const bitfield8_t *types, struct pldm_msg *msg)
{
	...
	size_t payload_length = sizeof(completion_code) + (PLDM_MAX_TYPES / 8);
			     ...
			     const bitfield8_t *commands, struct pldm_msg *msg)
{
	...
	size_t payload_length =
	    sizeof(completion_code) + (PLDM_MAX_CMDS_PER_TYPE / 8);
			   ...
			   struct pldm_msg *msg)
{
	...
	int rc = PLDM_SUCCESS;
	size_t payload_length =
	    sizeof(transfer_handle) + sizeof(transfer_opflag) + sizeof(type);
			    ...
			    size_t version_size, struct pldm_msg *msg)
{
	struct pldm_header_info header = {0};
	int rc = PLDM_SUCCESS;
	size_t payload_length = sizeof(completion_code) +
				sizeof(next_transfer_handle) +
				sizeof(transfer_flag) + version_size;
			    ...
			    uint8_t *transfer_flag, ver32_t *version)
{
	...
	*next_transfer_handle = le32toh(*((uint32_t *)(msg + sizeof(uint8_t))));
	*transfer_flag =
	    *(msg + sizeof(uint8_t) + sizeof(*next_transfer_handle));

	*version = *((ver32_t *)(msg + sizeof(uint8_t) +
				 sizeof(*next_transfer_handle) +
