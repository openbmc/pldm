					  ...
					  uint8_t *effecter_value)
{
	...
	*effecter_data_size = request->effecterDataSize;
	...
	if (*effecter_data_size > PLDM_SINT32) {
		return PLDM_ERROR_INVALID_DATA;
	}

	if ((*effecter_data_size == PLDM_UINT8 ||
	     *effecter_data_size == PLDM_SINT8) &&
	    payload_length != PLDM_SET_NUMERIC_EFFECTER_VALUE_MIN_REQ_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}

	if ((*effecter_data_size == PLDM_UINT16 ||
	     *effecter_data_size == PLDM_SINT16) &&
	    payload_length !=
		PLDM_SET_NUMERIC_EFFECTER_VALUE_MIN_REQ_BYTES + 1) {
		return PLDM_ERROR_INVALID_LENGTH;
	}

	if ((*effecter_data_size == PLDM_UINT32 ||
	     *effecter_data_size == PLDM_SINT32) &&
	    payload_length !=
    ...
    uint8_t *effecter_value, struct pldm_msg *msg, size_t payload_length)
{
	...
	} else if (effecter_data_size == PLDM_UINT16 ||
		   effecter_data_size == PLDM_SINT16) {
		if (payload_length !=
		    PLDM_SET_NUMERIC_EFFECTER_VALUE_MIN_REQ_BYTES + 1) {
			return PLDM_ERROR_INVALID_LENGTH;
	...
	} else if (effecter_data_size == PLDM_UINT32 ||
		   effecter_data_size == PLDM_SINT32) {
		if (payload_length !=
		    PLDM_SET_NUMERIC_EFFECTER_VALUE_MIN_REQ_BYTES + 3) {
			return PLDM_ERROR_INVALID_LENGTH;
