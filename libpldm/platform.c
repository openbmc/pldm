    ...
    struct pldm_msg *msg, size_t payload_length)
{
	...
	if (effecter_data_size == PLDM_EFFECTER_DATA_SIZE_UINT8 ||
		...
		*response->pending_value = *pending_value;
		*response->present_value = *present_value;
	} else if (effecter_data_size == PLDM_EFFECTER_DATA_SIZE_UINT16 ||
		   effecter_data_size == PLDM_EFFECTER_DATA_SIZE_SINT16) {
		if (payload_length !=
		...
		memcpy(response->pending_value, pending_value, 2);
		memcpy(response->present_value, present_value, 2);
		*response->pending_value = htole16(*response->pending_value);
		*response->present_value` = htole16(*response->present_value);
	} else if (effecter_data_size == PLDM_EFFECTER_DATA_SIZE_UINT32 ||
		   effecter_data_size == PLDM_EFFECTER_DATA_SIZE_SINT32) {
		if (payload_length !=
		...
		memcpy(response->pending_value, pending_value, 4);
		memcpy(response->present_value, present_value, 4);
		*response->pending_value = htole32(*response->pending_value);
		*response->present_value = htole32(*response->present_value);
	}
    ...
    uint8_t *pending_value, uint8_t *present_value)
{
	...
	*effecter_oper_state = response->effecter_oper_state;
	...
	if (*effecter_data_size == PLDM_EFFECTER_DATA_SIZE_UINT8 ||
		...
		*pending_value = response->pending_value[0];
		*present_value = response->present_value[0];
	} else if (*effecter_data_size == PLDM_EFFECTER_DATA_SIZE_UINT16 ||
		   *effecter_data_size == PLDM_EFFECTER_DATA_SIZE_SINT16) {
		if (payload_length !=
		    PLDM_GET_NUMERIC_EFFECTER_VALUE_MIN_RESP_BYTES + 2) {
		...
		memcpy(pending_value, response->pending_value, 2);
		memcpy(present_value, response->present_value, 2);
		*pending_value = le16toh(*pending_value);
		*present_value = le16toh(*present_value);
	} else if (*effecter_data_size == PLDM_EFFECTER_DATA_SIZE_UINT32 ||
		   *effecter_data_size == PLDM_EFFECTER_DATA_SIZE_SINT32) {
		if (payload_length !=
		    PLDM_GET_NUMERIC_EFFECTER_VALUE_MIN_RESP_BYTES + 6) {
		...
		memcpy(pending_value, response->pending_value, 4);
		memcpy(present_value, response->present_value, 4);
		*pending_value = le32toh(*pending_value);
		*present_value = le32toh(*present_value);
	}
	return PLDM_SUCCESS;
}
