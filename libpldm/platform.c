			...
			uint8_t *transfer_crc)
{
	...
	*resp_cnt = le16toh(response->response_count);
	...
	if (response->record_data != NULL && *resp_cnt > 0) {
		if (record_data_length > *resp_cnt) {
			memcpy(record_data, response->record_data, *resp_cnt);
		} else {
			memcpy(record_data, response->record_data,
			       record_data_length);
		}
	}
	...
	if (*transfer_flag == PLDM_END) {
		...
		memcpy(&resp_crc,
		       ...
		       1);
		*transfer_crc =
		    crc8(msg->payload, PLDM_GET_PDR_MIN_RESP_BYTES + *resp_cnt);
		if (resp_crc != *transfer_crc) {
			return PLDM_ERROR_INVALID_DATA;
		}
	}
