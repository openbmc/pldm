			       ...
			       uint32_t *present_reading)
{
	...
	*present_reading = 0;
	...
	case PLDM_SENSOR_DATA_SIZE_SINT16:
		...
		*present_reading =
		    le16toh(numeric_sensor_data->present_reading[1] |
			    (numeric_sensor_data->present_reading[0] << 8));
		break;
	case PLDM_SENSOR_DATA_SIZE_UINT32:
	case PLDM_SENSOR_DATA_SIZE_SINT32:
		if (sensor_data_length != 7) {
			return PLDM_ERROR_INVALID_LENGTH;
		}
		*present_reading =
		    le32toh(numeric_sensor_data->present_reading[3] |
			    (numeric_sensor_data->present_reading[2] << 8) |
			    (numeric_sensor_data->present_reading[1] << 16) |
			    (numeric_sensor_data->present_reading[0] << 24));
		break;
	default:
		return PLDM_ERROR_INVALID_DATA;
	}
