			     ...
			     uint8_t *day, uint8_t *month, uint16_t *year)
{
	...
	if (payload_length != PLDM_SET_DATE_TIME_REQ_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
			      ...
			      struct pldm_msg *msg)
{
	...
	struct pldm_set_date_time_resp *response =
		(struct pldm_set_date_time_resp *)msg->payload;
	response->completion_code = completion_code;
			      ...
			      uint8_t *completion_code)
{
	...
	if (payload_length != PLDM_SET_DATE_TIME_RESP_BYTES) {
		return PLDM_ERROR_INVALID_LENGTH;
	}
