			     ...
			     size_t payload_length)
{
	...
	if (!is_time_legal(seconds, minutes, hours, day, month, year)) {
		...
		uint8_t dseconds = dec2bcd8(seconds);
		uint8_t dminutes = dec2bcd8(minutes);
		uint8_t dhours = dec2bcd8(hours);
		uint8_t dday = dec2bcd8(day);
		uint8_t dmonth = dec2bcd8(month);
		uint16_t dyear = dec2bcd16(year);

		struct pldm_set_date_time_req *request =
		    (struct pldm_set_date_time_req *)msg->payload;
		request->seconds = dseconds;
		request->minutes = dminutes;
		request->hours = dhours;
		request->day = dday;
		request->month = dmonth;
		request->year = htole16(dyear);
