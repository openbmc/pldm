...
int encode_set_date_time_req(uint8_t instance_id, size_t payload_length,
			     uint8_t seconds, uint8_t minutes, uint8_t hours,
			     uint8_t day, uint8_t month, uint16_t year,
			     struct pldm_msg *msg)
{
	...
	if (judge_time_legal(seconds, minutes, hours, day, month, year) != 1) {
		return PLDM_ERROR_INVALID_DATA;
	} else {
		header.instance = instance_id;
		header.msg_type = PLDM_REQUEST;
			     ...
			     uint8_t *day, uint8_t *month, uint16_t *year)
{
	...
	*year = le16toh(request->year);
	...
	if (judge_time_legal(*seconds, *minutes, *hours, *day, *month, *year) !=
	    1) {
		return PLDM_ERROR_INVALID_DATA;
	}
...
int encode_set_date_time_resp(uint8_t instance_id, size_t payload_length,
			      uint8_t completion_code, struct pldm_msg *msg)
{
	struct pldm_header_info header = {0};
