    ...
    size_t max_fru_record_table_length)
{
	...
	*fru_record_table_length = payload_length - PLDM_GET_FRU_RECORD_TABLE_MIN_RESP_BYTES;

	if (*fru_record_table_length > max_fru_record_table_length) {
		return PLDM_ERROR_INVALID_LENGTH;
	}

	memcpy(fru_record_table_data, resp->fru_record_table_data, *fru_record_table_length);
...
int decode_get_fru_record_table_resp(
	...
	uint8_t *fru_record_table_data, size_t *fru_record_table_length)
{
	return decode_get_fru_record_table_resp_safe(msg, payload_length, completion_code,
	                                             next_data_transfer_handle, transfer_flag,
	                                             fru_record_table_data, fru_record_table_length,
	                                             (size_t)-1);
}
