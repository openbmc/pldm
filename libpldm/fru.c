...
int encode_get_fru_record_table_req(uint8_t instance_id, uint32_t data_transfer_handle, uint8_t transfer_operation_flag, struct pldm_msg *msg)
{
	...
	struct pldm_get_fru_record_table_req *req = (struct pldm_get_fru_record_table_req*)msg->payload;
...
{
if (msg == NULL || completion_code == NULL || next_data_transfer_handle == NULL || transfer_flag == NULL) {
...
struct pldm_get_fru_record_table_resp *resp = ( struct pldm_get_fru_record_table_resp*)msg->payload;
...
if(*completion_code == PLDM_SUCCESS)
{
	...
	*transfer_flag = resp->transfer_flag;
	if(payload_length != PLDM_GET_FRU_RECORD_TABLE_RESP_BYTES + *next_data_transfer_handle + *trasnfer_flag){
		return PLDM_ERROR_INVALID_LENGTH;
	}

	*fru_record_offset = sizeof(struct pldm_msg_hdr) + sizeof(*completion_code) + sizeof(*next_data_transfer_handle) + sizeof(*transfer_flag);
}

return PLDM_SUCCESS;
}

int encode_get_fru_record_table_resp(uint8_t completion_code, uint32_t next_data_tansfer_handle, uint8_t transfer_flag, struct pldm_msg *msg)
{
	struct pldm_header_info header = {0};
	int rc = PLDM_SUCCESS;
	...
	if(resp->completion_code == PLDM_SUCCESS){
		resp->next_data_transfer_handle = htole32(next_data_transfer_flag);
...
{
	if(msg == NULL || data_transfer_handle == NULL || transfer_operation_flag == NULL){
		return PLDM_ERROR_INVALID_DATA;
	}
