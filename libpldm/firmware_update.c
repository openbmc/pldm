			   ...
			   bitfield32_t *update_option_flags_enabled)
{
	...
	*completion_code = msg->payload[0];
	...
	if ((response->current_state == PLDM_FD_STATE_DOWNLOAD) ||
	    (response->current_state == PLDM_FD_STATE_VERIFY) ||
	    (response->current_state == PLDM_FD_STATE_APPLY)) {
		if (response->progress_percent ==
		    PLDM_FWUP_MAX_PROGRESS_PERCENT) {
			return PLDM_ERROR_INVALID_DATA;
		}
	} else {
		if (response->progress_percent !=
