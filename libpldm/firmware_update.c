...
static bool is_comp_resp_code_valid(uint8_t comp_resp_code)
{
	...
	case PLDM_COMP_CAN_BE_UPDATED_RESP_CODE:
	case PLDM_COMP_COMPARISON_STAMP_IDENTICAL:
	case PLDM_COMP_COMPARISON_STAMP_LOWER:
	case PLDM_INVALID_COMP_COMPARISON_STAMP:
	case PLDM_COMP_CONFLICT:
	case PLDM_COMP_PREREQUISITES:
	case PLDM_COMP_NOT_SUPPORTED:
	case PLDM_COMP_SECURITY_RESTRICTIONS:
	case PLDM_INCOMPLETE_COMP_IMAGE_SET:
	case PLDM_ACTIVE_IMAGE_NOT_UPDATEABLE_SUBSEQUENTLY:
	case PLDM_COMP_VER_STR_IDENTICAL:
	case PLDM_COMP_VER_STR_LOWER:
		return true;

	default:
		if (comp_resp_code >= PLDM_VENDOR_COMP_RESP_CODE_RANGE_MIN &&
		    comp_resp_code <= PLDM_VENDOR_COMP_RESP_CODE_RANGE_MAX) {
			return true;
		}
		return false;
	}
    ...
    struct pldm_msg *msg, size_t payload_length)
{
	...
	if (comp_ver_str->length != comp_ver_str_len) {
		return PLDM_ERROR_INVALID_DATA;
	}
