...
bool is_present(pldm_entity entity, pldm_entity **entities, size_t num_entities)
{
	if (entities == NULL || num_entities == 0) {
		printf("When entities or num_entities is 0\n");
		return true;
	}
	size_t i = 0;
	printf("Num_entities:  %d\n ", num_entities);
	while (i < num_entities) {
		printf("\nentities entity type: %d",
		       (*entities + i)->entity_type);
		printf("\n Entity entityType: %d", entity.entity_type);
