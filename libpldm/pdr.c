					...
					uint32_t record_handle)
{
	...
	if (data != NULL) {
		 ...
		 * a handle. For that reason, the computed handle needs to be
		 * populated in the PDR header. For a case where the caller
		 * supplied the record handle, it would exist in the header
		 * already
		 */
		if (!record_handle) {
