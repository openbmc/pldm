...
pldm_entity_node *entity_association_tree_find(pldm_entity_node *node,
					       pldm_entity *entity)
{
	if (node == NULL) {
		return node;
	...
	return entity_association_tree_find(node->next_sibling, entity);
	return entity_association_tree_find(node->first_child, entity);
}
				  ...
				  pldm_entity *entity)
{
	...
	return entity_association_tree_find(tree->root, entity);
}
					 ...
					 pldm_entity **entities)
{
	...
	*entities = malloc(sizeof(pldm_entity) * *num_entities);
	...
	while (i < *num_entities) {
		...
		curr_entity += sizeof(pldm_entity);
		++i;
	}
}
