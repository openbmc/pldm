					 ...
					 bool increment)
{
	...
	uint16_t id = tree->last_used_container_id + 1;
	if (increment) {
		tree->last_used_container_id = id;
	}
	return id;
}
					  ...
					  pldm_entity *entities)
{
	...
	pldm_entity *entity = &entities[node->entity.entity_container_id];
	entity->entity_type = node->entity.entity_type;
	entity->entity_instance_num = node->entity.entity_instance_num;
	entity->entity_container_id = node->entity.entity_container_id;

	entity_association_tree_visit(node->next_sibling, entities);
	entity_association_tree_visit(node->first_child, entities);
}
					...
					pldm_entity **entities, size_t *size)
{
	...
	*size = next_container_id(tree, false);
	*entities = malloc(*size * sizeof(pldm_entity));
	entity_association_tree_visit(tree->root, *entities);
}
...
uint8_t get_num_children(pldm_entity_node *node, uint8_t association_type)
{
	assert(node != NULL);
	assert(association_type == PLDM_ENTITY_ASSOCIAION_PHYSICAL ||
	       association_type == PLDM_ENTITY_ASSOCIAION_LOGICAL);
...
static void _entity_association_pdr_add_entry(pldm_entity_node *parent,
					      pldm_entity_node *curr,
					      ...
					      uint8_t association_type)
{
	...
	*container_id = curr->entity.entity_container_id;
	...
	*start = association_type;
	...
	if (parent != NULL) {
		entity->entity_type = parent->entity.entity_type;
...
static void entity_association_pdr_add_entry(pldm_entity_node *parent,
					     pldm_entity_node *curr,
					     pldm_pdr *repo)
{
	uint8_t num_logical_children =
	    get_num_children(curr, PLDM_ENTITY_ASSOCIAION_LOGICAL);
	uint8_t num_physical_children =
	    get_num_children(curr, PLDM_ENTITY_ASSOCIAION_PHYSICAL);
	...
	if (num_logical_children) {
		...
		_entity_association_pdr_add_entry(
		    parent, curr, repo, logical_pdr_size, num_logical_children,
	...
	if (num_physical_children) {
		...
		_entity_association_pdr_add_entry(
		    parent, curr, repo, physical_pdr_size,
		    num_physical_children, PLDM_ENTITY_ASSOCIAION_PHYSICAL);
	}
}

static void entity_association_pdr_add(pldm_entity_node *parent,
				       pldm_entity_node *curr, pldm_pdr *repo)
{
	if (curr == NULL) {
		return;
	}
	entity_association_pdr_add_entry(parent, curr, repo);
	entity_association_pdr_add(parent, curr->next_sibling, repo);
	entity_association_pdr_add(curr, curr->first_child, repo);
}
				     ...
				     pldm_pdr *repo)
{
	...
	entity_association_pdr_add(NULL, tree->root, repo);
}
