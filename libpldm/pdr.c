...
static void  add_hotplug_record(pldm_pdr *repo,pldm_pdr_record *record,uint32_t prev_record_handle)
{
...
uint32_t pldm_pdr_add_hotplug_record(pldm_pdr *repo, const uint8_t *data, uint32_t size,
        uint32_t record_handle, bool is_remote, uint32_t prev_record_handle)
{
    assert(size != 0);
    assert(data != NULL);
				     ...
				     uint32_t bmc_record_handle)
{
 ...
 if(hotplug)
  {
      return pldm_pdr_add_hotplug_record(repo, data, size, bmc_record_handle, 
                                         false, prev->record_handle);
  }
  else
  {
...
{
    /*printf("\nenter find_insertion_at start->entity.entity_type=%d and entity_type=%d",
        start->entity.entity_type,entity_type); */
	assert(start != NULL);
    ...
    uint8_t association_type, bool is_remote)
{
  ...
  if(parent)
  {
 /*     printf("\nenter pldm_entity_association_tree_add with parent entity_type=%d, entity_instance_num=%d,ntity_container_id=%d", parent->entity.entity_type, parent->entity.entity_instance_num, parent->entity.entity_container_id);*/
  }
	assert(tree != NULL);
	assert(entity != NULL);
 //uint16_t instance = 0; //sm00
    ...
    /* printf("\ncreating the node now\n"); */
      ...
      //printf("\nreturned from find_insertion_at with instance=%d",instance);
		assert(prev != NULL);
...
{
  /*  printf("\nenter pldm_entity_association_tree_visit with tree=%p\n",(void*)tree); */
	assert(tree != NULL);
    ...
    pldm_entity_association_tree *tree, pldm_entity entity)
{
 ...
 //sm00
 printf("\nfound node to delete node->entity.entity_type=%d,node->entity.entity_instance_num=%d",
 ...
 while(start != NULL)
 {
     ...
     current_entity.entity_instance_num = start->entity.entity_instance_num;
     current_entity.entity_container_id = start->entity.entity_container_id;
     if(current_entity.entity_type == entity.entity_type &&
         current_entity.entity_instance_num == entity.entity_instance_num &&
         current_entity.entity_container_id == entity.entity_container_id)
     {
         break;
     }
     prev = start;
...
uint32_t find_record_handle_by_contained_entity(pldm_pdr *repo, pldm_entity entity,bool is_remote)
{
    printf("\nenter find_record_handle_by_contained_entity with entity_type=%d, entity_instance_num=%d, entity_container_id=%d,is_remote=%d",
         entity.entity_type,entity.entity_instance_num,entity.entity_container_id,is_remote);
    ...
    while (record != NULL && !found)
    {
        ...
        {
            printf("\ngot one PLDM_PDR_ENTITY_ASSOCIATION record_handle=%d",record->record_handle);
            struct pldm_pdr_entity_association *pdr = (struct pldm_pdr_entity_association *)
                                       ((uint8_t *)record->data + sizeof(struct pldm_pdr_hdr));
            printf("\npdr->container_id=%d,pdr->num_children=%d",pdr->container_id,pdr->num_children);                     
            ...
            for (int i = 0; i < pdr->num_children; ++i)
            {
                if (child->entity_type == entity.entity_type &&
                    child->entity_instance_num == entity.entity_instance_num &&
                    child->entity_container_id == entity.entity_container_id)
                {
                    printf("\nFOUND record_handle=%d",record->record_handle);
                    found = true;
 ...
 *event_data_op = PLDM_RECORDS_MODIFIED;
 updated_hdl = find_record_handle_by_contained_entity(repo, entity, is_remote);
 if(!updated_hdl)
 {
     *event_data_op = PLDM_INVALID_OP;
     return updated_hdl;
 }
 printf("\npldm_entity_association_pdr_remove_contained_entity found the record handle to delete %d",
        updated_hdl);
 ...
 //new_record->next = NULL; //sm00
	...
	while (record != NULL) {
		pldm_pdr_record *next = record->next;
		struct pldm_pdr_hdr *hdr = (struct pldm_pdr_hdr *)record->data;
		if (record->record_handle == updated_hdl)/*(record->is_remote == is_remote) &&*/
		    /*hdr->type == PLDM_PDR_ENTITY_ASSOCIATION)*/ {
      new_record->data = malloc(record->size - sizeof(pldm_entity)); //sm00
      new_record->next = NULL; //sm00
			new_record->record_handle =
			    htole32(record->record_handle);
			new_record->size = htole32(record->size - sizeof(pldm_entity)); //sm00
			new_record->is_remote = record->is_remote;
			uint8_t *new_start = new_record->data;//sm00  new_data;
			struct pldm_pdr_hdr *new_hdr =
			    (struct pldm_pdr_hdr *) new_record->data; //sm00 new_data;
			...
			/*new_hdr->length =
			    htole16(record->size - sizeof(struct pldm_pdr_hdr) -
				    sizeof(pldm_entity));*/
   new_hdr->length = htole16(hdr->length - sizeof(pldm_entity)); //sm00
			...
			for (int i = 0; i < pdr->num_children; ++i) {
				if (child->entity_type == entity.entity_type &&
					...
					entity.entity_container_id) {
					removed = true;
				//	updated_hdl = hdr->record_handle; sm00 not needed as we are getting
    //	earlier
   ...
   if(!new_pdr->num_children)//record will be deleted and new_record will not be added
   {
       removed = false;
...
uint32_t pldm_entity_association_pdr_add_contained_entity(pldm_pdr *repo,
         ...
         uint8_t* event_data_op,
							  bool is_remote)
{
    printf("\nenter pldm_entity_association_pdr_add_contained_entity entity type=%d entity ins=%d container id=%d", entity.entity_type,entity.entity_instance_num, entity.entity_container_id);
 ...
 *event_data_op = PLDM_RECORDS_MODIFIED;
 ...
 if(!found && !is_remote)//need to create a new entity assoc pdr
 {
     ...
     *event_data_op = PLDM_RECORDS_ADDED;
     ...
     uint16_t new_pdr_size = sizeof(struct pldm_pdr_hdr) + sizeof(uint16_t) +
                           sizeof(uint8_t) + sizeof(pldm_entity) + sizeof(uint8_t) +
     ...
     struct pldm_pdr_hdr *new_hdr = (struct pldm_pdr_hdr *)new_record->data;
     new_hdr->version = 1;
     new_hdr->record_handle = new_record->record_handle;
     new_hdr->type = PLDM_PDR_ENTITY_ASSOCIATION;
     new_hdr->record_change_num = 0;
     new_hdr->length = htole16(new_pdr_size - sizeof(struct pldm_pdr_hdr));

      struct pldm_pdr_entity_association *new_pdr =
          (struct pldm_pdr_entity_association *)
          ((uint8_t *)new_record->data + sizeof(struct pldm_pdr_hdr));
     new_pdr->container.entity_type = parent.entity_type;
     new_pdr->container.entity_instance_num = parent.entity_instance_num;
     new_pdr->container.entity_container_id = parent.entity_container_id;
