    ...
    uint8_t association_type)
{
	...
	if (entity_instance_number != 0) {
		...
		if (node) {
			pldm_entity new_parent = pldm_entity_get_parent(node);
			pldm_entity old_parent = pldm_entity_extract(parent);
			if (new_parent.entity_type == old_parent.entity_type &&
			    new_parent.entity_instance_num ==
				old_parent.entity_instance_num) {
				return NULL;
			}
		}
	}
