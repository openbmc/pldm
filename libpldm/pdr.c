...
static void entity_association_pdr_add(pldm_entity_node *curr, pldm_pdr *repo,
				      pldm_entity **entities, size_t num_entities, bool is_remote)
{
	if (curr == NULL) {
		return;
	}
    ...
    if(to_add)
    {
        printf("\nadding curr->entity.entity_type %d " ,curr->entity.entity_type);
	entity_association_pdr_add_entry(curr, repo, is_remote);
    }
    else
    {
        printf("\n no need to create this entity assoc PDR %d ",  curr->entity.entity_type);
    }
	//entity_association_pdr_add_entry(curr, repo, is_remote);
	entity_association_pdr_add(curr->next_sibling, repo, entities, num_entities, is_remote);
	entity_association_pdr_add(curr->first_child, repo,entities, num_entities, is_remote);
}
...
void pldm_entity_association_pdr_add_from_node(pldm_entity_node *node,
					       pldm_pdr *repo,pldm_entity **entities, size_t num_entities, bool is_remote)
{
	assert(repo != NULL);
