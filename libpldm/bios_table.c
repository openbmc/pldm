...
static struct table_entry_length *
find_attr_table_entry_by_type(uint8_t attr_type,
			      struct table_entry_length *handlers, size_t count)
{
	size_t i;
	for (i = 0; i < count; i++) {
...
static size_t attr_table_entry_length(const void *table_entry)
{
	...
	struct table_entry_length *attr_table_entry =
	    find_attr_table_entry_by_type(entry->attr_type, attr_table_entries,
    ...
    size_t *length)
{
	...
	size_t value_length = entry_length - header_length;
	BUFFER_SIZE_EXPECT(*length, value_length);
	memcpy(buffer, entry->value, value_length);
	*length = value_length;
...
{
	uint16_t handle = *(uint16_t *)key;
	if (pldm_bios_table_attr_value_entry_decode_handle(entry) == handle)
		return true;
	return false;
}
