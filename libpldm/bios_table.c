...
int pldm_bios_table_attr_entry_string_encode_fields(
    void *entry, size_t entry_length,
...
{
	const void *entry;
	while ((entry = pldm_bios_table_entry_next(iter)) != NULL) {
		if (equal(entry, key))
			return entry;
	}
	return NULL;
...
pldm_bios_table_string_find_by_string(const void *table, const size_t length,
				      const char *str)
{
