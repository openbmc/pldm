...
TEST(GetPackageData, testGoodEncodeResponse)
{
    constexpr uint8_t metaDataLen = 8;
    struct variable_field portionOfMetaData;
    struct pldm_get_package_data_resp inResp = {};
    ...
    std::array<uint8_t, hdrSize + sizeof(struct pldm_get_package_data_resp) +
                            metaDataLen>
        responseMsg{};
    ...
    auto rc =
        encode_get_package_data_resp(instanceId, responseMsg.size() - hdrSize,
...
TEST(GetPackageData, testBadEncodeResponse)
{
    constexpr uint8_t metaDataLen = 8;
    struct variable_field portionOfMetaData;
    uint8_t instanceId = 0x01;
    ...
    auto rc = encode_get_package_data_resp(instanceId, 0, responsePtr, &inResp,
                                           &portionOfMetaData);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
...
TEST(GetPackageData, testGoodDecodeRequest)
{
    uint32_t dataTransferHandle = 0;
    uint8_t transferOperationFlag = PLDM_GET_NEXTPART;
    uint32_t handleIn = 0xFFAB;
    std::array<uint8_t, hdrSize + sizeof(struct pldm_get_package_data_req)>
        requestMsg{};
    struct pldm_get_package_data_req* request =
        reinterpret_cast<struct pldm_get_package_data_req*>(requestMsg.data() +
                                                            hdrSize);
...
TEST(GetPackageData, testBadDecodeRequest)
{
    uint32_t dataTransferHandle = 0;
    uint8_t transferOperationFlag = PLDM_GET_NEXTPART;
    uint32_t handleIn = 0xFFAB;
    ...
    auto rc = decode_get_pacakge_data_req(NULL, requestMsg.size() - hdrSize,
                                          &dataTransferHandle,
                                          &transferOperationFlag);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);
...
TEST(GetMetaData, testGoodEncodeResponse)
{
    constexpr uint8_t metaDataLen = 8;
    struct variable_field portionOfMetaData;
    struct pldm_get_meta_data_req inResp = {};
    std::array<uint8_t, metaDataLen> metaData{};
    portionOfMetaData.length = metaDataLen;
    portionOfMetaData.ptr = metaData.data();
    std::fill(metaData.data(), metaData.end(), 0xFF);
...
TEST(GetPackageData, testGoodDecodeRequest)
{
    ...
    struct pldm_get_meta_data_req* request =
        reinterpret_cast<struct pldm_get_meta_data_req*>(requestMsg.data() + hdrSize);
    request->data_transfer_handle = htole32(handleIn);
    request->transfer_operation_flag = PLDM_GET_NEXTPART;
    auto requestPtr = reinterpret_cast<pldm_msg*>(requestMsg.data());
    auto rc =
