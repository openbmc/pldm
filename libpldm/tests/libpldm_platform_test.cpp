...
TEST(GetNumericEffecterValue, testGoodEncodeResponse)
{
    ...
    uint8_t pendingValue = 0x78;
    uint8_t presentValue = 0xEF;
...
TEST(GetNumericEffecterValue, testGoodDecodeResponse)
{
    ...
    uint8_t effecter_dataSize = PLDM_EFFECTER_DATA_SIZE_UINT8;
    uint8_t effecter_operState = EFFECTER_OPER_STATE_ENABLED_NOUPDATEPENDING;
    uint8_t pendingValue = 0x34;
    uint8_t presentValue = 0xCD;

    uint8_t retcompletionCode;
    uint8_t reteffecter_dataSize;
    uint8_t reteffecter_operState;
    uint8_t retpendingValue;
    uint8_t retpresentValue;
    ...
    memcpy(resp->pending_and_present_values, &pendingValue,
           sizeof(pendingValue));
    memcpy(&resp->pending_and_present_values[1], &presentValue,
           sizeof(presentValue));
    ...
    EXPECT_EQ(pendingValue, retpendingValue);
    EXPECT_EQ(presentValue, retpresentValue);
}
