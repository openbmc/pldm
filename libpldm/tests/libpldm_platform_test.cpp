...
TEST(GetSensorReading, testGoodEncodeResponse)
{
    ...
    EXPECT_EQ(presentReading,
              *(reinterpret_cast<uint16_t*>(&resp->present_reading[0])));
}
...
TEST(GetSensorReading, testGoodDecodeResponse)
{
    ...
    memcpy(resp->present_reading, &presentReading, sizeof(presentReading));
    ...
    EXPECT_EQ(presentReading,
              *(reinterpret_cast<uint32_t*>(&retpresentReading[0])));
}
