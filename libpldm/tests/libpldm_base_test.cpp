...
TEST(MultipartReceive, testDecodeResponsePass)
{
    constexpr size_t max_msg_len = 256;
...
TEST(MultipartReceive, testDecodeResponseFailBadParams)
{
    EXPECT_EQ(decode_multipart_receive_resp(NULL, 0, NULL, NULL, NULL, NULL,
                                            NULL, NULL),
              PLDM_ERROR_INVALID_DATA);
}

TEST(MultipartReceive, testDecodeResponseFailBadLength)
{
    constexpr size_t max_msg_len = 256;
    constexpr uint8_t kCompletionCode = PLDM_SUCCESS;
    constexpr uint8_t kOpFlag = PLDM_GET_FIRSTPART;
    ...
    pldm_msg* pldm_response = reinterpret_cast<pldm_msg*>(resp.data());
    EXPECT_EQ(decode_multipart_receive_resp(
                  pldm_response,
                  PLDM_MULTIPART_RECEIVE_RESP_BYTES + kDataLength + 1,
                  &completion_code, &opflag, &next_transfer_handle, data,
                  &data_length, &data_crc32),
              PLDM_ERROR_INVALID_LENGTH);
}

TEST(MultipartReceive, testDecodeResponseFailBadXferFlag)
{
    constexpr size_t max_msg_len = 256;
    constexpr uint8_t kCompletionCode = PLDM_SUCCESS;
    constexpr uint8_t kOpFlag = 0xff;
    constexpr uint32_t kNextTransferHandle = 0x0;
    constexpr uint8_t kData[] = {0x01, 0x02, 0x03, 0x04, 0x05,
                                 0x06, 0x07, 0x08, 0x09};
    // Arbitrary, doesn't matter for this test.
    ...
    EXPECT_EQ(decode_multipart_receive_resp(
                  pldm_response,
                  PLDM_MULTIPART_RECEIVE_RESP_BYTES + kDataLength,
                  &completion_code, &opflag, &next_transfer_handle, data,
                  &data_length, &data_crc32),
              PLDM_INVALID_TRANSFER_OPERATION_FLAG);
}
