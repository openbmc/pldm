...
TEST(EntityAssociationPDR, testBuild)
{
    ...
    pldm_entity_association_tree_visit(l1, &out, &num);
    EXPECT_EQ(num, 9u);
...
TEST(EntityAssociationPDR, testCopyTree)
{
    ...
    pldm_entity_association_tree_visit(l1, &orgOut, &orgNum);
    EXPECT_EQ(orgNum, 4u);

    auto newRoot = pldm_entity_association_tree_copy_root(orgTree, newTree);
    size_t newNum{};
    pldm_entity* newOut = nullptr;
    pldm_entity_association_tree_visit(newRoot, &newOut, &newNum);
