...
TEST(PDRUpdate, testAddFruRecordSet)
{
    ...
    auto handle = pldm_pdr_add_fru_record_set(repo, 1, 10, 1, 0, 100, 1);
    EXPECT_EQ(handle, 1u);
    EXPECT_EQ(pldm_pdr_get_record_count(repo), 1u);
    ...
    handle = pldm_pdr_add_fru_record_set(repo, 2, 11, 2, 1, 101, 2);
    EXPECT_EQ(handle, 2u);
    EXPECT_EQ(pldm_pdr_get_record_count(repo), 2u);
    EXPECT_EQ(pldm_pdr_get_repo_size(repo),
...
TEST(PDRUpdate, tesFindtFruRecordSet)
{
    ...
    auto second = pldm_pdr_add_fru_record_set(repo, 1, 2, 1, 1, 100, 1);
    auto third = pldm_pdr_add_fru_record_set(repo, 1, 3, 1, 2, 100, 1);
