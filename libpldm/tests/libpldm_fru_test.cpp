...
TEST(SetFRURecordTable, testGoodEncodeResponse)
{
    ...
    std::array<uint8_t,
               sizeof(pldm_msg_hdr) + PLDM_SET_FRU_RECORD_TABLE_REQ_BYTES>
        responseMsg{};
    struct pldm_msg* response =
        reinterpret_cast<struct pldm_msg*>(responseMsg.data());
    auto rc = encode_set_fru_record_table_resp(
        instanceId, completionCode, nextDataTransferHandle, response);
    EXPECT_EQ(rc, PLDM_SUCCESS);
...
{
    auto rc = encode_set_fru_record_table_resp(0, PLDM_SUCCESS, 1, NULL);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);
}
...
TEST(SetFRURecordTable, testGoodDecodeRequest)
{
    ...
    std::array<uint8_t, sizeof(pldm_msg_hdr) +
                            PLDM_SET_FRU_RECORD_TABLE_REQ_BYTES +
                            sizeof(tableData)>
        requestMsg{};
