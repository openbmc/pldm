...
class TestFileTable : public testing::Test
{
	...
	void SetUp() override
	{
		// Create a temporary directory to hold the config file and
		// files to populate the file table.
	...
	Table attrTable = {0x00, 0x00, 0x00, 0x00, 0x0b, 0x00, 0x4e, 0x56, 0x52,
			   0x41, 0x4d, 0x2d, 0x49, 0x4d, 0x41, 0x47, 0x45, 0x00,
			   0x04, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00,
			   0x00, 0x00, 0x11, 0x00, 0x4e, 0x56, 0x52, 0x41, 0x4d,
			   0x2d, 0x49, 0x4d, 0x41, 0x47, 0x45, 0x2d, 0x43, 0x4b,
			   0x53, 0x55, 0x4d, 0x10, 0x00, 0x00, 0x00, 0x04, 0x00,
			   0x00, 0x00, 0x4e, 0xfa, 0x78, 0x7c};
};
...
class MockDMA
{
	...
	MOCK_METHOD5(transferDataHost,
		     int(int fd, uint32_t offset, uint32_t length,
...
TEST(TransferDataHost, GoodPath)
{
	...
	// returns the default value of 0 (the return type of transferDataHost
	// is int, the default value for int is 0)
	uint32_t length = minSize;
	EXPECT_CALL(dmaObj, transferDataHost(_, 0, length, 0, true)).Times(1);
	auto response = transferAll<MockDMA>(
	    &dmaObj, PLDM_READ_FILE_INTO_MEMORY, path, 0, length, 0, true, 0);
	auto responsePtr = reinterpret_cast<pldm_msg *>(response.data());
	ASSERT_EQ(responsePtr->payload[0], PLDM_SUCCESS);
	ASSERT_EQ(0,
		  memcmp(responsePtr->payload + sizeof(responsePtr->payload[0]),
	...
	EXPECT_CALL(dmaObj,
		    transferDataHost(_, maxSize, minSize, maxSize, true))
	    .Times(1);
	response = transferAll<MockDMA>(&dmaObj, PLDM_READ_FILE_INTO_MEMORY,
					path, 0, length, 0, true, 0);
	responsePtr = reinterpret_cast<pldm_msg *>(response.data());
...
TEST(TransferDataHost, BadPath)
{
	...
	uint32_t length = minSize;
	EXPECT_CALL(dmaObj, transferDataHost(_, _, _, _, _))
	    .WillOnce(Return(-1));
	auto response = transferAll<MockDMA>(
	    &dmaObj, PLDM_READ_FILE_INTO_MEMORY, path, 0, length, 0, true, 0);
	auto responsePtr = reinterpret_cast<pldm_msg *>(response.data());
...
TEST_F(TestFileTable, ReadFileInvalidFileHandle)
{
	...
	oem_ibm::Handler handler(oemPlatformHandler.get(), hostSocketFd,
				 host_eid, nullptr, nullptr);
	auto response =
	    handler.readFileIntoMemory(request, requestPayloadLength);
	auto responsePtr = reinterpret_cast<pldm_msg *>(response.data());
...
TEST_F(TestFileTable, WriteFileInvalidFileHandle)
{
	...
	oem_ibm::Handler handler(oemPlatformHandler.get(), hostSocketFd,
				 host_eid, nullptr, nullptr);
	auto response =
	    handler.writeFileFromMemory(request, requestPayloadLength);
	auto responsePtr = reinterpret_cast<pldm_msg *>(response.data());
	ASSERT_EQ(responsePtr->payload[0], PLDM_INVALID_FILE_HANDLE);
	// Clear the file table contents.
	table.clear();
...
TEST_F(TestFileTable, ValidateFileTable)
{
	...
	auto table = tableObj();
	ASSERT_EQ(true, std::equal(attrTable.begin(), attrTable.end(),
				   table.begin()));
}
...
TEST_F(TestFileTable, GetFileTableCommand)
{
	...
	oem_ibm::Handler handler(oemPlatformHandler.get(), hostSocketFd,
				 host_eid, nullptr, nullptr);
	auto response =
	    handler.getFileTable(requestMsgPtr, requestPayloadLength);
	auto responsePtr = reinterpret_cast<pldm_msg *>(response.data());
	ASSERT_EQ(responsePtr->payload[0], PLDM_SUCCESS);
	size_t offsetSize = sizeof(responsePtr->payload[0]);
	ASSERT_EQ(0, memcmp(responsePtr->payload + offsetSize,
...
TEST(getHandlerByType, allPaths)
{
	...
	handler =
	    getHandlerByType(PLDM_FILE_TYPE_RESOURCE_DUMP_PARMS, fileHandle);
	...
	handler =
	    getHandlerByType(PLDM_FILE_TYPE_CERT_SIGNING_REQUEST, fileHandle);
...
TEST(readFileByType, testBadPath)
{
	...
	int hostSocketFd = 0;
	const auto hdr_size = sizeof(pldm_msg_hdr);
	std::array<uint8_t, hdr_size + PLDM_RW_FILE_BY_TYPE_REQ_BYTES>
	    requestMsg{};
