...
FruProps PLDMFRUParser::fruProps(const uint8_t* fruRecord)
{
    ...
    // uint8_t fruFieldNumber = fruRecordData->num_fru_fields;
    ...
    FruProperties* fruProperties)
{
    ...
    while (fruFeildNumber)
    {
        ...
        else if (dataPtr->type == PLDM_FRU_FIELD_TYPE_MANUFAC_DATE)
        {
            ...
            const auto ts = reinterpret_cast<const timestamp104_t*>(dataPtr->value);
            fruProperties->emplace("BuildDate", pldm::utils::timestamp104DateToString(ts));
        }
        dataOffsetptr +=
