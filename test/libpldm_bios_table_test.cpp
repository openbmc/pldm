...
TEST(AttrValTable, IteratorTest)
{
    ...
    EXPECT_THAT(enumEntry, ElementsAreArray((uint8_t*)entry, enumEntry.size()));

    pldm_bios_table_iter_next(iter);
    entry = pldm_bios_table_iter_attr_value_entry_value(iter);
    EXPECT_THAT(stringEntry,
                ElementsAreArray((uint8_t*)entry, stringEntry.size()));

    pldm_bios_table_iter_next(iter);
    entry = pldm_bios_table_iter_attr_value_entry_value(iter);
    EXPECT_THAT(integerEntry,
                ElementsAreArray((uint8_t*)entry, integerEntry.size()));
