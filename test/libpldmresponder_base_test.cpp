...
TEST(GetPLDMTypes, testGoodRequest)
{
    ...
    ASSERT_EQ(response[RESPONSE_HEADER_LEN_BYTES], 1);
    ASSERT_EQ(response[RESPONSE_HEADER_LEN_BYTES + 1], 0);
    ASSERT_EQ(
        response[RESPONSE_HEADER_LEN_BYTES + GET_TYPES_RESP_DATA_BYTES - 1],
        0);
}
...
TEST(GetPLDMCommands, testGoodRequest)
{
    ...
    ASSERT_EQ(response[RESPONSE_HEADER_LEN_BYTES], 48); // 48 = 0b110000
    ASSERT_EQ(response[RESPONSE_HEADER_LEN_BYTES + 1], 0);
    ASSERT_EQ(
        response[RESPONSE_HEADER_LEN_BYTES + GET_TYPES_RESP_DATA_BYTES - 1],
        0);
