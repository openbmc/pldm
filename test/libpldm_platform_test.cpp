...
{
    auto rc = encode_get_state_sensor_readings_resp(0, PLDM_SUCCESS, NULL, NULL,
                                                    NULL);
...
TEST(GetStateSensorReadings, testGoodDecodeResponse)
{
    ...
    auto rc = decode_get_state_sensor_readings_resp(
        const struct pldm_msg* msg, responseMsg.size() - hdrSize,
        &retcompletion_code, &retcomp_sensorCnt, retstateField.data());
...
TEST(GetStateSensorReadings, testBadDecodeResponse)
{
    ...
    auto rc = decode_get_state_sensor_readings_resp(
        const struct pldm_msg* msg, responseMsg.size() - hdrSize, NULL, NULL,
        NULL);
...
TEST(GetStateSensorReadings, testGoodDecodeRequest)
{
    ...
    auto rc = decode_get_state_sensor_readings_req(
        const struct pldm_msg* msg, requestMsg.size() - hdrSize, &retsensorId,
        &retsensorRearm, &retreserved);
