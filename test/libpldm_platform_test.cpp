...
TEST(PlatformEventMessage, testGoodDecodeRequest)
{
    std::array<uint8_t, hdrSize + PLDM_PLATFORM_EVENT_MESSAGE_MIN_REQ_BYTES + PLDM_PLATFORM_EVENT_MESSAGE_STATE_SENSOR_STATE_REQ_BYTES>
        requestMsg{};
    ...
    uint8_t eventDataLength= PLDM_PLATFORM_EVENT_MESSAGE_STATE_SENSOR_STATE_REQ_BYTES;

    uint8_t retFormatVersion = 0;
    uint8_t retTid = 0;
    uint8_t retEventClass = 0;
    uint8_t retEventDataLength = 0;
    ...
    struct pldm_sensor_event_state_sensor_state* eventClassData = reinterpret_cast<struct pldm_sensor_event_state_sensor_state*>(eventData.event_class);
    ...
    memcpy(request->event_data,&eventData,eventDataLength);
    std::array<uint8_t, PLDM_PLATFORM_EVENT_MESSAGE_STATE_SENSOR_STATE_REQ_BYTES> retEventData{};

    auto rc = decode_platform_event_message_req(
        req, requestMsg.size() - hdrSize, &retFormatVersion,
        &retTid, &retEventClass, reinterpret_cast<uint8_t*>(&retEventData),&retEventDataLength);
    ...
    struct pldm_sensor_event_data* retEventDataPtr = reinterpret_cast<struct pldm_sensor_event_data*>(retEventData.data());
    EXPECT_EQ(sensorId, retEventDataPtr->sensor_id);
    EXPECT_EQ(sensorEventClassType, retEventDataPtr->sensor_event_class_type);
    struct pldm_sensor_event_state_sensor_state* retEventClassData = reinterpret_cast<struct pldm_sensor_event_state_sensor_state*>(retEventDataPtr->event_class);
...
{
    const struct pldm_msg* msg = NULL;
    std::array<uint8_t, hdrSize + PLDM_PLATFORM_EVENT_MESSAGE_MIN_REQ_BYTES + PLDM_PLATFORM_EVENT_MESSAGE_STATE_SENSOR_STATE_REQ_BYTES-1>
        requestMsg{};
    auto req = reinterpret_cast<pldm_msg*>(requestMsg.data());
    uint8_t retFormatVersion;
    uint8_t retTid = 0;
    uint8_t retEventClass = 0;
    uint8_t retEventDataLength;
    std::array<uint8_t, PLDM_PLATFORM_EVENT_MESSAGE_STATE_SENSOR_STATE_REQ_BYTES> retEventData{};

    auto rc = decode_platform_event_message_req(msg, sizeof(*msg), NULL, NULL, NULL,NULL, NULL);

    EXPECT_EQ(rc, PLDM_ERROR_INVALID_DATA);

    rc = decode_platform_event_message_req(
        req, requestMsg.size() - hdrSize - PLDM_PLATFORM_EVENT_MESSAGE_STATE_SENSOR_STATE_REQ_BYTES, &retFormatVersion,
        &retTid, &retEventClass, retEventData.data(),&retEventDataLength);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
}

TEST(PlatformEventMessage, testGoodEncodeResponse)
{
  std::array<uint8_t, hdrSize + PLDM_PLATFORM_EVENT_MESSAGE_MIN_REQ_BYTES + PLDM_PLATFORM_EVENT_MESSAGE_STATE_SENSOR_STATE_REQ_BYTES-1> responseMsg{};
    auto response = reinterpret_cast<pldm_msg*>(responseMsg.data());
    uint8_t completionCode = 0, instanceId=0x01, status=1;

    auto rc = encode_platform_event_message_resp(
        instanceId, PLDM_SUCCESS, status,response);
