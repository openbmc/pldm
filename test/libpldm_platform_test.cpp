...
TEST(GetPDR, testGoodDecodeResponse)
{
    ...
    EXPECT_EQ(0, memcmp(recordData, resp->record_data, 9));
}

TEST(GetPDR, testBadDecodeResponse)
{
    std::array<uint8_t, hdrSize + PLDM_GET_PDR_MIN_RESP_BYTES + 1>
        responseMsg{};
    auto response = reinterpret_cast<pldm_msg*>(responseMsg.data());

    auto rc = decode_get_pdr_resp(response, responseMsg.size(), NULL, NULL,
    ...
    rc = decode_get_pdr_resp(response, responseMsg.size() - hdrSize,
                             &retCompletionCode, &retNextRecordHndl,
                             &retNextDataTransferHndl, &retTransferFlag,
                             &retRespCnt, retRecordData, 32, &retTransferCRC);
    EXPECT_EQ(rc, PLDM_ERROR_INVALID_LENGTH);
}