...
TEST(SetStateEffecterStates, testEncodeRequest)
{
    ...
    auto rc = encode_set_state_effecter_states_req(
        0, effecterId, compEffecterCnt, stateField.data(), stateField.size(),
        &request);

    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(0, memcmp(request.body.payload, &effecterId, sizeof(effecterId)));
    ASSERT_EQ(0, memcmp(request.body.payload + sizeof(effecterId),
                        &compEffecterCnt, sizeof(compEffecterCnt)));
    ASSERT_EQ(0, memcmp(request.body.payload + sizeof(effecterId) +
