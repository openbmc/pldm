...
TEST(GetPDR, testGoodEncodeRequest)
{
    ...
    auto rc =
        encode_get_pdr_req(0, record_hndl, data_transfer_hndl, transfer_op_flag,
                           request_cnt, record_chg_num, request);
    ASSERT_EQ(rc, PLDM_SUCCESS);
    struct pldm_get_pdr_req* req =
        reinterpret_cast<struct pldm_get_pdr_req*>(request->payload);
    ASSERT_EQ(record_hndl, req->record_handle);
    ASSERT_EQ(data_transfer_hndl, req->data_transfer_handle);
    ASSERT_EQ(transfer_op_flag, req->transfer_op_flag);
    ASSERT_EQ(request_cnt, req->request_count);
    ASSERT_EQ(record_chg_num, req->record_change_number);
}
...
TEST(GetPDR, testBadEncodeRequest)
{
    ...
    auto rc =
        encode_get_pdr_req(0, record_hndl, data_transfer_hndl, transfer_op_flag,
                           request_cnt, record_chg_num, nullptr);
    ASSERT_EQ(rc, PLDM_ERROR_INVALID_DATA);
}
...
TEST(GetPDR, testGoodDecodeResponse)
{
    ...
    uint8_t transferFlag = PLDM_END;
    uint16_t respCnt = 9;
    uint8_t transferCRC = 244;
    std::array<uint8_t, hdrSize + PLDM_GET_PDR_MIN_RESP_BYTES + 9 + 1>
        responseMsg{};
    ...
    resp->next_record_handle = nextRecordHndl;
    resp->next_data_transfer_handle = nextDataTransferHndl;
    resp->transfer_flag = transferFlag;
    resp->response_count = respCnt;
    memcpy(resp->record_data, recordData, 9);

    auto rc = decode_get_pdr_resp(response, responseMsg.size() - hdrSize,
                                  &retCompletionCode, &retNextRecordHndl,
                                  &retNextDataTransferHndl, &retTransferFlag,
                                  &retRespCnt, retRecordData, &retTransferCRC);
    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(retCompletionCode, completionCode);
    ASSERT_EQ(retNextRecordHndl, nextRecordHndl);
    ASSERT_EQ(retNextDataTransferHndl, nextDataTransferHndl);
    ASSERT_EQ(retTransferFlag, transferFlag);
    ASSERT_EQ(retRespCnt, respCnt);
    ASSERT_EQ(retTransferCRC, transferCRC);
    ASSERT_EQ(0, memcmp(recordData, resp->record_data, 9));
}

TEST(GetPDR, testBadDecodeResponse)
{
    std::array<uint8_t, hdrSize + PLDM_GET_PDR_MIN_RESP_BYTES> responseMsg{};
    auto req = reinterpret_cast<pldm_msg*>(responseMsg.data());

    auto rc = decode_get_pdr_req(req, responseMsg.size(), NULL, NULL, NULL,
                                 NULL, NULL);
