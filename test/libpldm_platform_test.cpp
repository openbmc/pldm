...
TEST(SetNumericEffecterValue, testGoodDecodeRequest)
{
    ...
    uint8_t effecterDataSize = 0x0;
    uint8_t effecterValue = 1;

    uint16_t reteffecterId = 0;
    uint8_t reteffecterDataSize = 0;
    uint8_t reteffecterValue = 0;
    ...
    EXPECT_EQ(0,
              memcmp(&reteffecterValue, &effecterValue, sizeof(effecterValue)));
}
...
TEST(SetNumericEffecterValue, testGoodEncodeRequest)
{
    ...
    EXPECT_EQ(
        0, memcmp(&effecterValue, req->effecterValue, sizeof(effecterValue)));
}
