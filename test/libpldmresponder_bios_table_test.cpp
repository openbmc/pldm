...
TEST_F(TestBIOSTable, testResponse)
{
    std::vector<uint8_t> resp{10, 34, 56, 100, 44, 55, 69, 21, 48, 2, 7, 82};
    std::vector<uint8_t> out{10, 34, 56, 100, 44, 55, 69, 21, 48, 2, 7, 82};
    fs::path file(dir / "t2");
    BIOSTable t(file.string().c_str());

    ASSERT_EQ(false, t.hasResponse());

    t.cacheGetTableResponse(std::move(resp));
    resp = t.getTableResponse();
    ASSERT_EQ(true, std::equal(resp.begin(), resp.end(), out.begin()));
}
