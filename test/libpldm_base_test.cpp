...
TEST(GetPLDMCommands, testDecodeRequest)
{
    ...
    auto rc =
        decode_get_commands_req(requestMsg.data(), &pldmTypeOut, &versionOut);
    ASSERT_EQ(rc, PLDM_SUCCESS);
...
TEST(GetPLDMCommands, testEncodeResponse)
{
    ...
    std::array<uint8_t, sizeof(pldm_msg_hdr) + PLDM_GET_COMMANDS_RESP_BYTES> responseMsg{};
...
{
    uint8_t completionCode = 0;
    std::array<uint8_t, sizeof(pldm_msg_hdr) + PLDM_GET_TYPES_RESP_BYTES> responseMsg{};
    pldm_msg* response = reinterpret_cast<pldm_msg*>(responseMsg.data());
...
TEST(GetPLDMTypes, testDecodeResponse)
{
    ...
    auto rc = decode_get_types_resp(responseMsg.data(), &completion_code,
                                    outTypes.data());
...
TEST(GetPLDMCommands, testDecodeResponse)
{
    ...
    auto rc = decode_get_commands_resp(responseMsg.data(), &completion_code,
                                       outTypes.data());
...
TEST(GetPLDMVersion, testDecodeRequest)
{
    ...
    auto rc = decode_get_version_req(requestMsg.data(), &retTransferHandle,
                                     &retFlag, &retType);
...
TEST(GetPLDMVersion, testDecodeResponse)
{
    ...
    auto rc =
        decode_get_version_resp(responseMsg.data(), &completion_code,
