...
TEST(GetPLDMVersion, testEncodeRequest)
{
    ...
    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(0,
              memcmp(request.body.payload, &transferHandle,
                     sizeof(transferHandle)));
    ASSERT_EQ(0, memcmp(request.body.payload + sizeof(transferHandle), &opFlag,
                        sizeof(opFlag)));
    ASSERT_EQ(0,
              memcmp(request.body.payload + sizeof(transferHandle) +
                         sizeof(opFlag),
...
TEST(GetPLDMVersion, testEncodeResponse)
{
    ...
    auto rc = encode_get_version_resp(0, 0, PLDM_START_AND_END, &version,
                                      sizeof(pldm_version_t), &response);

    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(completionCode, response.body.payload[0]);
    ASSERT_EQ(0, memcmp(response.body.payload +
                            sizeof(response.body.payload[0]),
                        &transferHandle, sizeof(transferHandle)));
    ASSERT_EQ(0, memcmp(response.body.payload +
                            sizeof(response.body.payload[0]) +
                            sizeof(transferHandle),
                        &flag, sizeof(flag)));
    ASSERT_EQ(0, memcmp(response.body.payload + sizeof(response.body.payload[0])
