...
TEST(PackPLDMMessage, BadPathTest)
{
    ...
    auto rc = pack_pldm_header((uint8_t*)(uint8_t*)&arr, hdr_ptr);
    ASSERT_EQ(rc, FAIL);

    // PLDM message pointer is NULL
    rc = pack_pldm_header(ptr, &hdr);
    ASSERT_EQ(rc, FAIL);

    // PLDM header information pointer and PLDM message pointer is NULL
    rc = pack_pldm_header(ptr, hdr_ptr);
    ASSERT_EQ(rc, FAIL);

    // RESERVED message type
    hdr.msg_type = RESERVED;
    rc = pack_pldm_header((uint8_t*)(uint8_t*)&arr, &hdr);
    ASSERT_EQ(rc, FAIL);

    // Instance ID out of range
    hdr.msg_type = REQUEST;
    hdr.instance = 32;
    rc = pack_pldm_header((uint8_t*)&arr, &hdr);
    ASSERT_EQ(rc, FAIL);
...
TEST(PackPLDMMessage, RequestMessageGoodPath)
{
    ...
    auto rc = pack_pldm_header((uint8_t*)&arr, &hdr);
    ASSERT_EQ(rc, OK);
...
TEST(UnpackPLDMMessage, BadPathTest)
{
    ...
    // header size
    arr[0] = 0x80;
    unpack_pldm_header((uint8_t*)&arr, 2, &hdr, &offset, &resp_size);
    ASSERT_EQ(rc, -1);

    // Response message and size of PLDM message is less than the PLDM response
    // header size
    arr[0] = 0x00;
    unpack_pldm_header((uint8_t*)&arr, 3, &hdr, &offset, &resp_size);
    ASSERT_EQ(rc, -1);
}
...
TEST(UnpackPLDMMessage, ResponseMessageGoodPath)
{
    ...
    unpack_pldm_header((uint8_t*)&arr, sizeof(arr), &hdr, &offset, &resp_size);
    ...
    unpack_pldm_header((uint8_t*)&arr1, sizeof(arr1), &hdr, &offset,
                       &resp_size);
    ASSERT_EQ(hdr.msg_type, RESPONSE);
    ASSERT_EQ(hdr.instance, 31);
    ASSERT_EQ(hdr.pldm_type, 63);
...
{
    std::array<uint8_t, REQUEST_HEADER_LEN_BYTES +
                            // PLDM version
                            sizeof(ver32) +
                            // PLDM Type
                            sizeof(uint8_t)>
    ...
    ASSERT_EQ(0, memcmp(buffer.data() + REQUEST_HEADER_LEN_BYTES, &pldmType,
                        sizeof(pldmType)));
    ASSERT_EQ(
        0, memcmp(buffer.data() + REQUEST_HEADER_LEN_BYTES + sizeof(pldmType),
...
{
    std::array<uint8_t, RESPONSE_HEADER_LEN_BYTES + MAX_CMDS_PER_TYPE> buffer{};
...
{
    std::array<uint8_t, RESPONSE_HEADER_LEN_BYTES + MAX_TYPES> buffer{};
