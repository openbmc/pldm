...
TEST(FruParser, allScenarios)
{
    ...
    // Get an item with a 1 standard and 2 OEM PLDM FRU records
    ...
    ASSERT_EQ(motherBoardInfos.size(), 3);
    ASSERT_EQ(motherBoard == motherBoardInfos, true);

    // Get an item with a 1 standard and 1 OEM PLDM FRU record
    FruRecordInfos bmc{
        {1,
         1,
         {{"xyz.openbmc_project.Inventory.Decorator.Asset", "Model", "string",
    ...
    // Get an item type with 1 PLDM FRU records
    FruRecordInfos brd{
        {1,
         1,
         {{"xyz.openbmc_project.Inventory.Decorator.Asset", "Model", "string",
    ...
    ASSERT_EQ(boardInfos.size(), 1);
    ASSERT_EQ(brd == boardInfos, true);
