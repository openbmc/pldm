...
class MockPLDMFRUParser : public PLDMFRUParser<FruTableIntf>
{
  public:
    MockPLDMFRUParser(sdbusplus::bus::bus& bus, const std::string& objPath) :
        PLDMFRUParser<FruTableIntf>(bus, objPath)
    {}
};
...
class TestFRUParser : public testing::Test
{
  public:
    ...
    TestFRUParser() :
        ...
        mockfruparser(bus, "/xyz/openbmc_project/pldm/fru_record_set")
    {}
    ~TestFRUParser()
    {}
};
...
TEST_F(TestFRUParser, testTableParsed)
{
    ...
    auto& fruObjs = mockfruparser.getvecObjs();
    int it = 0;

    for (auto const& obj : fruObjs)
    {
        if (it == 0)
        {
            EXPECT_EQ(obj->dBusMap.propertyName, A1);
        }
        if (it == 1)
        {
            EXPECT_EQ(obj->dBusMap.propertyName, A2);
        }
        else if (it == 2)
        {
            EXPECT_EQ(obj->dBusMap.propertyName, B1);
        }
        else if (it == 3)
        {
            EXPECT_EQ(obj->dBusMap.propertyName, B2);
        }
        it++;
    }
}
