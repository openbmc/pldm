...
TEST(parseEffecterData, testGoodDecodeEffecterData)
{
    ...
    uint8_t effecter_count = 2;
    set_effecter_state_field stateField0 = {1, 1};
    set_effecter_state_field stateField1 = {0, 1};

    std::vector<set_effecter_state_field> stateField = {};
    auto rc = parseEffecterData(effecterData, effecter_count, stateField);

    EXPECT_EQ(rc, true);
    EXPECT_EQ(effecter_count, stateField.size());
...
TEST(parseEffecterData, testBadDecodeEffecterData)
{
    ...
    uint16_t retEffecter_count = 2;
    std::vector<set_effecter_state_field> stateField = {};
    auto rc = parseEffecterData(effecterData, retEffecter_count, stateField);
