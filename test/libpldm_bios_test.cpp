...
{
    std::array<uint8_t, sizeof(pldm_msg_hdr) + PLDM_GET_BIOS_TABLE_REQ_BYTES> requestMsg{};
    ...
    ASSERT_EQ(0, memcmp(request->payload + sizeof(transferHandle), &transferOpFlag,
                        sizeof(transferOpFlag)));
    ASSERT_EQ(0,
              memcmp(request->payload + sizeof(transferHandle) + sizeof(transferOpFlag),
