...
{
    return sizeof(mctpPrefix) + sizeof(pldm_msg_hdr) +
           PLDM_SET_STATE_EFFECTER_STATES_RESP_BYTES;
    ;
}
...
TEST(RequesterAPI, SendRecv)
{
    ...
    rc = pldm_recv_any(mctpEid, 0, &responseMsg, &responseMsgSize);
    ASSERT_EQ(rc, 0);

    rc = pldm_recv(mctpEid, 0, instanceId, &responseMsg, &responseMsgSize);
    ASSERT_EQ(rc, 0);

    rc = pldm_recv_any(0, 0, &responseMsg, &responseMsgSize);
    ASSERT_EQ(rc, -1);

    rc = pldm_recv(mctpEid, 0, 0, &responseMsg, &responseMsgSize);
    ASSERT_EQ(rc, -1);
}
