...
char biosPath[17]; // path to store bios tables

TEST(GetBIOSTable, testBadRequest)
{
    char tmpbios[] = "/tmp/bios.XXXXXX";
    char* dirName = mkdtemp(tmpbios);
    strcpy(biosPath, dirName); // to use in other tables
...
TEST(buildBIOSTables, testBadRequest)
{
    std::array<uint8_t, sizeof(pldm_msg_hdr) + PLDM_GET_BIOS_TABLE_REQ_BYTES>
        requestPayload{};
    auto request = reinterpret_cast<pldm_msg*>(requestPayload.data());
