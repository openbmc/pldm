...
class TestAllBIOSTables : public ::testing::Test
{
  public:
                                ...
                                // TestAllBIOSTables objects
    {
        ...
        char tmpstrdir[] = "/tmp/string_allBiosTables.XXXXXX";
        strBiosPath = fs::path(mkdtemp(tmpstrdir));
    }

    static void TearDownTestCase() // will be executed once at th eend of all
                                   // TestAllBIOSTables objects
    {
        // fs::remove_all(biosPath); // wkr
        // fs::remove_all(strBiosPath); // wkr
    }

    static fs::path biosPath;
    static fs::path strBiosPath;
};

fs::path TestAllBIOSTables::biosPath;
fs::path TestAllBIOSTables::strBiosPath;
...
TEST_F(TestAllBIOSTables, getBIOSAttributeValueTableBasedOnStringTypeTest)
{
    std::array<uint8_t, sizeof(pldm_msg_hdr) + PLDM_GET_BIOS_TABLE_REQ_BYTES>
        requestPayload{};
    auto request = reinterpret_cast<pldm_msg*>(requestPayload.data());
    ...
    size_t requestPayloadLength = requestPayload.size() - sizeof(pldm_msg_hdr);
    auto str_response = internal::buildBIOSTables(
        request, requestPayloadLength, "./bios_string_json",
        TestAllBIOSTables::strBiosPath.c_str());
    ...
    auto attr_response = internal::buildBIOSTables(
        request, requestPayloadLength, "./bios_string_json",
        TestAllBIOSTables::strBiosPath.c_str());
    ...
    std::string attrName("str_example3");
    bios_parser::bios_string::CurrentValue currVals =
        bios_parser::bios_string::getAttrValue(attrName);
