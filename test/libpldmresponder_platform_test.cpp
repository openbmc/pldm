...
TEST(getPDR, testGoodPath)
{
    ...
    Handler handler("./pdr_jsons/state_effecter/good", "./event_jsons/good",
                    pdrRepo, entityTree, nullptr, nullptr);
...
TEST(setStateEffecterStatesHandler, testGoodRequest)
{
    ...
    Handler handler("./pdr_jsons/state_effecter/good", "./event_jsons/good",
                    inPDRRepo, entityTree, nullptr, nullptr);
...
TEST(parseStateSensor, allScenarios)
{
    ...
    std::vector<uint8_t> sample1PDR{
        ...
        0x00, 0x01, 0x00, 0x43, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    std::vector<uint8_t> sample2PDR{
        0x00, 0x00, 0x00, 0x00, 0x01, 0x04, 0x00, 0x00, 0x17, 0x00, 0x00,
        0x00, 0x02, 0x00, 0x1F, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x00, 0x02, 0x18, 0x2E};
    ...
    std::vector<uint8_t> sample3PDR{
        0x00, 0x00, 0x00, 0x00, 0x01, 0x04, 0x00, 0x00, 0x17, 0x00, 0x00, 0x00,
        0x03, 0x00, 0x21, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
        0x00, 0x00, 0x02, 0x21, 0x00, 0x01, 0x06, 0x0F, 0x00, 0x01, 0x1E};
...
TEST(getStateSensorReadingsHandler, testGoodRequest)
{
    ...
    EXPECT_CALL(handlerObj,
                getDbusPropertyVariant(
                    _, StrEq("CriticalAlarmLow"),
                    StrEq("xyz.openbmc_project.Sensor.Threshold.Critical")))
        .WillOnce(Return(PropertyValue(bool(true))));

    auto rc =
        platform_state_sensor::getStateSensorReadingsHandler<MockdBusHandler,
                                                             Handler>(
            handlerObj, handler, 0x1, sensorRearm, compSensorCnt, stateField);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(compSensorCnt, 1);
    ASSERT_EQ(stateField[0].sensor_op_state, ENABLED);
    ASSERT_EQ(stateField[0].present_state, LOWERCRITICAL);
    ASSERT_EQ(stateField[0].previous_state, UNKNOWN);
    ASSERT_EQ(stateField[0].event_state, UNKNOWN);
...
TEST(getStateSensorReadingsHandler, testBadRequest)
{
    ...
    auto rc =
        platform_state_sensor::getStateSensorReadingsHandler<MockdBusHandler,
                                                             Handler>(
            handlerObj, handler, 0x1, sensorRearm, compSensorCnt, stateField);
    ASSERT_EQ(rc, PLDM_PLATFORM_INVALID_SENSOR_REARM);
