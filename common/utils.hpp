...
/**
 * @brief A functor that converts a variant to an address.
 *
 * This struct template provides an operator() that converts a variant to an
 * address. It checks for the type of the variant and converts it to an address.
 *
 * @tparam Target The target integral type to cast to.
 * @tparam Element The element integral type to cast from.
 *
 ...
 * @return The casted value of type Target.
 */
template <std::integral Target, std::integral Element> struct AddressVisitor
{
    template <typename Source> Target operator()(const Source& source) const
    {
        if constexpr (std::ranges::range<Source>)
        {
            using SourceVT = std::ranges::range_value_t<Source>;
            if constexpr (std::is_integral_v<SourceVT>)
            {
                Target address = 0;
                for (const auto& byte : source)
                {
                    address = (address << 8 * sizeof(Element)) |
                              VariantToIntegralVisitor<Element>{}(byte);
                }
                return address;
            }
            throw std::invalid_argument(
                "Cannot translate type " + std::string(typeid(Source).name()) +
                " to address");
        }
        else if constexpr (std::is_integral_v<Source>)
        {
            return VariantToIntegralVisitor<Target>{}(source);
        }
        throw std::invalid_argument(
            "Cannot translate type " + std::string(typeid(Source).name()) +
            " to address");
