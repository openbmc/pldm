...
TEST(GetStateSensorPDRsByType, testMultipleMatches)
{
    ...
    auto rec1 = reinterpret_cast<pldm_state_sensor_pdr*>(pdr1.data());
    auto state1 =
        reinterpret_cast<state_sensor_possible_states*>(rec1->possible_states);
    ...
    // Add second PDR with same entity type
    std::vector<uint8_t> pdr2(
        sizeof(struct pldm_state_sensor_pdr) - sizeof(uint8_t) +
        sizeof(struct state_sensor_possible_states));

    auto rec2 = reinterpret_cast<pldm_state_sensor_pdr*>(pdr2.data());
    auto state2 =
        reinterpret_cast<state_sensor_possible_states*>(rec2->possible_states);
...
TEST(GetStateSensorPDRsByType, testNoMatch)
{
    ...
    rec->hdr.record_handle = 1;
    rec->entity_type = 99; // Different entity type
    rec->composite_sensor_count = 1;
    state->state_set_id = 1;
    state->possible_states_size = 1;
...
TEST(GetStateEffecterPDRsByType, testMultipleMatches)
{
    ...
    auto rec1 = reinterpret_cast<pldm_state_effecter_pdr*>(pdr1.data());
    auto state1 = reinterpret_cast<state_effecter_possible_states*>(
        rec1->possible_states);
    ...
    auto rec2 = reinterpret_cast<pldm_state_effecter_pdr*>(pdr2.data());
    auto state2 = reinterpret_cast<state_effecter_possible_states*>(
        rec2->possible_states);
