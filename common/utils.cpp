    ...
    try
    {
        do
        {
            ...
            if (record)
            {
                ...
                if (rc)
                {
                    error("Failed to decode state sensor PDR, error {RC}", "RC",
                          rc);
                    continue;
                }
                ...
                {
                    error(
                        "Failed to walk state sensor PDR possible states, error {RC}",
                        "RC", rc);
                }
            }
                           ...
                           uint16_t containerId, uint16_t stateSetId)
{
    ...
    for (const auto& pdr : pdrs)
    {
        ...
        {
            error(
                "Failed to decode state sensor PDR for entity type '{ENTITY_TYPE}' instance '{ENTITY_INSTANCE}', error {RC}",
                "ENTITY_TYPE", entityType, "ENTITY_INSTANCE", entityInstance,
                "RC", rc);
            continue;
        }
