...
std::vector<std::vector<uint8_t>> findStateEffecterPDR(uint8_t /*tid*/,
  ...
  try {
      ...
      if (record) {
        ...
        for (auto effecters = 0x00; effecters < compositeEffecterCount;
          ...
          if (pdr->entity_type == entityID && setId == stateSetId) {
            std::vector<uint8_t> effecter_pdr(&outData[0], &outData[size]);
    ...
    std::cerr << " Failed to obtain a record. ERROR =" << e.what() << std::endl;
  }
...
std::vector<std::vector<uint8_t>> findStateSensorPDR(uint8_t /*tid*/,
  ...
  try {
    do {
      record = pldm_pdr_find_record_by_type(repo, PLDM_STATE_SENSOR_PDR, record,
                                            &outData, &size);
      if (record) {
        ...
        for (auto sensors = 0x00; sensors < compositeSensorCount; sensors++) {
          ...
          if (pdr->entity_type == entityID && setId == stateSetId) {
            std::vector<uint8_t> sensor_pdr(&outData[0], &outData[size]);
...
std::string DBusHandler::getService(const char *path,
  ...
  auto mapper = bus.new_method_call(mapperBusName, mapperPath, mapperInterface,
                                    "GetObject");
...
PropertyValue
DBusHandler::getDbusPropertyVariant(const char *objPath, const char *dbusProp,
...
int emitStateSensorEventSignal(uint8_t tid, uint16_t sensorId,
  ...
  try {
    auto &bus = DBusHandler::getBus();
    auto msg =
        bus.new_signal("/xyz/openbmc_project/pldm",
                       "xyz.openbmc_project.PLDM.Event", "StateSensorEvent");
...
uint16_t findStateSensorId(const pldm_pdr *pdrRepo, uint8_t tid,
  ...
  for (auto pdr : pdrs) {
    ...
    for (auto sensors = 0x00; sensors < compositeSensorCount; sensors++) {
      auto possibleStates = reinterpret_cast<state_sensor_possible_states *>(
          possible_states_start);
      auto setId = possibleStates->state_set_id;
      auto possibleStateSize = possibleStates->possible_states_size;
      if (entityType == sensorPdr->entity_type &&
          entityInstance == sensorPdr->entity_instance && stateSetId == setId &&
