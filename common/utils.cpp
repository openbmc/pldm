    ...
    fd_(fd), ownsFd_(false) // External fd, not owned by MMapHandler
{
    ...
    if (size.has_value())
    {
        ...
        if (size_ > static_cast<size_t>(sb.st_size))
        {
            error(
                "Requested size {REQSIZE} exceeds file size {FILESIZE}",
                "REQSIZE", size_, "FILESIZE", sb.st_size);
            data_ = nullptr;
            throw std::invalid_argument(
                "Requested size exceeds file size");
        }
    }
    else
    {
    ...
    if (data_ == MAP_FAILED)
    {
        ...
        data_ = nullptr;
        error("mmap failed: size {SIZE}, error: {ERROR}",
              "SIZE", size_, "ERROR", savedErrno);
