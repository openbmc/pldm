                                     ...
                                     ObjectPathMaps& objPathMap)
{
    ...
    {
        error(
            "Entity type '{TYPE}' fetched from remote PLDM terminal does not exist.",
                             ...
                             ObjectPathMaps& objPathMap)
{
    ...
    for (const auto& entity : parentsEntity)
    {
        ...
        while (node)
        {
            ...
            {
                error(
                    "Parent entity type '{TYPE}' with instance ID '{INSTANCEID}' not found in the entityMaps, error - {ERROR}",
...
uint8_t readHostEID()
{
    ...
    if (!eidFile.good())
    {
        error("Could not open host EID file at path '{PATH}'", "PATH",
...
void reportError(const char* errorMsg, const Severity& sev)
{
    ...
    {
        error(
            "Dbus call to create error log for '{ERRMSG}' at path '{PATH}' and interface '{INTERFACE}' failed - {ERROR}",
                               ...
                               uint8_t previousEventState)
{
    ...
    {
        error("Emitting pldm event signal, failed - {ERROR}", "ERROR", e);
        return PLDM_ERROR;
    }
...
void setFruPresence(const std::string& fruObjPath, bool present)
{
    ...
    {
        error("Set the present property on path '{PATH}' failed - {ERROR}.",
