...
the meson option `fw-update-inotify`. When enabled (default), PLDM will
automatically monitor the `/tmp/images` directory for new firmware packages and
process them automatically. When disabled, only D-Bus API-based firmware updates
will be supported.
