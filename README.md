...
2. **Inotify monitoring**: Automatic detection of firmware packages placed in `/tmp/images`

The inotify-based firmware update monitoring can be enabled or disabled using the
meson option `fw-update-inotify`. When enabled (default), PLDM will automatically
...
To disable inotify-based firmware update monitoring:
```bash
meson setup build -Dfw-update-inotify=disabled
```

To enable inotify-based firmware update monitoring (default):
```bash
meson setup build -Dfw-update-inotify=enabled
```
