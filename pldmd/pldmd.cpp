...
{
  public:
    WorkerPool(size_t n, Invoker& invoker,
               requester::Handler<requester::Request>& handler,
               fw_update::Manager* fwManager, PldmTransport& transport,
               bool verbose, std::mutex& transportMtx,
               std::queue<WorkItem>& readyQ, std::mutex& readyMtx,
               int resultEfd) :
        invoker(invoker), handler(handler), fwManager(fwManager),
        transport(transport), verbose(verbose), transportMtx(transportMtx),
...
int main(int argc, char** argv)
{
    ...
    WorkerPool workerPool(workerCount, invoker, reqHandler, fwManager.get(),
                          pldmTransport, verbose, transportMtx, readyQ,
                          readyMtx, resultEfd);
