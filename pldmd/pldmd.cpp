...
static Response processRxMsg(const std::vector<uint8_t> &requestMsg,
  ...
  if (PLDM_SUCCESS != unpack_pldm_header(hdr, &hdrFields)) {
    ...
    size_t requestLen = requestMsg.size() - sizeof(struct pldm_msg_hdr) -
                        sizeof(eid) - sizeof(type);
    try {
      response = invoker.handle(hdrFields.pldm_type, hdrFields.command, request,
                                requestLen);
...
int main(int argc, char **argv) {
  ...
  bool verbose = false;
  static struct option long_options[] = {{"verbose", required_argument, 0, 'v'},
                                         {0, 0, 0, 0}};
  ...
  std::unique_ptr<pldm::host_effecters::HostEffecterParser> hostEffecterParser;
  std::unique_ptr<DbusToPLDMEvent> dbusToPLDMEventHandler;
  auto dbusHandler = std::make_unique<DBusHandler>();
  auto hostEID = pldm::utils::readHostEID();
  if (hostEID) {
    hostPDRHandler = std::make_unique<HostPDRHandler>(
        sockfd, hostEID, event, pdrRepo.get(), entityTree.get(), dbusImplReq);
    hostEffecterParser =
  ...
  // clearDirPath(LID_STAGING_DIR);

#ifdef OEM_IBM
  std::unique_ptr<pldm::responder::CodeUpdate> codeUpdate =
      std::make_unique<pldm::responder::CodeUpdate>(dbusHandler.get());
  oemPlatformHandler = std::make_unique<oem_ibm_platform::Handler>(
      dbusHandler.get(), codeUpdate.get(), sockfd, hostEID, dbusImplReq, event);
  ...
  auto fruHandler = std::make_unique<fru::Handler>(FRU_JSONS_DIR, pdrRepo.get(),
                                                   entityTree.get());
    ...
    if (0 == peekedLength) {
      ...
      if (recvDataLength == peekedLength) {
        ...
        if (MCTP_MSG_TYPE_PLDM != requestMsg[1]) {
          ...
          auto response = processRxMsg(requestMsg, invoker, dbusImplReq);
          if (!response.empty()) {
            if (verbose) {
            ...
            if (-1 == result) {
              returnCode = -errno;
              std::cerr << "sendto system call failed, RC= " << returnCode
                        << "\n";
            }
          }
        }
      } else {
        std::cerr << "Failure to read peeked length packet. peekedLength= "
