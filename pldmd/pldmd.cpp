...
int main(int argc, char** argv)
{
    ...
    pldm_transport* pldmTransport = transport_impl_init(transportImpl, pollfd);
    ...
    auto callback = [verbose, &invoker, &reqHandler, &fwManager, pldmTransport,
        ...
        returnCode = pldm_transport_recv_msg(
            pldmTransport, &TID, reinterpret_cast<void**>(&requestMsg),
            &recvDataLength);

        if (returnCode == PLDM_REQUESTER_SUCCESS)
        {
            std::vector<uint8_t> requestMsgVec(requestMsg,
                                               requestMsg + recvDataLength);
            ...
            if (response.has_value())
            {
                ...
                returnCode = pldm_transport_send_msg(
                    pldmTransport, TID, (*response).data(), (*response).size());
                if (returnCode != PLDM_REQUESTER_SUCCESS)
                {
                    std::cerr
                        << "## pldm transport send failed, RC= " << returnCode
                        << "\n";
                }
            }
        }
        ...
        {
            std::cerr << "Failure to read packet. rc=" << returnCode
