...
class PLDMFRUParser : public FruTableIntf
{
  ...
  public:
    ...
    {
        std::cerr << "objectPath" << objectPath << std::endl;
    ...
    /** @brief Method to get a vector of objects created by PLDMFRUParser.
     *  @return vector of unique pointers to objects of PLDMFRUParser
     **/
    std::vector<std::unique_ptr<PLDMFRUParser<T>>>& getvecObjs();
