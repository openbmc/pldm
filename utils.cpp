                                  ...
                                  const PropertyValue& value) const
{
    auto setDbusValue = [&dBusMap, this](const auto& variant) {
        setDbusProperty(dBusMap.objectPath.c_str(),
                        dBusMap.propertyName.c_str(), dBusMap.interface.c_str(),
                        variant);
    };
...
std::any DBusHandler::getPropertyImpl(const char* objPath, const char* dbusProp,
...
{
    auto result = getPropertyImpl(objPath, dbusProp, dbusInterface);
    return std::any_cast<PropertyValue>(result);
}
