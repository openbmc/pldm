    ...
    handlers(list)
{
    ...
    for (const auto& mapIt : currentMctpInfoMap)
    {
        if (mapIt.second)
        {
            ...
            {
                if (auto matchedTid =
                        eidUuidPairExists(candidateEid, candidateUuid))
                {
                    info(
                        ...
                        "NETWORK", std::get<NetworkId>(mapIt.first), "TID",
                        matchedTid.value());
                    continue;
                }
            }
            ...
            for (auto* handler : handlers)
            {
                if (auto tid = handler->allocateOrGetTid(mapIt.first))
                {
                    ...
                    {
                        transport->mapTid(tid.value(), network, eid);
                    }
                    ...
                    types.end())
                {
                    ...
                    if (uuid != emptyUUID)
                    {
                        if (auto matchedTid = eidUuidPairExists(eid, uuid))
                        {
                            info(
                                "Skipping EID '{EID}' UUID '{UUID}' on NETWORK '{NETWORK}': already registered for terminus TID '{TID}'",
                                "EID", unsigned(eid), "UUID", uuid, "NETWORK",
                    ...
                    if (epUuid != emptyUUID)
                    {
                        if (auto matchedTid = eidUuidPairExists(epEid, epUuid))
                        {
                            info(
                                ...
                                "NETWORK", std::get<NetworkId>(mctpInfo), "TID",
                                matchedTid.value());
                            return;
                        }
                    }
