...
constexpr const char* MCTP_INTERFACE = "xyz.openbmc_project.MCTP.Endpoint";
constexpr const char* MCTP_OBJECT_PATH = "/xyz/openbmc_project/mctp";
...
MctpDiscovery::MctpDiscovery(
    ...
    mctpEndpointAddedSignal(
        bus, interfacesAdded(MCTP_OBJECT_PATH),
        std::bind_front(&MctpDiscovery::discoverEndpoints, this)),
    mctpEndpointRemovedSignal(
        bus, interfacesRemoved(MCTP_OBJECT_PATH),
...
void MctpDiscovery::getMctpInfos(MctpInfos& mctpInfos)
{
    ...
    catch (const sdbusplus::exception_t& e)
    {
        error(" getSubtree call failed with, {ERROR} {PATH} {INTERFACE}",
              "ERROR", e, "PATH", MCTP_OBJECT_PATH, "INTERFACE",
              MCTP_INTERFACE);
        return;
    }
