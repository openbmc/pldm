    ...
    handlers(list)
{
    ...
    for (const auto& mapIt : currentMctpInfoMap)
    {
        if (mapIt.second)
        {
            if (isDuplicateEidUuid(std::get<pldm::eid>(mapIt.first),
            ...
            for (auto* handler : handlers)
            {
                if (auto tid = handler->allocateOrGetTid(mapIt.first))
                {
                    ...
                    {
                        if (transport->mapTid(tid.value(), network, eid) != 0)
                        {
                            error(
            ...
            if (singleInfo.empty())
            {
                error(
                    ...
                    "EID", std::get<pldm::eid>(mapIt.first), "NETWORK",
                    std::get<NetworkId>(mapIt.first));
            }
        ...
        if (intfName == MCTPEndpoint::interface)
        {
                    ...
                    MCTPEndpoint::property_names::supported_message_types))
            {
                    ...
                    types.end())
                {
                    ...
                    if (!isDuplicateEidUuid(eid, uuid, networkId))
                    {
...
bool MctpDiscovery::isDuplicateEidUuid(pldm::eid endpointEid, const UUID& uuid,
                                       NetworkId networkId) const
{
    ...
    if (auto matchedTid = eidUuidPairExists(endpointEid, uuid))
    {
        info(
                                                ...
                                                TerminusInfos& removedInfos)
{
    ...
    for (const auto& [tid, mctpInfo] : removedInfos)
    {
        ...
        // Unmap from transport (all handlers share the same PldmTransport
        // instance; unmap once to avoid double-unmapping)
        for (auto* handler : handlers)
        {
            if (auto* transport = handler->getTransport())
            {
                if (transport->unmapTid(tid) != 0)
                {
                    lg2::error(
                        "Failed to unmap TID {TID} EID {EID} networkId {NETWORK} from transport.",
                        "TID", tid, "EID", std::get<0>(mctpInfo), "NETWORK",
                        std::get<3>(mctpInfo));
                }
                break;
            }
    ...
    for (const auto& [key, valueVariant] : properties)
    {
        ...
        if (key == MCTPConnectivityProp)
        {
            ...
            if (!found)
            {
                if (availability)
                {
                    if (isDuplicateEidUuid(std::get<pldm::eid>(mctpInfo),
                                           std::get<1>(mctpInfo),
                    ...
                    for (auto* handler : handlers)
                    {
                        if (auto tid = handler->allocateOrGetTid(mctpInfo))
                        {
                            ...
                            {
                                if (transport->mapTid(
                                        tid.value(), std::get<3>(mctpInfo),
                                        std::get<0>(mctpInfo)) != 0)
                                {
                                    error(
                                        "Failed to map TID '{TID}' for EID '{EID}' network '{NETWORK}' in transport",
                                        "TID", tid.value(), "EID",
                                        std::get<pldm::eid>(mctpInfo),
                                        "NETWORK",
                                        std::get<NetworkId>(mctpInfo));
                                    break;
                                }
                    ...
                    if (singleInfo.empty())
                    {
                        error(
                            "Failed to allocate TID for EID '{EID}' network '{NETWORK}', skipping endpoint",
                            "EID", std::get<pldm::eid>(mctpInfo), "NETWORK",
                            std::get<NetworkId>(mctpInfo));
                    }
                    addToExistingMctpInfos(singleInfo);
