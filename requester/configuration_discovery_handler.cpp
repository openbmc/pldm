...
void ConfigurationDiscoveryHandler::searchConfigurationFor(MctpInfo mctpInfo)
{
    ...
    try
    {
        ...
        configurations.insert_or_assign(associatedObjPath,
                                        MCTPEndpoint{eidValue, address, bus});
        debug("Added MCTP endpoint {EID} with address {ADDRESS} and bus {BUS}",
              "EID", eidValue, "ADDRESS", address, "BUS", bus);
    }
    catch (const std::exception& e)
    {
