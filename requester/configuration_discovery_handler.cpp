    ...
    if (!inventoryPath.empty())
    {
        try
        {
            auto properties = dBusIntf->getDbusPropertiesVariant(
                entityManagerService, inventoryPath.c_str(),
                PLDMDeviceInterface);
            auto addressArray =
                std::visit(utils::VariantToIntegralArrayVisitor<uint8_t>(),
            ...
            auto bus = std::visit(utils::VariantToIntegralVisitor<uint8_t>(),
                                  properties.at("Bus"));
            auto iana = std::visit(utils::VariantToStringVisitor(),
                                   properties.at("IANA"));
            auto name = std::visit(utils::VariantToStringVisitor(),
                                   properties.at("Name"));
