...
void ConfigurationDiscoveryHandler::searchConfigurationFor(MctpInfo mctpInfo)
{
    ...
    try
    {
        auto constexpr subTreeDepth =
            3; //"/{board or chassis type}/{board or chassis}/{device}"
    ...
    try
    {
        ...
        auto mctpEndpoint =
            MCTPEndpoint::create(eidValue, mctpTargetProperties);
        if (!mctpEndpoint)
        {
            error(
                "Failed to create MCTPEndpoint for endpoint ID {EID}: {ERROR}",
                "EID", eidValue, "ERROR", mctpEndpoint.error());
            return;
        }
        debug("Endpoint {EID} identified as device '{NAME}'", "EID", eidValue,
              "NAME", mctpEndpoint->name());
...
void ConfigurationDiscoveryHandler::removeConfigByEid(uint8_t eid)
{
    std::erase_if(configurations, [eid](const auto& config) {
        ...
        auto& [eidValue, ___] = mctpEndpoint;
        return eidValue == eid;
    });
}
