...
struct MCTPEndpoint : public std::pair<eid, std::string>
{
    using std::pair<eid, std::string>::pair;
    static std::expected<MCTPEndpoint, std::string> create(
        eid eidValue, const utils::PropertyMap& properties)
    {
        ...
        return MCTPEndpoint(eidValue, name);
    }
