...
TEST_F(HandlerTest, singleRequestResponseScenarioUsingCoroutine)
{
    ...
    ctx.spawn([&]() -> sdbusplus::async::task<void> {
        int rc;
        pldm::Request request(sizeof(pldm_msg_hdr) + sizeof(uint8_t));
    ...
    ctx.spawn(stdexec::just() | stdexec::then([&]() {
        pldm::Response mockResponse(sizeof(pldm_msg_hdr) + sizeof(uint8_t));
...
TEST_F(HandlerTest, asyncRequestResponseByCoroutine)
{
    ...
    ctx.spawn(stdexec::just() | stdexec::then([&]() {
        pldm::Response mockResponse(sizeof(pldm_msg_hdr) +
