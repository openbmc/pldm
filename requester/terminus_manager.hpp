...
/** @class Manager
 *
 * This class manages the MCTP terminus
 */
class Manager
{
  ...
  public:
    ...
    int addDevices(const std::vector<mctp_eid_t>& eids)
    {
        for (const auto& it : eids)
        {
            ...
            auto dev =
                new TerminusHandler(mctp_fd, it, event, bus, repo, entityTree,
                                    bmcEntityTree, requester, handler);
            mDevices.insert(std::pair<mctp_eid_t, TerminusHandler*>(it, dev));
        }
        return PLDM_SUCCESS;
    }
  ...
  private:
    ...
    std::map<mctp_eid_t, TerminusHandler*> mDevices;
};
