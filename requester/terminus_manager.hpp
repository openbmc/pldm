...
class Manager
{
  ...
  public:
    ...
    /** @brief Constructor
     *
     *  @param[in] handler - PLDM request manager
     */
    explicit Manager(
    ...
    /** @brief Add the discovered PLDM endpoints to the managed devices list
     *
     *  @param[in] eids - Array of PLDM endpoints
     *
     *  @return return PLDM_SUCCESS
     */
    int addDevices(const std::vector<uint8_t>& eids)
    {
        for (const auto& it : eids)
        {
            std::cerr << "Adding Device EID : " << unsigned(it) << std::endl;
            auto dev = new TerminusHandler(it, event, bus, repo, entityTree,
                                           bmcEntityTree, requester, handler);
            mDevices.insert(std::pair<uint8_t, TerminusHandler*>(it, dev));
        }
        return PLDM_SUCCESS;
    }

    /** @brief Remove the PLDM endpoints from the managed devices list
     *
     *  @param[in] eids - Array of PLDM endpoints
     *
     *  @return return PLDM_SUCCESS
     */
    int removeDevices(const std::vector<uint8_t>& eids)
    {
        for (const auto& it : eids)
        {
  ...
  private:
    ...
    /** @brief List terminus which are managerd by terminus manager */
    std::map<uint8_t, TerminusHandler*> mDevices;
};
