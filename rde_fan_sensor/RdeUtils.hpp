...
const constexpr char *rdeDeviceInterface =
 ...
 * @param[in] rdeServiceName Name of the service that handles D-bus requests for Rde device
 ...
 *
 *  Container that stores data and method needed to get the Rde device object paths
 */
struct GetRdeDevices : std::enable_shared_from_this<GetRdeDevices>
{
    ...
    GetRdeDevices(
        ...
        std::function<void(ObjPathToServiceMapType &serviceMap)> &&callbackFunc) : dbusConnection(std::move(connection)),
    ...
    void getRdeDevices(size_t retries = 0)
    {
        ...
        dbusConnection->async_method_call(
                            ...
                            const GetSubTreeType &subtree)
            {
                if (ec)
                {
                    std::cerr << "Error in the mapper call. ec : "
                              << ec << std::endl;
                    if (retries == 0U)
                    {
                        return;
                ...
                for (const auto &[path, objDict] : subtree)
                {
                    ...
                    std::cerr << " Got path : " << path << " with owner " << owner << std::endl;
                    self->rdeDevObjPathsToServiceMap[path] = objDict.begin()->first;
                }
            },
