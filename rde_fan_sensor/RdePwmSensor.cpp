...
PwmSensor::PwmSensor(const std::string& name,
                     ...
                     bool isValueMutable) :
    io(io), conn(conn), dbusConn(dbusConn), objectServer(objectServer), timer(io, boost::asio::chrono::seconds(fanPWMTimeInterval)),
    name(name), rdeSensorUri(rdeSensorUri), rdeDevObjPath(rdeDevObjPath), rdeDevService(rdeDevService),
    rdeDeviceId(rdeDeviceId), pwmValue(0)
{
    ...
    sensorInterface->register_property(
        ...
        if (currInt != pwmVal)
        {
            std::cerr << "Inside RDE Sensor Get Callback. curVal : " << curVal
                << " pwmValue : " << pwmVal << std::endl;
            double getScaled = 100.0 * (static_cast<double>(pwmVal) / pwmMax);
            curVal = getScaled;
            controlInterface->signal_property("Target");
    ...
    controlInterface->register_property(
        ...
        std::cerr << "inside Control set " << req
         << " Resp Value : " << resp << " pwmValue : " << this->pwmValue << std::endl;
        ...
        resp = req;
        double pwmDutyCycle = (static_cast<double>(pwmVal)/pwmMax) * 100.0;
        std::string pwmPatchStr = "{\"Reading\":" + std::to_string(pwmDutyCycle) + "}";
        std::cerr << "Sending fanPWMDuty : " << pwmDutyCycle << " for sensor : " << this->rdeSensorUri
            << " on device : " << this->rdeDeviceId << std::endl;
        this->io.post([&,pwmPatchStr{pwmPatchStr}](){
            std::string resp =
                sendRdeDbusRequest(this->dbusConn, this->rdeSensorUri, this->rdeDeviceId, this->rdeDevService,
                    this->rdeDevObjPath, PLDM_RDE_OPERATION_UPDATE, pwmPatchStr);
    ...
    /*
       ...
       is different from the previous one to avoid unnecessary dbus calls and so accordingly
       the rde_fan_sensor daemon sends it to the RDE device. However if
       the RDE device does not recieve a value, it may think that BMC<->RDE communication
       is broken. So setup a  timer to send fanPWM value periodically.
       The timer feature can be removed once, b/277832479 is addressed in phosphor-pid-control
    */
    ...
    sendFanPWMFunc =
        ...
        double pwmDutyCycle = (static_cast<double>(this->pwmValue)/pwmMax) * 100.0;
        std::string pwmPatchStr = "{\"Reading\":" + std::to_string(pwmDutyCycle) + "}";
        std::cerr <<  "Sending fanPWMDuty after 120 seconds in a timed event : " << pwmDutyCycle
        << " for sensor : " << this->rdeSensorUri << " on device : " << this->rdeDeviceId << std::endl;
        std::string resp =
            sendRdeDbusRequest(this->dbusConn, this->rdeSensorUri,
                               this->rdeDeviceId, this->rdeDevService,
                               this->rdeDevObjPath, PLDM_RDE_OPERATION_UPDATE, pwmPatchStr);
        if (isResponseError(resp))
        {
            std::cerr << "Received dbus error response while patch pwm"
        ...
        this->timer.expires_after(boost::asio::chrono::seconds(fanPWMTimeInterval));
    ...
    // TODO Nikhil : Check if we need to associate rde pwm/tach sensors with chassis ?
}
PwmSensor::~PwmSensor()
{
