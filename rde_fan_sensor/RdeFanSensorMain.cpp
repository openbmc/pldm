    ...
    boost::container::flat_map<std::string, std::string>& rdeIdToTrayIdMap)
{
    ...
    if (rdeIdToTrayIdMap.find(rdeDeviceId) == rdeIdToTrayIdMap.end())
    {
        ...
        std::cerr << "Adding entry in trayCountMap " << trayType << ":" << count
                  << std::endl;
                      ...
                      const std::string& rdeDevObjPath)
{
    ...
    for (int i = 0; i < sensorsCount; i++)
    {
        ...
        if (regex_match(sensorId, std::regex(R"(host_fan(\d+)_pwm)")))
        {
            ...
            std::cerr << "findRdePwmSensors) Found : "
                      << sensorId + "_" + trayId << std::endl;
        }
    }
        ...
        [&trayCountMap, &rdeIdToTrayIdMap, &io, &objectServer, &pwmSensors,
         ...
         rdeDevicesChanged](const std::map<std::string, std::string>
                 ...
                 it1 != rdeDevObjPathsToServiceMap.end(); ++it1)
            {
                ...
                std::string trayResp = sendRdeDbusRequest(
                    dbusConnection, "/redfish/v1/Chassis/Tray", rdeDeviceId,
                ...
                std::string trayId =
                    getTrayId(trayRespJson["Model"], trayCountMap, rdeDeviceId,
                              rdeIdToTrayIdMap);

                // Get Sensor Collection
                std::string sensorCollectionResp = sendRdeDbusRequest(
                    dbusConnection, "/redfish/v1/Chassis/Tray/Sensors",
                ...
                std::map<std::string, std::string> rdePwmSensors =
                    findRdePwmSensors(dbusConnection, sensorCollectionRespJson,
                                      ...
                                      rdeDevObjPath);
                for (auto it2 = rdePwmSensors.begin();
                     it2 != rdePwmSensors.end(); ++it2)
                {
                    std::string fanPwmName = it2->first;
                    std::cerr << "Update sensor signal request received for : "
                              << fanPwmName << std::endl;

                    if ((pwmSensors.count(fanPwmName) == 0))
                    {
                        std::cerr
                            << "Rde sensor found in config : " << fanPwmName
                            << std::endl;
                        pwmSensors[fanPwmName] = std::make_unique<PwmSensor>(
                            fanPwmName, rdePwmSensors[fanPwmName],
                            rdeDevObjPath, rdeDevService, rdeDeviceId, io,
                            asioConnection, dbusConnection, objectServer, true);
...
int main()
{
    ...
    std::function<void(sdbusplus::message_t&)> rdeDeviceDetectionHandler =
        [&](sdbusplus::message_t& message) {
            ...
            std::cerr << "Inserting " << message.get_path()
                      << " in rdeDevicesChanged" << std::endl;
            ...
            filterTimer.async_wait([&](const boost::system::error_code& ec) {
                ...
                createSensors(trayCountMap, rdeIdToTrayIdMap, io, objectServer,
                              pwmSensors, tachSensors, systemBus,
                              dbusConnection, rdeDevicesChanged, 5);
            });
        };
    ...
    std::function<void(sdbusplus::message_t&)> rdeDeviceRemovalHandler =
        [&](sdbusplus::message_t& message) {
            ...
            std::cerr << "Removing " << message.get_path() << " from pwmSensors"
                      << std::endl;
            ...
            auto intfIt =
                std::find(interfacesRemoved.begin(), interfacesRemoved.end(),
                          rdeDeviceInterface);
            if (intfIt != interfacesRemoved.end())
            {
                std::cerr << "Need to remove " << removedObjectPath
                          << " since RDE device got removed" << std::endl;
                ...
                {
                    std::string& pwmObjPath =
                        it1->second->getRdeDeviceObjPath();
                ...
                {
                    std::string& tachObjPath =
                        it2->second->getRdeDeviceObjPath();
