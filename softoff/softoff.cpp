...
Json SoftPowerOff::parseConfig()
{
    ...
    {
        error("Failed to parse softoff config JSON file, file does not exist");
        return PLDM_ERROR;
    }
                                ...
                                pldm::pdr::StateSetId& stateSetId)
{
    ...
    {
        error("Failed to get state sensor PDR, error - {ERROR}", "ERROR", e);
        return PLDM_ERROR;
    }
...
int SoftPowerOff::hostSoftOff(sdeventplus::Event& event)
{
    ...
    auto callback = [=, &pldmTransport, this](IO& io, int fd,
        ...
        if (rc)
        {
            error("Soft off failed to receive pldm data, response code '{RC}'",
        ...
        {
            error(
                "Failure to start remote terminus soft off wait timer, response code '{RET}'. Exit the pldm-softpoweroff",
                "RET", ret);
            exit(-1);
        }
        else
    ...
    while (!isCompleted() && !isTimerExpired())
    {
        ...
        catch (const sdeventplus::SdEventError& e)
        {
            instanceIdDb.free(pldmTID, instanceID);
            error(
                "PLDM remote terminus soft off failed to process request, error - {ERROR}",
                "ERROR", e);
            return PLDM_ERROR;
        }
    }
