    ...
    timer(event), instanceIdDb(instanceIdDb)
{
    ...
    for (const auto& entry : entries)
    {
        ...
        if (effecterFound)
        {
            ...
            {
                error(
                    "Message get Sensor PDRs failed with response code '{RC}'",
...
int SoftPowerOff::getHostState()
{
    ...
    {
        error(
            "PLDM remote terminus soft off. Can't get current remote pldm terminus state, error - {ERROR}",
            "ERROR", e);
        hasError = true;
        return PLDM_ERROR;
    }
...
void SoftPowerOff::hostSoftOffComplete(sdbusplus::message_t& msg)
{
        ...
        msgEventState == PLDM_SW_TERM_GRACEFUL_SHUTDOWN && msgTID == TID)
    {
        ...
        {
            error(
                "PLDM soft off failure to STOP the timer with response code '{RC}'",
                "RC", rc);
        }
                                 ...
                                 pldm::pdr::StateSetId& stateSetId)
{
    ...
    {
        error("PLDM soft off failed to get softPowerOff PDR, error - {ERROR}",
              "ERROR", e);
        return false;
    }
    return true;
                                ...
                                pldm::pdr::StateSetId& stateSetId)
{
    ...
    {
        error("PLDM soft off failed to get State Sensor PDR, error - {ERROR}",
              "ERROR", e);
        return PLDM_ERROR;
    }
...
int SoftPowerOff::hostSoftOff(sdeventplus::Event& event)
{
    ...
    auto callback = [=, &pldmTransport, this](IO& io, int fd,
        ...
        if (rc)
        {
            error("Soft off failed to recv pldm data, response code '{RC}'",
        ...
        {
            error(
                "Failure to start remote pldm terminus soft off wait timer, response code '{RET}'. Exit the pldm-softpoweroff",
                "RET", ret);
            exit(-1);
        }
        else
        {
            error(
                "Timer started waiting for remote pldm terminus soft off, timeout in sec '{TIMEOUT_SEC}'",
    ...
    while (!isCompleted() && !isTimerExpired())
    {
        ...
        catch (const sdeventplus::SdEventError& e)
        {
            instanceIdDb.free(pldmTID, instanceID);
            error(
                "PLDM remote pldm terminus soft off failed in processing request, error - {ERROR}",
                "ERROR", e);
            return PLDM_ERROR;
        }
    }
