    ...
    timer(event), instanceIdDb(instanceIdDb)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        error("Message get Sensor PDRs error, response code '{RC}'", "RC",
...
void SoftPowerOff::hostSoftOffComplete(sdbusplus::message_t& msg)
{
        ...
        msgEventState == PLDM_SW_TERM_GRACEFUL_SHUTDOWN)
    {
        ...
        {
            error(
                "PLDM soft off. Failure to STOP the timer,  response code '{RC}'",
                "RC", rc);
        }
...
int SoftPowerOff::getEffecterID()
{
    ...
    {
        error("PLDM soft off: Failed to get VMM PDR,error - {ERROR}", "ERROR",
              e);
        VMMPdrExist = false;
    }
    ...
    {
        error(
            "PLDM soft off: Failed to get system firmware PDR, error - {ERROR}",
            "ERROR", e);
        completed = true;
        return PLDM_ERROR;
    }
...
int SoftPowerOff::hostSoftOff(sdeventplus::Event& event)
{
    ...
    while (!isCompleted() && !isTimerExpired())
    {
        ...
        catch (const sdeventplus::SdEventError& e)
        {
            instanceIdDb.free(pldmTID, instanceID);
            error(
                "PLDM host soft off: Failure in processing request, error - {ERROR}",
                "ERROR", e);
            return PLDM_ERROR;
        }
    }
