...
PldmSoftPowerOff::PldmSoftPowerOff()
{
    ...
    try
    {
        pldm::utils::PropertyValue propertyValue = pldm::utils::DBusHandler().getDbusPropertyVariant(
            "/xyz/openbmc_project/state/host0", "CurrentHostState",
            "xyz.openbmc_project.State.Host");
                                  
        if(std::get<std::string>(propertyValue) != "xyz.openbmc_project.State.Host.HostState.Running")
        {
...
int PldmSoftPowerOff::getEffecterID()
{
    ...
    try
    {
        ...
        auto PHYP_method = bus.new_method_call("xyz.openbmc_project.PLDM.PDR", "/xyz/openbmc_project/pldm",
                                    "xyz.openbmc_project.PLDM.PDR", "FindStateEffecterPDR");                     
        PHYP_method.append("1", PHYP_EntityType, PHYP_State_Set_ID);
    ...
    if (PHYP_PDR_NOT_EXIST)
    {
        ...
        {                           
            ...
            auto HostBoot_method = bus.new_method_call("xyz.openbmc_project.PLDM.PDR", "/xyz/openbmc_project/pldm",
                                        "xyz.openbmc_project.PLDM.PDR", "FindStateEffecterPDR");                     
            HostBoot_method.append("1", HostBoot_EntityType, HostBoot_State_Set_ID);
            ...
            pldm_state_effecter_pdr* HostBoot_pdr =
                    reinterpret_cast<pldm_state_effecter_pdr*>(HostBoot_Response.data());
            effecterID = HostBoot_pdr->effecter_id;
           
        }
        catch (const SdBusError& e)
        {
            std::cerr << "PLDM soft off: Error get HostBoot PDR,ERROR=" << e.what()
                    << "\n";
            return PLDM_ERROR;
        }
    }
...
pldm_requester_rc_t PldmSoftPowerOff::soft_off_pldm_send_recv(mctp_eid_t eid, int mctp_fd,
