...
The effecter is used by OpenBMC to request a graceful shutdown through the
PLDM SetStateEffecterStates command.
...
The sensor is used by the remote endpoint to report software termination
status through PLDM state sensor events.
...
4. The application sends a PLDM SetStateEffecterStates request to the
   remote endpoint.
5. The remote endpoint receives the request and initiates a graceful shutdown.
6. During shutdown, the remote endpoint updates the software termination
   status sensor and generates PLDM state sensor events.
