...
1. **Configuration file not found**: Verify `SOFTOFF_CONFIG_JSON` path and file
   existence
2. **PDR not found**: Ensure host has published appropriate effecter and sensor
   PDRs
3. **Timeout errors**: Check host responsiveness and PLDM communication
4. **D-Bus errors**: Verify pldmd service is running and D-Bus permissions are
   correct
