        ...
        switch (attrType)
        {
            ...
            case PLDM_BIOS_STRING:
            {
                ...
                uint16_t Minlen =
                    pldm_bios_table_attr_entry_string_decode_min_length(
                        attrEntry);
                if (attrValue.size() < Minlen || attrValue.size() > Maxlen)
                {
                    std::cout << "Set Attribute Error: Length is wrong "
                              << std::endl;
                    return;
                }
                entryLength =
            ...
            case PLDM_BIOS_INTEGER:
            {
                int64_t value = std::stoll(attrValue);
                uint64_t lower, upper, def;
                uint32_t scalar;
                pldm_bios_table_attr_entry_integer_decode(
                ...
                {
                    std::cout << "Set Attribute Error: It's out of bound"
                              << std::endl;
                    return;
                }
                entryLength =
