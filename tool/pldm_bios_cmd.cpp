...
class GetBIOSTable: public CommandInterface
{
  public:
    ...
    void parseResponseMsg(pldm_msg* responsePtr, size_t payloadLength) override
    {
        ...
        auto tableDataPos = reinterpret_cast<char*>(responsePtr) + bios_table_offset;

        auto iter = pldm_bios_table_iter_create(tableDataPos, tableSize , pldmBIOSTableType);
        std::cout << "first iter: " << iter << std::endl;

        auto entry = pldm_bios_table_iter_string_entry_value(iter);

        auto strLength = pldm_bios_table_string_entry_decode_string_length(entry);  // string length
        std::vector<char> buffer(strLength + 1, 0);
        std::cout << " first string length: " << strLength << std::endl;

        auto handle = pldm_bios_table_string_entry_decode_handle(entry); //string handle
        std::cout << "first string handele : " << handle << std::endl;


        auto decodedString = pldm_bios_table_string_entry_decode_string(
        entry, buffer.data(), buffer.size());  //string itself
        std::cout << "first Decode string : " << decodedString << std::endl;
// second time

        pldm_bios_table_iter_next(iter);
        entry = pldm_bios_table_iter_string_entry_value(iter);

        strLength = pldm_bios_table_string_entry_decode_string_length(entry);  // string length
        std::cout << " string length: " << strLength << std::endl;
        handle = pldm_bios_table_string_entry_decode_handle(entry); //string handle
        std::cout << " string handele : " << handle << std::endl;


        decodedString = pldm_bios_table_string_entry_decode_string(
        entry, buffer.data(), buffer.size());  //string itself
        std::cout << "Decode string : " << decodedString << std::endl;
...
// third time
        ...
        auto endOfTable = pldm_bios_table_iter_is_end(iter);
        std::cout << "endOfTable ::: " << endOfTable << std::endl;
