    ...
    void parseResponseMsg(pldm_msg* responsePtr, size_t payloadLength) override
    {
        ...
        while (!pldm_bios_table_iter_is_end(iter.get()))
        {
            ...
            try
            {
                ...
                auto stringHandle = pldm_bios_table_string_entry_decode_handle(table_entry);
                std::cout << "stringHandle : " << stringHandle << std::endl;
                auto decodeString = pldm_bios_table_string_entry_decode_string(
                                     table_entry, strTableData.data(), strTableData.size());
                std::cout << "decodeString : " << decodeString << std::endl;
            }
        ...
        std::cout << "strTableData: " << strTableData << std::endl;
    }
    private:
