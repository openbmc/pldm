...
class GetPLDMCommands : public CommandInterface
{
  public:
    ...
    {
        uint8_t cc = 0;
        std::vector<bitfield8_t> outTypes(256);
        ...
        printPldmCommands(outTypes, pldmType);
    }
...

    {
        ...
        for (int i = 0; i < PLDM_MAX_CMDS_PER_TYPE; i++)
        {
            ...
            if (b.byte & (1 << i % 8))
            {
                ...
                {
                    auto it =
                        std::find_if(pldmBaseCmds.begin(), pldmBaseCmds.end(),
                                     [i](const auto& typePair) {
                                         return typePair.second == i;
                                     });
                    if (it != pldmBaseCmds.end())
                    {
                ...
                {
                    auto it =
                        std::find_if(pldmBiosCmds.begin(), pldmBiosCmds.end(),
                                     [i](const auto& typePair) {
                                         return typePair.second == i;
                                     });
                    if (it != pldmBiosCmds.end())
                    {
                ...
                {
                    auto it = std::find_if(pldmPlatformCmds.begin(),
                                           pldmPlatformCmds.end(),
                                           [i](const auto& typePair) {
                                               return typePair.second == i;
                                           });
                    if (it != pldmPlatformCmds.end())
                    {
                ...
                {
                    auto it =
                        std::find_if(pldmFruCmds.begin(), pldmFruCmds.end(),
                                     [i](const auto& typePair) {
                                         return typePair.second == i;
                                     });
                    if (it != pldmFruCmds.end())
                    {
