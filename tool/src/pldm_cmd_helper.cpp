...
int mctp_sock_init()
{
    ...
    {
        returnCode = -errno;
        LOG_E("Failed to connect to the socket : RC = [%d]",
                      returnCode);
        close(socketFd);
        return returnCode;
    }
    LOG_D("Success in connecting to the socket : RC = [%d]",
                  returnCode);
                  ...
                  std::vector<uint8_t> &responseMsg)
{
    ...
    if (0 == peekedLength)
    {
        LOG_E("Socket has been closed : peekedLength = [%d]",
                      peekedLength);
        close(socketFd);
        return returnCode;
    }
    ...
    else
    {
        ...
        if (recvDataLength == peekedLength)
        {
            LOG_I("Total length [%zu]\n"
                  "Loopback response message:",recvDataLength);
        ...
        returnCode = std::memcmp(requestMsg.data(), lbrespMsg.data(),
                                 requestMsg.size());
