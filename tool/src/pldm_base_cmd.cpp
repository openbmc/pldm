...
void GetPLDMTypes()
{
    ...
    std::vector<uint8_t> requestMsg(sizeof(pldm_msg_hdr) + PLDM_GET_COMMANDS_REQ_BYTES);
    ...
    std::array<uint8_t,64> lbBuf{};
    ssize_t peekedLength = recv(socketFd,
                           reinterpret_cast <void *>(lbBuf.data()),
                           lbBuf.size(), MSG_PEEK);
    if(0 == peekedLength)
    {
    ...
    else
    {
        ...
        auto recvDataLength = recv(socketFd,
                                   reinterpret_cast <void *>(lbrespMsg.data()),
                                   peekedLength, 0);
        ...
        auto result = std::memcmp(requestMsg.data(), lbrespMsg.data(),
                                  requestMsg.size());
        ...
        if ((!result) && (respEid == PLDM_ENTITY_ID &&
                          respType == MCTP_MSG_TYPE_PLDM))
        {

            Logger::Debug("requestMsg = lbrespMsg");
            std::array<uint8_t,64> recvBuf{};
            ssize_t peekedLength = recv(socketFd,
                                   reinterpret_cast <void *>(recvBuf.data()),
                                   recvBuf.size(), MSG_PEEK);
            std::vector<uint8_t> responseMsg(peekedLength);
            recvDataLength = recv(socketFd,
                                  reinterpret_cast <void *>(responseMsg.data()),
                                  peekedLength, 0);
            if (recvDataLength == peekedLength)
            {
                // print the recieve buffer
