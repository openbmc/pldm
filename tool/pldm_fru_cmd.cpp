...
class GetFruRecordTable : public CommandInterface
{
  public:
    ...
    void exec()
    {
        ...
        auto lc =
            encode_get_fru_record_table_metadata_req(instanceId, requestFru);
        if (lc != PLDM_SUCCESS)
        {
  ...
  private:
                       ...
                       int total_record_set_identifiers)
    {
        ...
        {
            std::cout << "FRU Record Set Identifier : " << j << std::endl;
            auto frutable =
                reinterpret_cast<pldm_fru_record_tlv*>(&fru->tlvs[0]);
            for (size_t i = 0; i < fru->num_fru_fields; ++i)
            {
                if (frutable->type < 16)
                {
                    std::cout
                        << "FRU Field Type: " << getFieldType(frutable->type)
                        << std::endl;
                    std::cout
                        << "FRU Field Length: " << unsigned(frutable->length)
                        << std::endl;
