...
class Logger
{
  public:
    ...
    virtual int log_msg(LogLevel level, const char* fmt,
...
#define LOG_D(fmt, ...) Logger::Debug(fmt, __VA_ARGS__)
#define LOG_E(fmt, ...) Logger::Error(FILE_LINE fmt, __VA_ARGS__)
#define LOG_W(fmt, ...) Logger::Warning(fmt, __VA_ARGS__)
#define LOG_I(fmt, ...) Logger::Info(fmt, __VA_ARGS__)
