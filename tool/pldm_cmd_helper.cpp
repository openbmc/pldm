...
void CommandInterface::exec(std::vector<std::string>& args)
{
    auto [rc, requestMsg] = createRequestMsg(args);
    if (rc != PLDM_SUCCESS)
    {
        std::cerr << "Failed to encode request message for " << commandName
                  << " rc = " << rc << std::endl;
        return;
    }
