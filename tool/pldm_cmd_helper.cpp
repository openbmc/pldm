...
{
/*
 * print the input buffer if pldmverbose is enabled
 *
 */
void printBuffer(const std::vector<uint8_t>& buffer, int pldm_verbose)
{
...
int mctpSockSendRecv(const std::vector<uint8_t>& requestMsg,
                     std::vector<uint8_t>& responseMsg, int pldm_verbose)
{
    ...
    Logger(pldm_verbose, "Success in creating the socket : RC = ", sockFd);
    ...
    Logger(pldm_verbose, "Success in connecting to socket : RC = ", returnCode);
    ...
    Logger(pldm_verbose, "Write to socket successful : RC = ", result);
    ...
    else
    {
        ...
        {
            printBuffer(loopBackRespMsg, pldm_verbose);
        }
        else
        {
        ...
        if (request == PLDM_REQUEST)
        {
            ...
            if (recvDataLength == peekedLength)
            {
                Logger(pldm_verbose, "Total length:", recvDataLength);
            }
            else
            {
    ...
    Logger(pldm_verbose, "Shutdown Socket successful :  RC = ", returnCode);
    return PLDM_SUCCESS;
}
