...
class GetPDR : public CommandInterface
{
  public:
    ...
    std::pair<int, std::vector<uint8_t>> createRequestMsg() override
    {
        ...
        auto rc = encode_get_pdr_req(PLDM_LOCAL_INSTANCE_ID, 0, 0,
    ...
    void parseResponseMsg(pldm_msg* responsePtr, size_t payloadLength) override
    {
        ...
        printPDRMsg(recordData, sizeof(recordData));
    }
  ...
  private:
    void printPDR11(uint8_t* data, size_t len)
    {
        ...
        std::cout << "record_handle: " << pdr->hdr.record_handle << std::endl;
        std::cout << "version: " << static_cast<uint16_t>(pdr->hdr.version)
                  << std::endl;
        std::cout << "type: " << static_cast<uint16_t>(pdr->hdr.type)
                  << std::endl;
        std::cout << "record_change_num: " << pdr->hdr.record_change_num
                  << std::endl;
        std::cout << "length: " << pdr->hdr.length << std::endl;
        std::cout << "terminus_handle: " << pdr->terminus_handle << std::endl;
        std::cout << "effecter_id: " << pdr->effecter_id << std::endl;
        std::cout << "entity_type: " << pdr->entity_type << std::endl;
        std::cout << "entity_instance: " << pdr->entity_instance << std::endl;
        std::cout << "container_id: " << pdr->container_id << std::endl;
        std::cout << "effecter_semantic_id: " << pdr->effecter_semantic_id
                  << std::endl;
        std::cout << "effecter_init: "
                  << static_cast<uint16_t>(pdr->effecter_init) << std::endl;
        if (pdr->has_description_pdr)
        {
            std::cout << "has_description_pdr: true" << std::endl;
        }
        else
        {
            std::cout << "has_description_pdr: false" << std::endl;
        }

        std::cout << "composite_effecter_count: "
                  << static_cast<uint16_t>(pdr->composite_effecter_count)
                  << std::endl;
        ...
        for (size_t i = 0; i < pdr->composite_effecter_count; ++i)
        {
            ...
            std::cout << "state_set_id: " << state->state_set_id << std::endl;
            std::cout << "possible_states_size: "
                      << static_cast<uint16_t>(state->possible_states_size)
                      << std::endl;
            bitfield8_t* bf = reinterpret_cast<bitfield8_t*>(state->states);
            std::cout << "states_type: " << static_cast<uint16_t>(bf->byte)
                      << std::endl;
        }
    }

    void printPDRMsg(uint8_t* data, size_t len)
    {
        if (data == NULL || len == 0)
        {
            return;
...
class SetStateEffecter : public CommandInterface
{
  public:
    ...
    void parseResponseMsg(pldm_msg* responsePtr, size_t payloadLength) override
    {
        ...
        std::cout << "Set state effecter successfully." << std::endl;
    }
