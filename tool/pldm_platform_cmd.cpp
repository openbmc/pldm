...
class SetNumericEffecterValue : public CommandInterface
{
  public:
    ...
    explicit SetNumericEffecterValue(const char* type, const char* name,
        ...
        CommandInterface(type, name, app)
    {
        app->add_option("-d,--data", effecterData,
                        "Set the value for a PLDM Numeric Effecter:\n"
                        "effecterID: A handle that is used to identify and "
                        "access the effecter\n"
                        "effecterDataSize: The bit width and format of the "
                        "setting value for the effecter. enum value: { uint8, "
                        "sint8, uint16, sint16, uint32, sint32 }\n"
                        "effecterValue: The setting value of numeric effecter "
                        "being requested\n"
                        "eg: effecterID effecterDataSize effecterValue")
            ->required();
    }
    ...
    std::pair<int, std::vector<uint8_t>> createRequestMsg() override
    {
        ...
        uint16_t effecterId = effecterData[0];
        uint8_t effecterDataSize = effecterData[1];
        uint8_t* effecterValue = (uint8_t*)&effecterData[2];
        ...
        auto rc = encode_set_numeric_effecter_value_req(
            PLDM_LOCAL_INSTANCE_ID, effecterId, effecterDataSize, effecterValue,
  ...
  private:
    std::vector<uint64_t> effecterData;
};
