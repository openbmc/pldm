...
class GetPDR : public CommandInterface
{
  ...
  private:
    void printPDR11(uint8_t* data, size_t len)
    {
        ...
        std::cout << "version: " << unsigned(pdr->hdr.version) << std::endl;
        std::cout << "type: " << unsigned(pdr->hdr.type) << std::endl;
        ...
        std::cout << "effecter_semantic_id: " << pdr->effecter_semantic_id
                  << std::endl;
        std::cout << "effecter_init: " << unsigned(pdr->effecter_init)
                  << std::endl;
        std::cout << "has_description_pdr: "
        ...
        for (size_t i = 0; i < pdr->composite_effecter_count; ++i)
        {
            ...
            std::cout << "states_type: " << unsigned(bf->byte) << std::endl;
        }
    }
