...
void registerCommand(CLI::App& app)
{
    ...
    auto oem_ibm_file = app.add_subcommand("oem-ibm-file", "oem type command");
    oem_ibm_file->require_subcommand(1);

    auto getFileTable =
        oem_ibm_file->add_subcommand("GetFileTable", "get file table");

    commands.push_back(std::make_unique<GetFileTable>(
        "oem_ibm_file", "getFileTable", getFileTable));
}
