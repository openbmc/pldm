...
{
    std::vector<uint8_t> dataVec = std::move(eventDataVec);
    ...
    auto rc = encode_platform_event_message_req(
        instanceId, 1 /*formatVersion*/, 0 /*tId*/, eventType, dataVec.data(),
        dataVec.size(), request,
        dataVec.size() + PLDM_PLATFORM_EVENT_MESSAGE_MIN_REQ_BYTES);
    if (rc != PLDM_SUCCESS)
    {
...
void DbusToPLDMEvent::sendStateSensorEvent(SensorId sensorId)
{
    ...
    for (uint8_t offset = 0; offset < dbusMappings.size(); ++offset)
    {
        ...
        auto stateSensorMatch = std::make_unique<sdbusplus::bus::match::match>(
            ...
            [this, eventDataVec{std::move(sensorEventDataVec)}](auto& /*msg*/) {
                std::vector<uint8_t> ev = std::move(eventDataVec);
                this->sendEventMsg(PLDM_SENSOR_EVENT, std::move(ev));
            });
