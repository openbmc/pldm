...
void DbusToSensorEventHandler::sendEventMsg(uint8_t eventType,
                                            ...
                                            size_t eventSize)
{
    ...
    // format version is 1 and tid use 0 by default.
    // see DSP0248_1.2.0 16.6 Table 15.
    uint8_t formatVersion = 1;
    uint8_t tId = 0;
    auto rc = encode_platform_event_message_req(
        instanceId, formatVersion, tId, eventType, eventData, eventSize,
        request, eventSize + PLDM_PLATFORM_EVENT_MESSAGE_MIN_REQ_BYTES);
    if (rc != PLDM_SUCCESS)
    {
...
void DbusToSensorEventHandler::sendStateSensorEvent(SensorId sensorId)
{
...
void DbusToSensorEventHandler::listenSensorEvent(const pdr_utils::Repo& repo,
