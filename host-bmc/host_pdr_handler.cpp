...
HostPDRHandler::HostPDRHandler(int mctp_fd, uint8_t mctp_eid,
  ...
  if (fs::exists(hostFruJson)) {
    ...
    try {
      ...
      std::cerr << "Parsing Host FRU json file failed, exception = " << e.what()
                << std::endl;
    }
  }
...
void HostPDRHandler::fetchPDRsOnStart() {
  ...
  do {
    ...
    if (requesterRc != PLDM_REQUESTER_SUCCESS) {
      std::cerr << "Failed to send msg to fetch pdrs, rc = " << requesterRc
                << std::endl;
      return;
    }
    ...
    if (rc != PLDM_SUCCESS) {
      ...
      rc = decode_get_pdr_resp(
          responsePtr, responseMsgSize - sizeof(pldm_msg_hdr), &completionCode,
          &nextRecordHandle, &nextDataTransferHandle, &transferFlag, &respCount,
          pdr.data(), respCount, &transferCRC);
      if (rc != PLDM_SUCCESS || completionCode != PLDM_SUCCESS) {
        ...
        if (pdrHdr->type == PLDM_PDR_ENTITY_ASSOCIATION) {
          ...
          if (pdrHdr->type == PLDM_TERMINUS_LOCATOR_PDR) {
            ...
            tlpdrInfo.emplace(
                static_cast<pdr::TerminusHandle>(tlpdr->terminus_handle),
                static_cast<pdr::TerminusID>(tlpdr->tid));

            auto tlEid =
                reinterpret_cast<const pldm_terminus_locator_type_mctp_eid *>(
                    pdr.data());
            tlinfo.emplace_back(TlInfo{tlpdr->validity, tlEid->eid, tlpdr->tid,
  ...
  setHostSensorState(stateSensorPDRs, tlinfo);
...
void HostPDRHandler::sendPDRRepositoryChgEvent(std::vector<uint8_t> &&pdrTypes,
  ...
  for (auto pdrType : pdrTypes) {
    ...
    do {
      record =
          pldm_pdr_find_record_by_type(repo, pdrType, record, nullptr, nullptr);
      if (record && pldm_pdr_record_is_remote(record)) {
        changeEntries[0].push_back(pldm_pdr_get_record_handle(repo, record));
      }
    } while (record);
  }
  ...
  if (rc != PLDM_SUCCESS) {
    std::cerr << "Failed to encode_pldm_pdr_repository_chg_event_data, rc = "
              << rc << std::endl;
    return;
  }
  auto instanceId = requester.getInstanceId(mctp_eid);
...
void HostPDRHandler::setHostState() {
  ...
  auto timerCallback = [=](Timer & /*source*/, Timer::TimePoint /*time*/) {
    timeOut = true;
    if (!responseReceived) {
      std::cerr << "PLDM did not get a response from Host"
  ...
  auto callback = [=](IO & /*io*/, int fd, uint32_t revents) {
    ...
    uint8_t *responseMsg = nullptr;
    size_t responseMsgSize{};
    auto rc = pldm_recv(mctp_eid, fd, insId, &responseMsg, &responseMsgSize);
    ...
    std::cout << "Getting the response. PLDM RC = " << std::hex << std::showbase
  ...
  if (0 > rc) {
    std::cerr << "Failed to send message RC = " << rc << ", errno = " << errno
              << "\n";
    return;
  }
  while (1) {
  ...
  DBusMapping dbusMapping{"/xyz/openbmc_project/state/host0",
...
bool HostPDRHandler::isHostUp() { return responseReceived; }
...
void HostPDRHandler::setHostSensorState(const PDRList &stateSensorPDRs,
  ...
  for (const auto &stateSensorPDR : stateSensorPDRs) {
    ...
    for (auto info : tlinfo) {
      if (info.terminus_handle == terminusHandle) {
        if (info.valid == 1) {
          mctp_eid = info.eid;
        }
      }
      ...
      auto response = reinterpret_cast<struct pldm_msg *>(responseMsgPtr.get());
