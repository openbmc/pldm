            ...
            else
            {
                ...
                else
                {
                    if (pdrHdr->type == PLDM_TERMINUS_LOCATOR_PDR)
                    {
                        ...
                        auto terminus_locator_type =
                            tlpdr->terminus_locator_type;
                            ...
                            PLDM_TERMINUS_LOCATOR_TYPE_MCTP_EID)
                        {
                            ...
                            tlEid = unsigned(locatorValue->eid);
                        }
                        if (tlpdr->validity == 0)
                        {
                            TLnotValid = true;
                    ...
                    // after we get the valid TLPDR from host
                    // check if we get invalid TLPDR from the same terminus
                    // and update the repo accordingly
                    if (TLnotValid && static_cast<uint8_t>(tlEid) == 9)
                    {
    ...
    for (const auto& stateSensorPDR : stateSensorPDRs)
    {
        ...
        for (auto info : tlinfo)
        {
            if (info.terminus_handle == pdr->terminus_handle)
            {
                ...
                if (requesterRc != PLDM_REQUESTER_SUCCESS)
                {
                    std::cerr << "Failed to send msg to get state sensor "
                                 "readings, rc = "
                              << requesterRc << std::endl;
                    return;
                }
                ...
                for (size_t i = 0; i < comp_sensor_count; i++)
                {
                    ...
                    {
                        std::cerr << " Error invalid_data" << std::endl;
                        return;
                    }
