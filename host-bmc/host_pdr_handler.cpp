    ...
    bmcEntityTree(bmcEntityTree), instanceIdDb(instanceIdDb), handler(handler)
{
    ...
    if (fs::exists(hostFruJson))
    {
        ...
        {
            lg2::error(
                "Parsing Host FRU json file failed, exception = {ERR_EXCEP}",
                "ERR_EXCEP", e.what());
        }
    }
...
void HostPDRHandler::getHostPDR(uint32_t nextRecordHandle)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        ...
        lg2::error("Failed to encode_get_pdr_req, rc = {RC}", "RC", lg2::hex,
                   rc);
        return;
    }
    ...
    const StateSensorEntry& entry, pdr::EventState state)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        lg2::error("Failed to fetch and update D-bus property, rc = {RC}", "RC",
                   lg2::hex, rc);
        return rc;
    }
    ...
    const PDRList& fruRecordSetPDRs)
{
    ...
    auto getFruRecordTableMetadataResponseHandler =
        ...
        this->getFRURecordTableByRemote(total, fruRecordSetPDRs);
    };
...
void HostPDRHandler::getFRURecordTableByRemote(uint16_t& total_table_records,
                                               const PDRList& fruRecordSetPDRs)
{
    fruRecordData.clear();
    ...
    auto getFruRecordTableResponseHandler =
        [total_table_records, this, fruRecordSetPDRs](
            mctp_eid_t /*eid*/, const pldm_msg* response, size_t respMsgLen) {
        if (response == nullptr || !respMsgLen)
        {
            lg2::error(
    ...
    uint8_t state, const std::string& path, pldm::pdr::StateSetId stateSetId)
{
    ...
    {
        instanceIdDb.free(mctp_eid, instanceId);
        lg2::error("Failed to encode_get_state_sensor_readings_req, Error:{RC}",
                   "RC", lg2::hex, rc);
    ...
    auto getStateSensorReadingsResponseHandler =
            ...
            stateSetId == PLDM_STATE_SET_HEALTH_STATE)
        {
            ...
            // command.
            lg2::error(
                "creating operational status for :{PATH}, {STATE_SET_ID}",
