                                           ...
                                           pdr::EventState state)
{
    for (auto& entity : objPathMap)
    {
        ...
        uint16_t node_entity_container_id =
            pldm_extract_host_container_id(entity.second);

        if (node_entity.entity_type != entry.entityType ||
            node_entity.entity_instance_num != entry.entityInstance ||
            node_entity_container_id != entry.containerId)
        {
            continue;
        }
                                                    ...
                                                    const std::string& path)
{
    ...
    auto getStateSensorReadingsResponseHandler = [&path, &state](
                                                     mctp_eid_t /*eid*/,
                                                     const pldm_msg* response,
                                                     size_t respMsgLen) {
        if (response == nullptr || !respMsgLen)
