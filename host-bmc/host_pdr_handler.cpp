    ...
    if (sensorMapIndex != objPathMap.end())
    {
        ...
        for (auto& sensor : sensorMap)
        {
                ...
                stateSetIds[0] == PLDM_STATE_SET_OPERATIONAL_FAULT_STATUS)
            {
                ...
                std::cerr << "entity type : " << (unsigned)entityType
                          << std::endl;
                std::cerr << "entity instance : " << (unsigned)entityInstance
                          << std::endl;
                std::cerr << "entity contained : " << (unsigned)containerId
                          << std::endl;
                std::cerr << "node entity type : " << (unsigned)node.entity_type
                          << std::endl;
                std::cerr << "node entity instance : "
                          << (unsigned)node.entity_instance_num << std::endl;
                std::cerr << "node entity contained : "
                          << (unsigned)node.entity_container_id << std::endl;
                std::cerr << "asking the status of sensor id : "
                          << sensor.first.sensorID << std::endl;
                std::cerr << "dbus path is : " << sensorMapIndex->first
                          << std::endl;
