                                             ...
                                             const PDRList& fruRecordSetPDRs)
{
    ...
    auto getFruRecordTableResponseHandler = [total_table_records, this,
        ...
        auto it = objPathMap.begin();
        setOperationStatus(it);
...
void HostPDRHandler::getPresentStateBySensorReadigs(
    uint16_t sensorId, uint8_t state, const std::string& path,
    ObjectPathMaps::iterator sensorMapIndex)
{
   
           std::cerr << "entered getPresentStateByreading\n";
    std::cerr << "path : " << sensorMapIndex->first << std::endl;

    pldm_entity node = pldm_entity_extract(sensorMapIndex->second);
    std::cerr << "entity type :" << (unsigned)node.entity_type << std::endl;
    std::cerr << "instance : " << (unsigned)node.entity_instance_num << std::endl;
	...
	std::cerr << "before incrementing the index" << std::endl;
        if (sensorMapIndex != objPathMap.end())
        {
		std::cout << "inside if check\n";
            ++sensorMapIndex;
	    std::cerr << "incremented index"<<std::endl;
            setOperationStatus(sensorMapIndex);
        }
    };
    ...
    ObjectPathMaps::iterator sensorMapIndex)
{
    ...
    std::cerr << "entered set Operation status\n";
    std::cerr << "path : " << sensorMapIndex->first << std::endl;
