...
template <typename T> uint16_t extractTerminusHandle(std::vector<uint8_t>& pdr)
{
    T* var = nullptr;
                                               ...
                                               uint8_t eventDataFormat)
{
    ...
    auto platformEventMessageResponseHandler = [](mctp_eid_t /*eid*/,
        ...
        uint8_t status{};
        auto responsePtr = new (response) const pldm_msg;
...
void HostPDRHandler::setHostSensorState(const PDRList& stateSensorPDRs)
{
    ...
    {
        auto request = new (stateSensorPDR.data()) const pldm_state_sensor_pdr;
