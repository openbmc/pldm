...
void HostPDRHandler::getHostPDR(uint32_t nextRecordHandle)
{
    ...
    if (rc != PLDM_SUCCESS)
    {
        ...
        error("Failed to encode get pdr request, response code '{RC}'", "RC", rc);
        return;
    }
                                               ...
                                               uint8_t eventDataFormat)
{
    ...
    if (rc)
    {
        error("Failed to send the PDR repository changed event request, response code '{RC}'", "RC", rc);
    }
}
                                     ...
                                     size_t respMsgLen)
{
    ...
    {
        error(
            "Failed to decode get pdr response for next record handle '{NEXT_RECORD_HANDLE}', response code '{RC}'",
    ...
    else
    {
        ...
        {
            error(
                "Failed to decode get pdr response for next record handle '{NEXT_RECORD_HANDLE}', response code '{RC}' and completion code '{CC}'",
        ...
        for (const auto& [terminusHandle, terminusInfo] : tlPDRInfo)
        {
            if (terminusHandle == pdr->terminus_handle)
            {
                ...
                if (rc != PLDM_SUCCESS)
                {
                    instanceIdDb.free(mctp_eid, instanceId);
                    error(
                        "Failed to encode get state sensor readings request, response code '{RC}'",
                        "RC", rc);
                    pldm::utils::reportError(
                ...
                auto getStateSensorReadingRespHandler =
                    ...
                    {
                        error(
                            "Failed to decode get state sensor readings response, response code'{RC}' and completion code '{CC}'",
                            "RC", rc, "CC", completionCode);
                    }
                         ...
                         sensorOffset < comp_sensor_count; sensorOffset++)
                    {
                        ...
                        {
                            error("Error Invalid data, Invalid sensor offset '{SENSOR_OFFSET}'", "SENSOR_OFFSET", sensorOffset);
                            return;
                        }
                        ...
                        if (possibleStates.find(eventState) ==
                            possibleStates.end())
                        {
                            error("Error invalid_data, Invalid event state '{STATE}'", "STATE", eventState);
                            return;
                        }
                                               ...
                                               uint16_t totalTableRecords)
{
    ...
    {
        instanceIdDb.free(mctp_eid, instanceId);
        error("Failed to encode get fru record table request, response code '{RC}'",
              "RC", lg2::hex, rc);
        return;
    }
