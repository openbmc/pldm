...
void HostPDRHandler::getHostPDR(uint32_t nextRecordHandle)
{
    ...
    {
        error("Failed to allocate instance id in getHostPDR: {ERROR}", "ERROR",
                   e.what());
        return;
    }
                                               ...
                                               uint8_t eventDataFormat)
{
    ...
    {
        error("Failed to allocate instance id in sendPDRRepositoryChgEvent: {ERROR}", "ERROR",
                   e.what());
        return;
    }
    std::vector<uint8_t> requestMsg(
...
void HostPDRHandler::setHostFirmwareCondition()
{
    ...
    {
        error("Failed to allocate instance id in setHostFirmwareCondition: {ERROR}", "ERROR",
                   e.what());
        return;
    }
    std::vector<uint8_t> requestMsg(
    ...
    for (const auto& stateSensorPDR : stateSensorPDRs)
    {
        ...
        for (const auto& [terminusHandle, terminusInfo] : tlPDRInfo)
        {
            if (terminusHandle == pdr->terminus_handle)
            {
                ...
                {
                    error("Failed to allocate instance id in setHostSensorState: {ERROR}", "ERROR",
                               e.what());
                    return;
                }
                std::vector<uint8_t> requestMsg(
    ...
    const PDRList& fruRecordSetPDRs)
{
    ...
    {
        error("Failed to allocate instance id in getFRURecordTableMetadataByRemote: {ERROR}", "ERROR",
                   e.what());
        return;
    }
    std::vector<uint8_t> requestMsg(
                                               ...
                                               uint16_t totalTableRecords)
{
    ...
    {
        error("Failed to allocate instance id in getFRURecordTableByRemote: {ERROR}", "ERROR",
                   e.what());
        return;
    }
    std::vector<uint8_t> requestMsg(
