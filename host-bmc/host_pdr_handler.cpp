...
template <typename T>
uint16_t extractTerminusId(std::vector<uint8_t>& pdr)
{
    T* var = nullptr;
...
void HostPDRHandler::parseStateSensorPDRs(const PDRList& stateSensorPDRs)
{
    for (const auto& pdr : stateSensorPDRs)
    {
        ...
        try
        {
            sensorEntry.terminusID = std::get<1>(tlPDRInfo.at(terminusHandle));
        }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    pdrTerminusHandle =
                        extractTerminusId<pldm_terminus_locator_pdr>(pdr);
                    auto tlpdr =
                ...
                {
                    pdrTerminusHandle =
                        extractTerminusId<pldm_state_sensor_pdr>(pdr);
                    stateSensorPDRs.emplace_back(pdr);
                }
                else if (pdrHdr->type == PLDM_PDR_FRU_RECORD_SET)
                {
                    pdrTerminusHandle =
                        extractTerminusId<pldm_pdr_fru_record_set>(pdr);
                }
                else if (pdrHdr->type == PLDM_STATE_EFFECTER_PDR)
                {
                    pdrTerminusHandle =
                        extractTerminusId<pldm_state_effecter_pdr>(pdr);
                }
                else if (pdrHdr->type == PLDM_NUMERIC_EFFECTER_PDR)
                {
                    pdrTerminusHandle =
                        extractTerminusId<pldm_numeric_effecter_value_pdr>(pdr);
                }
...
void HostPDRHandler::setTerminusInfo(uint8_t tid, uint16_t terminusHandle,
                                     uint8_t validity, uint8_t eid)
{
    tlPDRInfo[terminusHandle] = std::make_tuple(tid, eid, validity);
}
