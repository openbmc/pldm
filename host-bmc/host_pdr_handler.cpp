...
void HostPDRHandler::fetchPDRsOnStart(uint32_t nextRecordHandle)
{
    ...
    else
    {
        ...
        rc = handler.registerRequest(
            ...
            std::move(requestMsg), std::move(fetchPDRResponseHandler));
        if(rc != PLDM_SUCCESS)
        {
...
void HostPDRHandler::processHostPDRs(const pldm_msg* response,size_t respMsgLen)
{
            ...
            auto rc = decode_get_pdr_resp(response, respMsgLen /*- sizeof(pldm_msg_hdr)*/, 
            ...
            memcpy(responsePDRMsg.data(), response,
                ...
                if (merged)
                {
                    ...
                    deferredPDRRepoChgEvent =
                        std::make_unique<sdeventplus::source::Defer>(
                            event,
                            std::bind(std::mem_fn(
                                          (&HostPDRHandler::_processPDRRepoChgEvent)),
            ...
            else
            {
                deferredFetchPDREvent = std::make_unique<sdeventplus::source::Defer>(
                    event,
                    std::bind(std::mem_fn(
                        (&HostPDRHandler::_processFetchPDREvent)),
                        this, nextRecordHandle, std::placeholders::_1));
            }

}

void HostPDRHandler::_processPDRRepoChgEvent(sdeventplus::source::EventBase& /*source */)
{
...
void HostPDRHandler::_processFetchPDREvent(uint32_t nextRecordHandle,
