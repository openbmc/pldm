        ...
        for (size_t i = 1; i < numEntities; ++i)
        {
                ...
                parent.entity_container_id != node.entity_container_id)
            {
                cNode = pldm_entity_association_tree_add(
                    entityTree, &entities[i], 0xFFFF, pNode,
...
void HostPDRHandler::getFRURecordTableMetadataByHost(uint16_t& total)
{
    auto instanceId = requester.getInstanceId(mctp_eid);
    std::vector<uint8_t> requestMsg(
    ...
    auto getFruRecordTableMetadataResponseHandler = [&total](
                                                        mctp_eid_t /*eid*/,
                                                        const pldm_msg*
                                                            response,
                                                        size_t respMsgLen) {
...
void HostPDRHandler::getFRURecordTableByHost()
{
    ...
    getFRURecordTableMetadataByHost(total_table_records);
                                                    ...
                                                    uint8_t& state)
{
    ...
    auto getStateSensorReadingsResponseHandler = [&state](
                                                     mctp_eid_t /*eid*/,
                                                     const pldm_msg* response,
                                                     size_t respMsgLen) {
        if (response == nullptr || !respMsgLen)
