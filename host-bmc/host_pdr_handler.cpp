...
void HostPDRHandler::updateOperationStatus(const char* path,
                                           const pldm_entity& entity)
{
    for (auto& sensor : sensorMap)
    {
        ...
        auto operStatus =
            std::make_unique<sdbusplus::xyz::openbmc_project::State::Decorator::
                                 server::OperationalStatus>(
                pldm::utils::DBusHandler::getBus(), path);
        operStatus->functional(persentState);
    }
}
...
void HostPDRHandler::parseFruRecordSetPDRs(const PDRList& fruRecordSetPDRs)
{
    ...
    for (auto& entity : objPathMap)
    {
        ...
        updateOperationStatus(entity.first.c_str(), entity.second);
        ...
        for (auto& data : fruRecordData)
        {
            ...
            {
                // ifdef OEM_IBM, get the Location Code value
#ifdef OEM_IBM
                for (auto& tlv : data.fruTLV)
                {
                    ...
                    {
                        auto location = std::make_unique<
                            sdbusplus::com::ibm::ipzvpd::server::Location>(
                            pldm::utils::DBusHandler::getBus(),
                            entity.first.c_str());
                        location->locationCode(reinterpret_cast<const char*>(
