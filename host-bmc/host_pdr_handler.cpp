...
{
    // std::cerr << "Entered set operation status" << std::endl;
    if (sensorMapIndex != objPathMap.end())
    {

        pldm_entity node = pldm_entity_extract(sensorMapIndex->second);
        /*           std::cerr << "begin..\n";
                   std::cerr << "dbus sensor entity type : "<< node.entity_type
           << std::endl; std::cerr << "dbus ensor entity instance : " <<
           node.entity_instance_num << std::endl; std::cerr << "dbus sensor
           entity container id : "<< node.entity_container_id << std::endl;
                   std::cerr << "end..\n"; */

        for (auto& sensor : sensorMap)
        {
            // std::cerr << "sensor id : " << sensor.first.sensorID <<
            // std::endl; check if the terminus is valid
            bool valid = getValidity(sensor.first.terminusID);
            if (valid)
            {
                pldm::pdr::EntityInfo entityInfo{};
                ...
                /* std::cerr << "comparing with begin..\n";
                 std::cerr << "sensor entity type : "<< entityType << std::endl;
                 std::cerr << "sensor entity instance : " << entityInstance <<
                 std::endl; std::cerr << "sensor entity container id : "<<
                 containerId << std::endl; std::cerr << "comparing with
                 end..\n";*/
                ...
                // std::cerr << "sensor path :" << sensorMapIndex->first <<
                // std::endl;
            ...
            {
                // Sensor is attched to a Terminus that is not valid
                continue;
            }
        }
