    ...
    for (const auto& entry : entries)
    {
        ...
        if (effecterPdrType == PLDM_NUMERIC_EFFECTER_PDR)
        {
            for (const auto& effecter : effecters)
            {
                 ...
                 */
                if (std::find(
                        pldm::utils::DBUS_VALUE_NUMERIC_TYPE_NAMES.begin(),
                        pldm::utils::DBUS_VALUE_NUMERIC_TYPE_NAMES.end(),
                        dbusInfo.dbusMap.propertyType) ==
                    pldm::utils::DBUS_VALUE_NUMERIC_TYPE_NAMES.end())
                {
                    lg2::error(
    ...
    size_t dbusInfoIndex, uint16_t effecterId)
{
    ...
    if (std::find(pldm::utils::DBUS_VALUE_NUMERIC_TYPE_NAMES.begin(),
                  pldm::utils::DBUS_VALUE_NUMERIC_TYPE_NAMES.end(),
                  propertyType) ==
        pldm::utils::DBUS_VALUE_NUMERIC_TYPE_NAMES.end())
    {
    ...
    if (it == chProperties.end())
    {
        lg2::error("Can't find property name \"{NAME}\" ", "NAME",
                   propertyName);
        return;
    }
