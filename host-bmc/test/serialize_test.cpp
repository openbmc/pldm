...
TEST(Serialize, SerializeKeyValueGoodPath)
{
    ...
    pldm::serialize::Serialize::getSerialize().serializeKeyVal(key, value);
    ...
    EXPECT_EQ(30, std::get<uint32_t>(savedKeyVal[key]));
}
...
TEST(Serialize, DeserializeBadPath)
{
    ...
    // Changin file location which does not present so function should return false.
