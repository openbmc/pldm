...
TEST(EntityAssociation, addObjectPathEntityAssociations1)
{
    ...
    auto l2 = pldm_entity_association_tree_add(
        tree, &entities[1], 1, l1, PLDM_ENTITY_ASSOCIAION_PHYSICAL, true);

    auto l3a = pldm_entity_association_tree_add(
        tree, &entities[2], 0, l2, PLDM_ENTITY_ASSOCIAION_PHYSICAL, true);
    auto l3b = pldm_entity_association_tree_add(
        tree, &entities[3], 1, l2, PLDM_ENTITY_ASSOCIAION_PHYSICAL, true);

    auto l4a = pldm_entity_association_tree_add(
        tree, &entities[4], 0, l3a, PLDM_ENTITY_ASSOCIAION_PHYSICAL, true);
    auto l4b = pldm_entity_association_tree_add(
        tree, &entities[5], 1, l3a, PLDM_ENTITY_ASSOCIAION_PHYSICAL, true);

    auto l5a = pldm_entity_association_tree_add(
        tree, &entities[6], 0, l3b, PLDM_ENTITY_ASSOCIAION_PHYSICAL, true);
    auto l5b = pldm_entity_association_tree_add(
        tree, &entities[7], 1, l3b, PLDM_ENTITY_ASSOCIAION_PHYSICAL, true);
