...
Entities getParentEntites(const EntityAssociations& entityAssoc)
{
    ...
    {
        uint16_t parent_host_contanied_id = pldm_entity_node_get_remote_container_id(*it);
        bool find = false;
                             ...
                             ObjectPathMaps& objPathMap)
{
    ...
    for (auto& entity : parentsEntity)
    {
        ...
        pldm_entity node_entity = pldm_entity_extract(entity);
        auto node =
            pldm_entity_association_tree_find_if_remote(entityTree, &node_entity, false);
        if (!node)
        {
            continue;
        }
        ...
        while (node)
        {
            ...
            node =
                pldm_entity_association_tree_find_if_remote(entityTree, &parent, false);
        }
