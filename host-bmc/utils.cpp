                                     ...
                                     ObjectPathMaps& objPathMap)
{
    ...
    {
        lg2::info(
            "{TYPE} Entity fetched from remote PLDM terminal does not exist.",
            "TYPE", node_entity.entity_type);
        return;
    }
    ...
    for (const auto& ev : entityAssoc)
    {
            ...
            ev_entity.entity_type == node_entity.entity_type)
        {
            ...
            if (!objPathMap.contians(entity_path))
            {
                             ...
                             ObjectPathMaps& objPathMap)
{
    ...
    for (const auto& entity : parentsEntity)
    {
        ...
        while (node)
        {
            ...
            {
                lg2::error(
                    "Parent entity not found in the entityMaps, type: {TYPE}, num: {NUM}, e: {ERROR}",
                    "TYPE", parent.entity_type, "NUM",
                    parent.entity_instance_num, "ERROR", e);
                found = false;
                break;
            }
