    ...
    for (auto& entity : parentsEntity)
    {
        ...
        while (node)
        {
            ...
            catch (const std::exception& e)
            {
                ...
                std::cerr << "george: start print entityMaps...\n";
                for (auto& map : entityMaps)
                {
                    std::cerr << "type = " << map.first << std::endl;
                }
                if (parent.entity_type == 66)
                {
                    std::cerr << "george: type equal 66...\n";
                }
                std::cerr << "george: end print entityMaps...\n\n";
                found = false;
                break;
            }
