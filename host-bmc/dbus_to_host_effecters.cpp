...
void HostEffecterParser::parseEffecterJson(const std::string& jsonPath)
{
    ...
    {
        error("Json at path '{PATH}' does not exist.", "PATH",
              jsonFilePath.c_str());
    ...
    size_t dbusInfoIndex, uint16_t effecterId)
{
    ...
    {
        error("New state not found in json, error - {ERROR}", "ERROR", e);
        return;
    }
    ...
    {
        error("Could not set host state effecter, error - {ERROR}", "ERROR", e);
        return;
    }
    if (rc != PLDM_SUCCESS)
    {
        error("Could not set the host state effecter, response code '{RC}'",
              "RC", rc);
    }
}
    ...
    uint16_t effecterId)
{
    ...
    auto setStateEffecterStatesRespHandler =
        ...
        {
            error(
                "Failed to decode setStateEffecterStates, response code '{RC}'",
                "RC", rc);
    ...
    {
        error(
            "Failed to send request to set an effecter on Host, response code '{RC}'",
            "RC", rc);
    }
    return rc;
}
