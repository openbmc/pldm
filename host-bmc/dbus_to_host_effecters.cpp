                                            ...
                                            std::string interface)
{
    ...
    {
        std::cerr << "Failed to get effecter value." << std::endl;
    }
        ...
        else if (effecterPdrType == PLDM_NUMERIC_EFFECTER_PDR)
        {
            for (const auto& effecter : effecters)
            {
                ...
                dbusInfo.propertyValue = getEffecterValue(
                    dbusInfo.dbusMap.objectPath, dbusInfo.dbusMap.propertyName,
                    dbusInfo.dbusMap.interface);
                auto effecterInfoIndex = hostEffecterInfo.size();
                auto dbusInfoIndex = effecterInfo.dbusInfo.size();
                createHostEffecterMatch(
    ...
    size_t dbusInfoIndex, uint16_t effecterId)
{
    ...
    {
        processHostNumericEffecterChangeNotification(
...
void HostEffecterParser::processHostNumericEffecterChangeNotification(
    const DbusChgHostEffecterProps& chProperties, size_t effecterInfoIndex,
    size_t dbusInfoIndex, uint16_t effecterId)
{
    ...
    try
    {
        rc = setHostNumericEffecter(effecterInfoIndex, effecterId,
...
int HostEffecterParser::setHostNumericEffecter(size_t effecterInfoIndex,
                                               uint16_t effecterId,
                                               uint8_t dataSize,
                                               double rawValue)
{
    ...
    std::cerr << " rawValue " << rawValue << std::endl;
    ...
    for (auto i = requestMsg.begin(); i != requestMsg.end(); ++i)
