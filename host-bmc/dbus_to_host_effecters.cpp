...
void HostEffecterParser::parseEffecterJson(const std::string& jsonPath)
{
    ...
    {
        error(
            "Remote pldm terminus effecter json file '{PATH}' does not exist.",
            "PATH", jsonPath);
        return;
    }
    ...
    size_t dbusInfoIndex, uint16_t effecterId)
{
    ...
    try
    {
        ...
        {
            info(
                "Remote pldm terminus is not up and current remote pldm terminus state is '{CURRENT_HOST_STATE}'",
    ...
    {
        error(
            "Error in getting current remote pldm terminus state. Will still continue to set the remote pldm terminus effecter, error - {ERROR}",
            "ERROR", e);
    }
    uint8_t newState{};
    try
    ...
    {
        error("New state '{NEW_STATE}' not found in json, error - {ERROR}",
    ...
    {
        error(
            "Could not set remote pldm terminus state effecter for effecter ID '{EFFECTERID}', error - {ERROR}",
    ...
    {
        error(
            "Could not set the remote pldm terminus state effecter for effecter ID '{EFFECTERID}', response code '{RC}'",
    ...
    uint16_t effecterId)
{
    ...
    {
        error(
            "Set state effecter states message encode failure for effecter ID '{EFFECTERID}' and instanceID '{INSTANCE}' with response code '{RC}'",
    ...
    auto setStateEffecterStatesRespHandler =
        ...
        {
            error(
                "Failed to set a remote pldm terminus effecter, completion code '{CC}'",
                "CC", completionCode);
            pldm::utils::reportError(
                "xyz.openbmc_project.bmc.pldm.SetHostEffecterFailed");
        }
    ...
    {
        error(
            "Failed to send request to set an effecter on remote pldm terminus for effecter ID '{EFFECTERID}', response code '{RC}'",
            "EFFECTERID", effecterId, "RC", rc);
    }
    return rc;
}
