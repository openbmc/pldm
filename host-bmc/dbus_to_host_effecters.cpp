    ...
    size_t dbusInfoIndex, uint16_t effecterId)
{
    ...
    if (effecterId == PLDM_INVALID_EFFECTER_ID)
    {
        ...
        if (effecterId == PLDM_INVALID_EFFECTER_ID)
        {
            error("Effecter ID '{EFFECTERID}' not found in pdr repo",
    ...
    {
        error(
            "Could not set host state effecter for effecter ID '{EFFECTERID}', error - {ERROR}",
    ...
    {
        error(
            "Could not set the host state effecter for effecter ID '{EFFECTERID}', response code '{RC}'",
    ...
    uint16_t effecterId)
{
    ...
    {
        error(
            "Set state effecter states message encode failure for effecter ID '{EFFECTERID}' with response code '{RC}'",
